...
my $sFileins;
#my @aFilein = ("../db/re/mob_db.txt","../db/pre-re/mob_db.txt","../db/import-tmpl/mob_db.txt,"../db/import/mob_db.txt");
my @aFilein = ("../db/re/mob_db.txt");
my $sFileouts;
#my @aFileout = ("../db/re/mob_db.txt","../db/pre-re/mob_db.txt","../db/import-tmpl/mob_db.txt","../db/import/mob_db.txt");
my @aFileout = ("../db/re/mob_db.txt");
my $sHelp = 0;
...
sub Main {
	...
	GetArgs();
	print "Running rAthena's mob atk resetter...\n";
	...
	foreach my $sFile (@aFilein){
		...
		while (<FHIN>){
			...
			if( $_ =~ /^\/\// ) { # // line
				if(scalar(@champ)>11){
					$champ[0] =~ s!\/\/!!g;
					$sDoconvertion=looks_like_number($champ[0]) and looks_like_number($champ[10]) and looks_like_number($champ[11]);
			...
			if(scalar(@champ)>11 and !($champ[11]-$champ[10] < 0)){
				$champ[11]=$champ[11]-$champ[10];
