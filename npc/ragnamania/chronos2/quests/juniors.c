...
prontera,0,0,3	script	HuntingQuestsSetup	-1,{
	...
	OnInit:
		...
		$@MAX_LEVEL_GAP = 5; // Maximum level difference between members of the same party to share the quests
	OnTime0000:
		$@HM_LAST_TICK = gettimetick(2); //Used to limit the number of hunting quests a player can do in a day
	end;
...
function	script	F_Share	{
	...
	for( .@i=0; .@i < .@members; .@i++ ) {
		if ( isloggedin(.@GID[.@i]) == 1 && .@GID[.@i] != .@oacc ) {
			if( attachrid(.@GID[.@i]) == 1 ) {
				...
				if( HM_IN_MISSION || .@lvDiff > $@MAX_LEVEL_GAP ) {
	...
	attachrid(.@oacc);
	dispbottom "Hunting Quest succesfully shared among all party members.";
...
BuildMission:
		...
		// if( $@HM_LAST_TICK > HM_LAST_TICK ) {
		// 	// free to go... last quests were finished before mid-night last night
...
function	script	Build_Mission	{
	...
	if(BaseLevel > 50) .@lv = 50;
	else if(BaseLevel - 20 > 1) .@lv = BaseLevel - 20;
	else .@lv = 1;
	.@maxlv = .@lv + 5;
