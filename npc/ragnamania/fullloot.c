...
OnInit:
    ...
    .FL_ARMOR_BREAK_CHANCE = 20;	// % Chance bound items will break when killed by a player
    .FL_WEAPON_BREAK_CHANCE = 20;	// % Chance bound items will break when killed by a player
    end;
...
OnCollectLootBag:
	...
	function	full_loot	{
		...
		for(.@c=0;.@c<@inventorylist_count;.@c++) {
			...
			//Bound items sometiems break (40% chance)
			if (@inventorylist_bound[.@c] > 0) {
				if(rand(100) <= .FL_BOUND_BREAK_CHANCE) {
					delitem @inventorylist_id[.@c],@inventorylist_amount[.@c];
					continue;
				}
			}
			...
			//As I said, costumes never break, never drop. Even the ones created via NPC
			...
			if(getiteminfo(@inventorylist_id[.@c],5) == 4) {
				if(rand(100) <= .FL_ARMOR_BREAK_CHANCE) {
					delitem @inventorylist_id[.@c],1;
					continue;
				}
			}
			//...So does the weapon
			if(getiteminfo(@inventorylist_id[.@c],5) == 5) {
				if(rand(100) <= .FL_WEAPON_BREAK_CHANCE) {
					delitem @inventorylist_id[.@c],1;
					continue;
				}
			}
