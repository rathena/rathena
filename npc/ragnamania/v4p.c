		...
		case 1:
			if(gettime(DT_DAYOFMONTH)<.ThirdDday) {
				...
				if(select("Yes, please:No, not now") == 2){
					...
					callfunc "F_UpdatePoints",getcharid(3),.@votes;
					mes .n$;
					mes "All done!";
...
OnHour00:	//-- Monthly Rewards
	...
	callfunc "F_SetTargets";
	callfunc "F_AdjustRates";
	end;
...
function	script	F_UpdatePoints	{
...
function	script	F_SetTargets	{
	switch(gettime(DT_DAYOFMONTH)) {
		case 1:	case 2:	case 3:	case 4:	case 5:
			set $@VotePointsTarget, 1000;
			break;
		case 6:	case 7:	case 8:	case 9:	case 10:
			set $@VotePointsTarget, 2500;
			break;
		default:
			set $@VotePointsTarget, 4000;
			break;
	}
	return;
}

function	script	F_AdjustRates	{
	// Muito cedo no mes... do dia 1 ao dia 5 nada acontece:
	if(gettime(DT_DAYOFMONTH)<getvariableofnpc(.FirstDday,"v4p")) {
		callfunc "F_SetBF";
		return;
	}
	...
	if(gettime(DT_DAYOFMONTH)>=getvariableofnpc(.ExtensionStart,"v4p")) {
		// Se coletamos zeny o bastante, Extendemos os bonus por mais 24 horas:
		if($vp_donations >= getvariableofnpc(.ExtensionCost,"v4p")) {
			callfunc "F_SetBF",6000,5000;
			set $vp_donations, $vp_donations - getvariableofnpc(.ExtensionCost,"v4p");
			logmes "F_AdjustRates : Expandido bonus. Ajustado $vp_donations";
			return;
		// Caso contrario volta as rates originais:
		} else
			callfunc "F_SetBF";
	} else if(gettime(DT_DAYOFMONTH)>=getvariableofnpc(.ThirdDday,"v4p"))
		callfunc "F_SetBF",6000,5000;
	else if(gettime(DT_DAYOFMONTH)>=getvariableofnpc(.SecondDday,"v4p"))
		callfunc "F_SetBF",5500,4800;
	else if(gettime(DT_DAYOFMONTH)>=getvariableofnpc(.FirstDday,"v4p"))
		callfunc "F_SetBF",5000,4700;
		
	return;
}

function	script	F_SetBF	{
