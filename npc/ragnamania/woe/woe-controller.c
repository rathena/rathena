...
// Information NPC
//============================================================
prontera,162,194,4	script	WOE Information	835,{
...
// Castle("<castle map name>", "<event name when woe end>", "<event killmonster name>", "<map name special warp>",<x>,<y>)
	...
	Region("Kindergarten");
		Castle("nguild_alde",	"Agit_N01::OnAgitEnd",	"Agit_N01::OnAgitBreak",	"n_castle",102,93+rand(14));
		Castle("nguild_gef", 	"Agit_N02::OnAgitEnd",	"Agit_N02::OnAgitBreak",	"n_castle",102,93+rand(14));
		Castle("nguild_pay", 	"Agit_N03::OnAgitEnd",	"Agit_N03::OnAgitBreak",	"n_castle",102,93+rand(14));
		Castle("nguild_prt", 	"Agit_N04::OnAgitEnd",	"Agit_N04::OnAgitBreak",	"n_castle",102,93+rand(14));
...
OnPCLoadMapEvent:
	.@compare_val = ((compare(strcharinfo(3),"cas0") && (!compare(strcharinfo(3),"gl_cas"))) || (compare(strcharinfo(3),"nguild_")));
	if (!.@compare_val) end;
...
while(1) {
	...
	while(1) {
		...
		next;
		switch(select(" ~ Add a session...: ~ Delete a session...: ~ Reload settings...: ~ Castle Transfer...:"+((agitcheck() || agitcheck2() || agitcheck3())?" ~ End WOE session...":"")+": ~ ^777777Go back^000000")) {
		case 1:
			mes "[New Session]";
			mes "Select a day.";
			next;
		...
		case 3:
			...
			next;
			break;
		case 4: //Transfer castle - Biali
			mes "[Castle Transfer]";
			mes "Please choose which castle we are transferring:";
			if (.Size) {
				freeloop(1);

				for(set .@j,0; .@j<.total_castle; set .@j,.@j+1) {
					if (.@id = $WOE_CONTROL[.@i+3]&(1<<.@j)){
						.@actual$ += "  ~ "+getcastlename(.Castles$[.@j])+" ^777777("+.Castles$[.@j]+")^000000:";
						setarray .@actual[getarraysize(.@actual)], .@id;
						setarray .@mapname$[getarraysize(.@mapname$)],.Castles$[.@j];
						// debugmes "Merda : " + .@id;
					} 
				}

				.@actual$ += "^FF0000  ~ Cancel^000000";
				freeloop(0);
			} else {
				mes "No times are configured.";
				close;
			}
			set .@c,select(.@actual$) -1;

			// debugmes ".@c : " + .@c;
			// debugmes "array : " + getarraysize(.@actual);

			if(.@c == getarraysize(.@actual))
				close;

			// debugmes "Castelo : " + .@actual[.@c];

			clear;

			mes "[Castle Transfer]";
			mes "Now please choose the destination:";

			set .@Castle, 0;
			for(set .@i,0; .@i<.total_castle; set .@i,.@i+1) {
				set .@menu$, .@menu$+" ~ ^0055FF"+getcastlename(.Castles$[.@i])+" ("+.Castles$[.@i]+")^000000:";
				// setarray .@disponivel[.@i], 1<<(.@i);
				// setarray .@mapname2$[.@i],.Castles$[.@i];
				// debugmes "merda : " + .@disponivel[.@i];
				//set .@menu$, .@menu$+" ~ "+((.@Castle&(1<<.@i))?"^0055FF":"")+getcastlename(.Castles$[.@i])+" ("+.Castles$[.@i]+")^000000:";
			}
			set .@k, select(.@menu$)-1;
			// debugmes ".@i :" + .@i;
			...
			set .@Castle, 1<<(.@k);
			// debugmes ".@Castle : " + .@Castle + "(" + .Castles$[.@k] + ")";

			for(set .@i,0; .@i<.Size; set .@i,.@i+4) {
				debugmes ".@actual[.@c] : " + .@actual[.@c] + " - $WOE_CONTROL[.@i+3])" + $WOE_CONTROL[.@i+3];
				if (.@actual[.@c] == $WOE_CONTROL[.@i+3]) {
					debugmes "$WOE_CONTROL[.@i+3] was " + $WOE_CONTROL[.@i+3];
					set $WOE_CONTROL[.@i+3], .@Castle;
					debugmes "$WOE_CONTROL[.@i+3] is now " + $WOE_CONTROL[.@i+3];
					for( set .@j, 17; .@j > 0; set .@j, .@j - 1 ) {
						set .@TempData[.@j], getcastledata(.@mapname$[.@c],.@j);
						setcastledata .@mapname$[.@c],.@j,0; // Clear current Data
					}
					maprespawnguildid .@mapname$[.@c],0,3; // Kick Everybody inside
					
					killmonsterall .@mapname$[.@c]; // Remove Monsters and Treasures
					setmapflag .@mapname$[.@c],mf_blocked; // Block Castle
					//donpcevent "::OnRecvCastle"+.@castle$;
					donpcevent .@mapname$[.@c] + "::OnRecvCastle"; // Refresh Flags
					
					for( set .@j, 1; .@j < 18; set .@j, .@j + 1 )
						setcastledata .@Castle$,.@j,.@TempData[.@j];

					killmonsterall .Castles$[.@k];
					removemapflag .Castles$[.@k],mf_blocked; // UnBlock Castle
					donpcevent .Castles$[.@k] + "::OnRecvCastle"; // Refresh Flags

					debugmes "alterado castelo " + .@actual[.@c] + " para " + .@Castle + " em $WOE_CONTROL["+.@i+3+"]";
				}
			}
