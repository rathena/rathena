...
int32 Sql_Connect(Sql* self, const char* user, const char* passwd, const char* host, uint16 port, const char* db) {
	...
	if (!mysql_real_connect(
			&self->handle, host, user, passwd, db, (uint32)port, nullptr /*unix_socket*/, 0 /*clientflag*/)) {
...
///
/// @private
static int32 Sql_P_BindSqlDataType(MYSQL_BIND* bind,
...
void SqlStmt::ShowDebugTruncatedColumn(size_t i) {
	...
	Sql_P_ShowDebugMysqlFieldInfo(
		"data   - ", field->type, field->flags & UNSIGNED_FLAG, this->column_lengths[i].length, "");
...
int32 SqlStmt::Execute() {
	...
	if ((this->bind_params && mysql_stmt_bind_param(this->stmt, this->params)) || mysql_stmt_execute(this->stmt)) {
		ShowSQL("DB error - %s\n", mysql_stmt_error(this->stmt));
		ra_mysql_error_handler(mysql_stmt_errno(this->stmt));
		return SQL_ERROR;
	}
...
int32 SqlStmt::BindColumn(
	size_t idx, enum SqlDataType buffer_type, void* buffer, size_t buffer_len, uint32* out_length, int8* out_is_null) {
	...
	if (idx < this->max_columns) {
		...
		return Sql_P_BindSqlDataType(
			this->columns + idx, buffer_type, buffer, buffer_len, &this->column_lengths[idx].length, out_is_null);
	}
	else {
		// TODO: for real...? Check this! [Lemongrass]
		return SQL_SUCCESS; // out of range - ignore
