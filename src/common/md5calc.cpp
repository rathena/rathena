...
static void MD5_Round_Calculate(const unsigned char *block, uint32 *A2, uint32 *B2, uint32 *C2, uint32 *D2) {
...
static void MD5_String2binary(const char *string, unsigned char *output) {
	...
	*D = 0x10325476;
	...
	string_bit_len = string_byte_len * 8; // From the byte chief to bit length (32 bytes of low
										  // rank)
