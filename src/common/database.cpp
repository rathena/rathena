...
bool YamlDatabase::nodesExist(const ryml::NodeRef& node,
...
bool YamlDatabase::load(const std::string& path) {
	...
	if (!this->verifyCompatibility(tree)) {
		ShowError("Failed to verify compatibility with %s database file from '" CL_WHITE
				  "%s" CL_RESET "'.\n",
				  this->type.c_str(),
				  this->currentFile.c_str());
		aFree(buf);
...
void YamlDatabase::parse(const ryml::Tree& tree) {
	...
	if (this->nodeExists(tree.rootref(), "Body")) {
		...
		for (const ryml::NodeRef& node : bodyNode) {
			...
			ShowStatus("Loading [%" PRIdPTR "/%" PRIdPTR "] entries from '" CL_WHITE "%s" CL_RESET
					   "'" CL_CLL "\r",
		...
		ShowStatus("Done reading '" CL_WHITE "%" PRIu64 CL_RESET "' entries in '" CL_WHITE
				   "%s" CL_RESET "'" CL_CLL "\n",
				   count,
				   fileName);
	}
...
bool YamlDatabase::asType(const ryml::NodeRef& node, const std::string& name, R& out) {
	if (this->nodeExists(node, name)) {
		...
		try {
			...
			this->invalidWarning(
				node, "Node \"%s\" cannot be parsed as %s.\n", name.c_str(), typeid(R).name());
			return false;
		}
...
bool YamlDatabase::asUInt16Rate(const ryml::NodeRef& node,
	...
	if (this->asUInt16(node, name, out)) {
		if (out > maximum) {
			...
			this->invalidWarning(
				node[c4::to_csubstr(name)], "Node \"%s\" needs to be at least 1.\n", name.c_str());
...
bool YamlDatabase::asUInt32Rate(const ryml::NodeRef& node,
								const std::string& name,
								uint32& out,
								uint32 maximum) {
