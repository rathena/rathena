...
static void logclif_auth_ok(struct login_session_data* sd) {
	...
	if (login_config.group_id_to_connect >= 0 && sd->group_id != login_config.group_id_to_connect) {
		ShowStatus("Connection refused: the required group id for connection is %d (account: %s, group: %d).\n",
				   login_config.group_id_to_connect,
				   sd->userid,
				   sd->group_id);
		logclif_sent_auth_result(fd, 1); // server closed
		return;
		...
		ShowStatus("Connection refused: the minimum group id required for connection is %d (account: %s, group: %d).\n",
				   login_config.min_group_id_to_connect,
				   sd->userid,
				   sd->group_id);
		logclif_sent_auth_result(fd, 1); // server closed
		return;
	...
	for (i = 0, n = 0; i < ARRAYLENGTH(ch_server); ++i) {
		...
		ShowDebug(
			...
			CONVIP(ip),
			ch_server[i].name,
			CONVIP((subnet_char_ip) ? subnet_char_ip : ch_server[i].ip),
			ch_server[i].port);
#endif
...
public:
	LoginPacketDatabase() {
		...
		this->add(HEADER_CA_LOGIN_PCBANG,
				  true,
				  sizeof(PACKET_CA_LOGIN_PCBANG),
				  logclif_parse_reqauth_raw<PACKET_CA_LOGIN_PCBANG>);
		this->add(HEADER_CA_LOGIN_CHANNEL,
				  true,
				  sizeof(PACKET_CA_LOGIN_CHANNEL),
				  logclif_parse_reqauth_raw<PACKET_CA_LOGIN_CHANNEL>);
		...
		this->add(HEADER_CA_SSO_LOGIN_REQ,
				  false,
				  sizeof(PACKET_CA_SSO_LOGIN_REQ),
				  logclif_parse_reqauth_sso<PACKET_CA_SSO_LOGIN_REQ>);
