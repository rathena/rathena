...
void ipban_log(uint32 ip) {
	...
	failures = loginlog_failedattempts(
		ip, login_config.dynamic_pass_failure_ban_interval); // how many times failed account? in one ip.

	// if over the limit, add a temporary ban entry
	if (failures >= login_config.dynamic_pass_failure_ban_limit) {
		uint8* p = (uint8*)&ip;
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s`(`list`,`btime`,`rtime`,`reason`) VALUES ('%u.%u.%u.*', NOW() , "
								   "NOW() +  INTERVAL %d MINUTE ,'Password error ban')",
								   ipban_table.c_str(),
								   p[3],
								   p[2],
								   p[1],
...
void ipban_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(sql_handle,
								 ...
								 ipban_db_password.c_str(),
								 ipban_db_hostname.c_str(),
								 ipban_db_port,
								 ipban_db_database.c_str())) {
