...
static bool account_db_sql_get_property(AccountDB* self, const char* key, char* buf, size_t buflen)
{
	...
	if( strncmpi(key, signature, strlen(signature)) == 0 ) {
		...
		else if (strcmpi(key, "account_table") == 0)
			safesnprintf(buf, buflen, "%s", db->account_table.c_str());
...
static bool account_db_sql_create(AccountDB* self, struct mmo_account* acc) {
	...
	{// ask the database
		...
		if( SQL_SUCCESS != Sql_Query(sql_handle, "SELECT MAX(`account_id`)+1 FROM `%s`", db->account_table.c_str()) )
		{
			Sql_ShowDebug(sql_handle);
			return false;
		}
...
static bool account_db_sql_remove(AccountDB* self, const uint32 account_id) {
	...
	||  SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = %d", db->account_table.c_str(), account_id)
...
static bool account_db_sql_load_str(AccountDB* self, struct mmo_account* acc, const char* userid) {
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id` FROM `%s` WHERE `userid`= %s '%s'",
		db->account_table.c_str(), (db->case_sensitive ? "BINARY" : ""), esc_userid) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
static bool account_db_sql_iter_next(AccountDBIterator* self, struct mmo_account* acc) {
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id` FROM `%s` WHERE `account_id` > '%d' ORDER BY `account_id` ASC LIMIT 1",
		db->account_table.c_str(), iter->last_account_id) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
#endif
		db->account_table.c_str(), account_id )
	) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
#endif
			db->account_table.c_str(), acc->account_id)
	...
	if (count) {
		...
		for (i = 0; i < count; i++) {
			...
			switch (RFIFOB(fd, cursor++)) {
				// int
				case 0:
					if( SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%" PRIu32 "','%s','%" PRIu32 "','%" PRId64 "')", db->global_acc_reg_num_table.c_str(), account_id, esc_key, index, RFIFOQ(fd, cursor)) )
						Sql_ShowDebug(sql_handle);
					cursor += 8;
					break;
				case 1:
					if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%" PRIu32 "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1", db->global_acc_reg_num_table.c_str(), account_id, esc_key, index) )
						Sql_ShowDebug(sql_handle);
					break;
				// str
				case 2:
					...
					if( SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%" PRIu32 "','%s','%" PRIu32 "','%s')", db->global_acc_reg_str_table.c_str(), account_id, esc_key, index, esc_sval) )
						Sql_ShowDebug(sql_handle);
					break;
				case 3:
					if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%" PRIu32 "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1", db->global_acc_reg_str_table.c_str(), account_id, esc_key, index) )
						Sql_ShowDebug(sql_handle);
					break;
				default:
...
void mmo_send_global_accreg(AccountDB* self, int fd, uint32 account_id, uint32 char_id) {
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'", db->global_acc_reg_str_table.c_str(), account_id) )
		Sql_ShowDebug(sql_handle);
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'", db->global_acc_reg_num_table.c_str(), account_id) )
		Sql_ShowDebug(sql_handle);
