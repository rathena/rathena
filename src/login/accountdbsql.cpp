...
bool AccountDBSql::init() {
	...
	if (SQL_ERROR == Sql_Connect(sql_handle, db_username_.c_str(), db_password_.c_str(),
								 db_hostname_.c_str(), db_port_, db_database_.c_str())) {
...
bool AccountDBSql::create(struct mmo_account& acc) {
	...
	if (acc.account_id != -1) {
		...
		if (SQL_SUCCESS !=
			Sql_Query(sql_handle, "SELECT MAX(`account_id`)+1 FROM `%s`", account_db_.c_str())) {
			Sql_ShowDebug(sql_handle);
			return false;
		}
...
bool AccountDBSql::remove(const uint32 account_id) {
	...
	if (SQL_SUCCESS != Sql_QueryStr(sql_handle, "START TRANSACTION") ||
		SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = %d",
								 account_db_.c_str(), account_id) ||
		SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = %d",
								 global_acc_reg_num_table_.c_str(), account_id) ||
		SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = %d",
								 global_acc_reg_str_table_.c_str(), account_id))
		Sql_ShowDebug(sql_handle);
	else
		result = true;
...
/**
 * Retrieve data from db and store it in the provided data structure.
 *  Doesn't actually retrieve data yet: escapes and checks userid, then transforms it to accid for
 * fetching. Filled data structure is done by delegation to load_num.
 * @param acc: pointer of mmo_account to fill
...
bool AccountDBSql::load_str(struct mmo_account& acc, const char* userid) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id` FROM `%s` WHERE `userid`= %s '%s'",
							   account_db_.c_str(), (case_sensitive_ ? "BINARY" : ""),
							   esc_userid)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (Sql_NumRows(sql_handle) > 1) {
		...
		ShowError(
			"account_db_sql_load_str: multiple accounts found when retrieving data for account "
			"'%s'!\n",
			userid);
		Sql_FreeResult(sql_handle);
		return false;
...
bool AccountDBSql::from_sql(struct mmo_account& acc, uint32 account_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(
			sql_handle,
			"SELECT "
			"`account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`"
			"expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`"
			"pincode`, `pincode_change`, `vip_time`, `old_group` FROM `%s` WHERE `account_id` = %d",
			account_db_.c_str(), account_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
		...
		if (is_new) {  // insert into account table
			if (SQL_SUCCESS !=
					SqlStmt_Prepare(
						...
						"`lastlogin`, `last_ip`, `birthdate`, `character_slots`, `pincode`, "
						"`pincode_change`, `vip_time`, `old_group` ) VALUES (?, ?, ?, ?, ?, ?, ?, "
						"?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
						account_db_.c_str()) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_INT, (void*)&acc.account_id,
												 sizeof(acc.account_id)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (void*)acc.userid,
												 strlen(acc.userid)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 2, SQLDT_STRING, (void*)acc.pass, strlen(acc.pass)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 3, SQLDT_ENUM, (void*)&acc.sex, sizeof(acc.sex)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 4, SQLDT_STRING, (void*)&acc.email,
												 strlen(acc.email)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 5, SQLDT_INT, (void*)&acc.group_id,
												 sizeof(acc.group_id)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 6, SQLDT_UINT, (void*)&acc.state, sizeof(acc.state)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 7, SQLDT_LONG, (void*)&acc.unban_time,
												 sizeof(acc.unban_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 8, SQLDT_INT, (void*)&acc.expiration_time,
												 sizeof(acc.expiration_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 9, SQLDT_UINT, (void*)&acc.logincount,
												 sizeof(acc.logincount)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 10,
												 acc.lastlogin[0] ? SQLDT_STRING : SQLDT_NULL,
												 (void*)&acc.lastlogin, strlen(acc.lastlogin)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 11, SQLDT_STRING, (void*)&acc.last_ip,
												 strlen(acc.last_ip)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 12,
												 acc.birthdate[0] ? SQLDT_STRING : SQLDT_NULL,
												 (void*)&acc.birthdate, strlen(acc.birthdate)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 13, SQLDT_UCHAR, (void*)&acc.char_slots,
												 sizeof(acc.char_slots)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 14, SQLDT_STRING, (void*)&acc.pincode,
												 strlen(acc.pincode)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 15, SQLDT_LONG, (void*)&acc.pincode_change,
												 sizeof(acc.pincode_change)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 16, SQLDT_LONG, (void*)&acc.vip_time,
												 sizeof(acc.vip_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 17, SQLDT_INT, (void*)&acc.old_group,
												 sizeof(acc.old_group)) ||
			...
			if (SQL_SUCCESS !=
					SqlStmt_Prepare(
						stmt,
						"UPDATE `%s` SET "
						"`userid`=?,`user_pass`=?,`sex`=?,`email`=?,`group_id`=?,`state`=?,`unban_"
						"time`=?,`expiration_time`=?,`logincount`=?,`lastlogin`=?,`last_ip`=?,`"
						"birthdate`=?,`character_slots`=?,`pincode`=?, `pincode_change`=?, "
						"`vip_time`=?, `old_group`=? WHERE `account_id` = '%d'",
						account_db_.c_str(), acc.account_id) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, (void*)acc.userid,
												 strlen(acc.userid)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (void*)acc.pass, strlen(acc.pass)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 2, SQLDT_ENUM, (void*)&acc.sex, sizeof(acc.sex)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 3, SQLDT_STRING, (void*)acc.email, strlen(acc.email)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 4, SQLDT_INT, (void*)&acc.group_id,
												 sizeof(acc.group_id)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 5, SQLDT_UINT, (void*)&acc.state, sizeof(acc.state)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 6, SQLDT_LONG, (void*)&acc.unban_time,
												 sizeof(acc.unban_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 7, SQLDT_LONG, (void*)&acc.expiration_time,
												 sizeof(acc.expiration_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 8, SQLDT_UINT, (void*)&acc.logincount,
												 sizeof(acc.logincount)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 9,
												 acc.lastlogin[0] ? SQLDT_STRING : SQLDT_NULL,
												 (void*)&acc.lastlogin, strlen(acc.lastlogin)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 10, SQLDT_STRING, (void*)&acc.last_ip,
												 strlen(acc.last_ip)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 11,
												 acc.birthdate[0] ? SQLDT_STRING : SQLDT_NULL,
												 (void*)&acc.birthdate, strlen(acc.birthdate)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 12, SQLDT_UCHAR, (void*)&acc.char_slots,
												 sizeof(acc.char_slots)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 13, SQLDT_STRING, (void*)&acc.pincode,
												 strlen(acc.pincode)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 14, SQLDT_LONG, (void*)&acc.pincode_change,
												 sizeof(acc.pincode_change)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 15, SQLDT_LONG, (void*)&acc.vip_time,
												 sizeof(acc.vip_time)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 16, SQLDT_INT, (void*)&acc.old_group,
												 sizeof(acc.old_group)) ||
				SQL_SUCCESS != SqlStmt_Execute(stmt)) {
				SqlStmt_ShowDebug(stmt);
				break;
		...
		if (acc.sex != 'S' && login_config.use_web_auth_token && refresh_token) {
			...
			while (!initialized) {
				if (SQL_SUCCESS == Sql_Query(sql_handle, "SELECT SHA2( 'test', 256 )")) {
					query =
						"UPDATE `%s` SET `web_auth_token` = LEFT( SHA2( CONCAT( UUID(), RAND() ), "
						"256 ), %d ), `web_auth_token_enabled` = '1' WHERE `account_id` = '%d'";
					initialized = true;
					break;
				}
				...
				if (SQL_SUCCESS == Sql_Query(sql_handle, "SELECT MD5( 'test' )")) {
					query =
						"UPDATE `%s` SET `web_auth_token` = LEFT( MD5( CONCAT( UUID(), RAND() ) ), "
						"%d ), `web_auth_token_enabled` = '1' WHERE `account_id` = '%d'";
					initialized = true;
					break;
				}

				ShowWarning(
					"Your MySQL does not support SHA2 and MD5 - no hashing will be used for login "
					"token creation.\n");
				ShowWarning(
					"If you are using an old version of MySQL consider upgrading to a newer "
					"release.\n");
				query =
					"UPDATE `%s` SET `web_auth_token` = LEFT( CONCAT( UUID(), RAND() ), %d ), "
			...
			do {
				if (SQL_SUCCESS == Sql_Query(sql_handle, query, account_db_.c_str(),
											 WEB_AUTH_TOKEN_LENGTH - 1, acc.account_id)) {
					success = true;
					break;
				}
		...
		for (i = 0; i < count; i++) {
			...
			switch (RFIFOB(fd, cursor++)) {
				...
				case 0:
					if (SQL_ERROR ==
						Sql_Query(sql_handle,
								  "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES "
								  "('%" PRIu32 "','%s','%" PRIu32 "','%" PRId64 "')",
								  global_acc_reg_num_table_.c_str(), account_id, esc_key, index,
								  RFIFOQ(fd, cursor)))
						Sql_ShowDebug(sql_handle);
					cursor += 8;
					break;
				...
				case 2:
					...
					if (SQL_ERROR ==
						Sql_Query(sql_handle,
								  "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES "
								  "('%" PRIu32 "','%s','%" PRIu32 "','%s')",
								  global_acc_reg_str_table_.c_str(), account_id, esc_key, index,
								  esc_sval))
						Sql_ShowDebug(sql_handle);
					break;
				case 3:
...
void AccountDBSql::send_global_accreg(int fd, uint32 account_id, uint32 char_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'",
				  global_acc_reg_str_table_.c_str(), account_id))
		Sql_ShowDebug(sql_handle);
	...
	while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		...
		if (plen > 60000) {
			...
			WFIFOL(fd, 8) = char_id;
			WFIFOB(fd, 12) =
				0;	// var type (only set when all vars have been sent, regardless of type)
			WFIFOB(fd, 13) = 1;	 // is string type
			WFIFOW(fd, 14) = 0;	 // count
			plen = 16;
		}
...
bool AccountDBSql::enable_webtoken(const uint32 account_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "UPDATE `%s` SET `web_auth_token_enabled` = '1' WHERE `account_id` = '%u'",
				  account_db_.c_str(), account_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
bool AccountDBSql::disable_webtoken_timer(const uint32 account_id) {
	add_timer(gettick() + login_config.disable_webtoken_delay, account_disable_webtoken_timer,
			  account_id, reinterpret_cast<intptr_t>(this));
	return true;
}
...
int AccountDBSql::disable_webtoken(const uint32 account_id) {
	...
	if (p == nullptr) {
		...
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "UPDATE `%s` SET `web_auth_token_enabled` = '0' WHERE `account_id` = '%u'",
					  account_db_.c_str(), account_id)) {
			Sql_ShowDebug(sql_handle);
			return 0;
		}
...
bool AccountDBSql::remove_webtokens() {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "UPDATE `%s` SET `web_auth_token` = NULL, `web_auth_token_enabled` = '0'",
				  account_db_.c_str())) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
Sql* AccountDBSql::get_handle() { return accounts_; }
