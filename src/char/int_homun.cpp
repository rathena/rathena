	...
	if (hd->hom_id == 0) { // new homunculus
		...
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  ...
					  "`luk`='%d', `hp`='%u',`max_hp`='%u',`sp`='%u',`max_sp`='%u',`skill_point`='%d', "
			...
			// Save skills
			if (SQL_ERROR == stmt.Prepare("REPLACE INTO `%s` (`homun_id`, `id`, `lv`) VALUES (%d, ?, ?)",
			...
			if (SQL_ERROR == stmt.Prepare("INSERT INTO `%s` (`homun_id`, `skill`, `tick`) VALUES (%d, ?, ?)",
...
bool mapif_homunculus_load(int32 homun_id, struct s_homunculus* hd) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, "
				  "`agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`rename_flag`, "
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `id`,`lv` FROM `%s` WHERE `homun_id`=%d",
							   schema_config.skill_homunculus_db,
							   homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `skill`,`tick` FROM `%s` WHERE `homun_id`=%d",
							   schema_config.skillcooldown_homunculus_db,
							   homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (count > 0) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "DELETE FROM `%s` WHERE `homun_id`='%d'",
								   schema_config.skillcooldown_homunculus_db,
								   homun_id)) {
			Sql_ShowDebug(sql_handle);
		}
...
bool mapif_homunculus_delete(int32 homun_id) {
	if (SQL_ERROR ==
		...
		SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `homun_id` = '%u'",
							   schema_config.skillcooldown_homunculus_db,
							   homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
...
int32 inter_homunculus_parse_frommap(int32 fd) {
	...
	switch (cmd) {
		case 0x3090:
			mapif_parse_homunculus_create(
				fd, (int32)RFIFOW(fd, 2), (int32)RFIFOL(fd, 4), (struct s_homunculus*)RFIFOP(fd, 8));
			break;
		case 0x3091:
			mapif_parse_homunculus_load(fd, (int32)RFIFOL(fd, 2), (int32)RFIFOL(fd, 6));
			break;
		case 0x3092:
			mapif_parse_homunculus_save(
				fd, (int32)RFIFOW(fd, 2), (int32)RFIFOL(fd, 4), (struct s_homunculus*)RFIFOP(fd, 8));
			break;
		case 0x3093:
			mapif_parse_homunculus_delete(fd, (int32)RFIFOL(fd, 2));
			break;
		case 0x3094:
			mapif_parse_homunculus_rename(fd, (int32)RFIFOL(fd, 2), (int32)RFIFOL(fd, 6), RFIFOCP(fd, 10));
			break;
		default:
			return 0;
	}
