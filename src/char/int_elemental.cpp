	...
	if (ele->elemental_id == 0) { // Create new DB entry
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "INSERT INTO `%s` "
					  "(`char_id`,`class`,`mode`,`hp`,`sp`,`max_hp`,`max_sp`,`atk1`,`atk2`,`matk`,`aspd`,`def`,`mdef`,`"
					  "flee`,`hit`,`life_time`)"
...
int32 inter_elemental_parse_frommap(int32 fd) {
	...
	switch (cmd) {
		case 0x307c:
			...
			break;
		case 0x307d:
			mapif_parse_elemental_load(fd, (int32)RFIFOL(fd, 2), (int32)RFIFOL(fd, 6));
			break;
		case 0x307e:
			mapif_parse_elemental_delete(fd, (int32)RFIFOL(fd, 2));
			break;
		case 0x307f:
