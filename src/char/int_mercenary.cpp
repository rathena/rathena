...
bool mercenary_owner_delete(uint32 char_id) {
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "DELETE FROM `%s` WHERE `mer_id` IN ( SELECT `merc_id` FROM `%s` WHERE `char_id` = '%d' )",
...
bool mapif_mercenary_save(struct s_mercenary* merc) {
	if (merc->mercenary_id == 0) { // Create new DB entry
		if (SQL_ERROR == Sql_Query(sql_handle,
			...
			merc->mercenary_id = (int32)Sql_LastInsertId(sql_handle);
									...
									merc->kill_count,
									merc->life_time,
									merc->mercenary_id)) { // Update DB entry
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == stmt.Prepare("INSERT INTO `%s` (`mer_id`, `skill`, `tick`) VALUES (%d, ?, ?)",
								  schema_config.skillcooldown_mercenary_db,
								  merc->mercenary_id)) {
	...
	for (uint16 i = 0; i < MAX_SKILLCOOLDOWN; ++i) {
		...
		if (SQL_ERROR == stmt.BindParam(0, SQLDT_USHORT, &merc->scd[i].skill_id, 0) ||
			SQL_ERROR == stmt.BindParam(1, SQLDT_LONGLONG, &merc->scd[i].tick, 0) || SQL_ERROR == stmt.Execute()) {
			SqlStmt_ShowDebug(stmt);
			return false;
		}
	}
...
bool mapif_mercenary_load(int32 merc_id, uint32 char_id, struct s_mercenary* merc) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `class`, `hp`, `sp`, `kill_counter`, `life_time` FROM `%s` WHERE `mer_id` = "
							   "'%d' AND `char_id` = '%d'",
							   schema_config.mercenary_db,
							   merc_id,
							   char_id)) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `skill`,`tick` FROM `%s` WHERE `mer_id`=%d",
							   schema_config.skillcooldown_mercenary_db,
							   merc_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
...
int32 inter_mercenary_parse_frommap(int32 fd) {
	...
	switch (cmd) {
		case 0x3070:
			...
			break;
		case 0x3071:
			mapif_parse_mercenary_load(fd, (int32)RFIFOL(fd, 2), (int32)RFIFOL(fd, 6));
			break;
		case 0x3072:
			mapif_parse_mercenary_delete(fd, (int32)RFIFOL(fd, 2));
			break;
		case 0x3073:
