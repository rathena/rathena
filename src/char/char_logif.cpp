...
int32 chlogif_parse_ackconnect(int32 fd) {
	...
	if (RFIFOB(fd, 2)) {
		...
		ShowError(
			"Also, please make sure your login db has the correct communication username/passwords and the gender of "
			"the account is S.\n");
...
void chlogif_parse_change_sex_sub(int32 sex, int32 acc, int32 char_id, int32 class_, int32 guild_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "UPDATE `%s` SET `class` = '%d', `weapon` = '0', `shield` = '0', `head_top` = '0', "
							   "`head_mid` = '0', `head_bottom` = '0', `sex` = '%c' WHERE `char_id` = '%d'",
...
int32 chlogif_parse_ackchangesex(int32 fd) {
	if (RFIFOREST(fd) < 7) {
		...
		if (acc > 0) { // TODO: Is this even possible?
			...
			// get characters
			if (SQL_ERROR == stmt.Prepare("SELECT `char_id`, `class`, `guild_id` FROM `%s` WHERE `account_id` = '%d'",
										  schema_config.char_db,
										  acc) ||
				stmt.Execute()) {
		...
		if (character != nullptr) {
			if (character->server > -1) { // Kick it from the map server it is on.
				...
				ARR_FIND(0,
						 fd_max,
						 i,
						 session[i] && (tsd = (struct char_session_data*)session[i]->session_data) &&
							 tsd->account_id == aid);
				if (i < fd_max) {
