...
TIMER_FUNC(guild_save_timer) {
	...
	int32 state = 0; // 0: Have not reached last guild. 1: Reached last guild, ready for save. 2: Some guild saved,
					 // don't do further saving.
	...
	if (((flag & GS_BASIC_MASK) && !new_guild) || ((flag & (GS_BASIC_MASK & ~GS_BASIC)) && new_guild)) {
		...
		if (flag & GS_LEVEL) {
			...
			StringBuf_Printf(&buf,
							 "`guild_lv`=%d, `skill_point`=%d, `exp`=%" PRIu64 ", `next_exp`=%" PRIu64
							 ", `max_member`=%d",
							 g.guild_lv,
							 g.skill_point,
							 g.exp,
	...
	if (flag & GS_EXPULSION) {
		...
		for (i = 0; i < MAX_GUILDEXPULSION; i++) {
			...
			if (e->account_id > 0) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle,
										   "REPLACE INTO `%s` (`guild_id`,`account_id`,`name`,`mes`,`char_id`) "
										   "VALUES ('%u','%u','%s','%s','%u')",
...
std::shared_ptr<CharGuild> inter_guild_fromsql(int32 guild_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "g.`name`,c.`name`,g.`guild_lv`,g.`connect_member`,g.`max_member`,g.`average_lv`,g.`exp`"
							   ",g.`next_exp`,g.`skill_point`,g.`mes1`,g.`mes2`,g.`emblem_len`,g.`emblem_id`,COALESCE("
							   "UNIX_TIMESTAMP(g.`last_master_change`),0), g.`emblem_data` "
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT "
				  "`c`.`account_id`,`m`.`char_id`,`c`.`hair`,`c`.`hair_color`,`c`.`sex`,`c`.`class`,`c`.`base_level`,`"
				  "m`.`exp`,`c`.`online`,`m`.`position`,`c`.`name`,coalesce(UNIX_TIMESTAMP(`c`.`last_login`),0) "
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `account_id`,`name`,`mes`,`char_id` FROM `%s` WHERE `guild_id`='%d'",
							   schema_config.guild_expulsion_db,
							   guild_id)) {
		Sql_ShowDebug(sql_handle);
		return nullptr;
		...
		if (g->guild.member[i].account_id > 0) {
			if (g->guild.member[i].lv >= 0) {
				...
				ShowWarning("Guild %d:%s, member %d:%s has an invalid level %d\n",
							g->guild.guild_id,
							g->guild.name,
							g->guild.member[i].char_id,
							g->guild.member[i].name,
							g->guild.member[i].lv);
			}
...
// Send member info
int32 mapif_guild_memberinfochanged(
	int32 guild_id, uint32 account_id, uint32 char_id, int32 type, const void *data, int32 len) {
	unsigned char buf[2048];
	if (len > 2030) {
		len = 2030;
	}
...
// ACK guild alliance
int32 mapif_guild_alliance(int32 guild_id1,
						   int32 guild_id2,
						   uint32 account_id1,
						   uint32 account_id2,
						   int32 flag,
...
int32 mapif_parse_GuildLeave(int32 fd, int32 guild_id, uint32 account_id, uint32 char_id, int32 flag, const char *mes) {
	...
	ARR_FIND(0,
			 g->guild.max_member,
			 i,
			 g->guild.member[i].account_id == account_id && g->guild.member[i].char_id == char_id);
	if (i == g->guild.max_member) {
...
// Change member info
int32 mapif_parse_GuildChangeMemberInfoShort(
	int32 fd, int32 guild_id, uint32 account_id, uint32 char_id, int32 online, int32 lv, int32 class_) {
...
int32 mapif_parse_BreakGuild(int32 fd, int32 guild_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(
			sql_handle, "UPDATE `%s` SET `guild_id`='0' WHERE `guild_id`='%d'", schema_config.char_db, guild_id)) {
		Sql_ShowDebug(sql_handle);
	}
 ...
 * @return
 */
int32 mapif_parse_GuildMemberInfoChange(
	int32 fd, int32 guild_id, uint32 account_id, uint32 char_id, int32 type, const char *data, int32 len) {
...
int32 mapif_parse_GuildDeleteAlliance(
	std::shared_ptr<CharGuild> &g, int32 guild_id, uint32 account_id1, uint32 account_id2, int32 flag) {
	int32 i;
 ...
 * @return
 */
int32 mapif_parse_GuildAlliance(
	int32 fd, int32 guild_id1, int32 guild_id2, uint32 account_id1, uint32 account_id2, int32 flag) {
	// Could speed up
	...
	if (g[0] != nullptr && g[1] == nullptr && (flag & GUILD_ALLIANCE_REMOVE)) {
		return mapif_parse_GuildDeleteAlliance(
			g[0], guild_id2, account_id1, account_id2, flag); // Try to do a manual removal of said guild.
	}
...
int32 inter_guild_parse_frommap(int32 fd) {
	switch (RFIFOW(fd, 0)) {
		...
		case 0x303F:
			...
			break;
		case 0x3040:
			mapif_parse_GuildCastleDataLoad(fd, RFIFOW(fd, 2), (int32 *)RFIFOP(fd, 4));
			break;
		case 0x3041:
