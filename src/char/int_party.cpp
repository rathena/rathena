...
int32 mapif_parse_PartyLeave(
	int32 fd, int32 party_id, uint32 account_id, uint32 char_id, char *name, enum e_party_member_withdraw type);
...
std::shared_ptr<struct party_data> inter_party_fromsql(int32 party_id) {
	...
	// Load members
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `account_id`,`char_id`,`name`,`base_level`,`last_map`,`online`,`class` FROM "
							   "`%s` WHERE `party_id`='%d'",
							   schema_config.char_db,
							   party_id)) {
		Sql_ShowDebug(sql_handle);
...
int32 mapif_parse_PartyChangeMap(
	int32 fd, int32 party_id, uint32 account_id, uint32 char_id, int32 online, uint32 lv, const char *map) {
	int32 i;
