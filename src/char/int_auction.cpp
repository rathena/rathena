...
void auction_save(std::shared_ptr<struct auction_data> auction) {
	...
	StringBuf_Printf(&buf,
					 ...
					 auction->buynow,
					 auction->hours,
					 (unsigned long)auction->timestamp,
					 auction->item.nameid,
					 auction->type,
					 auction->item.refine,
					 auction->item.attribute,
					 auction->item.enchantgrade);
	...
	if (SQL_SUCCESS != stmt.PrepareStr(StringBuf_Value(&buf)) ||
		SQL_SUCCESS !=
			stmt.BindParam(0, SQLDT_STRING, auction->seller_name, strnlen(auction->seller_name, NAME_LENGTH)) ||
		SQL_SUCCESS !=
			stmt.BindParam(1, SQLDT_STRING, auction->buyer_name, strnlen(auction->buyer_name, NAME_LENGTH)) ||
		SQL_SUCCESS !=
			stmt.BindParam(2, SQLDT_STRING, auction->item_name, strnlen(auction->item_name, ITEM_NAME_LENGTH)) ||
...
uint32 auction_create(std::shared_ptr<struct auction_data> auction) {
	...
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` "
					 "(`seller_id`,`seller_name`,`buyer_id`,`buyer_name`,`price`,`buynow`,`hours`,`timestamp`,`nameid`,"
					 "`item_name`,`type`,`refine`,`attribute`,`unique_id`,`enchantgrade`",
					 schema_config.auction_db);
	for (j = 0; j < MAX_SLOTS; j++) {
	...
	StringBuf_Printf(&buf,
					 ") VALUES ('%d',?,'%d',?,'%d','%d','%d','%lu','%u',?,'%d','%d','%d','%" PRIu64 "','%d'",
					 auction->seller_id,
					 auction->buyer_id,
					 auction->price,
					 auction->buynow,
...
void inter_auctions_fromsql(void) {
	...
	StringBuf_AppendStr(&buf,
						...
						"`price`,`buynow`,`hours`,`timestamp`,`nameid`,`item_name`,`type`,`refine`,`attribute`,`unique_"
						"id`,`enchantgrade`");
