...
void inter_storage_checkDB(void) {
	...
	for (auto storage_table : interServerDb) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   ...
								   "`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_"
								   "parm0`,`option_id1`,`option_val1`,`option_parm1`,"
...
bool mapif_parse_itembound_retrieve(int32 fd) {
	...
	StringBuf_AppendStr(&buf,
						"SELECT `id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, "
	...
	ShowInfo("Found '" CL_WHITE "%d" CL_RESET "' guild bound item(s) from CID = " CL_WHITE "%d" CL_RESET
	...
	StringBuf_Printf(&buf,
					 "DELETE FROM `%s` WHERE `char_id` = %d AND `bound` = %d",
	...
	if (j) {
		...
		if (SQL_ERROR == stmt.PrepareStr(StringBuf_Value(&buf2)) || SQL_ERROR == stmt.Execute()) {
			SqlStmt_ShowDebug(stmt);
			StringBuf_Destroy(&buf);
