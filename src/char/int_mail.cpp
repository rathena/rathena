...
void mapif_Mail_return(
	int32 fd, uint32 char_id, int32 mail_id, uint32 account_id_receiver = 0, uint32 account_id_sender = 0);
...
int32 mail_savemessage(struct mail_message* msg) {
	...
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`send_name`, `send_id`, `dest_name`, `dest_id`, `title`, `message`, `time`, "
					 "`status`, `zeny`,`type`",
					 schema_config.mail_db);
	StringBuf_Printf(&buf,
					 ") VALUES (?, '%d', ?, '%d', ?, ?, '%lu', '%d', '%d', '%d'",
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`id`, `index`, `amount`, `nameid`, `refine`, `attribute`, `identify`, "
	...
	for (i = 0; i < MAIL_MAX_ITEM; i++) {
		...
		StringBuf_Printf(&buf,
						 "('%" PRIu64 "', '%hu', '%d', '%u', '%d', '%d', '%d', '%" PRIu64 "', '%d', '%d'",
						 (uint64)msg->id,
						 i,
						 msg->item[i].amount,
						 msg->item[i].nameid,
						 msg->item[i].refine,
						 msg->item[i].attribute,
						 msg->item[i].identify,
						 msg->item[i].unique_id,
						 msg->item[i].bound,
						 msg->item[i].enchantgrade);
		for (j = 0; j < MAX_SLOTS; j++) {
...
bool mail_loadmessage(int32 mail_id, struct mail_message* msg) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "`id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,`time`,`status`,`"
							   "zeny`,`type` FROM `%s` WHERE `id` = '%d'",
							   schema_config.mail_db,
							   mail_id) ||
	...
	StringBuf_Init(&buf);
	StringBuf_AppendStr(&buf,
						"SELECT `amount`,`nameid`,`refine`,`attribute`,`identify`,`unique_id`,`bound`,`enchantgrade`");
	for (j = 0; j < MAX_SLOTS; j++) {
		StringBuf_Printf(&buf, ",`card%d`", j);
	}
	for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
...
int32 mail_timer_sub(int32 limit, enum mail_inbox_type type) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s`WHERE `type` = '%d' AND `send_id` = '0' AND `time` <= UNIX_TIMESTAMP( "
							   "NOW() - INTERVAL %d DAY )",
							   schema_config.mail_db,
							   type,
							   limit)) {
...
void mapif_parse_Mail_send(int32 fd) {
	...
	if (msg.dest_id != 0) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT `char_id`, `name` FROM `%s` WHERE `char_id` = '%u'",
								   schema_config.char_db,
								   msg.dest_id)) {
			Sql_ShowDebug(sql_handle);
			return;
...
bool mail_sendmail(int32 send_id,
...
void mapif_Mail_receiver_send(
	int32 fd, int32 requesting_char_id, int32 char_id, int32 class_, int32 base_level, const char* name) {
	WFIFOHEAD(fd, 38);
	WFIFOW(fd, 0) = 0x384e;
...
void mapif_parse_Mail_receiver_check(int32 fd) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `char_id`,`class`,`base_level` FROM `%s` WHERE `name` = '%s'",
							   schema_config.char_db,
							   esc_name)) {
		Sql_ShowDebug(sql_handle);
	}
