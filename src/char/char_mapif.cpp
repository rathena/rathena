...
int32 chmapif_parse_getmapname(int32 fd, int32 id) {
	...
	ShowStatus("Map-Server %d connected: %" PRIuPTR " maps, from IP %d.%d.%d.%d port %d.\n",
			   id,
			   map_server[id].maps.size(),
			   CONVIP(map_server[id].ip),
			   map_server[id].port);
	...
	if (RFIFOREST(fd) < 10) {
		...
		if (Sql_NumRows(sql_handle) > 0) {
			...
			for (count = 0; count < 50 && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
				...
				memcpy(WFIFOP(fd, 14 + count * sizeof(struct status_change_data)),
					   &scdata,
	...
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2)) {
		...
		if (count > 0) {
			...
			StringBuf_Printf(&buf,
							 "INSERT INTO `%s` (`account_id`, `char_id`, `skill`, `tick`) VALUES ",
...
int32 chmapif_parse_req_skillcooldown(int32 fd) {
	if (RFIFOREST(fd) < 10) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT skill, tick FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
								   schema_config.skillcooldown_db,
								   aid,
								   cid)) {
			Sql_ShowDebug(sql_handle);
...
int32 chmapif_parse_reqchangemapserv(int32 fd) {
	if (RFIFOREST(fd) < (37 + MAP_NAME_LENGTH_EXT)) {
		...
		map_id = char_search_mapserver(RFIFOCP(fd, 18),
									   ntohl(RFIFOL(fd, offset + 4)),
									   ntohs(RFIFOW(fd, offset + 8))); // Locate mapserver by ip and port.
		if (map_id >= 0) {
...
int32 chmapif_parse_askrmfriend(int32 fd) {
	if (RFIFOREST(fd) < 10) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "DELETE FROM `%s` WHERE `char_id`='%d' AND `friend_id`='%d' LIMIT 1",
...
int32 chmapif_parse_fwlog_changestatus(int32 fd) {
	if (RFIFOREST(fd) < 44) {
		...
		int32 result = 0; // 0-login-server request done, 1-player not found, 2-gm level too low, 3-login-server
						  // offline, 4-current group level > VIP group level
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT `account_id`, `char_id` FROM `%s` WHERE `name` = '%s'",
			...
			//		result = 2; // 2-gm level too low
				...
				switch (operation) {
					...
					case CHRIF_OP_LOGIN_VIP: // vip
						answer =
							(val1 &
							 4); // vip_req val1=type, &1 login send return, &2 update timestamp, &4 map send answer
...
int32 chmapif_parse_reqcharban(int32 fd) {
	if (RFIFOREST(fd) < 10 + NAME_LENGTH) {
		...
		Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT `account_id`,`char_id`,`unban_time` FROM `%s` WHERE `name` = '%s'",
								   schema_config.char_db,
								   esc_name)) {
			Sql_ShowDebug(sql_handle);
		}
			...
			if (SQL_SUCCESS != stmt.Prepare("UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1",
											schema_config.char_db) ||
...
int32 chmapif_parse_reqcharunban(int32 fd) {
	if (RFIFOREST(fd) < 6 + NAME_LENGTH) {
		...
		Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "UPDATE `%s` SET `unban_time` = '0' WHERE `name` = '%s' LIMIT 1",
								   schema_config.char_db,
								   esc_name)) {
			Sql_ShowDebug(sql_handle);
			return 1;
...
int32 chmapif_bonus_script_get(int32 fd) {
	if (RFIFOREST(fd) < 6) {
		...
		if (SQL_ERROR ==
			...
			SQL_ERROR ==
				stmt.BindColumn(
					0, SQLDT_STRING, &tmp_bsdata.script_str, sizeof(tmp_bsdata.script_str), nullptr, nullptr) ||
		...
		if ((num_rows = (uint8)stmt.NumRows()) > 0) {
			...
			if (SQL_ERROR ==
					stmt.Prepare("DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.bonus_script_db, cid) ||
				SQL_ERROR == stmt.Execute()) {
				SqlStmt_ShowDebug(stmt);
			}
		}
...
int32 chmapif_bonus_script_save(int32 fd) {
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2)) {
		...
		if (count > 0) {
			...
			StringBuf_Init(&buf);
			StringBuf_Printf(&buf,
							 "INSERT INTO `%s` (`char_id`, `script`, `tick`, `flag`, `type`, `icon`) VALUES ",
							 schema_config.bonus_script_db);
			for (i = 0; i < count; ++i) {
