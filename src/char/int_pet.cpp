...
int32 inter_pet_tosql(int32 pet_id, struct s_pet *p) {
	...
	if (pet_id == -1) { // New pet.
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s` "
								   "(`class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`hungry`"
								   ",`rename_flag`,`incubate`,`autofeed`) "
		...
		if (SQL_ERROR ==
			Sql_Query(
				sql_handle,
				"UPDATE `%s` SET "
				"`class`='%d',`name`='%s',`account_id`='%d',`char_id`='%d',`level`='%d',`egg_id`='%u',`equip`='%u',`"
				"intimate`='%d',`hungry`='%d',`rename_flag`='%d',`incubate`='%d',`autofeed`='%d' WHERE `pet_id`='%d'",
				schema_config.pet_db,
				p->class_,
				esc_name,
				p->account_id,
...
int32 inter_pet_fromsql(int32 pet_id, struct s_pet *p) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `pet_id`, "
							   "`class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`hungry`,`"
							   "rename_flag`,`incubate`,`autofeed` FROM `%s` WHERE `pet_id`='%d'",
							   schema_config.pet_db,
							   pet_id)) {
		Sql_ShowDebug(sql_handle);
		return 0;
...
int32 mapif_create_pet(int32 fd,
					   uint32 account_id,
					   uint32 char_id,
					   short pet_class,
					   short pet_lv,
