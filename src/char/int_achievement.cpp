...
struct achievement *mapif_achievements_fromsql(uint32 char_id, int32 *count) {
	...
	StringBuf_AppendStr(&buf,
						"SELECT `id`, COALESCE(UNIX_TIMESTAMP(`completed`),0), COALESCE(UNIX_TIMESTAMP(`rewarded`),0)");
...
int32 mapif_parse_achievement_reward(int32 fd) {
	...
	if (Sql_Query(sql_handle,
				  "UPDATE `%s` SET `rewarded` = FROM_UNIXTIME('%u') WHERE `char_id`='%u' AND `id` = '%d' AND "
				  "`completed` IS NOT NULL AND `rewarded` IS NULL",
				  schema_config.achievement_table,
				  (uint32)current,
				  char_id,
