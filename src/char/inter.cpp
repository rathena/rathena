...
const char *geoip_countryname[253] = {"Unknown",
									  ...
									  "Saint32 Vincent and the Grenadines",
									  "Venezuela",
...
void mapif_parse_accinfo(int32 fd) {
	...
	if (account_id < START_ACCOUNT_NUM) { // is string
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` FROM `%s` "
 ...
 **/
void inter_savereg(uint32 account_id,
				   uint32 char_id,
				   const char *key,
				   uint32 index,
				   int64 int_value,
...
int32 inter_accreg_fromsql(uint32 account_id, uint32 char_id, int32 fd, int32 type) {
	...
	switch (type) {
		...
		case 1: // account2 reg
			ShowError(
				"inter_accreg_fromsql: Char server shouldn't handle type 1 registry values (##). That is the login "
				"server's job!\n");
			return 0;
		default:
...
// broadcast sending
int32 mapif_broadcast(unsigned char *mes,
					  int32 len,
	...
	switch (cmd) {
		...
		default:
			if (inter_party_parse_frommap(fd) || inter_guild_parse_frommap(fd) || inter_storage_parse_frommap(fd) ||
				inter_pet_parse_frommap(fd) || inter_homunculus_parse_frommap(fd) ||
				inter_mercenary_parse_frommap(fd) || inter_elemental_parse_frommap(fd) ||
				inter_mail_parse_frommap(fd) || inter_auction_parse_frommap(fd) || inter_quest_parse_frommap(fd) ||
