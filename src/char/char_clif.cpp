...
std::vector<struct s_point_str> accessible_maps{s_point_str{MAP_PRONTERA, 116, 73},
...
int32 chclif_parse_moveCharSlot(int32 fd, struct char_session_data* sd) {
	...
	if ((charserv_config.charmove_config.char_moves_unlimited) == 0) {
		...
		Sql_Query(sql_handle,
				  "UPDATE `%s` SET `moves`='%d' WHERE `char_id`='%d'",
				  schema_config.char_db,
				  sd->char_moves[from],
				  sd->found_char[from]);
	}
...
int32 chclif_parse(int32 fd) {
	...
	while (RFIFOREST(fd) >= 2) {
			...
			switch (cmd) {
				...
				default:
					...
					if (sd != nullptr) {
						...
						// The pincode was not entered correctly, yet the player (=bot) tried to send a different packet
						// => Goodbye!
						set_eof(fd);
						return 0;
					}
						...
						ShowError("chclif_parse: Received unknown packet " CL_WHITE "0x%x" CL_RESET
		...
		switch (cmd) {
			...
			// unknown packet received
			default:
				ShowError("chclif_parse: Received unknown packet " CL_WHITE "0x%x" CL_RESET " from ip '" CL_WHITE
						  "%s" CL_RESET "'! Disconnecting!\n",
						  cmd,
						  ip2str(ipl, nullptr));
				set_eof(fd);
				return 0;
