...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->hotkey_rowshift2 != cp->hotkey_rowshift2) || (p->faction_id != cp->faction_id) || (p->infamy != cp->infamy)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			p->hotkey_rowshift, p->clan_id, p->title_id, p->show_equip, p->hotkey_rowshift2, p->faction_id, p->infamy,
	...
	if( memcmp(p->rep, cp->rep, sizeof(p->rep)) )
	{
		StringBuf_Clear(&buf);
		...
		for( i = 1, count = 0; i <= MAX_FACTION; ++i )
		{
			...
			if( count )
				StringBuf_AppendStr(&buf, ",");
			StringBuf_Printf(&buf, "('%d', '%d', '%d', '%d')", p->account_id, p->char_id, i, p->rep[i].value);
			++count;
...
int char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`faction_id`,`infamy`,`hotkey_rowshift2`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 45, SQLDT_INT,	&p.faction_id, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 46, SQLDT_UINT,   &p.infamy, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 47, SQLDT_UCHAR,  &p.hotkey_rowshift2, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`clan_id`,`title_id`,`show_equip`,`faction_id`,`infamy`,`hotkey_rowshift2`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 62, SQLDT_UINT,   &p->faction_id, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 63, SQLDT_UINT,   &p->infamy, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 64, SQLDT_UCHAR,  &p->hotkey_rowshift2, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `value` FROM `reputation` WHERE `char_id`=? ORDER by `faction_id` ASC")
	||	SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0)
	||	SQL_ERROR == SqlStmt_Execute(stmt)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_INT,  	&tmp_rep.value, 0, NULL, NULL) )
		SqlStmt_ShowDebug(stmt);

	
	p->rep[0].value = 0; // there will never be a faction_id == 0 so we nullify the first position of the array
	strncpy(p->rep[0].desc, "", sizeof(p->rep[0].desc)); // there will never be a faction_id == 0 so we nullify the first position of the array

	for( i = 1; i <= MAX_FACTION && SQL_SUCCESS == SqlStmt_NextRow(stmt); ++i )
	{
		memcpy(&p->rep[i].value, &tmp_rep.value, sizeof(tmp_rep.value));
		memcpy(&p->rep[i].desc, "", 12);
	}
