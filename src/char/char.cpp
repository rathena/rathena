...
void char_set_char_online(int map_id, uint32 char_id, uint32 account_id) {
	...
	//Update DB
	if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `online`='1', `last_login`=NOW() WHERE `char_id`='%d' LIMIT 1", charserv_table(char_table), char_id) )
...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	{	//Save friends
		...
		StringBuf_Clear(&buf);
		StringBuf_Printf(&buf, "INSERT INTO `%s` (`char_id`, `friend_id`) VALUES ", charserv_table(friend_table));
...
int char_memitemdata_to_sql(const struct item items[], int max, int id, enum storage_type tableswitch, uint8 stor_id) {
	...
	std::string tablename, selectoption, printname;
	...
	switch (tableswitch) {
		case TABLE_INVENTORY:
			printname = "Inventory";
			tablename = charserv_table(inventory_table);
			selectoption = "char_id";
			break;
		case TABLE_CART:
			printname = "Cart";
			tablename = charserv_table(cart_table);
			selectoption = "char_id";
			break;
		case TABLE_STORAGE:
		...
		case TABLE_GUILD_STORAGE:
			...
			tablename = charserv_table(guild_storage_table);
	...
	StringBuf_Printf(&buf, " FROM `%s` WHERE `%s`='%d'", tablename.c_str(), selectoption.c_str(), id);
	...
	while( SQL_SUCCESS == SqlStmt_NextRow(stmt) )
	{
			...
			&&  items[i].unique_id == item.unique_id
				...
				else
				{
					...
					StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%u', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `bound`='%d', `unique_id`='%" PRIu64 "'",
						tablename.c_str(), items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].bound, items[i].unique_id);
					if (tableswitch == TABLE_INVENTORY)
		...
		{// Item not present in inventory, remove it.
			if( SQL_ERROR == Sql_Query(sql_handle, "DELETE from `%s` where `id`='%d' LIMIT 1", tablename.c_str(), item.id) )
			{
				Sql_ShowDebug(sql_handle);
				errors++;
			}
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, `unique_id`", tablename.c_str(), selectoption.c_str());
	if (tableswitch == TABLE_INVENTORY)
		StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
	for( j = 0; j < MAX_SLOTS; ++j )
	...
	ShowInfo("Saved %s (%d) data to table %s for %s: %d\n", printname.c_str(), stor_id, tablename.c_str(), selectoption.c_str(), id);
	StringBuf_Destroy(&buf);
	aFree(flag);
...
bool char_memitemdata_from_sql(struct s_storage* p, int max, int id, enum storage_type tableswitch, uint8 stor_id) {
	...
	StringBuf_Printf(&buf, " FROM `%s` WHERE `%s`=? ORDER BY `nameid`", tablename.c_str(), selectoption.c_str());
	...
	p->amount = i;
	ShowInfo("Loaded %s data from table %s for %s: %d (total: %d)\n", printname.c_str(), tablename.c_str(), selectoption.c_str(), id, p->amount);
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN `%s` f ON f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d", charserv_table(char_table), charserv_table(friend_table), MAX_FRIENDS)
	||	SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0)
	||	SQL_ERROR == SqlStmt_Execute(stmt)
...
int char_make_new_char( struct char_session_data* sd, char* name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style, short start_job, int sex ){
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
		...
		charserv_table(char_table), sd->account_id , slot, esc_name, start_job, charserv_config.start_zeny, status_points, str, agi, vit, int_, dex, luk,
	...
	for (k = 0; k <= MAX_STARTITEM && tmp_start_items[k].nameid != 0; k++) {
		if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, `identify`) VALUES ('%d', '%u', '%hu', '%u', '%d')", charserv_table(inventory_table), char_id, tmp_start_items[k].nameid, tmp_start_items[k].amount, tmp_start_items[k].pos, 1) )
			Sql_ShowDebug(sql_handle);
	}
...
int char_divorce_char_sql(int partner_id1, int partner_id2){
	if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `partner_id`='0' WHERE `char_id`='%d' OR `char_id`='%d' LIMIT 2", charserv_table(char_table), partner_id1, partner_id2) )
		Sql_ShowDebug(sql_handle);
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE (`nameid`='%u' OR `nameid`='%u') AND (`char_id`='%d' OR `char_id`='%d') LIMIT 2", charserv_table(inventory_table), WEDDING_RING_M, WEDDING_RING_F, partner_id1, partner_id2) )
		Sql_ShowDebug(sql_handle);
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id){
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `name`,`account_id`,`party_id`,`guild_id`,`base_level`,`homun_id`,`partner_id`,`father`,`mother`,`elemental_id`,`delete_date` FROM `%s` WHERE `account_id`='%u' AND `char_id`='%u'", charserv_table(char_table), sd->account_id, char_id)){
		Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` IN ( SELECT `id` FROM `%s` WHERE `dest_id`='%d' )", charserv_table(mail_attachment_table), charserv_table(mail_table), char_id))
		Sql_ShowDebug(sql_handle);
	...
	/* Quest Data */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", charserv_table(quest_table), char_id))
		Sql_ShowDebug(sql_handle);

	/* Achievement Data */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", charserv_table(achievement_table), char_id))
		Sql_ShowDebug(sql_handle);
...
void char_read_fame_list(void)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d", charserv_table(char_table), JOB_TAEKWON, JOB_BABY_TAEKWON, fame_list_size_taekwon) )
		Sql_ShowDebug(sql_handle);
...
TIMER_FUNC(char_clan_member_cleanup){
	...
	if( SQL_ERROR == Sql_Query( sql_handle, "UPDATE `%s` SET `clan_id`='0' WHERE `online`='0' AND `clan_id`<>'0' AND `last_login` IS NOT NULL AND `last_login` <= NOW() - INTERVAL %d DAY", charserv_table(char_table), charserv_config.clan_remove_inactive_days ) ){
		Sql_ShowDebug(sql_handle);
	}
...
bool char_checkdb(void){
	...
	std::string sqltable[] = {
		...
		charserv_table(auction_table), charserv_table(interlog_table),
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`account_id`,`char_num`,`name`,`class`,"
		...
		"`hotkey_rowshift2`"
		" FROM `%s` LIMIT 1;", charserv_table(char_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking charlog_db
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `guild_id`,`char_id`,`exp`,`position` FROM `%s` LIMIT 1;", charserv_table(guild_member_table)) ) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	//checking friend_db
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`friend_id` FROM `%s` LIMIT 1;", charserv_table(friend_table)) ) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking mail_db
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`index`,`nameid`,`amount`,`refine`,`attribute`,`identify`,"
			...
			" FROM `%s` LIMIT 1;", charserv_table(mail_attachment_table)) ){
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`id`,`count1`,`count2`,`count3`,`count4`,`count5`,`count6`,`count7`,`count8`,`count9`,`count10`,`completed`,`rewarded`"
		" FROM `%s` LIMIT 1;", charserv_table(achievement_table))) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	Sql_FreeResult(sql_handle);
...
#define SCHEMA_CONF(var,str) \
	...
	SCHEMA_CONF(clan_alliance_table, "clan_alliance_table")
	SCHEMA_CONF(clan_table, "clan_table")
	...
	SCHEMA_CONF(mail_attachment_table, "mail_attachment_table")
...
static void char_schema_config_init(void) {
	...
	schema_config.char_table = "char";
	schema_config.charlog_table = "charlog";
	schema_config.bonus_script_table = "bonus_script";
	schema_config.cart_table = "cart_inventory";
	schema_config.inventory_table = "inventory";
	schema_config.memo_table = "memo";
	schema_config.scdata_table = "sc_data";
	schema_config.skill_table = "skill";
	schema_config.skillcooldown_table = "skillcooldown";
	schema_config.storage_table = "storage";
	schema_config.friend_table = "friends";
	schema_config.hotkey_table = "hotkey";
	schema_config.quest_table = "quest";
	schema_config.pet_table = "pet";
	schema_config.elemental_table = "elemental";
	schema_config.party_table = "party";
	schema_config.auction_table = "auction";
	schema_config.achievement_table = "achievement";

	// Mail tables
	schema_config.mail_table = "mail";
	schema_config.mail_attachment_table = "mail_attachments";

	// Homunculus tables
	schema_config.homunculus_table = "homunculus";
	schema_config.homunculus_skill_table = "skill_homunculus";

	// Mercenary Tables
	schema_config.mercenary_table = "mercenary";
	schema_config.mercenary_owner_table = "mercenary_owner";

	// Guild tables
	schema_config.guild_table = "guild";
	schema_config.guild_alliance_table = "guild_alliance";
	schema_config.guild_castle_table = "guild_castle";
	schema_config.guild_expulsion_table = "guild_expulsion";
	schema_config.guild_member_table = "guild_member";
	schema_config.guild_position_table = "guild_position";
	schema_config.guild_skill_table = "guild_skill";
	schema_config.guild_storage_table = "guild_storage";

	// Clan tables
	schema_config.clan_table = "clan";
	schema_config.clan_alliance_table = "clan_alliance";

	// Other
	schema_config.acc_reg_num_table = "acc_reg_num";
	schema_config.acc_reg_str_table = "acc_reg_str";
	schema_config.char_reg_num_table = "char_reg_num";
	schema_config.char_reg_str_table = "char_reg_str";
	schema_config.interlog_table = "interlog";
}
...
int do_init(int argc, char **argv)
{
	...
	//guildmemberdb clean
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '0' AND `char_id` = '0'", charserv_table(guild_member_table)) )
		Sql_ShowDebug(sql_handle);
