...
void char_set_char_online(int32 map_id, uint32 char_id, uint32 account_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "UPDATE `%s` SET `online`='1', `last_login`=NOW() WHERE `char_id`='%d' LIMIT 1",
...
void char_set_char_offline(uint32 char_id, uint32 account_id) {
	if (char_id == -1) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "UPDATE `%s` SET `online`='0' WHERE `char_id`='%d' LIMIT 1",
								   schema_config.char_db,
								   char_id)) {
			Sql_ShowDebug(sql_handle);
		}
...
int32 char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p) {
		...
		for (i = 0, count = 0; i < MAX_SKILL; ++i) {
			if (p->skill[i].id != 0 && p->skill[i].flag != SKILL_FLAG_TEMPORARY) {
				...
				StringBuf_Printf(
					...
					p->skill[i].flag == SKILL_FLAG_PERM_GRANTED ? p->skill[i].flag
																: 0); /* other flags do not need to be saved */
				++count;
			}
		}
		if (count) {
	...
	StringBuf_Printf(&buf,
					 "REPLACE INTO `%s` (`char_id`, `hotkey`, `type`, `itemskill_id`, `skill_lvl`) VALUES ",
	...
	for (i = 0; i < ARRAYLENGTH(p->hotkeys); i++) {
		if (memcmp(&p->hotkeys[i], &cp->hotkeys[i], sizeof(struct hotkey))) {
			...
			StringBuf_Printf(&buf,
							 ...
							 char_id,
							 (uint32)i,
							 (uint32)p->hotkeys[i].type,
							 p->hotkeys[i].id,
							 (uint32)p->hotkeys[i].lv);
			diff = 1;
		}
	}
	if (diff) {
...
int32 char_memitemdata_to_sql(
		...
		for (i = 0; i < max; ++i) {
			...
			if (items[i].nameid == item.nameid && items[i].card[0] == item.card[0] &&
				...
				ARR_FIND(0,
						 MAX_ITEM_RDM_OPT,
						 k,
						 items[i].option[k].id != item.option[k].id ||
							 items[i].option[k].value != item.option[k].value ||
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, "
...
bool char_memitemdata_from_sql(struct s_storage* p, int32 max, int32 id, enum storage_type tableswitch, uint8 stor_id) {
	...
	StringBuf_AppendStr(&buf,
						"SELECT "
						"`id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`attribute`,`expire_time`,`bound`,`unique_"
						"id`,`enchantgrade`");
	if (tableswitch == TABLE_INVENTORY) {
		StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
		offset = 2;
	...
	if (SQL_ERROR == stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT, &id, 0) ||
...
int32 char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count) {
	...
	if (SQL_ERROR ==
			stmt.Prepare("SELECT "
						 ...
						 sd->account_id,
						 MAX_CHARS) ||
		SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT, &p.char_id, 0, nullptr, nullptr) ||
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if (SQL_ERROR ==
			stmt.Prepare(
				...
				schema_config.char_db) ||
		SQL_ERROR == stmt.BindParam(0, SQLDT_INT, &char_id, 0) || SQL_ERROR == stmt.Execute() ||
		...
		SQL_ERROR ==
			stmt.BindColumn(39, SQLDT_STRING, &p->last_point.map, sizeof(p->last_point.map), nullptr, nullptr) ||
		...
		SQL_ERROR ==
			stmt.BindColumn(42, SQLDT_STRING, &p->save_point.map, sizeof(p->save_point.map), nullptr, nullptr) ||
	...
	// read memo data
	//`memo` (`memo_id`,`char_id`,`map`,`x`,`y`)
	if (SQL_ERROR == stmt.Prepare("SELECT `map`,`x`,`y` FROM `%s` WHERE `char_id`=? ORDER by `memo_id` LIMIT %d",
	...
	// read skill
	//`skill` (`char_id`, `id`, `lv`)
	if (SQL_ERROR == stmt.Prepare("SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d",
	...
	if (SQL_ERROR == stmt.Prepare("SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN `%s` f ON "
								  "f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d",
								  schema_config.char_db,
	...
	if (SQL_ERROR == stmt.Prepare("SELECT `hotkey`, `type`, `itemskill_id`, `skill_lvl` FROM `%s` WHERE `char_id`=?",
								  schema_config.hotkey_db) ||
		SQL_ERROR == stmt.BindParam(0, SQLDT_INT, &char_id, 0) || SQL_ERROR == stmt.Execute() ||
	...
	if (charserv_config.save_log) {
		ShowInfo("Loaded char (%d - %s): %s\n",
				 char_id,
				 p->name,
				 StringBuf_Value(&msg_buf)); // ok. all data load successfully!
	}
...
int32 char_make_new_char(struct char_session_data* sd,
	...
	// check char slot
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT 1 FROM `%s` WHERE `account_id` = '%d' AND `char_num` = '%d' LIMIT 1",
							   schema_config.char_db,
							   sd->account_id,
							   slot)) {
		Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`, "
							   "`str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
							   "`max_sp`, `sp`, `hair`, `hair_color`, `last_map`, `last_x`, `last_y`, `save_map`, "
							   "`save_x`, `save_y`, `sex`, `last_instanceid`) VALUES ("
							   "'%d', '%d', '%s', '%d', '%d',  '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', "
							   "'%u', '%u', '%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%c', '0')",
							   schema_config.char_db,
							   sd->account_id,
							   slot,
							   esc_name,
							   start_job,
							   charserv_config.start_zeny,
							   status_points,
							   str,
							   agi,
							   vit,
							   ...
							   hair_style,
							   hair_color,
							   tmp_start_point[start_point_idx].map,
							   tmp_start_point[start_point_idx].x,
							   tmp_start_point[start_point_idx].y,
							   tmp_start_point[start_point_idx].map,
							   tmp_start_point[start_point_idx].x,
							   tmp_start_point[start_point_idx].y,
							   sex)) {
	...
	for (k = 0; k <= MAX_STARTITEM && tmp_start_items[k].nameid != 0; k++) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, `identify`) VALUES ('%d', "
								   "'%u', '%hu', '%u', '%d')",
								   schema_config.inventory_db,
								   char_id,
								   tmp_start_items[k].nameid,
								   tmp_start_items[k].amount,
								   tmp_start_items[k].pos,
								   1)) {
			Sql_ShowDebug(sql_handle);
		}
	}
...
int32 char_divorce_char_sql(int32 partner_id1, int32 partner_id2) {
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "UPDATE `%s` SET `partner_id`='0' WHERE `char_id`='%d' OR `char_id`='%d' LIMIT 2",
							   schema_config.char_db,
							   partner_id1,
							   partner_id2)) {
		Sql_ShowDebug(sql_handle);
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT "
				  "`name`,`account_id`,`party_id`,`guild_id`,`base_level`,`homun_id`,`partner_id`,`father`,`mother`,`"
				  "elemental_id`,`delete_date` FROM `%s` WHERE `account_id`='%u' AND `char_id`='%u'",
				  schema_config.char_db,
				  sd->account_id,
				  char_id)) {
		Sql_ShowDebug(sql_handle);
	...
	if (father_id || mother_id) { // Char is Baby
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "UPDATE `%s` SET `child`='0' WHERE `char_id`='%d' OR `char_id`='%d'",
								   schema_config.char_db,
								   father_id,
								   mother_id)) {
			Sql_ShowDebug(sql_handle);
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "DELETE FROM `%s` WHERE `id` = '410' AND (`char_id`='%d' OR `char_id`='%d')",
								   schema_config.skill_db,
								   father_id,
								   mother_id)) {
			Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = `card1`|`card2`<<16 WHERE "
							   "`%s`.char_id = '%d' AND card0 = 256",
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `id` IN ( SELECT `id` FROM `%s` WHERE `dest_id`='%d' )",
	...
	/* Quest Data */
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", schema_config.quest_db, char_id)) {
		Sql_ShowDebug(sql_handle);
	}
	...
	if (charserv_config.log_char) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`char_msg`,`name`) VALUES (NOW(), "
								   "'%d', '%d', 'Deleted char (CID %d)', '%s')",
								   schema_config.charlog_db,
								   account_id,
								   0,
								   char_id,
...
int32 char_family(int32 cid1, int32 cid2, int32 cid3) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
		...
		while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
			...
			if ((cid1 == charid &&
				 ((cid2 == partnerid && cid3 == childid) || (cid2 == childid && cid3 == partnerid))) ||
...
void char_read_fame_list(void) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR "
							   "`class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
							   schema_config.char_db,
							   JOB_TAEKWON,
							   JOB_BABY_TAEKWON,
							   fame_list_size_taekwon)) {
		Sql_ShowDebug(sql_handle);
	}
...
TIMER_FUNC(char_clan_member_cleanup) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "UPDATE `%s` SET `clan_id`='0' WHERE `online`='0' AND `clan_id`<>'0' AND `last_login` "
							   "IS NOT NULL AND `last_login` <= NOW() - INTERVAL %d DAY",
							   schema_config.char_db,
							   charserv_config.clan_remove_inactive_days)) {
		Sql_ShowDebug(sql_handle);
	}
...
bool char_checkdb(void) {
	int32 i;
	const char* sqltable[] = {schema_config.char_db,
							  schema_config.hotkey_db,
							  schema_config.scdata_db,
							  schema_config.cart_db,
							  schema_config.inventory_db,
							  ...
							  schema_config.skillcooldown_homunculus_db,
							  schema_config.mercenary_db,
							  schema_config.mercenary_owner_db,
							  schema_config.skillcooldown_mercenary_db,
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `char_id`,`key`,`index`,`value` from `%s` LIMIT 1;",
							   schema_config.char_reg_str_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `account_id`,`key`,`index`,`value` from `%s` LIMIT 1;",
							   schema_config.acc_reg_str_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `guild_id`,`opposition`,`alliance_id`,`name` FROM `%s` LIMIT 1;",
							   schema_config.guild_alliance_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `guild_id`,`account_id`,`name`,`mes`,`char_id` FROM `%s` LIMIT 1;",
							   schema_config.guild_expulsion_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `guild_id`,`char_id`,`exp`,`position` FROM `%s` LIMIT 1;",
							   schema_config.guild_member_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `guild_id`,`position`,`name`,`mode`,`exp_mode` FROM `%s` LIMIT 1;",
							   schema_config.guild_position_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  ",`option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_parm3`,"
				  "`option_id4`,`option_val4`,`option_parm4`,`unique_id`, `bound`, `enchantgrade`"
				  " FROM `%s` LIMIT 1;",
				  schema_config.mail_attachment_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "`str`,`agi`,`vit`,`int`,`dex`,`luk`,`hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`alive`,`rename_flag`,"
				  "`vaporize`,`autofeed` "
				  " FROM `%s` LIMIT 1;",
				  schema_config.homunculus_db)) {
		Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `homun_id`,`skill`,`tick` FROM `%s` LIMIT 1;",
							   schema_config.skillcooldown_homunculus_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `mer_id`,`skill`,`tick` FROM `%s` LIMIT 1;",
							   schema_config.skillcooldown_mercenary_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `account_id`,`char_id`,`skill`,`tick` FROM `%s` LIMIT 1;",
							   schema_config.skillcooldown_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `char_id`,`script`,`tick`,`flag`,`type`,`icon` FROM `%s` LIMIT 1;",
							   schema_config.bonus_script_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
void char_sql_config_read(const char* cfgName) {
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (!strcmpi(w1, "char_db")) {
			...
			safestrncpy(
				schema_config.skillcooldown_homunculus_db, w2, sizeof(schema_config.skillcooldown_homunculus_db));
		}
...
void char_set_default_sql() {
	...
	safestrncpy(schema_config.skillcooldown_homunculus_db,
				"skillcooldown_homunculus",
				sizeof(schema_config.skillcooldown_homunculus_db));
	...
	safestrncpy(schema_config.skillcooldown_mercenary_db,
				"skillcooldown_mercenary",
				sizeof(schema_config.skillcooldown_mercenary_db));
...
void char_set_defaults() {
	...
	safestrncpy(
		charserv_config.char_config.unknown_char_name,
		"Unknown",
		sizeof(
			charserv_config.char_config.unknown_char_name)); // Name to use when the requested name cannot be determined
	safestrncpy(
		charserv_config.char_config.char_name_letters,
		"",
		sizeof(charserv_config.char_config
				   .char_name_letters)); // list of letters/symbols allowed (or not) in a character name. by [Yor]
 ...
 */
void char_config_split_startpoint(char* w1_value,
								  char* w2_value,
...
bool char_config_read(const char* cfgName, bool normal) {
	...
	while (fgets(line, sizeof(line), fp)) {
			...
			if (strcmpi(w1, "userid") == 0) {
				...
				if (charserv_config.bind_ip) {
					...
					ShowStatus("Character server binding IP address : %s -> %s\n",
							   w2,
							   ip2str(charserv_config.bind_ip, ip_str));
				}
			}
		...
		if (strcmpi(w1, "timestamp_format") == 0) {
			...
			safestrncpy(charserv_config.char_config.unknown_char_name,
						w2,
						sizeof(charserv_config.char_config.unknown_char_name));
			...
			safestrncpy(charserv_config.char_config.char_name_letters,
						w2,
						sizeof(charserv_config.char_config.char_name_letters));
		}
...
void char_config_adjust() {
	...
	if (charserv_config.char_config.char_del_option & CHAR_DEL_BIRTHDATE) {
		ShowWarning(
			"conf/char_athena.conf:char_del_option birthdate is enabled but it requires PACKETVER 2010-08-03 or newer, "
			"defaulting to email...\n");
		charserv_config.char_config.char_del_option &= ~CHAR_DEL_BIRTHDATE;
	}
#endif
...
bool CharacterServer::initialize(int32 argc, char* argv[]) {
	...
	if (strcmp(charserv_config.userid, "s1") == 0 && strcmp(charserv_config.passwd, "p1") == 0) {
		...
		ShowNotice(
			"And then change the user/password to use in conf/char_athena.conf (or conf/import/char_conf.txt)\n");
	}
#endif
	...
	// guildmemberdb clean
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `guild_id` = '0' AND `char_id` = '0'",
							   schema_config.guild_member_db)) {
		Sql_ShowDebug(sql_handle);
	}
