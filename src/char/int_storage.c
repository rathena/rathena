 ...
 * @return 0 if sucess, or error count
 */
...
{
	char_memitemdata_to_sql(p->u.items_guild, MAX_GUILD_STORAGE, guild_id, TABLE_GUILD_STORAGE);
	ShowInfo ("guild storage save to DB - guild: %d\n", guild_id);
	return 0;
}

/// Load guild_storage data to mem
int guild_storage_fromsql(int guild_id, struct s_storage* p)
{
	StringBuf buf;
	int i, j;
 ...
 * @param result
 */
static int mapif_storage_data_loaded(int fd, uint32 account_id, char type, struct s_storage entries, bool result) {
 ...
 * @param type
 */
int mapif_storage_saved(int fd, uint32 account_id, bool sucess, char type) {
	WFIFOHEAD(fd,8);
	WFIFOW(fd, 0) = 0x388b;
	WFIFOL(fd, 2) = account_id;
	WFIFOB(fd, 6) = sucess;
 ...
 * @param fd
 */
int mapif_parse_StorageLoad(int fd) {
	uint32 aid, cid;
	int type;
 ...
 * @param fd
 */
int mapif_parse_StorageSave(int fd) {
	int aid, cid, type;
	struct s_storage stor;
