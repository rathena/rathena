	...
	for (i = 0; i < 10; i++) {
		...
		if (inventory[index].amount < sd->deal.item[i].amount) { // if more than the player have -> hack
			sprintf(
				...
				sd->status.name,
				sd->status
					.account_id); // Hack on trade: character '%s' (account: %d) try to trade more items that he has.
			...
			sprintf(
				...
				sd->deal.item[i].amount); // This player has %d of a kind of item (id: %u), and try to trade %d of them.
			intif_wis_message_to_gm(wisp_server_name, PC_PERM_RECEIVE_HACK_INFO, message_to_gm);
			// if we block people
			if (battle_config.ban_hack_trade < 0) {
				...
				chrif_req_login_operation(-1,
										  sd->status.name,
										  CHRIF_OP_LOGIN_BAN,
										  battle_config.ban_hack_trade * 60,
										  0,
										  0); // type: 2 - ban (year, month, day, hour, minute, second)
				set_eof(sd->fd); // forced to disconnect because of the hack
				// message about the ban
				sprintf(message_to_gm,
						msg_txt(sd, 507),
						battle_config.ban_hack_trade); //  This player has been banned for %d minute(s).
			}
			else {
				// message about the ban
...
void trade_tradeadditem(map_session_data *sd, short index, short amount) {
	...
	if (!amount) { // Why do this.. ~.~ just send an ack, the item won't display on the trade window.
		clif_tradeitemok(
			*sd, -2, EXITEM_ADD_SUCCEED); // We pass -2 which will becomes 0 in clif_tradeitemok (Official behavior)
		return;
	}
