...
static int32 add_path(struct node_heap *heap,
					  struct path_node *tp,
					  int16 x,
					  int16 y,
					  int32 g_cost,
 ...
 *------------------------------------------*/
bool path_search(
	struct walkpath_data *wpd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int32 flag, cell_chk cell) {
...
#define chk_dir(d) ((allowed_dirs & (d)) == (d))
			...
			if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y - 1, cell)) {
				e += add_path(&g_open_set,
							  ...
							  current,
							  heuristic(x + 1, y - 1, x1, y1)); // (x+1, y-1) 5
			}
			if (chk_dir(PATH_DIR_EAST)) {
				e += add_path(
					&g_open_set, tp, x + 1, y, g_cost + MOVE_COST, current, heuristic(x + 1, y, x1, y1)); // (x+1, y) 6
			}
			if (chk_dir(PATH_DIR_NORTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y + 1, cell)) {
				e += add_path(&g_open_set,
							  ...
							  g_cost + MOVE_DIAGONAL_COST,
							  current,
							  heuristic(x + 1, y + 1, x1, y1)); // (x+1, y+1) 7
			}
			if (chk_dir(PATH_DIR_NORTH)) {
				e += add_path(
					&g_open_set, tp, x, y + 1, g_cost + MOVE_COST, current, heuristic(x, y + 1, x1, y1)); // (x, y+1) 0
			}
			if (chk_dir(PATH_DIR_NORTH | PATH_DIR_WEST) && !map_getcellp(mapdata, x - 1, y + 1, cell)) {
				e += add_path(&g_open_set,
							  ...
							  g_cost + MOVE_DIAGONAL_COST,
							  current,
							  heuristic(x - 1, y + 1, x1, y1)); // (x-1, y+1) 1
			}
			if (chk_dir(PATH_DIR_WEST)) {
				e += add_path(
					&g_open_set, tp, x - 1, y, g_cost + MOVE_COST, current, heuristic(x - 1, y, x1, y1)); // (x-1, y) 2
			}
			if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_WEST) && !map_getcellp(mapdata, x - 1, y - 1, cell)) {
				e += add_path(&g_open_set,
							  ...
							  g_cost + MOVE_DIAGONAL_COST,
							  current,
							  heuristic(x - 1, y - 1, x1, y1)); // (x-1, y-1) 3
			}
			if (chk_dir(PATH_DIR_SOUTH)) {
				e += add_path(
					&g_open_set, tp, x, y - 1, g_cost + MOVE_COST, current, heuristic(x, y - 1, x1, y1)); // (x, y-1) 4
			}
#undef chk_dir
			if (e) {
				return false;
