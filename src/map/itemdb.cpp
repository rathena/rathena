...
uint64 ItemDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Range")) {
		...
		if (range > AREA_SIZE) {
			this->invalidWarning(node["Range"],
	...
	if (this->nodeExists(node, "EquipLevelMin")) {
		...
		if (lv > MAX_LEVEL) {
			this->invalidWarning(node["EquipLevelMin"],
	...
	if (this->nodeExists(node, "Flags")) {
		...
		if (this->nodeExists(flagNode, "DropEffect")) {
			...
			if (!script_get_constant(effect_constant.c_str(), &constant) || constant < DROPEFFECT_NONE ||
				...
				this->invalidWarning(flagNode["DropEffect"],
	...
	if (this->nodeExists(node, "Script")) {
		...
		item->script = parse_script(script.c_str(),
	...
	if (this->nodeExists(node, "EquipScript")) {
		...
		item->equip_script = parse_script(script.c_str(),
										  this->getCurrentFile().c_str(),
										  this->getLineNumber(node["EquipScript"]),
										  SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	}
	else {
		if (!exists) {
	...
	if (this->nodeExists(node, "UnEquipScript")) {
		...
		item->unequip_script = parse_script(script.c_str(),
											this->getCurrentFile().c_str(),
...
uint64 LaphineUpgradeDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "RequiredRandomOptions")) {
		...
		if (amount > MAX_ITEM_RDM_OPT) {
			this->invalidWarning(node["RequiredRandomOptions"],
	...
	if (this->nodeExists(node, "ResultRefineMinimum")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["ResultRefineMinimum"],
	...
	if (this->nodeExists(node, "ResultRefineMaximum")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["ResultRefineMaximum"],
	...
	if (this->nodeExists(node, "BaseItems")) {
		for (const ryml::NodeRef& baseNode : node["BaseItems"]) {
			...
			if (this->nodeExists(baseNode, "MinimumRefine")) {
				...
				if (refine > MAX_REFINE) {
					this->invalidWarning(baseNode["MinimumRefine"],
			...
			if (this->nodeExists(baseNode, "MaximumRefine")) {
				...
				if (refine > MAX_REFINE) {
					this->invalidWarning(baseNode["MaximumRefine"],
...
std::shared_ptr<s_item_group_entry> ItemGroupDatabase::get_random_itemsubgroup(
	std::shared_ptr<s_item_group_random> random, e_group_algorithm_type algorithm) {
	if (random == nullptr) {
		return nullptr;
	}
	...
	switch (algorithm) {
		case GROUP_ALGORITHM_DROP: {
			// We pick a random item from the group and then do a drop check based on the rate. On fail, do not return
			// any item
 ...
 */
void ItemGroupDatabase::pc_get_itemgroup_sub(map_session_data& sd,
											 bool identify,
...
bool itemdb_canpartnertrade_sub(struct item_data* item, int32 gmlv, int32 gmlv2) {
	return (item && (item->flag.trade_restriction.trade_partner || gmlv >= item->gm_lv_trade_override ||
					 gmlv2 >= item->gm_lv_trade_override));
}
		...
		for (const auto& subit : subNode) {
			if (this->nodeExists(subit, "Clear")) {
				...
				if (group->random.erase(id) == 0) {
					this->invalidWarning(subit["Clear"],
			...
			for (const auto& listit : listNode) {
				...
				if (this->nodeExists(listit, "Clear")) {
					...
					if (random->data.erase(index) == 0) {
						this->invalidWarning(listit["Clear"],
											 "Index %u doesn't exist in the SubGroup %hu (group %s). Clear failed.\n",
											 index,
											 subgroup,
											 group_name.c_str());
					}
				...
				if (random->algorithm == GROUP_ALGORITHM_ALL && entry->rate > 0) {
					this->invalidWarning(listit["Item"],
										 "Item cannot have a rate with \"All\" algorithm. Defaulting Rate to 0.\n");
					entry->rate = 0;
				}
				if (random->algorithm != GROUP_ALGORITHM_ALL && entry->rate == 0) {
				...
				entry->adj_rate =
					cap_value(entry->adj_rate, battle_config.item_group_drop_min, battle_config.item_group_drop_max);
				...
				if (this->nodeExists(listit, "RefineMinimum")) {
					...
					if (refine > MAX_REFINE) {
						this->invalidWarning(listit["RefineMinimum"],
											 "Minimum refine % hu is too high, capping to MAX_REFINE...\n",
											 refine);
						refine = MAX_REFINE;
					}
				...
				if (this->nodeExists(listit, "RefineMaximum")) {
					...
					if (refine > MAX_REFINE) {
						this->invalidWarning(listit["RefineMaximum"],
											 "Maximum refine %hu is too high, capping to MAX_REFINE...\n",
											 refine);
						refine = MAX_REFINE;
					}
...
uint64 ComboDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	for (const auto& itemsit : items_list) {
		...
		if (this->nodeExists(node, "Script")) {
			...
			combo->script = parse_script(script.c_str(),
										 this->getCurrentFile().c_str(),
										 this->getLineNumber(node["Script"]),
										 SCRIPT_IGNORE_EXTERNAL_BRACKETS);
		}
...
uint64 RandomOptionDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Script")) {
		...
		randopt->script = parse_script(script.c_str(),
									   this->getCurrentFile().c_str(),
									   this->getLineNumber(node["Script"]),
									   SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	}
...
bool RandomOptionGroupDatabase::add_option(const ryml::NodeRef& node,
	...
	if (entry->min_value > entry->max_value) {
		this->invalidWarning(node["MaxValue"],
	...
	if (this->nodeExists(node, "Slots")) {
		...
		for (const ryml::NodeRef& slotNode : slotsNode) {
			if (randopt->slots.size() >= MAX_ITEM_RDM_OPT) {
				this->invalidWarning(
					slotNode,
					"Reached maximum of %d Random Option group options. Skipping the remaining slots...\n",
					MAX_ITEM_RDM_OPT);
				break;
			}
			...
			if (slot < 1 || slot > MAX_ITEM_RDM_OPT) {
				this->invalidWarning(slotNode["Slot"],
...
void itemdb_gen_itemmoveinfo() {
	...
	for (auto it = sorted_itemdb.begin(); it != sorted_itemdb.end(); ++it) {
		...
		if (memcmp(&it->second->flag.trade_restriction,
				   &tmp_id.flag.trade_restriction,
				   sizeof(tmp_id.flag.trade_restriction)) == 0) {
			continue;
		}
