...
* @param nameid
*/
static struct item_data *itemdb_create_item(nameid_t nameid) {
	struct item_data *id;
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		*p = '\0';
		...
		if ( str[1][strlen(str[1])-1] != '}' ) {
			...
			id->combos[idx]->nameid = (nameid_t *)aMalloc(retcount * sizeof(nameid_t));
...
bool itemdb_parse_roulette_db(void)
{
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
		...
		for (k = 0; k < limit && SQL_SUCCESS == Sql_NextRow(mmysql_handle); k++) {
			...
			j = rd.items[i];
			RECREATE(rd.nameid[i], nameid_t, ++rd.items[i]);
