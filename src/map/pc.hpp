...
public:
	...
	struct s_state {
		...
		uint32 rest : 1;
		uint32
			storage_flag : 3; // 0: closed, 1: Normal Storage open, 2: guild storage open [Skotlex], 3: Premium Storage
	...
	std::bitset<PC_PERM_MAX>
		permissions; // group permissions have to be copied, because they might be adjusted by atcommand addperm
	...
	struct item_data
		*inventory_data[MAX_INVENTORY]; // direct pointers to itemdb entries (faster than doing item_id lookups)
	...
	int32 npc_item_flag; // Marks the npc_id with which you can use items during interactions with said npc (see script
						 // command enable_itemuse)
	...
	std::vector<s_item_bonus> skillatk, skillusesprate, skillusesp, skillheal, skillheal2, skillblown, skillcastrate,
		skillfixcastrate, subskill, skillcooldown, skillfixcast, skillvarcast, skilldelay, itemhealrate, add_def,
		add_mdef, add_mdmg, reseff, itemgrouphealrate, itemsphealrate, itemgroupsphealrate;
	...
	std::vector<std::shared_ptr<s_autobonus>> autobonus, autobonus2,
		autobonus3; // Auto script on attack, when attacked, on skill usage
	...
	struct s_bonus {
		...
		int32 sp_gain_value, hp_gain_value, magic_sp_gain_value, magic_hp_gain_value, long_sp_gain_value,
			long_hp_gain_value;
	...
	int32
		avail_quests; ///< Number of Q_ACTIVE and Q_INACTIVE entries in quest log (index of the first Q_COMPLETE entry)
	...
	struct s_bonus_script_list {
		struct linkdb_node
			*head; ///< Bonus script head node. data: struct s_bonus_script_entry *entry, key: (intptr_t)entry
		uint16 count;
	} bonus_script;
	...
	short setlook_head_top, setlook_head_mid, setlook_head_bottom,
		setlook_robe; ///< Stores 'setlook' script command values.
...
struct s_job_info {
	std::vector<uint32> base_hp, base_sp,
		base_ap; // Storage for the first calculation with hp/sp/ap factor and multiplicator
...
#define EQP_SHADOW_GEAR \
	(EQP_SHADOW_ARMOR | EQP_SHADOW_WEAPON | EQP_SHADOW_SHIELD | EQP_SHADOW_SHOES | EQP_SHADOW_ACC_R | EQP_SHADOW_ACC_L)
...
#define pc_isidle_party(sd) \
	((sd)->chatID || (sd)->state.vending || (sd)->state.buyingstore || \
	 DIFF_TICK(last_tick, (sd)->idletime) >= battle_config.idle_no_share)
#define pc_isidle_hom(sd) \
	((sd)->hd && ((sd)->chatID || (sd)->state.vending || (sd)->state.buyingstore || \
				  DIFF_TICK(last_tick, (sd)->idletime_hom) >= battle_config.hom_idle_no_share))
#define pc_isidle_mer(sd) \
	((sd)->md && ((sd)->chatID || (sd)->state.vending || (sd)->state.buyingstore || \
				  DIFF_TICK(last_tick, (sd)->idletime_mer) >= battle_config.mer_idle_no_share))
...
static bool pc_cant_act2(map_session_data *sd) {
	return sd->state.vending || sd->state.buyingstore || (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING) ||
		   ...
		   sd->state.barter_open || sd->state.barter_extended_open || sd->state.laphine_synthesis ||
		   sd->state.laphine_upgrade || sd->state.roulette_open || sd->state.enchantgrade_open ||
...
#define pc_check_weapontype(sd, type) \
	((type) & ((sd)->status.weapon < MAX_WEAPON_TYPE \
				   ? 1 << (sd)->status.weapon \
				   : (1 << (sd)->weapontype1) | (1 << (sd)->weapontype2) | (1 << (sd)->status.weapon)))
// Checks if the given class value corresponds to a player class. [Skotlex]
// JOB_NOVICE isn't checked for class_ is supposed to be unsigned
#define pcdb_checkid_sub(class_) \
	(((class_) < JOB_MAX_BASIC) || ((class_) >= JOB_NOVICE_HIGH && (class_) <= JOB_DARK_COLLECTOR) || \
	 ((class_) >= JOB_RUNE_KNIGHT && (class_) <= JOB_MECHANIC_T2) || \
	 ((class_) >= JOB_BABY_RUNE_KNIGHT && (class_) <= JOB_BABY_MECHANIC2) || \
	 ((class_) >= JOB_SUPER_NOVICE_E && (class_) <= JOB_SUPER_BABY_E) || \
	 ((class_) >= JOB_KAGEROU && (class_) <= JOB_OBORO) || (class_) == JOB_REBELLION || (class_) == JOB_SUMMONER || \
	 (class_) == JOB_BABY_SUMMONER || ((class_) >= JOB_BABY_NINJA && (class_) <= JOB_BABY_REBELLION) || \
	 ...
	 ((class_) >= JOB_SKY_EMPEROR && (class_) <= JOB_SPIRIT_HANDLER) || (class_) == JOB_SKY_EMPEROR2)
...
// clientside display macros (values to the left/right of the "+")
	...
	#define pc_leftside_matk(sd) \
		(status_base_matk_min(&(sd)->bl, status_get_status_data((sd)->bl), (sd)->status.base_level))
	#define pc_rightside_matk(sd) \
		((sd)->battle_status.rhw.matk + (sd)->battle_status.lhw.matk + (sd)->bonus.ematk + \
		 status_calc_consumablematk(&(sd)->sc, 0) + status_calc_pseudobuff_matk((sd), &(sd)->sc, 0))
#else
...
void pc_setnewpc(
	map_session_data *sd, uint32 account_id, uint32 char_id, int32 login_id1, t_tick client_tick, int32 sex, int32 fd);
bool pc_authok(map_session_data *sd,
			   uint32 login_id2,
			   time_t expiration_time,
			   int32 group_id,
...
enum e_additem_result pc_cart_additem(map_session_data *sd,
...
bool pc_addautobonus(std::vector<std::shared_ptr<s_autobonus>> &bonus,
					 const char *script,
					 short rate,
					 uint32 dur,
					 uint16 atk_type,
					 ...
					 uint32 pos,
					 bool onskill);
void pc_exeautobonus(map_session_data &sd,
					 std::vector<std::shared_ptr<s_autobonus>> *bonus,
					 std::shared_ptr<s_autobonus> autobonus);
...
enum e_addskill_type {
	...
	ADDSKILL_TEMP = 1, ///< Temporary skill. If player learned the skill and the given level is higher, level will be
					   ///< replaced and learned level will be palced in skill flag. `flag = learned +
					   ///< SKILL_FLAG_REPLACED_LV_0; learned_level = level;`
	ADDSKILL_TEMP_ADDLEVEL = 2, ///< Like PCSKILL_TEMP, except the level will be stacked. `learned_level += level`. The
								///< flag is used to store original learned level
...
void pc_gainexp_disp(
	map_session_data *sd, t_exp base_exp, t_exp next_base_exp, t_exp job_exp, t_exp next_job_exp, bool lost);
...
struct s_bonus_script_entry *pc_bonus_script_add(
	map_session_data *sd, const char *script_str, t_tick dur, enum efst_type icon, uint16 flag, uint8 type);
...
#define pc_is_taekwon_ranker(sd) \
	(((sd)->class_ & MAPID_UPPERMASK) == MAPID_TAEKWON && \
...
uint16 pc_level_penalty_mod(map_session_data *sd,
							e_penalty_type type,
