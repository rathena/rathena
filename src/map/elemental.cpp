...
static int32 elemental_ai_sub_timer(s_elemental_data *ed, map_session_data *sd, t_tick tick) {
	...
	if (mode == EL_MODE_AGGRESSIVE) {
		...
		if (!target) {
			map_foreachinallrange(elemental_ai_sub_timer_activesearch,
								  &ed->bl,
								  view_range,
								  BL_CHAR,
								  ed,
								  &target,
								  status_get_mode(&ed->bl));
		}
...
uint64 ElementalDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (!((id >= ELEMENTALID_AGNI_S && id <= ELEMENTALID_TERA_L) ||
		...
		this->invalidWarning(node["Id"],
							 "Invalid Id %d (valid ranges: %d-%d and %d-%d).\n",
							 id,
							 ELEMENTALID_AGNI_S,
							 ELEMENTALID_TERA_L,
							 ELEMENTALID_DILUVIO,
	...
	if (this->nodeExists(node, "Mode")) {
		...
		for (const auto &Modeit : ModeNode) {
			...
			if (skill_id == 0) {
				this->invalidWarning(SkillNode["Skill"],
									 "Skipping invalid skill %s for mode %s.\n",
									 skill_name.c_str(),
									 mode_name.c_str());
				return 0;
			}
			...
			if (!SKILL_CHK_ELEM(skill_id)) {
				this->invalidWarning(SkillNode["Skill"],
									 "Skill %s (%u) is out of the skill range [%u-%u], skipping.\n",
									 skill_name.c_str(),
									 skill_id,
									 EL_SKILLBASE,
									 EL_SKILLBASE + MAX_ELEMENTALSKILL - 1);
				return 0;
			}
			...
			if (this->nodeExists(SkillNode, "Level")) {
				...
				if (level > skill_get_max(skill_id)) {
					this->invalidWarning(SkillNode["Level"],
										 "Skill %s's level %hu exceeds max level %hu.\n",
										 skill_name.c_str(),
										 level,
