...
void do_init_party(void) {
	...
	add_timer_interval(gettick() + battle_config.party_update_interval,
					   party_send_xy_timer,
					   0,
					   0,
...
bool party_invite(map_session_data& sd, map_session_data* tsd) {
	...
	if (!pc_has_permission(&sd, PC_PERM_PARTY) || !pc_has_permission(tsd, PC_PERM_PARTY)) {
		clif_displaymessage(
			sd.fd,
			msg_txt(&sd, 81)); // Your GM level doesn't authorize you to perform this action on the specified player.
		return false;
	}
...
bool party_booking_load(uint32 account_id, uint32 char_id, struct s_party_booking_requirement* booking) {
	...
	if (Sql_Query(mmysql_handle,
				  "SELECT `minimum_level`, `maximum_level` FROM `%s` WHERE `world_name` = '%s' AND `account_id` = '%u' "
				  "AND `char_id` = '%u'",
				  partybookings_table,
				  world_name,
				  account_id,
...
bool party_removemember(map_session_data& sd, uint32 account_id, const char* name) {
	...
	intif_party_leave(p->party.party_id,
					  account_id,
...
int32 party_removemember2(map_session_data* sd, uint32 char_id, int32 party_id) {
	if (sd) {
		...
		party_trade_bound_cancel(*sd);
		intif_party_leave(sd->status.party_id,
...
int32 party_member_withdraw(
	int32 party_id, uint32 account_id, uint32 char_id, char* name, enum e_party_member_withdraw type) {
...
int32 party_broken(int32 party_id) {
	...
	for (i = 0; i < MAX_PARTY; i++) {
		if (p->data[i].sd != nullptr) {
			clif_party_withdraw(*p->data[i].sd,
...
int32 party_recv_movemap(int32 party_id, uint32 account_id, uint32 char_id, int32 online, int32 lv, const char* map) {
	...
	ARR_FIND(0, MAX_PARTY, i, p->party.member[i].account_id == account_id && p->party.member[i].char_id == char_id);
	if (i == MAX_PARTY) {
		ShowError("party_recv_movemap: char %d/%d not found in party %s (id:%d)",
				  account_id,
				  char_id,
				  p->party.name,
				  party_id);
