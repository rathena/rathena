...
export_constant2("VAR_HEAD_TOP",
				 LOOK_HEAD_TOP); // This one was actually pointing to LOOK_HEAD_BOTTOM until now, so we might be safe to
								 // remove them anyway(since we got no bug report until today)
export_constant2("VAR_HEAD_MID",
				 LOOK_HEAD_MID); // This one was actually pointing to LOOK_HEAD_TOP until now, so we might be safe to
								 // remove them anyway(since we got no bug report until today)
export_constant2("VAR_HEAD_BOTTOM",
				 LOOK_HEAD_BOTTOM); // This one was actually pointing to LOOK_HEAD_MID until now, so we might be safe to
									// remove them anyway(since we got no bug report until today)
