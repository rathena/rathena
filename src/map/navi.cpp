 ...
 *------------------------------------------*/
bool navi_path_search(struct navi_walkpath_data *wpd,
					  const struct navi_pos *from,
					  const struct navi_pos *dest,
					  cell_chk cell) {
	...
	if (from->x < 0 || from->x > mapdata->xs || from->y < 0 ||
		from->y > mapdata->ys /*|| map_getcellp(mapdata,x0,y0,cell)*/) {
		return false;
	}

	// Check destination cell
	if (dest->x < 0 || dest->x > mapdata->xs || dest->y < 0 || dest->y > mapdata->ys ||
		map_getcellp(mapdata, dest->x, dest->y, cell)) {
		return false;
	}
	...
	#define chk_dir(d) ((allowed_dirs & (d)) == (d))
		...
		if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y - 1, cell)) {
			e += add_path(&g_open_set,
						  x + 1,
						  y - 1,
						  g_cost + MOVE_DIAGONAL_COST,
						  current,
						  heuristic(x + 1, y - 1, dest->x, dest->y)); // (x+1, y-1) 5
		}
		if (chk_dir(PATH_DIR_EAST)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_COST,
						  current,
						  heuristic(x + 1, y, dest->x, dest->y)); // (x+1, y) 6
		}
		if (chk_dir(PATH_DIR_NORTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y + 1, cell)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_DIAGONAL_COST,
						  current,
						  heuristic(x + 1, y + 1, dest->x, dest->y)); // (x+1, y+1) 7
		}
		if (chk_dir(PATH_DIR_NORTH)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_COST,
						  current,
						  heuristic(x, y + 1, dest->x, dest->y)); // (x, y+1) 0
		}
		if (chk_dir(PATH_DIR_NORTH | PATH_DIR_WEST) && !map_getcellp(mapdata, x - 1, y + 1, cell)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_DIAGONAL_COST,
						  current,
						  heuristic(x - 1, y + 1, dest->x, dest->y)); // (x-1, y+1) 1
		}
		if (chk_dir(PATH_DIR_WEST)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_COST,
						  current,
						  heuristic(x - 1, y, dest->x, dest->y)); // (x-1, y) 2
		}
		if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_WEST) && !map_getcellp(mapdata, x - 1, y - 1, cell)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_DIAGONAL_COST,
						  current,
						  heuristic(x - 1, y - 1, dest->x, dest->y)); // (x-1, y-1) 3
		}
		if (chk_dir(PATH_DIR_SOUTH)) {
			e += add_path(&g_open_set,
						  ...
						  g_cost + MOVE_COST,
						  current,
						  heuristic(x, y - 1, dest->x, dest->y)); // (x, y-1) 4
		}
	#undef chk_dir
		if (e) {
			return false;
...
int32 map_type(const struct map_data *m) {
	...
	if (std::find_if(m->navi.warps_outof.begin(), m->navi.warps_outof.end(), [&m](const navi_link *link) {
			return std::find_if(
					   m->navi.warps_outof.begin(), m->navi.warps_outof.end(), [&link](const navi_link *link2) {
...
void write_spawn(
	std::ostream &os, const struct map_data *m, const std::shared_ptr<s_mob_db> mobinfo, int32 amount, int32 idx) {
	os << "\t{";
	os << "\"" << m->name << "\", ";
	os << "" << idx << ", ";
	...
	os << "" << mobinfo->lv << ", ";
	os << ""
	   << (((mobinfo->status.ele_lv * 20 + mobinfo->status.def_ele) << 16) | (mobinfo->status.size << 8) |
		   mobinfo->status.race);
	os << "},\n";
}
	...
	for (int32 mapid = 0; mapid < map_num; mapid++) {
		...
		for (int32 npcidx = 0; npcidx < m->npc_num; npcidx++) {
			...
			if (nd->subtype == NPCTYPE_WARP) {
				...
				if (nd->class_ == -1 || nd->class_ == JT_HIDDEN_NPC || nd->class_ == JT_HIDDEN_WARP_NPC ||
					nd->class_ == JT_GUILD_FLAG || nd->navi.hidden) {
					continue;
				}
...
void write_npc_distance(std::ostream &os, const struct npc_data *nd, const struct map_data *msrc) {
	os << "\t\t{ " << nd->navi.id << ", -- (" << nd->name << " " << msrc->name << ", " << nd->navi.pos.x << ", "
	   << nd->navi.pos.y << ")\n";
	for (const auto warp : msrc->navi.warps_into) {
		...
		os << "\t\t\t{ \"" << mdest->name << "\", " << warp->id << ", " << std::to_string(wpd.path_len) << "}, -- ("
		   << msrc->name << ", " << warp->pos.x << ", " << warp->pos.y << ")\n";
	}
	os << "\t\t\t{\"\", 0, 0}\n";
	os << "\t\t},\n";
}
...
void write_map_distance(std::ostream &os, const struct navi_link *warp1, const struct map_data *m) {
	os << "\t\t{ " << warp1->id << ", -- ("
	   << " " << m->name << ", " << warp1->pos.x << ", " << warp1->pos.y << ")\n";
	...
	// 		continue;
	// 	os << "\t\t\t{ \"P\", " << warp2->id << ", " << std::to_string(wpd.path_len) << "}, -- ReachableFromSrc warp ("
	// << m->name << ", " << warp2->pos.x << ", " << warp2->pos.y << ")\n";
	// }
	...
	for (const auto warp3 : map_getmapdata(warp1->warp_dest.m)->navi.warps_outof) {
		...
		os << "\t\t\t{ \"E\", " << warp3->id << ", " << std::to_string(wpd.path_len) << "}, -- ReachableFromDst warp ("
		   << map_getmapdata(warp3->pos.m)->name << ", " << warp3->pos.x << ", " << warp3->pos.y << ")\n";
	}
