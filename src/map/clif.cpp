...
void Clif::setbindip(const char* ip)
{
	bind_ip = host2ip(ip);
	if (bind_ip) {
		char ip_str[16];
...
/*==========================================
 * Returns map port which is set by clif.setport()
 *------------------------------------------*/
uint16 Clif::getport(void)
{
	return map_port;
}
...
/*==========================================
 * sub process of clif.send
 * Called from a map_obj.foreachinallarea (grabs all players in specific area and subjects them to this function)
 ...
 *------------------------------------------*/
int Clif::send(const uint8* buf, int len, struct block_list* bl, enum send_target type)
{
	int i;
	...
	switch(type) {
		...
		case AREA_WOC:
		case AREA_WOS:
			map_obj.foreachinallarea(clif_send_sub, bl->m, bl->x-AREA_SIZE, bl->y-AREA_SIZE, bl->x+AREA_SIZE, bl->y+AREA_SIZE,
					BL_PC, buf, len, bl, type);
			break;
		case AREA_CHAT_WOC:
			map_obj.foreachinallarea(clif_send_sub, bl->m, bl->x-(AREA_SIZE-5), bl->y-(AREA_SIZE-5),
...
void Clif::authok(struct map_session_data *sd)
{
...
void Clif::authrefuse(int fd, uint8 error_code)
{
...
void Clif::authfail_fd(int fd, int type)
{
...
///     ? = nothing
void Clif::charselectok(int id, uint8 ok)
{
	struct map_session_data* sd;
	int fd;

	if ((sd = map_obj.id2sd(id)) == NULL || !sd->fd)
		return;
...
void Clif::dropflooritem(struct flooritem_data* fitem, bool canShowEffect)
{
	...
	clif.send(buf, packet_len(header), &fitem->bl, AREA);
}
...
void Clif::clearflooritem(struct flooritem_data *fitem, int fd)
{
...
///     3 = teleport
///     4 = trickdead
void Clif::clearunit_single(int id, clr_type type, int fd)
{
...
///     3 = teleport
///     4 = trickdead
void Clif::clearunit_area(struct block_list* bl, clr_type type)
{
	unsigned char buf[8];
...
void Clif::clearunit_delayed(struct block_list* bl, clr_type type, t_tick tick)
{
...
void Clif::get_weapon_view(struct map_session_data* sd, unsigned short *rhand, unsigned short *lhand)
{
...
static int clif_set_unit_idle(struct block_list* bl, unsigned char* buffer, bool spawn)
{
	...
	if ( battle_config.monster_hp_bars_info && !map_obj.getmapflag(bl->m, MF_HIDEMOBHPBAR) && bl->type == BL_MOB && (status_get_hp(bl) < status_get_max_hp(bl)) ) {
...
void class_change_target(struct block_list *bl,int class_,int type, enum send_target target, struct map_session_data *sd)
{
	...
	{// player classes yield missing sprites
		...
		WBUFB(buf,6)=type;
		WBUFL(buf,7)=class_;
		clif.send(buf,packet_len(0x1b0),(sd == NULL ? bl : &(sd->bl)),target);
	}
}
...
static void clif_weather_check(struct map_session_data *sd)
{
	...
	if (map_obj.getmapflag(m, MF_SNOW)
			|| map_obj.getmapflag(m, MF_CLOUDS)
			|| map_obj.getmapflag(m, MF_FOG)
			|| map_obj.getmapflag(m, MF_FIREWORKS)
			|| map_obj.getmapflag(m, MF_SAKURA)
			|| map_obj.getmapflag(m, MF_LEAVES)
			|| map_obj.getmapflag(m, MF_CLOUDS2))
	{
		if (map_obj.getmapflag(m, MF_SNOW))
			clif.specialeffect_single(&sd->bl, EF_SNOW, fd);
		if (map_obj.getmapflag(m, MF_CLOUDS))
			clif.specialeffect_single(&sd->bl, EF_CLOUD3, fd);
		if (map_obj.getmapflag(m, MF_CLOUDS2))
			clif.specialeffect_single(&sd->bl, EF_CLOUD5, fd);
		if (map_obj.getmapflag(m, MF_FOG))
			clif.specialeffect_single(&sd->bl, EF_CLOUD4, fd);
		if (map_obj.getmapflag(m, MF_FIREWORKS)) {
			clif.specialeffect_single(&sd->bl, EF_POKJUK, fd);
			clif.specialeffect_single(&sd->bl, EF_THROWITEM2, fd);
			clif.specialeffect_single(&sd->bl, EF_POKJUK_SOUND, fd);
		}
		if (map_obj.getmapflag(m, MF_SAKURA))
			clif.specialeffect_single(&sd->bl, EF_SAKURA, fd);
		if (map_obj.getmapflag(m, MF_LEAVES))
			clif.specialeffect_single(&sd->bl, EF_MAPLE, fd);
	}
}
/**
 * Run when the weather on a map changes, throws all players in map id 'm' to clif_weather_check function
 **/
void Clif::weather(int16 m)
{
	struct s_mapiterator* iter;
 ...
 **/
int Clif::spawn(struct block_list *bl)
{
		...
		case BL_PC:
			{
				...
				if(sd->state.size==SZ_BIG) // tiny/big players [Valaris]
					clif.specialeffect(bl,EF_GIANTBODY2,AREA);
				else if(sd->state.size==SZ_MEDIUM)
					clif.specialeffect(bl,EF_BABYBODY2,AREA);
				if( sd->bg_id && map_obj.getmapflag(sd->bl.m, MF_BATTLEGROUND) )
					clif.sendbgemblem_area(sd);
				if (sd->spiritcharm_type != CHARM_TYPE_NONE && sd->spiritcharm > 0)
					clif.spiritcharm(sd);
				if (sd->status.robe)
					clif.refreshlook(bl,bl->id,LOOK_ROBE,sd->status.robe,AREA);
				clif.efst_status_change_sub(bl, bl, AREA);
				clif.hat_effects(sd,bl,AREA);
			}
			break;
		case BL_MOB:
			{
				...
				if(md->special_state.size==SZ_BIG) // tiny/big mobs [Valaris]
					clif.specialeffect(&md->bl,EF_GIANTBODY2,AREA);
				else if(md->special_state.size==SZ_MEDIUM)
					clif.specialeffect(&md->bl,EF_BABYBODY2,AREA);
			}
			break;
		case BL_NPC:
			{
				...
				if( nd->size == SZ_BIG )
					clif.specialeffect(&nd->bl,EF_GIANTBODY2,AREA);
				else if( nd->size == SZ_MEDIUM )
					clif.specialeffect(&nd->bl,EF_BABYBODY2,AREA);
				clif.efst_status_change_sub(bl, bl, AREA);
...
void Clif::hominfo(struct map_session_data *sd, struct homun_data *hd, int flag)
{
...
///     3 = accessory?
///     ? = ignored
void Clif::send_homdata(struct map_session_data *sd, int state, int param)
{	//[orn]
	int fd = sd->fd;
...
int Clif::homskillinfoblock(struct map_session_data *sd)
{	//[orn]
...
void Clif::homskillup(struct map_session_data *sd, uint16 skill_id)
{	//[orn]
	struct homun_data *hd;
	int fd;
	short idx = -1;
...
int Clif::hom_food(struct map_session_data *sd,int foodid,int fail)	//[orn]
{
	int fd=sd->fd;
...
void Clif::walkok(struct map_session_data *sd)
{
	int fd=sd->fd;
		...
		case BL_PC:
			{
				...
				//			clif.movepc(sd);
				if(sd->state.size==SZ_BIG) // tiny/big players [Valaris]
					clif.specialeffect(&sd->bl,EF_GIANTBODY2,AREA);
				else if(sd->state.size==SZ_MEDIUM)
					clif.specialeffect(&sd->bl,EF_BABYBODY2,AREA);
				if (sd->status.robe)
					clif.refreshlook(bl,bl->id,LOOK_ROBE,sd->status.robe,AREA);
			}
			break;
...
void Clif::move(struct unit_data *ud)
{
	unsigned char buf[16];
	struct view_data* vd;
	...
	//This performance check is needed to keep GM-hidden objects from being notified to bots.
		...
		// If the player was disguised we still need to update the disguised unit, since the main unit will be updated through clif.walkok
		if(disguised(bl)) {
			WBUFW(buf,0)=0x86;
			WBUFL(buf,2)=-bl->id;
...
/*==========================================
 * Delays the map_obj.quit of a player after they are disconnected. [Skotlex]
 *------------------------------------------*/
static TIMER_FUNC(clif_delayquit){
	struct map_session_data *sd = NULL;

	//Remove player from map server
	if ((sd = map_obj.id2sd(id)) != NULL && sd->fd == 0) //Should be a disconnected player.
		map_obj.quit(sd);
	return 0;
}
...
void Clif::changemap(struct map_session_data *sd, short m, int x, int y)
{
	int fd;
	nullpo_retv(sd);
	fd = sd->fd;
	...
	WFIFOW(fd,0) = 0x91;
	mapindex_getmapname_ext(map_obj.mapid2mapname(m), WFIFOCP(fd,2));
	WFIFOW(fd,18) = x;
	WFIFOW(fd,20) = y;
...
void Clif::changemapserver(struct map_session_data* sd, unsigned short map_index, int x, int y, uint32 ip, uint16 port)
{
	int fd;
...
/// In many situations (knockback, backslide, etc.) Aegis sends both clif.slide and clif_fixpos
/// This function combines both calls and allows to simplify the calling code
void Clif::blown(struct block_list *bl)
{
...
/// 0088 <id>.L <x>.W <y>.W
void Clif::fixpos(struct block_list *bl)
{
...
/// 00c4 <shop id>.L
void Clif::npcbuysell(struct map_session_data* sd, int id)
{
	int fd;
...
void Clif::buylist(struct map_session_data *sd, struct npc_data *nd)
{
	int fd,i,c;
	bool discount;
...
void Clif::selllist(struct map_session_data *sd)
{
	int fd,i,c=0,val;
	struct npc_data *nd;

	nullpo_retv(sd);
	if (!sd->npc_shopid || (nd = map_obj.id2nd(sd->npc_shopid)) == NULL)
		return;
 ...
 **/
void Clif::npc_market_open(struct map_session_data *sd, struct npc_data *nd) {
#if PACKETVER >= 20131223
...
void clif_npc_market_purchase_ack(struct map_session_data *sd, uint8 res, uint8 n, struct s_npc_buy_list *list) {
	...
	nullpo_retv(sd);
	nullpo_retv((nd = map_obj.id2nd(sd->npc_shopid)));
...
/// - append this text
void Clif::scriptmes(struct map_session_data *sd, int npcid, const char *mes)
{
	int fd = sd->fd;
...
/// - remove 'next' button
void Clif::scriptnext(struct map_session_data *sd,int npcid)
{
	int fd;
...
void Clif::scriptclose(struct map_session_data *sd, int npcid)
{
	int fd;
 ...
 */
void Clif::scriptclear(struct map_session_data *sd, int npcid)
{
	struct s_packet_db* info;
	int16 len;
	int cmd = 0;
...
void Clif::scriptmenu(struct map_session_data* sd, int npcid, const char* mes)
{
	int fd = sd->fd;
	int slen = strlen(mes) + 9;
	struct block_list *bl = NULL;

	if (!sd->state.using_fake_npc && (npcid == fake_nd->bl.id || ((bl = map_obj.id2bl(npcid)) && (bl->m!=sd->bl.m ||
...
void Clif::scriptinput(struct map_session_data *sd, int npcid)
{
	int fd;
	struct block_list *bl = NULL;
...
void Clif::scriptinputstr(struct map_session_data *sd, int npcid)
{
	int fd;
	struct block_list *bl = NULL;
...
/// color:
///     0x00RRGGBB
void Clif::viewpoint(struct map_session_data *sd, int npc_id, int type, int x, int y, int id, int color)
{
	int fd;
...
void Clif::cutin(struct map_session_data* sd, const char* image, int type)
{
	int fd;
...
void Clif::additem(struct map_session_data *sd, int n, int amount, unsigned char fail)
{
...
void Clif::dropitem(struct map_session_data *sd,int n,int amount)
{
	int fd;
...
void Clif::delitem(struct map_session_data *sd,int n,int amount, short reason)
{
...
void Clif::inventorylist(struct map_session_data *sd) {
	int i,n,ne,arrow=-1;
	unsigned char *buf;
	unsigned char *bufe;
...
void Clif::equiplist(struct map_session_data *sd)
{
	int i,n,fd = sd->fd;
	unsigned char *buf;
#if PACKETVER < 20071002
...
void Clif::storagelist(struct map_session_data* sd, struct item* items, int items_length, const char *storename)
{
...
void Clif::cartlist(struct map_session_data *sd)
{
	struct item_data *id;
	int i,n,ne;
	unsigned char *buf;
...
void Clif::guild_xy(struct map_session_data *sd)
{
	unsigned char buf[10];
 ...
 *------------------------------------------*/
void Clif::guild_xy_single(int fd, struct map_session_data *sd)
{
	if( sd->bg_id )
		return;
...
void Clif::guild_xy_remove(struct map_session_data *sd)
{
	unsigned char buf[10];
...
{
	nullpo_ret(sd);
	map_obj.foreachinallarea(clif_hpmeter_sub, sd->bl.m, sd->bl.x-AREA_SIZE, sd->bl.y-AREA_SIZE, sd->bl.x+AREA_SIZE, sd->bl.y+AREA_SIZE, BL_PC, sd);
	return 0;
}
...
void Clif::updatestatus(struct map_session_data *sd,int type)
{
	int fd,len=8;
...
void Clif::changestatus(struct map_session_data* sd,int type,int val)
{
	...
	switch(type)
	{
		case SP_MANNER:
			...
			break;
		default:
			ShowError("clif.changestatus : unrecognized type %d.\n",type);
			return;
	}
...
void Clif::sprite_change(struct block_list *bl, int id, int type, int val, int val2, enum send_target target) {
	...
	WBUFW(buf,9) = val2;
#endif
	clif.send(buf,packet_len(cmd), bl, target);
}


/// Updates sprite/style properties of an object.
void Clif::changelook(struct block_list *bl, int type, int val) {
	struct map_session_data* sd;
...
#if PACKETVER < 4
	clif.sprite_change(bl, bl->id, type, val, 0, target);
#else
	...
	if( disguised(bl) ) {
		clif.sprite_change(bl, bl->id, type, val, val2, AREA_WOS);
		clif.sprite_change(bl, -bl->id, type, val, val2, SELF);
	} else
		clif.sprite_change(bl, bl->id, type, val, val2, target);
#endif
}


//Sends a change-base-look packet required for traps as they are triggered.
void Clif::changetraplook(struct block_list *bl,int val) {
	clif.sprite_change(bl, bl->id, LOOK_BASE, val, 0, AREA);
}


/// For the stupid cloth-dye bug. Resends the given view data to the area specified by bl.
void Clif::refreshlook(struct block_list *bl, int id, int type, int val, enum send_target target) {
	clif.sprite_change(bl, id, type, val, 0, target);
}
...
/// 013c <index>.W
void Clif::arrowequip(struct map_session_data *sd,int val) {
	int fd;

	nullpo_retv(sd);

#if PACKETVER >= 20121128
	clif.status_change(&sd->bl, EFST_CLIENT_ONLY_EQUIP_ARROW, 1, INFINITE_TICK, 0, 0, 0);
#endif
	fd=sd->fd;
...
void Clif::arrow_fail(struct map_session_data *sd,int type) {
	int fd;
...
void Clif::arrow_create_list(struct map_session_data *sd)
{
	int i, c;
	int fd;
...
///     0 = failure
///     1 = success
void Clif::statusupack(struct map_session_data *sd,int type,int ok,int val) {
	int fd;
...
void Clif::equipitemack(struct map_session_data *sd,int n,int pos,uint8 flag)
{
...
///     0 = failure
///     1 = success
void Clif::unequipitemack(struct map_session_data *sd,int n,int pos,int ok)
{
	int fd, header, offs = 0;
#if PACKETVER >= 20130000
	header = 0x99a;
...
void Clif::misceffect(struct block_list* bl,int type)
{
	unsigned char buf[32];
...
void Clif::changeoption(struct block_list* bl)
{
	unsigned char buf[32];
	struct status_change *sc;
	struct map_session_data* sd;
	...
	if (sc->data[SC_PROVOKE]) {
		...
		clif.status_change(bl, StatusIconChangeTable[SC_PROVOKE], 1, (!td ? INFINITE_TICK : DIFF_TICK(td->tick, gettick())), 0, 0, 0);
	}
}
...
void Clif::changeoption2(struct block_list* bl)
{
...
void Clif::useitemack(struct map_session_data *sd,int index,int amount,bool ok)
{
	nullpo_retv(sd);
...
void Clif::createchat(struct map_session_data* sd, int flag)
{
	int fd;
...
void Clif::dispchat(struct chat_data* cd, int fd)
{
	unsigned char buf[128];
	uint8 type;
...
///     2 = arena (npc waiting room)
///     3 = PK zone (non-clickable)
void Clif::changechatstatus(struct chat_data* cd)
{
	unsigned char buf[128];
	uint8 type;
...
/// 00d8 <chat id>.L
void Clif::clearchat(struct chat_data *cd,int fd)
{
	unsigned char buf[32];
...
void Clif::joinchatfail(struct map_session_data *sd,int flag)
{
	int fd;
...
///     0 = owner (menu)
///     1 = normal
void Clif::joinchatok(struct map_session_data *sd,struct chat_data* cd)
{
	int fd;
	int i,t;
...
void Clif::addchat(struct chat_data* cd,struct map_session_data *sd)
{
...
///     0 = owner (menu)
///     1 = normal
void Clif::changechatowner(struct chat_data* cd, struct map_session_data* sd)
{
...
///     0 = left
///     1 = kicked
void Clif::leavechat(struct chat_data* cd, struct map_session_data* sd, bool flag)
{
	unsigned char buf[32];
...
void Clif::traderequest(struct map_session_data* sd, const char* name)
{
	int fd = sd->fd;
...
#else
	struct map_session_data* tsd = map_obj.id2sd(sd->trade_partner);
	if( !tsd ) return;
...
///     4 = Cancel
///     5 = Busy
void Clif::tradestart(struct map_session_data* sd, uint8 type)
{
	int fd = sd->fd;
	struct map_session_data* tsd = map_obj.id2sd(sd->trade_partner);
	if( PACKETVER < 6 || !tsd ) {
...
void Clif::tradeadditem(struct map_session_data* sd, struct map_session_data* tsd, int index, int amount)
{
	int fd;
	unsigned char *buf;
...
///     1 = overweight
///     2 = trade canceled
void Clif::tradeitemok(struct map_session_data* sd, int index, int fail)
{
	int fd;
	nullpo_retv(sd);
...
///     0 = self
///     1 = other player
void Clif::tradedeal_lock(struct map_session_data* sd, int fail)
{
	int fd;
	nullpo_retv(sd);
...
/// 00ee
void Clif::tradecancelled(struct map_session_data* sd)
{
	int fd;
	nullpo_retv(sd);
...
///     0 = success
///     1 = failure
void Clif::tradecompleted(struct map_session_data* sd, int fail)
{
	int fd;
	nullpo_retv(sd);
...
void Clif::tradeundo(struct map_session_data* sd)
{
	int fd = sd->fd;
...
void Clif::updatestorageamount(struct map_session_data* sd, int amount, int max_amount)
{
	int fd;
...
void Clif::storageitemadded(struct map_session_data* sd, struct item* i, int index, int amount)
{
#if PACKETVER < 5
	const int cmd = 0xf4;
#elif PACKETVER < 20150225
...
void Clif::storageitemremoved(struct map_session_data* sd, int index, int amount)
{
	int fd;
...
/// 00f8
void Clif::storageclose(struct map_session_data* sd)
{
	int fd;
...
static void clif_getareachar_pc(struct map_session_data* sd,struct map_session_data* dstsd)
{
	...
	if( dstsd->chatID ) {
		...
		if( (cd = (struct chat_data*)map_obj.id2bl(dstsd->chatID)) && cd->usersd[0]==dstsd)
	...
	if( dstsd->sc.data[SC_DEVOTION] && (d_bl = map_obj.id2bl(dstsd->sc.data[SC_DEVOTION]->val1)) != NULL )
		clif.devotion(d_bl, sd);
}

void clif_get_areachar_unit(struct map_session_data* sd,struct block_list *bl)
{
	uint8 buf[128];
	struct unit_data *ud;
	struct view_data *vd;
		...
		case BL_PC:
			{
				...
				if(tsd->state.size==SZ_BIG) // tiny/big players [Valaris]
					clif.specialeffect_single(bl,EF_GIANTBODY2,sd->fd);
				else if(tsd->state.size==SZ_MEDIUM)
					clif.specialeffect_single(bl,EF_BABYBODY2,sd->fd);
				if( tsd->bg_id && map_obj.getmapflag(tsd->bl.m, MF_BATTLEGROUND) )
					clif.sendbgemblem_single(sd->fd,tsd);
				if ( tsd->status.robe )
					clif.refreshlook(&sd->bl,bl->id,LOOK_ROBE,tsd->status.robe,SELF);
				clif.efst_status_change_sub(&sd->bl, bl, SELF);
				clif.hat_effects(sd,bl,SELF);
			}
			break;
		...
		case BL_NPC:
			{
				TBL_NPC* nd = (TBL_NPC*)bl;
				if( nd->chat_id )
					clif.dispchat((struct chat_data*)map_obj.id2bl(nd->chat_id),sd->fd);
				if( nd->size == SZ_BIG )
					clif.specialeffect_single(bl,EF_GIANTBODY2,sd->fd);
				else if( nd->size == SZ_MEDIUM )
					clif.specialeffect_single(bl,EF_BABYBODY2,sd->fd);
				clif.efst_status_change_sub(&sd->bl, bl, SELF);
				clif.progressbar_npc(nd, sd);
			}
			break;
		case BL_MOB:
			{
				...
				if (battle_config.monster_hp_bars_info && !map_obj.getmapflag(bl->m, MF_HIDEMOBHPBAR)) {
					int i;
...
///     11 = lucky dodge
///     12 = (touch skill?)
int Clif::damage(struct block_list* src, struct block_list* dst, t_tick tick, int sdelay, int ddelay, int64 sdamage, int div, enum e_damage_type type, int64 sdamage2, bool spdamage)
{
 ...
 * src picks up dst
 *------------------------------------------*/
void Clif::takeitem(struct block_list* src, struct block_list* dst)
{
	//clif.damage(src,dst,0,0,0,0,0,DMG_PICKUP_ITEM,0,false);
	unsigned char buf[32];
 ...
 *------------------------------------------*/
void Clif::sitting(struct block_list* bl)
{
	unsigned char buf[32];
	nullpo_retv(bl);
 ...
 *------------------------------------------*/
void Clif::standing(struct block_list* bl)
{
	unsigned char buf[32];
	nullpo_retv(bl);
...
void Clif::changemapcell(int fd, int16 m, int x, int y, int type, enum send_target target)
{
	unsigned char buf[32];
	...
	WBUFW(buf,4) = y;
	WBUFW(buf,6) = type;
	mapindex_getmapname_ext(map_obj.mapid2mapname(m),WBUFCP(buf,8));
...
static void clif_graffiti(struct block_list *bl, struct skill_unit *unit, enum send_target target) {
	...
	clif.send(buf,packet_len(0x1c9),bl,target);
}
...
void Clif::getareachar_skillunit(struct block_list *bl, struct skill_unit *unit, enum send_target target, bool visible) {
...
/// 0120 <id>.L
void Clif::skill_delunit(struct skill_unit *unit)
{
	unsigned char buf[16];
...
void Clif::skillunit_update(struct block_list* bl)
{
	unsigned char buf[6];
	nullpo_retv(bl);
	...
	if (tsd && tsd->fd) { //Tell tsd that bl entered into his view
		switch(bl->type){
			...
			case BL_SKILL:
				...
				break;
			default:
				clif_get_areachar_unit(tsd,bl);
				break;
		}
	}
	if (sd && sd->fd) { //Tell sd that tbl walked into his view
		clif_get_areachar_unit(sd,tbl);
	}
	return 0;
}
...
void Clif::skillinfoblock(struct map_session_data *sd)
{
	int fd;
	int i,len,id;
...
void Clif::addskill(struct map_session_data *sd, int skill_id)
{
	int fd;
	uint16 idx = 0;
...
/// 0441 <skill id>.W
void Clif::deleteskill(struct map_session_data *sd, int skill_id)
{
...
void Clif::skillup(struct map_session_data *sd, uint16 skill_id, int lv, int range, int upgradable) {
	int fd;
...
void Clif::skillinfo(struct map_session_data *sd,int skill_id, int inf)
{
	const int fd = sd->fd;
...
///     1 = no text
void Clif::skillcasting(struct block_list* bl, int src_id, int dst_id, int dst_x, int dst_y, uint16 skill_id, int property, int casttime)
{
...
/// 01b9 <id>.L
void Clif::skillcastcancel(struct block_list* bl)
{
	unsigned char buf[16];
...
void Clif::skill_fail(struct map_session_data *sd,uint16 skill_id,enum useskill_fail_cause cause,int btype)
{
	int fd;

	if (!sd) {	//Since this is the most common nullpo....
		ShowDebug("clif.skill_fail: Error, received NULL sd for skill %d\n", skill_id);
		return;
	}
...
void Clif::skill_cooldown(struct map_session_data *sd, uint16 skill_id, t_tick tick)
{
...
int Clif::skill_damage(struct block_list *src,struct block_list *dst,t_tick tick,int sdelay,int ddelay,int64 sdamage,int div,uint16 skill_id,uint16 skill_lv,enum e_damage_type type)
{
	unsigned char buf[64];
	struct status_change *sc;
	int damage = (int)cap_value(sdamage,INT_MIN,INT_MAX);
...
/*
   int clif.skill_damage2(struct block_list *src,struct block_list *dst,t_tick tick,int sdelay,int ddelay,int damage,int div,uint16 skill_id,uint16 skill_lv,enum e_damage_type type)
   {
   unsigned char buf[64];
   struct status_change *sc;
...
bool Clif::skill_nodamage(struct block_list *src,struct block_list *dst, uint16 skill_id, int heal, t_tick tick)
{
...
void Clif::skill_poseffect(struct block_list *src,uint16 skill_id,int val,int x,int y,t_tick tick)
{
	unsigned char buf[32];
...
void Clif::skill_warppoint(struct map_session_data* sd, uint16 skill_id, uint16 skill_lv, unsigned short map1, unsigned short map2, unsigned short map3, unsigned short map4)
{
	int fd;
	nullpo_retv(sd);
	fd = sd->fd;
...
void Clif::skill_memomessage(struct map_session_data* sd, int type)
{
	int fd;
...
/// @param sd Who receives the message
/// @param type What message
void Clif::skill_teleportmessage(struct map_session_data *sd, int type)
{
	int fd;
...
void Clif::skill_estimation(struct map_session_data *sd,struct block_list *dst)
{
	struct status_data *status;
	unsigned char buf[64];
	int i;//, fix;
...
void Clif::skill_produce_mix_list(struct map_session_data *sd, int skill_id , int trigger)
{
	int i,c,view,fd;
	nullpo_retv(sd);
...
///     5 = GN_MAKEBOMB
///     6 = GN_S_PHARMACY
void Clif::cooking_list(struct map_session_data *sd, int trigger, uint16 skill_id, int qty, int list_type)
{
	int fd;
	int i, c;
	int view;
	...
	if( c > 0 || skill_id == AM_PHARMACY) {
		...
		clif.msg_skill(sd,skill_id,INVENTORY_SPACE_FULL);
#else
		WFIFOW(fd,2) = 6 + 2 * c;
		WFIFOSET(fd,WFIFOW(fd,2));
#endif
...
#endif
	clif.send(buf, packet_len(WBUFW(buf,0)), bl, target_type);
}
 ...
 * @param val3
 */
void Clif::status_change(struct block_list *bl, int type, int flag, t_tick tick, int val1, int val2, int val3) {
	struct map_session_data *sd = NULL;
 ...
 */
void Clif::efst_status_change_sub(struct block_list *tbl, struct block_list *bl, enum send_target target) {
	unsigned char i;
	...
	for (i = 0; i < sc_display_count; i++) {
		...
		if( spheres_sent && type >= SC_SPHERE_1 && type <= SC_SPHERE_5 ){
			...
			clif.efst_status_change(tbl, bl->id, AREA_WOS, StatusIconChangeTable[type], tick, sc_display[i]->val1, sc_display[i]->val2, sc_display[i]->val3);
#else
...
#if PACKETVER > 20120418
			clif.efst_status_change(tbl, bl->id, target, StatusIconChangeTable[type], tick, sc_display[i]->val1, sc_display[i]->val2, sc_display[i]->val3);
#else
...
void Clif::efst_status_change(struct block_list *bl, int tid, enum send_target target, int type, t_tick tick, int val1, int val2, int val3) {
...
void Clif::displaymessage(const int fd, const char* mes)
{
	nullpo_retv(mes);
	...
	void Clif::broadcast(struct block_list* bl, const char* mes, int len, int type, enum send_target target)
	{
		...
		clif.send(buf.get(), WBUFW(buf.get(),2), bl, target);
	}
	 ...
	 *------------------------------------------*/
	void Clif::GlobalMessage(struct block_list* bl, const char* message, enum send_target target) {
		size_t len;
		nullpo_retv(bl);
		...
		if( len > CHAT_SIZE_MAX ) {
			ShowWarning("clif.GlobalMessage: Truncating too long message '%s' (len=%" PRIuPTR ").\n", message, len);
			len = CHAT_SIZE_MAX;
		}
		...
		clif.send((unsigned char *)buf.get(),WBUFW(buf.get(),2),bl,target);
	}
	...
	void Clif::broadcast2(struct block_list* bl, const char* mes, int len, unsigned long fontColor, short fontType, short fontSize, short fontAlign, short fontY, enum send_target target)
	{
	 ...
	 */
	void Clif::channel_msg(struct Channel *channel, const char *msg, unsigned long color) {
		DBIterator *iter;
		...
		if( msg_len > sizeof(buf)-12 ) {
			ShowWarning("clif.channel_msg: Truncating too long message '%s' (len=%u).\n", msg, msg_len);
		...
		for( user = (struct map_session_data *)dbi_first(iter); dbi_exists(iter); user = (struct map_session_data *)dbi_next(iter) ) {
			clif.send(buf, len, &user->bl, SELF);
		}
		dbi_destroy(iter);
	}
	...
	///     7 = SP (SP_SP)
	///     ? = ignored
	void Clif::heal(int fd,int type,int val) {
#if PACKETVER < 20141022
		const int cmd = 0x13d;
#else
		const int cmd = 0xa27;
	...
	/// type:
	///     ignored
	void Clif::resurrection(struct block_list *bl,int type)
	{
		unsigned char buf[16];
	...
	void Clif::map_property(struct block_list *bl, enum map_property property, enum send_target t)
	{
		...
		clif.send(buf,packet_len(cmd),bl,t);
	}
	...
	void Clif::pvpset(struct map_session_data *sd,int pvprank,int pvpnum,int type)
	{
		if(type == 2) {
			int fd = sd->fd;
	 ...
	 *
	 *------------------------------------------*/
	void Clif::map_property_mapall(int map_idx, enum map_property property)
	{
	...
	///     1 = failure
	///     2 = downgrade
	void Clif::refine(int fd, int fail, int index, int val)
	{
	...
	void Clif::upgrademessage(int fd, int result, unsigned short item_id)
	{
	...
	void Clif::wis_message(struct map_session_data* sd, const char* nick, const char* mes, int mes_len, int gmlvl){
		int fd;
		...
		WFIFOSET(fd,WFIFOW(fd,2));
#else
		struct map_session_data *ssd = map_obj.nick2sd(nick,false);
	...
	void Clif::wis_end(int fd, int result)
	{
	...
	void Clif::solved_charname(int fd, int charid, const char* name)
	{
	...
	void Clif::use_card(struct map_session_data *sd,int idx)
	{
		int i,c,ep;
		int fd=sd->fd;
	...
	///     0 = success
	///     1 = failure
	void Clif::insert_card(struct map_session_data *sd,int idx_equip,int idx_card,int flag)
	{
		int fd;
	...
	void Clif::item_identify_list(struct map_session_data *sd)
	{
		int i,c;
		int fd;
	...
	void Clif::item_identified(struct map_session_data *sd,int idx,int flag)
	{
		int fd;
	...
	void Clif::item_repair_list(struct map_session_data *sd,struct map_session_data *dstsd, int lv)
	{
		int i,c;
		int fd;
		...
		void Clif::item_repaireffect(struct map_session_data *sd,int idx,int flag)
		{
			int fd;
		...
		void Clif::item_damaged(struct map_session_data* sd, unsigned short position)
		{
			int fd = sd->fd;
		...
		void Clif::item_refine_list(struct map_session_data *sd)
		{
			int i,c;
			int fd;
			uint16 skill_lv;
		...
		void Clif::item_skill(struct map_session_data *sd,uint16 skill_id,uint16 skill_lv)
		{
			int fd;
		...
		void Clif::cart_additem(struct map_session_data *sd,int n,int amount,int fail)
		{
#if PACKETVER < 5
			const int cmd = 0x124;
#elif PACKETVER < 20150225
		 ...
		 */
		void Clif::cart_additem_ack(struct map_session_data *sd, uint8 flag)
		{
			int fd;
			unsigned char *buf;
			nullpo_retv(sd);
			...
			WBUFW(buf,0) = 0x12c;
			WBUFB(buf,2) = flag;
			clif.send(buf,packet_len(0x12c),&sd->bl,SELF);
		}
		...
		void clif_parse_BankOpen(int fd, struct map_session_data* sd) {
			...
			if( !battle_config.feature_banking ) {
				clif.messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1496),false,SELF); //Banking is disabled
				return;
			}
			else {
		 ...
		 */
		void Clif::bank_deposit(struct map_session_data *sd, enum e_BANKING_DEPOSIT_ACK reason) {
			unsigned char buf[17];
			struct s_packet_db* info;
			int16 len;
			int cmd =0;
		 ...
		 */
		void Clif::bank_withdraw(struct map_session_data *sd,enum e_BANKING_WITHDRAW_ACK reason) {
			unsigned char buf[17];
			struct s_packet_db* info;
			int16 len;
			int cmd;
		...
		void Clif::cart_delitem(struct map_session_data *sd,int n,int amount)
		{
			int fd;
		...
		void Clif::openvendingreq(struct map_session_data* sd, int num)
		{
			int fd;
		...
		void Clif::showvendingboard(struct block_list* bl, const char* message, int fd)
		{
			unsigned char buf[128];
		...
		/// 0132 <owner id>.L
		void Clif::closevendingboard(struct block_list* bl, int fd)
		{
			unsigned char buf[16];
		...
		void Clif::vendinglist(struct map_session_data* sd, int id, struct s_vending* vending)
		{
			int i,fd;
			int count;
			...
			nullpo_retv(sd);
			nullpo_retv(vending);
			nullpo_retv(vsd=map_obj.id2sd(id));
		...
		void Clif::buyvending(struct map_session_data* sd, int index, int amount, int fail)
		{
			int fd;
		...
		void Clif::openvending(struct map_session_data* sd, int id, struct s_vending* vending)
		{
			int i,fd;
			int count;
		...
		void Clif::vendingreport(struct map_session_data* sd, int index, int amount, uint32 char_id, int zeny) {
		...
		///     ? = nothing
		void Clif::party_created(struct map_session_data *sd,int result)
		{
			int fd;
		...
		///     0 = connected
		///     1 = disconnected
		void Clif::party_member_info(struct party_data *p, struct map_session_data *sd)
		{
			int i;
			...
			mapindex_getmapname_ext(map_obj.mapid2mapname(sd->bl.m), WBUFCP(buf,offset+63));
		...
		void Clif::party_info(struct party_data* p, struct map_session_data *sd)
		{
		...
		void Clif::party_invite(struct map_session_data *sd,struct map_session_data *tsd)
		{
		...
		void Clif::party_invite_reply(struct map_session_data* sd, const char* nick, enum e_party_invite_reply reply)
		{
			int fd;
			nullpo_retv(sd);
			fd=sd->fd;
			...
			if( reply == PARTY_REPLY_OFFLINE ) {
				clif.displaymessage(fd, msg_txt(sd,3));
				return;
			}
#endif
		...
		///     0 = send to party
		///     1 = send to sd
		void Clif::party_option(struct party_data *p,struct map_session_data *sd,int flag)
		{
			unsigned char buf[16];
		 ...
		 **/
		void Clif::party_withdraw(struct map_session_data *sd, uint32 account_id, const char* name, enum e_party_member_withdraw result, enum send_target target) {
			...
			clif.send(buf, 2+4+NAME_LENGTH+1, &sd->bl, target);
		}


		/// Party chat message (ZC_NOTIFY_CHAT_PARTY).
		/// 0109 <packet len>.W <account id>.L <message>.?B
		void Clif::party_message(struct party_data* p, uint32 account_id, const char* mes, int len)
		{
			struct map_session_data *sd;
			int i;
			...
			if(i < MAX_PARTY){
				...
				if( len > sizeof(buf)-8 )
				{
					ShowWarning("clif.party_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", party_id=%d).\n", mes, len, sizeof(buf)-8, p->party.party_id);
					len = sizeof(buf)-8;
				}
		...
		void Clif::party_xy(struct map_session_data *sd)
		{
			unsigned char buf[16];
		 ...
		 * Sends x/y dot to a single fd. [Skotlex]
		 *------------------------------------------*/
		void Clif::party_xy_single(int fd, struct map_session_data *sd)
		{
		...
		void Clif::party_hp(struct map_session_data *sd)
		{
			unsigned char buf[16];
#if PACKETVER < 20100126
			const int cmd = 0x106;
		...
		/// 0AB2 <GID>.L <dead>.B
		void Clif::party_dead( struct map_session_data *sd ){
#if PACKETVER >= 20170502
			unsigned char buf[7];
			...
			clif.send(buf, packet_len(0xab2), &sd->bl, PARTY);
#endif
		}

		/// Updates the job and level of a party member
		/// 0abd <account id>.L <job>.W <level>.W
		void Clif::party_job_and_level(struct map_session_data *sd){
#if PACKETVER >= 20170502
			unsigned char buf[10];
		 ...
		 *------------------------------------------*/
		void Clif::hpmeter_single(int fd, int id, unsigned int hp, unsigned int maxhp)
		{
#if PACKETVER < 20100126
			const int cmd = 0x106;
#else
		...
		void Clif::movetoattack(struct map_session_data *sd,struct block_list *bl)
		{
			int fd;
		...
		///     2 = success (alchemist)
		///     3 = failure (alchemist)
		void Clif::produceeffect(struct map_session_data* sd,int flag, unsigned short nameid)
		{
			int view,fd;
		...
		/// 019e
		void Clif::catch_process(struct map_session_data *sd)
		{
			int fd;
		...
		///     0 = failure
		///     1 = success
		void Clif::pet_roulette(struct map_session_data *sd,int data)
		{
			int fd;
		...
		void Clif::sendegg(struct map_session_data *sd)
		{
			int i,n=0,fd;
			...
			if (battle_config.pet_no_gvg && map_flag_gvg2(sd->bl.m)) { //Disable pet hatching in GvG grounds during Guild Wars [Skotlex]
				clif.displaymessage(fd, msg_txt(sd,666));
				return;
			}
			WFIFOHEAD(fd, MAX_INVENTORY * 2 + 4);
			WFIFOW(fd,0)=0x1a6;
		...
		void Clif::send_petstatus(struct map_session_data *sd)
		{
			int fd;
			struct s_pet *pet;
		...
		/// data:
		///     @see CZ_PET_ACT.
		void Clif::pet_emotion(struct pet_data *pd,int param)
		{
			unsigned char buf[16];
			s_pet_db *pet_db_ptr;
		...
		///     0 = failure
		///     1 = success
		void Clif::pet_food(struct map_session_data *sd,int foodid,int fail)
		{
			int fd;
		...
		/// 01cd { <skill id>.L }*7
		void Clif::autospell(struct map_session_data *sd,uint16 skill_lv)
		{
			int fd;
		...
		void Clif::devotion(struct block_list *src, struct map_session_data *tsd)
		{
		 ...
		 * 01e1 <id>.L <amount>.W (ZC_SPIRITS2)
		 *------------------------------------------*/
		void Clif::spiritball(struct block_list *bl) {
			unsigned char buf[16];
		...
		void Clif::combo_delay(struct block_list *bl,t_tick wait)
		{
			unsigned char buf[32];
		...
		///     0 = inactive
		///     1 = active
		void Clif::bladestop(struct block_list *src, int dst_id, int active)
		{
			unsigned char buf[32];
		...
		/// MVP effect (ZC_MVP).
		/// 010c <account id>.L
		void Clif::mvp_effect(struct map_session_data *sd)
		{
			unsigned char buf[16];
		...
		/// 010a <name id>.W
		void Clif::mvp_item(struct map_session_data *sd, unsigned short nameid)
		{
			int view,fd;
		...
		/// 010b <exp>.L
		void Clif::mvp_exp(struct map_session_data *sd, unsigned int exp) {
#if PACKETVER >= 20131223		// Kro remove this packet [Napster]
			if (battle_config.mvp_exp_reward_message) {
				char e_msg[CHAT_SIZE_MAX];
				sprintf(e_msg, msg_txt(sd, 717), exp);
				clif.messagecolor(&sd->bl, color_table[COLOR_CYAN], e_msg, false, SELF);		// Congratulations! You are the MVP! Your reward EXP Points are %u !!
			}
#else
			int fd;
		...
		///     you are overweight."
		void Clif::mvp_noitem(struct map_session_data* sd)
		{
			int fd = sd->fd;
		...
		void Clif::guild_created(struct map_session_data *sd,int flag)
		{
			int fd;
		...
		///     &0x01 = allow invite
		///     &0x10 = allow expel
		void Clif::guild_belonginfo(struct map_session_data *sd)
		{
			int ps,fd;
			struct guild* g;
		...
		///     0 = offline
		///     1 = online
		void Clif::guild_memberlogin_notice(struct guild *g,int idx,int flag)
		{
			unsigned char buf[64];
			struct map_session_data* sd;
		...
		// Function `clif.guild_memberlogin_notice` sends info about
		...
		void Clif::guild_send_onlineinfo(struct map_session_data *sd)
		{
			struct guild *g;
		...
		///     &0x80 = Notice
		void Clif::guild_masterormember(struct map_session_data *sd)
		{
			int fd;
		...
		void Clif::guild_basicinfo(struct map_session_data *sd) {
			int fd;
			struct guild *g;
		...
		void Clif::guild_allianceinfo(struct map_session_data *sd)
		{
			int fd,i,c;
			struct guild *g;
		...
		void Clif::guild_memberlist(struct map_session_data *sd)
		{
			int fd;
			int i,c;
			struct guild *g;
		...
		void clif_positionnamelist(struct map_session_data *sd)
		{
			int i,fd;
			struct guild *g;
		...
		/// ranking:
		///     TODO
		void clif_positioninfolist(struct map_session_data *sd)
		{
			int i,fd;
			struct guild *g;
		...
		/// ranking:
		///     TODO
		void Clif::guild_positionchanged(struct guild *g,int idx)
		{
		...
		void Clif::guild_memberpositionchanged(struct guild *g,int idx)
		{
			// FIXME: This packet is intended to update the clients after a
		...
		void Clif::guild_emblem(struct map_session_data *sd,struct guild *g)
		{
			int fd;
			nullpo_retv(sd);
			nullpo_retv(g);
		...
		void Clif::guild_emblem_area(struct block_list* bl)
		{
			uint8 buf[12];
		...
		void Clif::guild_skillinfo(struct map_session_data* sd)
		{
			int fd;
			struct guild* g;
			int i,c;
		...
		void Clif::guild_notice(struct map_session_data* sd)
		{
			int fd;
			struct guild* g;
		...
		void Clif::guild_invite(struct map_session_data *sd,struct guild *g)
		{
			int fd;
		...
		///     2 = Offer accepted.
		///     3 = Guild full.
		void Clif::guild_inviteack(struct map_session_data *sd,int flag)
		{
			int fd;
		...
		void Clif::guild_leave(struct map_session_data *sd,const char *name,const char *mes)
		{
			unsigned char buf[128];
		...
		void Clif::guild_expulsion(struct map_session_data* sd, const char* name, const char* mes, uint32 account_id)
		{
			unsigned char buf[128];
		...
		void Clif::guild_message(struct guild *g,uint32 account_id,const char *mes,int len)
		{// TODO: account_id is not used, candidate for deletion? [Ai4rei]
			...
			{
				ShowWarning("clif.guild_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", guild_id=%d).\n", mes, len, sizeof(buf)-5, g->guild_id);
				len = sizeof(buf)-5;
			}
		...
		void Clif::guild_reqalliance(struct map_session_data *sd,uint32 account_id,const char *name)
		{
			int fd;
		...
		void Clif::guild_allianceack(struct map_session_data *sd,int flag)
		{
			int fd;
		...
		///     0 = Ally
		///     1 = Enemy
		void Clif::guild_delalliance(struct map_session_data *sd,int guild_id,int flag)
		{
			int fd;
		...
		void Clif::guild_oppositionack(struct map_session_data *sd,int flag)
		{
			int fd;
		...
		/*
		   void Clif::guild_allianceadded(struct guild *g,int idx)
		   {
		   unsigned char buf[64];
		   WBUFW(buf,0)=0x185;
		...
		void Clif::guild_broken(struct map_session_data *sd,int flag)
		{
			int fd;
		...
		/// type:
		///     enum emotion_type
		void Clif::emotion(struct block_list *bl,int type)
		{
			unsigned char buf[8];
		...
		void Clif::talkiebox(struct block_list* bl, const char* talkie)
		{
		...
		/// 01ea <id>.L
		void Clif::wedding_effect(struct block_list *bl)
		{
			unsigned char buf[6];
		...
		void Clif::callpartner(struct map_session_data *sd)
		{
			...
			if( sd->status.partner_id ) {
				const char *p = map_obj.charid2nick(sd->status.partner_id);
		...
		/*
		   void Clif::marriage_process(struct map_session_data *sd)
		   {
		   int fd;
		   nullpo_retv(sd);
		...
		void Clif::divorced(struct map_session_data* sd, const char* name)
		{
			int fd;
			nullpo_retv(sd);
		...
		/// This packet while still implemented by the client is no longer being officially used.
		/*
		   void Clif::marriage_proposal(int fd, struct map_session_data *sd, struct map_session_data* ssd)
		   {
		   nullpo_retv(sd);
		 ...
		 *------------------------------------------*/
		void Clif::disp_message(struct block_list* src, const char* mes, int len, enum send_target target)
		{
			...
			if( len == 0 ) {
				...
				ShowWarning("clif.disp_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", aid=%d).\n", mes, len, sizeof(buf)-5, src->id);
				len = sizeof(buf)-5;
			}
		...
		///     0 = failure
		///     1 = success
		void Clif::GM_kickack(struct map_session_data *sd, int id)
		{
			int fd;
		...
		void Clif::GM_kick(struct map_session_data *sd, struct map_session_data *tsd)
		{
			int fd;
		...
		void Clif::manner_message(struct map_session_data* sd, uint32 type)
		{
			int fd;
			nullpo_retv(sd);
		...
		///     0 = positive (unmute)
		///     1 = negative (mute)
		void Clif::GM_silence(struct map_session_data* sd, struct map_session_data* tsd, uint8 type)
		{
			int fd;
			nullpo_retv(sd);
			nullpo_retv(tsd);
		...
		/// 07fe <bgm>.24B
		void Clif::playBGM(struct map_session_data* sd, const char* name)
		{
			int fd;
		...
		void Clif::soundeffect(struct map_session_data* sd, struct block_list* bl, const char* name, int type)
		{
			int fd;
		...
		void Clif::soundeffectall(struct block_list* bl, const char* name, int type, enum send_target coverage)
		{
		...
		void Clif::specialeffect(struct block_list* bl, int type, enum send_target target)
		{
		...
		void Clif::specialeffect_single(struct block_list* bl, int type, int fd)
		{
			WFIFOHEAD(fd,10);
			WFIFOW(fd,0) = 0x1f3;
			WFIFOL(fd,2) = bl->id;
		...
		void Clif::specialeffect_value(struct block_list* bl, int effect_id, int num, send_target target)
		{
			uint8 buf[14];
		...
		void Clif::messagecolor_target(struct block_list *bl, unsigned long color, const char *msg, bool rgb2bgr, enum send_target type, struct map_session_data *sd) {
			...
			if (msg_len > sizeof(buf) - 12) {
				ShowWarning("clif.messagecolor: Truncating too long message '%s' (len=%u).\n", msg, msg_len);
				msg_len = sizeof(buf) - 12;
			}
			...
			clif.send(buf, WBUFW(buf,2), (sd == NULL ? bl : &(sd->bl)), type);
		}
		 ...
		 */
		void Clif::refresh_storagewindow(struct map_session_data *sd) {
			...
			if( sd->state.storage_flag == 1 ) {
				...
				clif.storagelist(sd, sd->storage.u.items_storage, ARRAYLENGTH(sd->storage.u.items_storage), storage_getName(0));
				clif.updatestorageamount(sd, sd->storage.amount, MAX_STORAGE);
			}
			...
			if( sd->state.storage_flag == 2 ) {
				...
				if( !gstor ) // Shouldn't happen. The information should already be at the map-server
					...
					clif.updatestorageamount(sd, gstor->amount, MAX_GUILD_STORAGE);
				}
			}
		}

		// refresh the client's screen, getting rid of any effects
		void Clif::refresh(struct map_session_data *sd)
		{
			nullpo_retv(sd);
			...
			map_obj.foreachinallrange(clif_getareachar,&sd->bl,AREA_SIZE,BL_ALL,sd);
			clif_weather_check(sd);
			if( sd->chatID )
				chat_leavechat(sd,0);
			if( sd->state.vending )
			...
			clif.efst_status_change_sub(&sd->bl,&sd->bl,SELF);
		...
		void Clif::name( struct block_list* src, struct block_list *bl, send_target target ){
			...
			switch( bl->type )
			{
				...
				case BL_ELEM:
					...
					break;
				default:
					ShowError("clif.name: bad type %d(%d)\n", bl->type, bl->id);
					return;
			}

			clif.send(buf, packet_len(cmd), src, target);
		}
		...
		/// stays that way.
		void Clif::slide(struct block_list *bl, int x, int y)
		{
			unsigned char buf[10];
			nullpo_retv(bl);
		...
		void Clif::disp_overhead_(struct block_list *bl, const char* mes, enum send_target flag)
		{
			...
			if (len_mes > sizeof(buf)-8) {
				ShowError("clif.disp_overhead: Message too long (length %d)\n", len_mes);
		 ...
		 *--------------------------*/
		void Clif::party_xy_remove(struct map_session_data *sd)
		{
			unsigned char buf[16];
			nullpo_retv(sd);
			WBUFW(buf,0)=0x107;
		...
		///     ? = nothing
		void Clif::gospel_info(struct map_session_data *sd, int type)
		{
			int fd=sd->fd;
		...
		void Clif::starskill(struct map_session_data* sd, const char* mapname, int monster_id, unsigned char star, unsigned char result)
		{
			int fd = sd->fd;
		 ...
		 *------------------------------------------*/
		void Clif::feel_info(struct map_session_data* sd, unsigned char feel_level, unsigned char type)
		{
		 ...
		 *------------------------------------------*/
		void Clif::hate_info(struct map_session_data *sd, unsigned char hate_level,int class_, unsigned char type)
		{
			if( pcdb_checkid(class_) ) {
				...
				ShowWarning("clif.hate_info: Received invalid class %d for this packet (char_id=%d, hate_level=%u, type=%u).\n", class_, sd->status.char_id, (unsigned int)hate_level, (unsigned int)type);
			}
		}
		 ...
		 *------------------------------------------*/
		void Clif::mission_info(struct map_session_data *sd, int mob_id, unsigned char progress)
		{
		 ...
		 *------------------------------------------*/
		void Clif::feel_hate_reset(struct map_session_data *sd)
		{
		...
		///		false = disabled
		///		true = enabled
		void Clif::configuration( struct map_session_data* sd, enum e_config_type type, bool enabled ){
			int fd;
			nullpo_retv(sd);
			fd = sd->fd;
		...
		///     0 = disabled
		///     1 = enabled
		void Clif::equipcheckbox(struct map_session_data* sd)
		{
			int fd;
			nullpo_retv(sd);
			fd = sd->fd;
		...
		void Clif::viewequip_ack(struct map_session_data* sd, struct map_session_data* tsd)
		{
			uint8* buf;
			int i, n, fd, offset = 0;
#if PACKETVER < 20100629
		...
		/// 0291 <message>.W
		void Clif::msg(struct map_session_data* sd, unsigned short id)
		{
			int fd;
			nullpo_retv(sd);
			fd = sd->fd;
		...
		void Clif::msg_value(struct map_session_data* sd, unsigned short id, int value)
		{
			int fd = sd->fd;
		...
		///       "[SkillName] Message"
		void Clif::msg_skill(struct map_session_data* sd, uint16 skill_id, int msg_id)
		{
			int fd = sd->fd;
		...
		inline void clif_pk_mode_message(struct map_session_data * sd)
		{
			if (battle_config.pk_mode && battle_config.pk_mode_mes &&
					sd && map_obj.getmapflag(sd->bl.m, MF_PVP)) {
				if( (int)sd->status.base_level < battle_config.pk_min_level ) {
					...
					clif.showscript(&sd->bl, output, SELF);
				} else {
					// 1503: You've entered a PK Zone.
					clif.showscript(&sd->bl, msg_txt(sd,1503), SELF);
				}
			}
			return;
		}
		...
		void clif_parse_WantToConnection(int fd, struct map_session_data* sd)
		{
			...
			if( err ){ // connection rejected
				ShowInfo("clif.parse: Disconnecting session #%d with unknown connect packet 0x%04x(length:%d)%s\n", fd, cmd, RFIFOREST(fd), (
							err == 1 ? "." :
			...
			sd->cryptKey = (((((cryptKey[0] * cryptKey[1]) + cryptKey[2]) & 0xFFFFFFFF) * cryptKey[1]) + cryptKey[2]) & 0xFFFFFFFF;
#endif
		...
		void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
		{
			...
			// guild
			// (needs to go before clif.spawn() to show guild emblems correctly)
			...
			if(map_obj.addblock(&sd->bl))
				return;
			clif.spawn(&sd->bl);

			// Party
			// (needs to go after clif.spawn() to show hp bars correctly)
			if(sd->status.party_id) {
			...
			map_obj.foreachinallarea(clif_getareachar, sd->bl.m, sd->bl.x-AREA_SIZE, sd->bl.y-AREA_SIZE, sd->bl.x+AREA_SIZE, sd->bl.y+AREA_SIZE, BL_ALL, sd);

			// pet
			if( sd->pd ) {
				if( battle_config.pet_no_gvg && mapdata_flag_gvg(mapdata) ) { //Return the pet to egg. [Skotlex]
					clif.displaymessage(sd->fd, msg_txt(sd,666));
			...
			if(sd->state.connect_new) {
				...
				if (sd->sc.option&OPTION_FALCON)
					clif.status_load(&sd->bl, EFST_FALCON, 1);
				else if (sd->sc.option&(OPTION_RIDING|OPTION_DRAGON|OPTION_MADOGEAR))
					clif.status_load(&sd->bl, EFST_RIDING, 1);
				else if (sd->sc.option&OPTION_WUGRIDER)
					clif.status_load(&sd->bl, EFST_WUGRIDER, 1);
				else if (sd->sc.data[SC_ALL_RIDING])
					clif.status_load(&sd->bl, EFST_ALL_RIDING, 1);
				...
				if (night_flag && mapdata->flag[MF_NIGHTENABLED]) {
					sd->state.night = 1;
					clif.status_load(&sd->bl, EFST_SKE, 1);
				}

				// Notify everyone that this char logged in [Skotlex].
				map_obj.foreachpc(clif_friendslist_toggle_sub, sd->status.account_id, sd->status.char_id, 1);
			...
			if( sd->state.changemap ) {// restore information that gets lost on map-change
				...
				if( battle_config.homunculus_autofeed_always ){
					...
					if( sd->hd && battle_config.feature_homunculus_autofeed ){
						clif.configuration( sd, CONFIG_HOMUNCULUS_AUTOFEED, sd->hd->homunculus.autofeed );
					}else{
						clif.configuration( sd, CONFIG_HOMUNCULUS_AUTOFEED, false );
					}
				}else{
					// Only send when enabled
					if( sd->hd && battle_config.feature_homunculus_autofeed && sd->hd->homunculus.autofeed ){
						clif.configuration( sd, CONFIG_HOMUNCULUS_AUTOFEED, true );
					}
				}
#endif

				if (sd->guild && battle_config.guild_notice_changemap == 1){
					clif.guild_notice(sd); // Displays after VIP
				...
				map_obj.iwall_get(sd); // Updates Walls Info on this Map to Client
				...
				if (!sd->state.connect_new &&
						...
						map_obj.getmapflag(sd->state.pmap, MF_BEXP) != mapdata->flag[MF_BEXP]
				   )
				{
			...
			if( sd->guild && ( battle_config.guild_notice_changemap == 2 || guild_notice ) ){
				clif.guild_notice(sd); // Displays at end
			}
			...
			if (pc_checkskill(sd, SG_DEVIL) && pc_is_maxjoblv(sd))
				clif.status_load(&sd->bl, EFST_DEVIL1, 1);  //blindness [Komurka]
		...
		void Clif::hotkeys_send(struct map_session_data *sd) {
#ifdef HOTKEY_SAVING
			const int fd = sd->fd;
			int i;
			int offset = 2;
		...
		/// 02f0 <color>.L <time>.L
		void Clif::progressbar(struct map_session_data * sd, unsigned long color, unsigned int second)
		{
			int fd = sd->fd;
		...
		/// 02f2
		void Clif::progressbar_abort(struct map_session_data * sd)
		{
			int fd = sd->fd;
		...
		void Clif::progressbar_npc( struct npc_data *nd, struct map_session_data* sd ){
			...
			if( nd->progressbar.timeout > 0 ){
				...
				if( sd ){
					clif.send(buf, packet_len(0x9d1), &sd->bl, SELF);
				}else{
					clif.send(buf, packet_len(0x9d1), &nd->bl, AREA);
				}
			}
#endif
		}
		...
		void clif_parse_WalkToXY(int fd, struct map_session_data *sd)
		{
			...
			if(battle_config.official_cell_stack_limit > 0
					&& sd->bl.x == x+1 && sd->bl.y == y && map_obj.count_oncell(sd->bl.m, x, y, BL_CHAR|BL_NPC, 1))
				return;
		...
		void clif_parse_GlobalMessage(int fd, struct map_session_data* sd)
		{
			...
			clif.GlobalMessage(&sd->bl,output,sd->chatID ? CHAT_WOS : AREA_CHAT_WOC);
			...
			map_obj.foreachinallrange(npc_chat_sub, &sd->bl, AREA_SIZE, BL_NPC, output, strlen(output), &sd->bl);
#endif
		...
		///     6 = east
		///     7 = northeast
		void Clif::changed_dir(struct block_list *bl, enum send_target target)
		{
			unsigned char buf[64];
		...
		{
			clif_user_count(sd, map_obj.getusers());
		}


		void Clif::parse_ActionRequest_sub(struct map_session_data *sd, int action_type, int target_id, t_tick tick)
		{
			if (pc_isdead(sd)) {
				clif.clearunit_area(&sd->bl, CLR_DEAD);
				return;
		...
		{
			struct s_packet_db* info = &packet_db[RFIFOW(fd,0)];
			clif.parse_ActionRequest_sub(sd,
		...
		void clif_parse_WisMessage(int fd, struct map_session_data* sd)
		{
			...
			if (target[0] && (strncasecmp(target,"NPC:",4) == 0) && (strlen(target) > 4))
			{
				...
				if(channel && (pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) || ((channel->opt&CHAN_OPT_CAN_CHAT) && channel_pccheckgroup(channel,sd->group_id)))){
					if(channel_pc_haschan(sd,channel)>=0){ //we are in the chan
						...
						clif.displaymessage(fd, msg_txt(sd,1402)); //You're not in that channel, type '@join <#channel_name>'
					}
					return;
				}
			}
			...
			dstsd = map_obj.nick2sd(target,false);
			...
			if (dstsd->state.autotrade == 1){
				...
				clif.wis_message(sd, wisp_server_name, output, strlen(output) + 1, 0);
				return;
			}
			...
			// Normal message
			clif.wis_message(dstsd, sd->status.name, message, strlen(message)+1, 0);
		}
		...
		void clif_parse_TakeItem(int fd, struct map_session_data *sd)
		{
			...
			fitem = (struct flooritem_data*)map_obj.id2bl(map_object_id);
		...
		void clif_parse_EquipItem(int fd,struct map_session_data *sd)
		{
			...
			if(!sd->inventory.u.items_inventory[index].identify) {
				clif.equipitemack(sd,index,0,ITEM_EQUIP_ACK_FAIL);	// fail
				return;
			}
		...
		void clif_parse_NpcClicked(int fd,struct map_session_data *sd)
		{
			...
			bl = map_obj.id2bl(RFIFOL(fd,info->pos[0]));
			//type = RFIFOB(fd,info->pos[1]);
			if (!bl) return;
			switch (bl->type) {
				case BL_MOB:
				case BL_PC:
					clif.parse_ActionRequest_sub(sd, 0x07, bl->id, gettick());
					break;
				case BL_NPC:
#ifdef RENEWAL
		...
		static void clif_noask_sub(struct map_session_data *sd, struct map_session_data *tsd, int type)
		{
			...
			msg = msg_txt(sd,392);
			clif.messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msg, false, SELF);
			//Notice that a request was rejected.
			snprintf(output, 256, msg_txt(tsd,393+type), sd->status.name, 256);
			clif.messagecolor(&tsd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		...
		void clif_parse_TradeRequest(int fd,struct map_session_data *sd)
		{
			...
			t_sd = map_obj.id2sd(RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]));
		...
		void Clif::SelectCart(struct map_session_data *sd) {
...
#endif
					TBL_NPC* nd = map_obj.id2nd(npc_id);
		...
		void clif_parse_UseCard(int fd,struct map_session_data *sd)
		{
			if (sd->state.trading != 0)
				return;
			clif.use_card(sd,RFIFOW(fd,packet_db[RFIFOW(fd,0)].pos[0])-2);
		}
		...
		void clif_parse_MoveToKafra(int fd, struct map_session_data *sd)
		{
			...
			if( sd->inventory.u.items_inventory[item_index].equipSwitch ){
				clif.msg( sd, C_ITEM_EQUIP_SWITCH );
				return;
			}
		...
		void clif_parse_CreateParty(int fd, struct map_session_data *sd){
			...
			if( map_obj.getmapflag(sd->bl.m, MF_PARTYLOCK) ) {// Party locked.
				clif.displaymessage(fd, msg_txt(sd,227));
				return;
			}
		...
		void clif_parse_PartyInvite2(int fd, struct map_session_data *sd){
			...
			t_sd = map_obj.nick2sd(name,false);
		...
		void clif_parse_PartyChangeLeader(int fd, struct map_session_data* sd){
			party_changeleader(sd, map_obj.id2sd(RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0])),NULL);
		}
		...
		void Clif::PartyBookingRegisterAck(struct map_session_data *sd, int flag)
		{
			int fd = sd->fd;
		...
		///     0 = no
		///     1 = yes
		void Clif::PartyBookingSearchAck(int fd, struct party_booking_ad_info** results, int count, bool more_result)
		{
			int i, j;
		...
		void Clif::PartyBookingDeleteAck(struct map_session_data* sd, int flag)
		{
			int fd = sd->fd;
		...
		void Clif::PartyBookingInsertNotify(struct map_session_data* sd, struct party_booking_ad_info* pb_ad)
		{
			int i;
		...
		void Clif::PartyBookingUpdateNotify(struct map_session_data* sd, struct party_booking_ad_info* pb_ad)
		{
			int i;
		...
		/// 080b <index>.L
		void Clif::PartyBookingDeleteNotify(struct map_session_data* sd, int index)
		{
			uint8 buf[6];
		...
		void clif_parse_CreateGuild(int fd,struct map_session_data *sd){
			...
			if(map_obj.getmapflag(sd->bl.m, MF_GUILDLOCK)) { //Guild locked.
				clif.displaymessage(fd, msg_txt(sd,228));
				return;
			}
		...
		void clif_parse_GuildChangeMemberPosition(int fd, struct map_session_data *sd)
		{
			...
			if( len == 16 && RFIFOL(fd,12) == 0 ){
				...
				if( battle_config.guild_leaderchange_delay && DIFF_TICK(time(NULL),sd->guild->last_leader_change) < battle_config.guild_leaderchange_delay ){
					clif.msg(sd, GUILD_MASTER_DELAY);
					return;
				}
		...
		void clif_parse_GuildChangeEmblem(int fd,struct map_session_data *sd){
			...
			if(!(battle_config.emblem_woe_change) && is_agit_start() ){
				clif.messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,385),false,SELF); //"You not allowed to change emblem during woe"
				return;
			}
			...
			if(emb_val == EMBVALIDATE_ERR_RAW_FILEFORMAT){
				...
				clif.messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,386),false,SELF); //"The chosen emblem was detected invalid\n"
				return;
				...
				clif.messagecolor(&sd->bl,color_table[COLOR_RED],output,false,SELF); //"The chosen emblem was detected invalid as it contain too much transparency (limit=%d)\n"
				return;
			}
		...
		void clif_parse_GuildInvite(int fd,struct map_session_data *sd){
			struct s_packet_db* info = &packet_db[RFIFOW(fd,0)];
			struct map_session_data *t_sd = map_obj.id2sd(RFIFOL(fd,info->pos[0]));
		...
		void
			clif_parse_GuildInvite2(int fd, struct map_session_data *sd) {
				struct map_session_data *t_sd = map_obj.nick2sd(RFIFOCP(fd, packet_db[RFIFOW(fd,0)].pos[0]),false);
		...
		void clif_parse_GuildLeave(int fd,struct map_session_data *sd){
			...
			if( sd->bg_id ) {
				clif.displaymessage(fd, msg_txt(sd,670)); //"You can't leave battleground guilds."
				return;
			}
		...
		void clif_parse_GuildExpulsion(int fd,struct map_session_data *sd){
			struct s_packet_db* info = &packet_db[RFIFOW(fd,0)];
			if( map_obj.getmapflag(sd->bl.m, MF_GUILDLOCK) || sd->bg_id )
			{ // Guild locked.
				clif.displaymessage(fd, msg_txt(sd,228));
				return;
			}
		...
		{
			if(sd->pd)
				clif.pet_emotion(sd->pd,RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]));
		}
		...
		void clif_parse_GMRemove2(int fd, struct map_session_data* sd)
		{
			...
			if( (pl_sd = map_obj.id2sd(account_id)) != NULL ) {
				char command[NAME_LENGTH+8];
		...
		void Clif::account_name(int fd, uint32 account_id, const char* accname)
		{
		...
		void clif_parse_GMReqAccountName(int fd, struct map_session_data *sd)
		{
			...
			/*
			   ...
			   intif_request_accinfo(sd->fd, sd->bl.id, pc_get_group_level(sd), query, 1); //will call clif.account_name at return
			   */

			clif.account_name(fd, account_id, "");
		}
		...
		//Subfunction called from clif.foreachclient to toggle friends on/off [Skotlex]
		...
		void Clif::friendslist_send(struct map_session_data *sd)
		{
			int i = 0, n, fd = sd->fd;
#if PACKETVER >= 20180221
			const int size = 8;
			...
			for (n = 0; n < i; n++) { //Sending the online players
				if (map_obj.charid2sd(sd->status.friends[n].char_id))
		...
		void clif_friendslust_req(struct map_session_data* sd, uint32 account_id, uint32 char_id, const char* name)
		{
			int fd = sd->fd;
		...
		void clif_parse_FriendsListAdd(int fd, struct map_session_data *sd)
		{
			...
			f_sd = map_obj.nick2sd(RFIFOCP(fd,packet_db[RFIFOW(fd,0)].pos[0]),false);
			...
			for (i = 0; i < MAX_FRIENDS && sd->status.friends[i].char_id != 0; i++) {
				if (sd->status.friends[i].char_id == f_sd->status.char_id) {
					clif.displaymessage(fd, msg_txt(sd,671)); //"Friend already exists."
					return;
				}
			}
			...
			clif_friendslust_req(f_sd, sd->status.account_id, sd->status.char_id, sd->status.name);
		}
		...
		void clif_parse_FriendsListReply(int fd, struct map_session_data *sd)
		{
			...
			f_sd = map_obj.id2sd(account_id); //The account id is the same as the bl.id of players.
			if (f_sd == NULL)
				return;
		...
		void clif_parse_FriendsListRemove(int fd, struct map_session_data *sd)
		{
			...
			if (i == MAX_FRIENDS) {
				clif.displaymessage(fd, msg_txt(sd,672)); //"Name not found in list."
				return;
			}

			//remove from friend's list first
			if( (f_sd = map_obj.id2sd(account_id)) && f_sd->status.char_id == char_id) {
				for (i = 0; i < MAX_FRIENDS &&
				...
				if(chrif_removefriend(char_id,sd->status.char_id)) { // char-server offline, abort
					clif.displaymessage(fd, msg_txt(sd,673)); //"This action can't be performed at the moment. Please try again later."
					return;
				}
			}
			...
			clif.displaymessage(fd, msg_txt(sd,674)); //"Friend removed"
		...
		void clif_parse_FeelSaveOk(int fd,struct map_session_data *sd)
		{
			...
			//	clif.misceffect2(&sd->bl, 0x1b0);
			//	clif.misceffect2(&sd->bl, 0x21f);
		...
		///     1 = moon
		///     2 = star
		void Clif::feel_req(int fd, struct map_session_data *sd, uint16 skill_lv)
		{
		...
		void clif_parse_Check(int fd, struct map_session_data *sd)
		{
			...
			if( ( pl_sd = map_obj.nick2sd(charname,false) ) == NULL || pc_get_group_level(sd) < pc_get_group_level(pl_sd) )
			{
				return;
			}
		...
		void Clif::mail_getattachment(struct map_session_data* sd, struct mail_message *msg, uint8 result, enum mail_attachment_type type) {
			int fd = sd->fd;
			...
			clif.Mail_refreshinbox( sd, msg->type, 0 );
#endif
		}
		...
		void Clif::Mail_send(struct map_session_data* sd, enum mail_send_result result){
			int fd = sd->fd;
#if PACKETVER < 20150513
		...
		void Clif::mail_delete( struct map_session_data* sd, struct mail_message *msg, bool success ){
			int fd = sd->fd;
		...
		///     0 = success
		///     1 = failure
		void Clif::Mail_return(int fd, int mail_id, short fail)
		{
		...
		void Clif::Mail_new(struct map_session_data* sd, int mail_id, const char *sender, const char *title){
			int fd = sd->fd;
#if PACKETVER < 20150513
		...
		///     0 = open
		///     1 = close
		void Clif::Mail_window(int fd, int flag)
		{
		...
		void Clif::Mail_refreshinbox(struct map_session_data *sd,enum mail_inbox_type type,int64 mailID){
#if PACKETVER < 20150513
			int fd = sd->fd;
		...
		void clif_parse_Mail_refreshinbox(int fd, struct map_session_data *sd){
			...
			if( md->amount < MAIL_MAX_INBOX && (md->full || sd->mail.changed) )
				...
				clif.Mail_refreshinbox(sd, MAIL_INBOX_NORMAL,0);
			...
			clif.Mail_refreshinbox(sd,(enum mail_inbox_type)openType,mailId);
#endif
		}
		...
		void Clif::Mail_read(struct map_session_data *sd, int mail_id)
		{
			int i, fd = sd->fd;
			...
			if( i == MAIL_MAX_INBOX ) {
				...
				if (msg->status == MAIL_UNREAD) {
					...
					clif.Mail_new(sd, 0, msg->send_name, msg->title);
				}
			}
		}
		...
		void Clif::Mail_Receiver_Ack( struct map_session_data* sd, uint32 char_id, short class_, uint32 level, const char* name ){
			int fd = sd->fd;
		...
		void clif_parse_Mail_getattach( int fd, struct map_session_data *sd ){
			...
			if( attachment&MAIL_ATT_ZENY ){
				if( msg->zeny + sd->status.zeny > MAX_ZENY ){
					clif.mail_getattachment(sd, msg, 1, MAIL_ATT_ZENY); //too many zeny
					return;
				}else{
					...
					if( item->nameid > 0 && item->amount > 0 ){
						...
						switch( pc_checkadditem(sd, item->nameid, item->amount) ){
							...
							case CHKADDITEM_OVERAMOUNT:
								clif.mail_getattachment(sd, msg, 2, MAIL_ATT_ITEM);
								return;
						}
		...
		void Clif::mail_removeitem( struct map_session_data* sd, bool success, int index, int amount ){
			int fd = sd->fd;
		...
		///     0 = open
		///     1 = close
		void Clif::Auction_openwindow(struct map_session_data *sd)
		{
			int fd = sd->fd;
		...
		void Clif::Auction_results(struct map_session_data *sd, short count, short pages, uint8 *buf)
		{
		...
		void Clif::Auction_message(int fd, unsigned char flag)
		{
		...
		void Clif::Auction_close(int fd, unsigned char flag)
		{
		...
		void clif_parse_Auction_register(int fd, struct map_session_data *sd)
		{
			...
			if( sd->inventory.u.items_inventory[sd->auction.index].bound && !pc_can_give_bounded_items(sd) ) {
				clif.displaymessage(sd->fd, msg_txt(sd,293));
		...
		void Clif::cashshop_open( struct map_session_data* sd ){
			WFIFOHEAD( sd->fd, 10 );
		...
		void Clif::cashshop_show(struct map_session_data *sd, struct npc_data *nd)
		{
			int fd, i, cost[2] = { 0, 0 };
		...
		void clif_cashshop_ack(struct map_session_data* sd, int error)
		{
			...
			fd = sd->fd;
			nd = map_obj.id2nd(sd->npc_shopid);
		...
		void Clif::cashshop_result( struct map_session_data *sd, unsigned short item_id, uint16 result ){
			WFIFOHEAD( sd->fd, 16 );
		...
		void Clif::Adopt_reply(struct map_session_data *sd, int type)
		{
			int fd = sd->fd;
		...
		void Clif::Adopt_request(struct map_session_data *sd, struct map_session_data *src, int p_id)
		{
			int fd = sd->fd;
		...
		{
			TBL_PC *tsd = map_obj.id2sd(RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]));
		...
		void clif_parse_Adopt_reply(int fd, struct map_session_data *sd){
			...
			struct map_session_data* p1_sd = map_obj.id2sd(p1_id);
			struct map_session_data* p2_sd = map_obj.id2sd(p2_id);
		...
		void Clif::bossmapinfo(struct map_session_data *sd, struct mob_data *md, enum e_bossmap_info flag)
		{
			int fd = sd->fd;
		...
		void clif_parse_ViewPlayerEquip(int fd, struct map_session_data* sd)
		{
			...
			struct map_session_data* tsd = map_obj.id2sd(aid);
		...
		void clif_parse_configuration( int fd, struct map_session_data* sd ){
			...
			clif.configuration( sd, static_cast<e_config_type>(type), flag );
		}
		...
		void Clif::quest_send_list(struct map_session_data *sd)
		{
			int fd = sd->fd;
			int i;
			int offset = 8;
		...
		void Clif::quest_send_mission(struct map_session_data *sd)
		{
			int fd = sd->fd;
			int i, j, limit = 0;
		...
		void Clif::quest_add(struct map_session_data *sd, struct quest *qd)
		{
			int fd = sd->fd;
			int i, offset;
		...
		/// 02b4 <quest id>.L
		void Clif::quest_delete(struct map_session_data *sd, int quest_id)
		{
			int fd = sd->fd;
		...
		void Clif::quest_update_objective(struct map_session_data *sd, struct quest *qd, int mobid)
		{
			int fd = sd->fd;
			int i, offset;
			struct quest_db *qi = quest_search(qd->quest_id);
		...
		void Clif::quest_update_status(struct map_session_data *sd, int quest_id, bool active)
		{
			int fd = sd->fd;
		...
		///     2 = green
		///     3 = purple
		void Clif::quest_show_event(struct map_session_data *sd, struct block_list *bl, short effect, short color)
		{
		...
		void Clif::mercenary_updatestatus(struct map_session_data *sd, int type)
		{
		...
		void Clif::mercenary_info(struct map_session_data *sd)
		{
			int fd;
			struct mercenary_data *md;
			struct status_data *status;
		...
		void Clif::mercenary_skillblock(struct map_session_data *sd)
		{
			struct mercenary_data *md;
			int fd, i, len = 4;
		...
		void Clif::mercenary_message(struct map_session_data* sd, int message){
			clif.msg(sd, MERC_MSG_BASE + message);
		}


		/// Notification about the remaining time of a rental item (ZC_CASH_TIME_COUNTER).
		/// 0298 <name id>.W <seconds>.L
		void Clif::rental_time(int fd, unsigned short nameid, int seconds)
		...
		void Clif::rental_expired(int fd, int index, unsigned short nameid)
		...
		void Clif::readbook(int fd, int book_id, int page)
		{
		...
		void Clif::bg_hp(struct map_session_data *sd) {
		...
		void Clif::bg_xy(struct map_session_data *sd)
		{
		...
		void Clif::bg_xy_remove(struct map_session_data *sd)
		{
			unsigned char buf[36];
			nullpo_retv(sd);
		...
		void Clif::bg_message( struct battleground_data *bg, int src_id, const char *name, const char *mes, int len ){
		...
		void Clif::bg_updatescore(int16 m)
		{
			struct block_list bl;
			unsigned char buf[6];
		...
		void Clif::bg_updatescore_single(struct map_session_data *sd)
		{
			int fd;
			nullpo_retv(sd);
			fd = sd->fd;
		...
		void Clif::sendbgemblem_area(struct map_session_data *sd)
		{
		...
		void Clif::sendbgemblem_single(int fd, struct map_session_data *sd)
		{
			nullpo_retv(sd);
			WFIFOHEAD(fd,32);
			WFIFOW(fd,0) = 0x2dd;
		...
		void Clif::font(struct map_session_data *sd)
		{
#if PACKETVER >= 20080102
			unsigned char buf[8];
			nullpo_retv(sd);
		...
		void Clif::instance_create(unsigned short instance_id, int num)
		{
			...
			WBUFW(buf,63) = num;
			clif.send(buf,packet_len(0x2cb),&sd->bl,target);
#endif
		...
		void Clif::instance_changewait(unsigned short instance_id, int num)
		{
#if PACKETVER >= 20071128
			struct map_session_data *sd = NULL;
			enum send_target target = PARTY;
			...
			WBUFW(buf,0) = 0x2cc;
			WBUFW(buf,2) = num;
			clif.send(buf,packet_len(0x2cc),&sd->bl,target);
#endif
		...
		void Clif::instance_status(unsigned short instance_id, unsigned int limit1, unsigned int limit2)
		{
#if PACKETVER >= 20071128
			struct instance_db *db = NULL;
			struct map_session_data *sd = NULL;
			...
			WBUFL(buf,63) = limit1;
			WBUFL(buf,67) = limit2;
			clif.send(buf,packet_len(0x2cd),&sd->bl,target);
#endif
		...
		void Clif::instance_changestatus(unsigned int instance_id, int type, unsigned int limit)
		{
#if PACKETVER >= 20071128
			struct map_session_data *sd = NULL;
			enum send_target target = PARTY;
			...
			WBUFL(buf,2) = type;
			WBUFL(buf,6) = limit;
			clif.send(buf,packet_len(0x2ce),&sd->bl,target);
#endif
		...
		void Clif::party_show_picker(struct map_session_data * sd, struct item * item_data)
		{
#if PACKETVER >= 20071002
		 ...
		 */
		void Clif::displayexp(struct map_session_data *sd, unsigned int exp, char type, bool quest, bool lost)
		{
			int fd;
			int offset;
#if PACKETVER >= 20170830
		...
		void Clif::showdigit(struct map_session_data* sd, unsigned char type, int value)
		{
		 ...
		 * Elemental System
		 *==========================================*/
		void Clif::elemental_updatestatus(struct map_session_data *sd, int type) {
		...
		void Clif::elemental_info(struct map_session_data *sd) {
			int fd;
			struct elemental_data *ed;
			struct status_data *status;
		...
		/// 0810 <slots>.B
		void Clif::buyingstore_open(struct map_session_data* sd)
		{
			int fd = sd->fd;
		...
		///     ? = nothing
		void Clif::buyingstore_open_failed(struct map_session_data* sd, unsigned short result, unsigned int weight)
		{
			int fd = sd->fd;
		...
		void Clif::buyingstore_myitemlist(struct map_session_data* sd)
		{
			int fd = sd->fd;
			unsigned int i;
		...
		void Clif::buyingstore_entry(struct map_session_data* sd)
		{
		...
		void Clif::buyingstore_entry_single(struct map_session_data* sd, struct map_session_data* pl_sd)
		{
			int fd = sd->fd;
		...
		/// 0816 <account id>.L
		void Clif::buyingstore_disappear_entry(struct map_session_data* sd)
		{
			uint8 buf[6];
		...
		void Clif::buyingstore_disappear_entry_single(struct map_session_data* sd, struct map_session_data* pl_sd)
		{
			int fd = sd->fd;
		...
		void Clif::buyingstore_itemlist(struct map_session_data* sd, struct map_session_data* pl_sd)
		{
			int fd = sd->fd;
			unsigned int i;
		...
		///     ? = nothing
		void Clif::buyingstore_trade_failed_buyer(struct map_session_data* sd, short result)
		{
			int fd = sd->fd;
		...
		void Clif::buyingstore_update_item(struct map_session_data* sd, unsigned short nameid, unsigned short amount, uint32 char_id, int zeny) {
#if PACKETVER < 20141016		// TODO : not sure for client date [Napster]
			const int cmd = 0x81b;
#else
			const int cmd = 0x9e6;
		...
		///
		/// NOTE:   This function has to be called _instead_ of clif.delitem/clif_dropitem.
		void Clif::buyingstore_delete_item(struct map_session_data* sd, short index, unsigned short amount, int price)
		{
			int fd = sd->fd;
		...
		///     ? = nothing
		void Clif::buyingstore_trade_failed_seller(struct map_session_data* sd, short result, unsigned short nameid)
		{
			int fd = sd->fd;
		...
		void Clif::search_store_info_ack(struct map_session_data* sd)
		{
#if PACKETVER >= 20150225
		...
		void Clif::search_store_info_failed(struct map_session_data* sd, unsigned char reason)
		{
			int fd = sd->fd;
		...
		void Clif::open_search_store_info(struct map_session_data* sd)
		{
			int fd = sd->fd;
		...
		/// 083d <xPos>.W <yPos>.W
		void Clif::search_store_info_click_ack(struct map_session_data* sd, short x, short y)
		{
			int fd = sd->fd;
		...
		void clif_parse_debug(int fd,struct map_session_data *sd)
		{
			...
			// clif.parse ensures, that there is at least 2 bytes of data
			cmd = RFIFOW(fd,0);
			...
			if( sd ) {
				...
				{// variable length
					packet_len = RFIFOW(fd,2);  // clif.parse ensures, that this amount of data is already received
				}
		 ...
		 *------------------------------------------*/
		int Clif::elementalconverter_list(struct map_session_data *sd) {
			int i,c,view,fd;
		 ...
		 * Rune Knight
		 **/
		void Clif::millenniumshield(struct block_list *bl, short shields) {
#if PACKETVER >= 20081217
			unsigned char buf[10];
		 ...
		 *------------------------------------------*/
		int Clif::spellbook_list(struct map_session_data *sd)
		{
			int i, c;
			int fd;
		 ...
		 *------------------------------------------*/
		int Clif::magicdecoy_list(struct map_session_data *sd, uint16 skill_lv, short x, short y) {
			int i, c;
			int fd;
		 ...
		 *------------------------------------------*/
		int Clif::poison_list(struct map_session_data *sd, uint16 skill_lv) {
			int i, c;
			int fd;
		...
		int Clif::autoshadowspell_list(struct map_session_data *sd) {
			int fd, i, c;
			nullpo_ret(sd);
			fd = sd->fd;
		 ...
		 *------------------------------------------*/
		int Clif::skill_itemlistwindow( struct map_session_data *sd, uint16 skill_id, uint16 skill_lv ) {
#if PACKETVER >= 20090922
			int fd = sd->fd;
		...
		void Clif::spiritcharm(struct map_session_data *sd) {
			unsigned char buf[10];
...
void Clif::snap( struct block_list *bl, short x, short y ) {
	unsigned char buf[10];
	...
	if( disguised(bl) )
	{
		clif.send(buf, packet_len(0x8d2), bl, AREA_WOS);
		WBUFL(buf,2) = -bl->id;
		clif.send(buf, packet_len(0x8d2), bl, SELF);
	} else
		clif.send(buf,packet_len(0x8d2),bl, AREA);
}

/// 0977 <id>.L <HP>.L <maxHP>.L (ZC_HP_INFO).
void Clif::monster_hp_bar( struct mob_data* md, int fd ) {
#if PACKETVER >= 20120404
...
void Clif::partytickack(struct map_session_data* sd, bool flag) {
	...
	for (i = 0; i < min(10,size); i++) {
		if (list[i].id > 0) {
			...
			if (strcmp(list[i].name, "-") == 0 &&
					(name = map_obj.charid2nick(list[i].id)) != NULL)
			{
...
void clif_parse_Blacksmith( int fd, struct map_session_data *sd ){
	clif.ranklist(sd,RANK_BLACKSMITH);
}
...
void clif_parse_Alchemist( int fd, struct map_session_data *sd ){
	clif.ranklist(sd,RANK_ALCHEMIST);
}
...
void Clif::ranklist(struct map_session_data *sd, int16 rankingType) {
	enum e_rank rank;
	...
	switch( rankingType ){
		...
		default:
								ShowError( "clif.ranklist: Unsupported ranking type '%d'. Please report this.\n", rankingType );
								return;
	}
...
void Clif::update_rankingpoint(struct map_session_data *sd, int rankingtype, int point) {
	int fd=sd->fd;
#if PACKETVER < 20130710
	short cmd;
	switch(rankingtype){
		...
		default:
								ShowError( "clif.update_rankingpoint: Unsupported ranking type '%d'. Please report this.\n", rankingtype );
								return;
	}
 ...
 */
#ifdef VIP_ENABLE
void Clif::display_pinfo(struct map_session_data *sd, int cmdtype) {
	if (sd) {
		struct s_packet_db* info;
		 ...
		 */
		//0:PCRoom
		details_bexp[0] = map_obj.getmapflag(sd->bl.m, MF_BEXP);
 ...
 **/
void Clif::crimson_marker(struct map_session_data *sd, struct block_list *bl, bool remove) {
	struct s_packet_db* info;
	int cmd = 0;
	int16 len;
 ...
 **/
void Clif::notify_bindOnEquip(struct map_session_data *sd, int n) {
	struct s_packet_db *info = NULL;
	int cmd = 0;
 ...
 **/
void Clif::showscript(struct block_list* bl, const char* message, enum send_target flag) {
	char buf[256];
	size_t len;
	nullpo_retv(bl);
	...
	if( len > sizeof(buf)-8 ) {
		ShowWarning("clif.showscript: Truncating too long message '%s' (len=%" PRIuPTR ").\n", message, len);
		len = sizeof(buf)-8;
	}
	...
	clif.send((unsigned char *) buf, WBUFW(buf,2), bl, flag);
}
 ...
 **/
void Clif::party_leaderchanged(struct map_session_data *sd, int prev_leader_aid, int new_leader_aid) {
	unsigned char buf[10];
	...
	clif.send(buf, packet_len(0x7fc), &sd->bl, PARTY);
}
 ...
 **/
void Clif::clan_message(struct clan *clan,const char *mes,int len){
#if PACKETVER >= 20131223
	struct map_session_data *sd;
	uint8 buf[256];
	...
	if( len == 0 ){
		...
		ShowWarning("clif.clan_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", clan_id=%d).\n", mes, len, sizeof(buf)-5, clan->id);
 ...
 **/
void Clif::clan_basicinfo( struct map_session_data *sd ){
#if PACKETVER >= 20131223
 ...
 **/
void Clif::clan_onlinecount( struct clan* clan ){
#if PACKETVER >= 20131223
	uint8 buf[6];
	struct map_session_data *sd;
 ...
 **/
void Clif::clan_leave( struct map_session_data* sd ){
#if PACKETVER >= 20131223
	int fd;
...
void Clif::roulette_open( struct map_session_data* sd ){
	nullpo_retv(sd);
...
void clif_parse_roulette_open( int fd, struct map_session_data* sd ){
	...
	if (!battle_config.feature_roulette) {
		clif.messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,1497),false,SELF); //Roulette is disabled
		return;
	}
...
void clif_merge_item_ack(struct map_session_data *sd, unsigned short index, unsigned short count, enum MERGE_ITEM_ACK type) {
	...
	clif.send(buf, info->len, &sd->bl, SELF);
}
 ...
 * @param sd
 **/
void Clif::merge_item_open(struct map_session_data *sd) {
	...
	if (n < 2) { // No item need to be merged
		clif.msg(sd, MERGE_ITEM_NOT_AVAILABLE);
		return;
	}
 ...
 **/
void Clif::broadcast_obtain_special_item(const char *char_name, unsigned short nameid, unsigned short container, enum BROADCASTING_SPECIAL_ITEM_OBTAIN type) {
	...
	clif.send(buf, WBUFW(buf, 2), NULL, ALL_CLIENT);
}
...
void Clif::dressing_room(struct map_session_data *sd, int flag) {
#if PACKETVER >= 20150513
	int fd = sd->fd;
...
void Clif::navigateTo(struct map_session_data *sd, const char* mapname, uint16 x, uint16 y, uint8 flag, bool hideWindow, uint16 mob_id ){
...
void Clif::hat_effects( struct map_session_data* sd, struct block_list* bl, enum send_target target ){
#if PACKETVER >= 20150513
	unsigned char* buf;
	int len,i;
...
void Clif::hat_effect_single( struct map_session_data* sd, uint16 effectId, bool enable ){
#if PACKETVER >= 20150513
	unsigned char buf[13];
...
void Clif::sale_start( struct sale_item_data* sale_item, struct block_list* bl, enum send_target target ){
#if PACKETVER_SUPPORTS_SALES
	unsigned char buf[8];
	...
	clif.send(buf, 8, bl, target);
#endif
}

/// Notify the clien that a sale has ended
/// 09b3 <item id>.W (ZC_NOTIFY_BARGAIN_SALE_CLOSE)
void Clif::sale_end( struct sale_item_data* sale_item, struct block_list* bl, enum send_target target ){
#if PACKETVER_SUPPORTS_SALES
	unsigned char buf[4];
	...
	clif.send(buf, 4, bl, target);
#endif
}

/// Update the remaining amount of a sale item.
/// 09c4 <item id>.W <amount>.L (ZC_ACK_COUNT_BARGAIN_SALE_ITEM)
void Clif::sale_amount( struct sale_item_data* sale_item, struct block_list* bl, enum send_target target ){
#if PACKETVER_SUPPORTS_SALES
	unsigned char buf[8];
 ...
 */
void Clif::achievement_list_all(struct map_session_data *sd)
{
	int i, j, len, fd, *info;
	uint16 count = 0;
	...
	if (!battle_config.feature_achievement) {
		clif.messagecolor(&sd->bl,color_table[COLOR_RED],msg_txt(sd,772),false,SELF); // Achievements are disabled.
		return;
	}
 ...
 */
void Clif::achievement_update(struct map_session_data *sd, struct achievement *ach, int count)
{
	int fd, i, *info;
 ...
 */
void Clif::achievement_reward_ack(int fd, unsigned char result, int achievement_id)
{
...
/// 0AE2 <type>.B <data>.L
void Clif::ui_open( struct map_session_data *sd, enum out_ui_type ui_type, int32 data ){
	nullpo_retv(sd);
	...
	switch( RFIFOB(fd,2) ){
		case IN_UI_ATTENDANCE:
			if( !pc_has_permission( sd, PC_PERM_ATTENDANCE ) ){
				clif.messagecolor( &sd->bl, color_table[COLOR_RED], msg_txt( sd, 791 ), false, SELF ); // You are not allowed to use the attendance system.
			}else if( pc_attendance_enabled() ){
				clif.ui_open( sd, OUT_UI_ATTENDANCE, pc_attendance_counter( sd ) );
			}else{
...
void Clif::attendence_response( struct map_session_data *sd, int32 data ){
	nullpo_retv(sd);
...
/// 0ADE <percentage>.L
void Clif::weight_limit( struct map_session_data* sd ){
...
void clif_parse_private_airship_request( int fd, struct map_session_data* sd ){
	...
	if( !battle_config.feature_privateairship ){
		clif.messagecolor( &sd->bl, color_table[COLOR_RED], msg_txt( sd, 792 ), false, SELF ); // The private airship system is disabled.
		return;
	}

	// Check if the player is allowed to warp from the source map
	if( !map_obj.getmapflag( sd->bl.m, MF_PRIVATEAIRSHIP_SOURCE ) ){
	...
	int16 mapid = map_obj.mapindex2mapid( mapindex );
	...
	if( !map_obj.getmapflag( mapid, MF_PRIVATEAIRSHIP_DESTINATION ) ){
		clif_private_airship_response( sd, PRIVATEAIRSHIP_DESTINATION_MAP_INVALID );
		return;
	}
...
void Clif::equipswitch_list( struct map_session_data* sd ){
...
void Clif::equipswitch_remove( struct map_session_data* sd, uint16 index, uint32 pos, bool failed ){
#if PACKETVER >= 20170208
	int fd = sd->fd;
...
void Clif::equipswitch_add( struct map_session_data* sd, uint16 index, uint32 pos, bool failed ){
#if PACKETVER >= 20170208
	int fd = sd->fd;
...
void clif_parse_equipswitch_add( int fd, struct map_session_data* sd ){
	...
	if( sd->state.trading || sd->npc_shopid ){
		clif.equipswitch_add( sd, index, position, true );
		return;
	}
...
/// 0a9d <failed>.W
void Clif::equipswitch_reply( struct map_session_data* sd, bool failed ){
#if PACKETVER >= 20170208
	int fd = sd->fd;
...
static int clif_parse(int fd)
{
	...
	if (cmd > MAX_PACKET_DB || cmd < MIN_PACKET_DB || packet_db[cmd].len == 0) {
		ShowWarning("clif.parse: Received unsupported packet (packet 0x%04x, %d bytes received), disconnecting session #%d.\n", cmd, RFIFOREST(fd), fd);
		...
		if( !sd ){
			...
			if( cmd2 >= MIN_PACKET_DB && cmd2 < MAX_PACKET_DB && packet_db[cmd2].len != 0 && packet_db[cmd2].func == clif_parse_WantToConnection ){
				ShowWarning( "clif.parse: It looks like you have enabled PACKET_OBFUSCATION on server side, but disabled it on client side.\n" );
			}
#else
			...
			if( cmd >= MIN_PACKET_DB && cmd < MAX_PACKET_DB && packet_db[cmd].len != 0 && packet_db[cmd].func == clif_parse_WantToConnection ){
				ShowWarning( "clif.parse: It looks like you have disabled PACKET_OBFUSCATION on server side, but enabled it on client side.\n" );
			}
#endif
		}
#endif
	...
	if (packet_len == -1) { // variable-length packet
		...
		if (packet_len < 4 || packet_len > 32768) {
			ShowWarning("clif.parse: Received packet 0x%04x specifies invalid packet_len (%d), disconnecting session #%d.\n", cmd, packet_len, fd);
#ifdef DUMP_INVALID_PACKET
			ShowDump(RFIFOP(fd,0), RFIFOREST(fd));
#endif
			set_eof(fd);
	...
	RFIFOW(fd, 0) = cmd;
	if (sd)
		sd->cryptKey = ((sd->cryptKey * cryptKey[1]) + cryptKey[2]) & 0xFFFFFFFF; // Update key for the next packet
#endif
