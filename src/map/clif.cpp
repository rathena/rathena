...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if( hom_is_active(sd->hd) ) {
		...
		if( battle_config.hom_setting&HOMSET_COPY_SPEED )
			status_calc_bl_sub(&sd->hd->bl, { SCB_SPEED }); //Homunc mimic their master's speed on each map change
	...
	if( sd->md ) {
		...
		clif_mercenary_info(sd);
		clif_mercenary_skillblock(sd);
		status_calc_bl_sub(&sd->md->bl, { SCB_SPEED }); // Mercenary mimic their master's speed on each map change
	}
	...
	if( sd->ed ) {
		...
		status_calc_bl_sub(&sd->ed->bl, { SCB_SPEED }); //Elemental mimic their master's speed on each map change
	}
	...
	if( sd->state.changemap ) {// restore information that gets lost on map-change
		...
		if (battle_config.bg_flee_penalty != 100 || battle_config.gvg_flee_penalty != 100) {
			...
			if ((pmap != nullptr && (mapdata_flag_gvg(pmap) || pmap->flag[MF_BATTLEGROUND])) || (mapdata != nullptr && (mapdata_flag_gvg(mapdata) || mapdata->flag[MF_BATTLEGROUND])))
				status_calc_bl_sub(&sd->bl, { SCB_FLEE }); //Refresh flee penalty
		}
