...
static inline int32 itemtype(t_itemid nameid) {
	struct item_data* id = itemdb_search(nameid); // Use itemdb_search, so non-existance item will use dummy data and
												  // won't crash the server. bugreport:8468
	int32 type = id->type;
...
static inline void WBUFPOS2(
	uint8* p, unsigned short pos, short x0, short y0, short x1, short y1, uint8 sx0, uint8 sy0) {
	p += pos;
	p[0] = (uint8)(x0 >> 2);
...
static inline void RBUFPOS2(const uint8* p,
							unsigned short pos,
							short* x0,
							short* y0,
							short* x1,
...
static inline void RFIFOPOS2(
	int32 fd, unsigned short pos, short* x0, short* y0, short* x1, short* y1, unsigned char* sx0, unsigned char* sy0) {
...
int32 clif_send(const void* buf, int32 len, struct block_list* bl, enum send_target type) {
	...
	switch (type) {
		...
		case AREA_WOC:
		case AREA_WOS:
			map_foreachinallarea(clif_send_sub,
								 bl->m,
								 bl->x - AREA_SIZE,
								 bl->y - AREA_SIZE,
								 bl->x + AREA_SIZE,
		...
		case PARTY_SAMEMAP_WOS:
			...
			if (p) {
				for (i = 0; i < MAX_PARTY; i++) {
					...
					if (sd->bl.id == bl->id &&
						(type == PARTY_WOS || type == PARTY_SAMEMAP_WOS || type == PARTY_AREA_WOS)) {
						continue;
					}
		...
		case GUILD_NOBG: {
			...
			for (i = 0; i < g.max_member; i++) {
				if ((sd = g.member[i].sd) != nullptr) {
					...
					if (sd->bl.id == bl->id &&
						(type == GUILD_WOS || type == GUILD_SAMEMAP_WOS || type == GUILD_AREA_WOS)) {
						continue;
					}
 ...
 *------------------------------------------*/
static void clif_set_unit_walking(struct block_list& bl,
...
void clif_homunculus_updatestatus(map_session_data& sd, _sp type) {
	...
	switch (type) {
		case SP_BASEEXP:
			p.value = static_cast<decltype(p.value)>(std::min<decltype(sd.hd->homunculus.exp)>(
				sd.hd->homunculus.exp, std::numeric_limits<decltype(p.value)>::max()));
			break;
		case SP_HP:
...
void clif_hominfo(map_session_data* sd, struct homun_data* hd, int32 flag) {
	...
	p.exp = static_cast<decltype(p.exp)>(
		std::min<decltype(hd->homunculus.exp)>(hd->homunculus.exp, std::numeric_limits<decltype(p.exp)>::max()));
	p.expNext = static_cast<decltype(p.expNext)>(
		std::min<decltype(hd->exp_next)>(hd->exp_next, std::numeric_limits<decltype(p.expNext)>::max()));
...
void clif_changemapserver(map_session_data& sd, const char* map, uint16 x, uint16 y, uint32 ip, uint16 port) {
	...
	ShowDebug("Sending the client (%d %d.%d.%d.%d) to map-server with ip %d.%d.%d.%d and port %hu\n",
			  ...
			  CONVIP(session[sd.fd]->client_addr),
			  CONVIP(ip),
			  port);
#endif
...
void clif_inventorylist(map_session_data* sd) {
	...
	for (int32 i = 0; i < MAX_INVENTORY; i++) {
		...
		if (!itemdb_isstackable2(sd->inventory_data[i])) {
			clif_item_equip(client_index(i),
							&itemlist_equip.list[equip++],
			...
			clif_item_normal(client_index(i),
							 &itemlist_normal.list[normal++],
							 &sd->inventory.u.items_inventory[i],
							 sd->inventory_data[i]);
			...
			if (normal == MAX_INVENTORY_ITEM_PACKET_NORMAL) {
				...
				itemlist_normal.PacketLength = static_cast<decltype(itemlist_normal.PacketLength)>(
					(sizeof(itemlist_normal) - sizeof(itemlist_normal.list)) +
...
void clif_storagelist(map_session_data* sd, struct item* items, int32 items_length, const char* storename) {
	...
	for (int32 i = 0; i < items_length; i++) {
		...
		if (!itemdb_isstackable2(id)) {
			clif_item_equip(client_storage_index(i),
							&storage_itemlist_equip.list[equip++],
							&items[i],
							id,
							pc_equippoint_sub(sd, id));
...
static int32 clif_hpmeter(map_session_data* sd) {
	nullpo_ret(sd);
	map_foreachinallarea(clif_hpmeter_sub,
						 sd->bl.m,
						 sd->bl.x - AREA_SIZE,
						 sd->bl.y - AREA_SIZE,
						 sd->bl.x + AREA_SIZE,
	...
	switch (type) {
		...
		case SP_HP:
			...
			if (sd.battle_status.hp == 0) {
				clif_par_change(
					sd, type, (sd.class_ & MAPID_UPPERMASK) != MAPID_NOVICE ? 1 : sd.battle_status.max_hp / 2);
			}
			else {
		...
		case SP_UDEX:
		case SP_ULUK:
			clif_zc_status_change(sd,
								  static_cast<uint16>(type),
								  static_cast<uint8>(pc_need_status_point(&sd, type - SP_USTR + SP_STR, 1)));
			break;
		...
		case SP_UCRT:
			clif_zc_status_change(sd,
								  static_cast<uint16>(type),
								  static_cast<uint8>(pc_need_trait_point(&sd, type - SP_UPOW + SP_POW, 1)));
			break;
...
int32 clif_damage(block_list& src,
	...
	if (battle_config.hide_woe_damage && map_flag_gvg(src.m)) {
		...
		p.damage = static_cast<decltype(p.damage)>(
			std::min(damage, static_cast<decltype(damage)>(std::numeric_limits<decltype(p.damage)>::max())));
		p.damage2 = static_cast<decltype(p.damage2)>(
			std::min(damage2, static_cast<decltype(damage2)>(std::numeric_limits<decltype(p.damage2)>::max())));
	}
...
void clif_addskill(map_session_data& sd, uint16 skill_id) {
	...
	p.skill.range2 =
		static_cast<decltype(p.skill.range2)>(skill_get_range2(&sd.bl, skill_id, sd.status.skill[idx].lv, false));
...
void clif_skill_scale(
	struct block_list* bl, int32 src_id, int32 x, int32 y, uint16 skill_id, uint16 skill_lv, int32 casttime) {
...
void clif_skill_fail(
	map_session_data& sd, uint16 skill_id, enum useskill_fail_cause cause, int32 btype, t_itemid itemId) {
...
/// @param val2
/// @param val3
void clif_status_change_sub(struct block_list* bl,
							int32 id,
							int32 type,
							int32 flag,
							t_tick tick,
 ...
 * @param val3
 */
void clif_status_change(
	struct block_list* bl, int32 type, int32 flag, t_tick tick, int32 val1, int32 val2, int32 val3) {
	map_session_data* sd = nullptr;
...
void clif_efst_status_change_sub(struct block_list* tbl, struct block_list* bl, enum send_target target) {
	...
	for (i = 0; i < sc_display_count; i++) {
		...
		clif_efst_status_change(tbl,
								bl->id,
								target,
								status_db.getIcon(type),
								tick,
								sc_display[i]->val1,
								sc_display[i]->val2,
								sc_display[i]->val3);
#else
		clif_status_change_sub(tbl,
							   bl->id,
							   status_db.getIcon(type),
							   1,
							   tick,
...
void clif_heal(map_session_data& sd, int32 type, uint32 val) {
	...
	packet.type = static_cast<decltype(packet.type)>(type);
	packet.amount =
		std::min(static_cast<decltype(packet.amount)>(val), std::numeric_limits<decltype(packet.amount)>::max());
...
void clif_map_property(struct block_list* bl, enum map_property property, enum send_target t) {
	...
	WBUFL(buf, 4) =
		((mapdata->getMapFlag(MF_PVP) || (sd && sd->duel_group > 0))
		 << 0) | // PARTY - Show attack cursor on non-party members (PvP)
		((mapdata->getMapFlag(MF_BATTLEGROUND) || mapdata_flag_gvg2(mapdata))
		 << 1) | // GUILD - Show attack cursor on non-guild members (GvG)
		((mapdata->getMapFlag(MF_BATTLEGROUND) || mapdata_flag_gvg2(mapdata))
		 << 2) | // SIEGE - Show emblem over characters heads when in GvG (WoE castle)
		((mapdata->getMapFlag(MF_FORCEMINEFFECT) || mapdata_flag_gvg2(mapdata))
		 << 3) | // USE_SIMPLE_EFFECT - Forces simpler skill effects, like /mineffect command
		((mapdata->getMapFlag(MF_NOLOCKON) || mapdata_flag_vs(mapdata) || (sd && sd->duel_group > 0))
		 << 4) | // DISABLE_LOCKON - Only allow attacks on other players with shift key or /ns active
		((mapdata->getMapFlag(MF_PVP)) << 5) | // COUNT_PK - Show the PvP counter
		((mapdata->getMapFlag(MF_PARTYLOCK))
		 << 6) | // NO_PARTY_FORMATION - Prevents party creation/modification (Might be used for instance dungeons)
		((mapdata->getMapFlag(MF_BATTLEGROUND))
		 << 7) | // BATTLEFIELD - Unknown (Does something for battlegrounds areas)
		((mapdata->getMapFlag(MF_NOCOSTUME)) << 8) | // DISABLE_COSTUMEITEM - Disable costume sprites
		((!mapdata->getMapFlag(MF_NOUSECART))
		 << 9) | // USECART - Allow opening cart inventory (Well force it to always allow it)
		((!mapdata->getMapFlag(MF_NOSUNMOONSTARMIRACLE))
...
void clif_use_card(map_session_data* sd, int32 idx) {
	...
	for (i = c = 0; i < MAX_INVENTORY; i++) {
		...
		if (sd->inventory_data[i]->type == IT_ARMOR && (ep & EQP_ACC) && ((ep & EQP_ACC) != EQP_ACC) &&
			((sd->inventory_data[i]->equip & EQP_ACC) !=
			 (ep & EQP_ACC))) { // specific accessory-card can only be inserted to specific accessory.
			continue;
		}
...
#ifdef RENEWAL
	const std::vector<s_autospell_requirement> autospell_skills = {{MG_FIREBOLT, 0},
																   {MG_COLDBOLT, 0},
																   {MG_LIGHTNINGBOLT, 0},
																   {MG_SOULSTRIKE, 3},
																   {MG_FIREBALL, 3},
...
void clif_mvp_exp(map_session_data* sd, t_exp exp) {
	...
	if (battle_config.mvp_exp_reward_message) {
		...
		clif_messagecolor(&sd->bl,
						  color_table[COLOR_CYAN],
						  e_msg,
						  false,
						  SELF); // Congratulations! You are the MVP! Your reward EXP Points are %u !!
	}
#else
	int32 fd;
...
void clif_guild_memberlist(map_session_data& sd) {
	...
	for (int32 i = 0, c = 0; i < guild.max_member; i++) {
		...
		member_info.lastLoginTime = member.last_login;
#else
		memset(member_info.intro, 0, sizeof(member_info.intro)); //[Ind] - This is displayed in the 'note' column but
...
/// (ZC_ACK_CHANGE_GUILD_POSITIONINFO) mode:
///     &0x01 = allow invite
///     &0x10 = allow expel
/// ranking:
///     TODO
...
/// <upgradable>.B }* (ZC_GUILD_SKILLINFO)
...
void clif_viewequip_ack(map_session_data& sd, map_session_data& tsd) {
	...
	for (int32 i = 0, equip = 0; i < EQI_MAX; i++) {
		...
		if (k >= 0) {
			...
			clif_item_equip(client_index(k),
							&p->list[equip],
							&tsd.inventory.u.items_inventory[k],
...
void clif_parse_LoadEndAck(int32 fd, map_session_data* sd) {
	...
	map_foreachinallarea(clif_getareachar,
						 sd->bl.m,
						 sd->bl.x - AREA_SIZE,
						 sd->bl.y - AREA_SIZE,
						 sd->bl.x + AREA_SIZE,
	...
	if (sd->state.changemap) { // restore information that gets lost on map-change
		...
		status_calc_pc(
			sd, sd->state.autotrade ? SCO_FIRST : SCO_NONE); // Some conditions are map-dependent so we must recalculate
			...
			if (!sd.sc.empty() &&
				(sd.sc.getSCE(SC_DANCING) ||
				 (sd.sc.getSCE(SC_GRAVITATION) &&
				  sd.sc.getSCE(SC_GRAVITATION)->val3 == BCT_SELF))) { // No sitting during these states either.
				break;
			}
...
void clif_parse_ChatRoomStatusChange(int32 fd, map_session_data* sd) {
	...
	safestrncpy(s_title,
				title,
				min(len + 1, CHATROOM_TITLE_SIZE)); // NOTE: assumes that safestrncpy will not access the len+1'th byte
...
void clif_parse_PutItemToCart(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART* p =
		reinterpret_cast<PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART*>(RFIFOP(fd, 0));
...
void clif_parse_ChangeCart(int32 fd, map_session_data* sd) { // TODO: State tracking?
	...
	#endif
		(type == 9 && sd->status.base_level > 130) || (type == 8 && sd->status.base_level > 120) ||
		(type == 7 && sd->status.base_level > 110) || (type == 6 && sd->status.base_level > 100) ||
#endif
		(type == 5 && sd->status.base_level > 90) || (type == 4 && sd->status.base_level > 80) ||
		(type == 3 && sd->status.base_level > 65) || (type == 2 && sd->status.base_level > 40) || (type == 1))
		pc_setcart(sd, type);
}
...
static void clif_parse_UseSkillToId_homun(
	struct homun_data* hd, map_session_data* sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int32 target_id) {
	int32 lv;
...
static void clif_parse_UseSkillToPos_homun(struct homun_data* hd,
										   map_session_data* sd,
										   t_tick tick,
										   uint16 skill_id,
										   uint16 skill_lv,
...
static void clif_parse_UseSkillToId_mercenary(
	s_mercenary_data* md, map_session_data* sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int32 target_id) {
	int32 lv;
...
static void clif_parse_UseSkillToPos_mercenary(s_mercenary_data* md,
											   map_session_data* sd,
											   t_tick tick,
											   uint16 skill_id,
											   uint16 skill_lv,
...
void clif_parse_skill_toid(map_session_data* sd, uint16 skill_id, uint16 skill_lv, int32 target_id) {
	...
	if ((pc_cant_act2(sd) || sd->chatID) && skill_id != RK_REFRESH &&
		...
		!(sd->state.storage_flag &&
		  (inf & INF_SELF_SKILL))) { // SELF skills can be used with the storage open, issue: 8027
		return;
	}
...
static void clif_parse_UseSkillToPosSub(
	int32 fd, map_session_data& sd, uint16 skill_lv, uint16 skill_id, short x, short y, int32 skillmoreinfo) {
	t_tick tick = gettick();
...
void clif_parse_UseSkillToPos(int32 fd, map_session_data* sd) {
	...
	clif_parse_UseSkillToPosSub(fd,
								*sd,
...
void clif_parse_PurchaseReq(int32 fd, map_session_data* sd) {
	const PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC* p =
		reinterpret_cast<PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC*>(RFIFOP(fd, 0));

	vending_purchasereq(
		sd, p->AID, sd->vended_id, (uint8*)p->list, (p->packetLength - sizeof(*p)) / sizeof(p->list[0]));
...
#if PACKETVER >= 20100105
	const PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC2* p =
		reinterpret_cast<PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC2*>(RFIFOP(fd, 0));
...
static enum e_result_validate_emblem clif_validate_emblem(const uint8* emblem, unsigned long emblem_len) {
	...
	if (!((decode_zip(buf, &buf_len, emblem, emblem_len) == 0 &&
		   buf_len >= 18) // sizeof(BITMAPFILEHEADER) + sizeof(biSize) of the following info header struct
...
void clif_parse_GuildChangeEmblem(int32 fd, map_session_data* sd) {
	...
	if (!(inter_config.emblem_woe_change) && is_agit_start()) {
		clif_messagecolor(&sd->bl,
						  ...
						  msg_txt(sd, 385),
						  false,
						  SELF); //"You not allowed to change emblem during woe"
		return;
	}
	...
	if (emb_val == EMBVALIDATE_ERR_RAW_FILEFORMAT) {
		...
		clif_messagecolor(&sd->bl,
						  ...
						  msg_txt(sd, 386),
						  false,
						  SELF); //"The chosen emblem was detected invalid\n"
		return;
	}
		...
		clif_messagecolor(
			...
			output,
			false,
			SELF); //"The chosen emblem was detected invalid as it contain too much transparency (limit=%d)\n"
		return;
	}
...
void clif_parse_GMShift(
	int32 fd, map_session_data* sd) { // FIXME: remove is supposed to receive account name for clients prior 20100803RE
	char* player_name;
...
void clif_parse_GMRecall(
	int32 fd, map_session_data* sd) { // FIXME: recall is supposed to receive account name for clients prior 20100803RE
	char* player_name;
	char command[NAME_LENGTH + 8];
		...
		if (next) {
			...
			if (percent && (percent % 100) == 0) { // 10.0%, 20.0%, ..., 90.0%
				sc_start(&sd->bl,
						 ...
						 100,
						 17,
						 skill_get_time(MO_EXPLOSIONSPIRITS, 5)); // Lv17-> +50 critical (noted by Poki) [Skotlex]
				clif_skill_nodamage(&sd->bl,
									sd->bl,
									MO_EXPLOSIONSPIRITS,
									5); // prayer always shows successful Lv5 cast and disregards noskill restrictions
			}
		}
	}
}
...
void clif_parse_FriendsListRemove(int32 fd, map_session_data* sd) {
	...
	if ((f_sd = map_id2sd(account_id)) && f_sd->status.char_id == char_id) {
		...
		if (chrif_removefriend(char_id, sd->status.char_id)) { // char-server offline, abort
			clif_displaymessage(
				fd, msg_txt(sd, 673)); //"This action can't be performed at the moment. Please try again later."
			return;
		}
	}
	...
	if (i == MAIL_MAX_INBOX) {
		...
		for (int32 j = 0; j < MAIL_MAX_ITEM; j++) {
			...
			if (item->nameid > 0 && item->amount > 0 && data != nullptr) {
				PACKET_ZC_ACK_READ_RODEX_SUB* mailitem =
					reinterpret_cast<PACKET_ZC_ACK_READ_RODEX_SUB*>(WBUFP(p, p->PacketLength));
...
void clif_parse_Auction_setitem(int32 fd, map_session_data* sd) {
	...
	if (!pc_can_give_items(sd) || sd->inventory.u.items_inventory[idx].expire_time ||
		...
		!itemdb_canauction(&sd->inventory.u.items_inventory[idx],
						   pc_get_group_level(sd))) { // Quest Item or something else
		clif_Auction_setitem(sd->fd, idx, true);
		return;
	}
...
#endif
		WFIFOL(fd, offset) =
			((mob && qi->objectives[i]->mob_id > 0)
				 ? qi->objectives[i]->mob_id
				 : MOBID_PORING); // 0 can't be used as it displays "Novice" job regardless of the clif_mobtype_name
		offset += 4;
#if PACKETVER >= 20150513
...
/// (CZ_REQ_OPEN_BUYING_STORE) result:
///     0 = cancel
///     1 = open
static void clif_parse_ReqOpenBuyingStore(int32 fd, map_session_data* sd) {
	...
	if (p->packetLength < sizeof(*p)) {
		ShowError("clif_parse_ReqOpenBuyingStore: Malformed packet (expected length=%" PRIdPTR
...
void clif_buyingstore_itemlist(map_session_data& sd, map_session_data& pl_sd) {
	...
	for (int32 i = 0; i < pl_sd.buyingstore.slots; i++) {
		...
		entry.amount = pl_sd.buyingstore.items[i]
...
static void clif_parse_ReqTradeBuyingStore(int32 fd, map_session_data* sd) {
	...
	// minimum packet length
	if (p->packetLength < sizeof(*p)) {
		ShowError("clif_parse_ReqTradeBuyingStore: Malformed packet (expected length=%" PRIdPTR
				  ", length=%u, account_id=%d).\n",
				  sizeof(*p),
				  p->packetLength,
				  sd->bl.id);
...
/// <card>.W }* (CZ_SEARCH_STORE_INFO) type:
///     0 = Vending
///     1 = Buying Store
///
...
static void clif_parse_SearchStoreInfo(int32 fd, map_session_data* sd) {
	...
	// minimum packet length
	if (p->packetLength < sizeof(*p)) {
		ShowError("clif_parse_SearchStoreInfo: Malformed packet (expected length=%" PRIdPTR
				  ", length=%u, account_id=%d).\n",
				  sizeof(*p),
				  p->packetLength,
				  sd->bl.id);
	...
	searchstore_query(*sd,
					  static_cast<e_searchstore_searchtype>(p->searchType),
					  p->minPrice,
					  p->maxPrice,
					  &p->items[0],
					  p->itemsCount,
					  &p->items[p->itemsCount],
					  p->cardsCount);
}
...
/// name>.80B <nameid>.W <item type>.B <price>.L <amount>.W <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }*
/// (ZC_SEARCH_STORE_INFO_ACK) is first page:
...
void clif_search_store_info_ack(map_session_data& sd) {
	...
	p->usesCount = static_cast<decltype(p->usesCount)>(std::min<decltype(sd.searchstore.uses)>(
		sd.searchstore.uses, std::numeric_limits<decltype(p->usesCount)>::max()));
...
void clif_open_search_store_info(map_session_data& sd) {
	...
	#if PACKETVER_MAIN_NUM >= 20100701 || PACKETVER_RE_NUM >= 20100701 || defined(PACKETVER_ZERO)
	packet.remainingUses = static_cast<decltype(packet.remainingUses)>(std::min<decltype(sd.searchstore.uses)>(
		sd.searchstore.uses, std::numeric_limits<decltype(packet.remainingUses)>::max()));
	#endif
...
void clif_parse_debug(int32 fd, map_session_data* sd) {
	...
	if (sd) {
		...
		ShowDebug("Packet debug of 0x%04X (length %d), %s session #%d, %d/%d (AID/CID)\n",
				  cmd,
				  packet_len,
				  sd->state.active ? "authed" : "unauthed",
				  fd,
				  sd->status.account_id,
				  sd->status.char_id);
	}
...
static uint8 clif_roulette_getitem(map_session_data* sd) {
	...
	if ((res = pc_additem(
			 sd, &it, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx] * factor, LOG_TYPE_ROULETTE)) == 0) {
		; // onSuccess
	}
...
void clif_parse_roulette_generate(int32 fd, map_session_data* sd) {
	...
	clif_roulette_generate(sd,
						   result,
						   sd->roulette.prizeStage,
						   (sd->roulette.prizeIdx == -1 ? 0 : sd->roulette.prizeIdx),
...
#if PACKETVER_SUPPORTS_SALES
	const PACKET_CZ_REQ_CASH_BARGAIN_SALE_ITEM_INFO* p =
		reinterpret_cast<PACKET_CZ_REQ_CASH_BARGAIN_SALE_ITEM_INFO*>(RFIFOP(fd, 0));
...
void clif_parse_sale_add(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_REQ_APPLY_BARGAIN_SALE_ITEM* p =
		reinterpret_cast<PACKET_CZ_REQ_APPLY_BARGAIN_SALE_ITEM*>(RFIFOP(fd, 0));
...
void clif_parse_sale_remove(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM* p =
		reinterpret_cast<PACKET_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM*>(RFIFOP(fd, 0));
		...
		case IN_UI_ATTENDANCE:
			if (!pc_has_permission(sd, PC_PERM_ATTENDANCE)) {
				clif_messagecolor(&sd->bl,
								  ...
								  msg_txt(sd, 791),
								  false,
								  SELF); // You are not allowed to use the attendance system.
			}
...
void clif_parse_private_airship_request(int32 fd, map_session_data* sd) {
	...
	if (!battle_config.feature_privateairship) {
		clif_messagecolor(
			&sd->bl, color_table[COLOR_RED], msg_txt(sd, 792), false, SELF); // The private airship system is disabled.
		return;
	}
...
void clif_parse_camerainfo(int32 fd, map_session_data* sd) {
	...
	if (RFIFOB(fd, 2) == 1) {
		...
		safesnprintf(command,
					 sizeof(command),
					 "%ccamerainfo %03.03f %03.03f %03.03f",
					 atcommand_symbol,
					 RFIFOF(fd, 3),
					 RFIFOF(fd, 7),
					 RFIFOF(fd, 11));
...
void clif_parse_barter_buy(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_NPC_BARTER_MARKET_PURCHASE* p =
		reinterpret_cast<PACKET_CZ_NPC_BARTER_MARKET_PURCHASE*>(RFIFOP(fd, 0));
...
void clif_barter_extended_open(map_session_data& sd, struct npc_data& nd) {
	...
	PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO* p =
		reinterpret_cast<PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO*>(packet_buffer);
	...
	for (const auto& itemPair : barter->items) {
		...
		PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub* item =
			reinterpret_cast<PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub*>(WBUFP(p, p->packetLength));
		...
		for (const auto& requirementPair : itemPair.second->requirements) {
			// Needs dynamic calculation, because of variable currencies
			PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2* req =
				reinterpret_cast<PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2*>(WBUFP(p, p->packetLength));
			std::shared_ptr<s_npc_barter_requirement> requirement = requirementPair.second;
...
void clif_parse_barter_extended_buy(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE* p =
		reinterpret_cast<PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE*>(RFIFOP(fd, 0));
...
void clif_parse_laphine_synthesis(int32 fd, map_session_data* sd) {
	...
	size_t count =
		(p->packetLength - sizeof(PACKET_CZ_REQ_RANDOM_COMBINE_ITEM)) / sizeof(PACKET_CZ_REQ_RANDOM_COMBINE_ITEM_sub);
...
void clif_enchantgrade_add(map_session_data& sd,
						   uint16 index = UINT16_MAX,
						   std::shared_ptr<s_enchantgradelevel> gradeLevel = nullptr) {
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
...
void clif_parse_enchantgrade_add(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_GRADE_ENCHANT_SELECT_EQUIPMENT* p =
		reinterpret_cast<PACKET_CZ_GRADE_ENCHANT_SELECT_EQUIPMENT*>(RFIFOP(fd, 0));
...
void clif_parse_enchantwindow_general(int32 fd, map_session_data* sd) {
	...
	if (enchants_for_enchantgrade == nullptr) {
		clif_messagecolor(&sd->bl,
						  ...
						  msg_txt(sd, 829),
						  false,
						  SELF); // Enchanting is not possible for your item's enchant grade.
...
void clif_parse_captcha_register(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_REQ_UPLOAD_MACRO_DETECTOR* p =
		reinterpret_cast<PACKET_CZ_REQ_UPLOAD_MACRO_DETECTOR*>(RFIFOP(fd, 0));
...
void clif_parse_captcha_upload(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_UPLOAD_MACRO_DETECTOR_CAPTCHA* p =
		reinterpret_cast<PACKET_CZ_UPLOAD_MACRO_DETECTOR_CAPTCHA*>(RFIFOP(fd, 0));
...
void clif_parse_captcha_preview_request(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_REQ_PREVIEW_MACRO_DETECTOR* p =
		reinterpret_cast<PACKET_CZ_REQ_PREVIEW_MACRO_DETECTOR*>(RFIFOP(fd, 0));
...
void clif_captcha_preview_response(map_session_data& sd, std::shared_ptr<s_captcha_data> cd) {
	...
	if (cd != nullptr) {
		for (uint16 offset = 0; offset < cd->image_size;) {
			...
			PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA* p2 =
				reinterpret_cast<PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA*>(packet_buffer);
...
void clif_macro_detector_request(map_session_data& sd) {
	...
	for (uint16 offset = 0; offset < cd->image_size;) {
		uint16 chunk_size = min(cd->image_size - offset, MAX_CAPTCHA_CHUNK_SIZE);
		PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA* p2 =
			reinterpret_cast<PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA*>(packet_buffer);
...
void clif_parse_macro_detector_download_ack(int32 fd, map_session_data* sd) {
	...
	if (sd->macro_detect.retry != 0) {
		// const PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA* p =
		// reinterpret_cast<PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA*>( RFIFOP( fd, 0 ) );
...
void clif_parse_macro_detector_answer(int32 fd, map_session_data* sd) {
	...
	const PACKET_CZ_ACK_ANSWER_MACRO_DETECTOR* p =
		reinterpret_cast<PACKET_CZ_ACK_ANSWER_MACRO_DETECTOR*>(RFIFOP(fd, 0));
...
#if PACKETVER >= 20191204
	const PACKET_CZ_PARTY_REQ_ACK_MASTER_TO_JOIN* p =
		reinterpret_cast<PACKET_CZ_PARTY_REQ_ACK_MASTER_TO_JOIN*>(RFIFOP(fd, 0));
		...
		if (cmd > MAX_PACKET_DB || cmd < MIN_PACKET_DB || packet_db[cmd].len == 0) {
			...
			if (!sd) {
				...
				if (cmd2 >= MIN_PACKET_DB && cmd2 < MAX_PACKET_DB && packet_db[cmd2].len != 0 &&
					...
					ShowWarning(
						"clif_parse: It looks like you have enabled PACKET_OBFUSCATION on server side, but disabled it "
						"on client side.\n");
				}
	#else
				...
				if (cmd >= MIN_PACKET_DB && cmd < MAX_PACKET_DB && packet_db[cmd].len != 0 &&
					...
					ShowWarning(
						"clif_parse: It looks like you have disabled PACKET_OBFUSCATION on server side, but enabled it "
						"on client side.\n");
				}
	#endif
			}
...
void do_init_clif(void) {
	...
	if (battle_config.ping_timer_interval > 0) {
		add_timer_interval(gettick() + battle_config.ping_timer_interval * 1000,
						   clif_ping_timer,
						   0,
						   0,
