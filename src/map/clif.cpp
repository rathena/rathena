...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if( sd->state.changemap ) {// restore information that gets lost on map-change
		...
		if( channel_config.map_tmpl.name != NULL && (channel_config.map_tmpl.opt&CHAN_OPT_AUTOJOIN) && !map_getmapflag(sd->bl.m,MF_NOMAPCHANNELAUTOJOIN) && !map[sd->bl.m].instance_id )
