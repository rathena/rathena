...
int32 console = 0;
int32 enable_spy = 0; // To enable/disable @spy commands, which consume too much cpu time when sending packets.
					  // [Skotlex]
	...
	if (bl->type & BL_CHAR) {
		...
		if (sc != nullptr && !sc->empty()) {
			if (sc->getSCE(SC_DANCING)) {
				...
				if (sc->getSCE(SC_CLOAKING) && sc->getSCE(SC_CLOAKING)->val1 < 3 &&
					!skill_check_cloaking(bl, nullptr)) {
...
struct skill_unit *map_find_skill_unit_oncell(
	struct block_list *target, int16 x, int16 y, uint16 skill_id, struct skill_unit *out_unit, int32 flag) {
	int16 bx, by;
	struct block_list *bl;
 ...
 *------------------------------------------*/
int32 map_foreachinrangeV(int32 (*func)(struct block_list *, va_list),
...
int32 map_foreachinrange(
	int32 (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int32 type, ...) {
	int32 returnCount = 0;
	va_list ap;
	va_start(ap, type);
 ...
 *------------------------------------------*/
int32 map_foreachinareaV(int32 (*func)(struct block_list *, va_list),
						 int16 m,
						 int16 x0,
						 int16 y0,
						 int16 x1,
...
int32 map_foreachinallarea(
	int32 (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int32 type, ...) {
	int32 returnCount = 0;
	va_list ap;
	va_start(ap, type);
 ...
 *------------------------------------------*/
int32 map_forcountinrange(
	int32 (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int32 count, int32 type, ...) {
	int32 bx, by, m;
	int32 returnCount = 0; // total sum of returned values of func() [Skotlex]
	struct block_list *bl;
	int32 blockcount = bl_list_count, i;
...
int32 map_forcountinarea(int32 (*func)(struct block_list *, va_list),
						 int16 m,
						 int16 x0,
						 int16 y0,
						 int16 x1,
 ...
 *------------------------------------------*/
int32 map_foreachinmovearea(int32 (*func)(struct block_list *, va_list),
							struct block_list *center,
							int16 range,
							int16 dx,
							int16 dy,
 ...
 *------------------------------------------------------------*/
int32 map_foreachinpath(int32 (*func)(struct block_list *, va_list),
						int16 m,
						int16 x0,
						int16 y0,
						int16 x1,
			...
			for (bx = mx0 / BLOCK_SIZE; bx <= mx1 / BLOCK_SIZE; bx++) {
				for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != nullptr; bl = bl->next) {
					if (bl->prev && bl->type & type && bl_list_count < BL_LIST_MAX) {
						...
						// considering how it's int32 math.
 ...
 * @param type: Type of bl to search for
 *------------------------------------------*/
int32 map_foreachindir(int32 (*func)(struct block_list *, va_list),
					   int16 m,
					   int16 x0,
					   int16 y0,
					   int16 x1,
 ...
 *------------------------------------------*/
int32 map_search_freecell(
	struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry, int32 flag, int32 tries) {
	int32 spawn = 0;
	int32 bx, by;
 ...
 *------------------------------------------*/
int32 map_addflooritem(struct item *item,
					   int32 amount,
					   int16 m,
					   int16 x,
					   int16 y,
...
void map_delnickdb(int32 charid, const char *name) {
	...
	if (!nick_db->remove(nick_db, db_i2key(charid), &data) ||
		(p = (struct charid2nick *)db_data2ptr(&data)) == nullptr) {
		return;
	}
...
int32 map_sql_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(mmysql_handle,
								 map_server_id.c_str(),
								 map_server_pw.c_str(),
								 map_server_ip.c_str(),
								 map_server_port,
								 map_server_db.c_str()) ||
		SQL_ERROR == Sql_Connect(qsmysql_handle,
								 map_server_id.c_str(),
								 map_server_pw.c_str(),
								 map_server_ip.c_str(),
								 map_server_port,
...
int32 log_sql_init(void) {
	...
	ShowInfo("" CL_WHITE "[SQL]" CL_RESET ": Connecting to the Log Database " CL_WHITE "%s" CL_RESET " At " CL_WHITE
			 "%s" CL_RESET "...\n",
			 log_db_db.c_str(),
			 log_db_ip.c_str());
	if (SQL_ERROR ==
	...
	ShowStatus("" CL_WHITE "[SQL]" CL_RESET ": Successfully '" CL_GREEN "connected" CL_RESET "' to Database '" CL_WHITE
			   "%s" CL_RESET "'.\n",
			   log_db_db.c_str());
	...
	switch (mapflag) {
		...
		case MF_PVP:
			...
			if (!status) {
				...
				if (mapdata->getMapFlag(MF_GVG)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set GvG and PvP flags for the same map! Removing GvG flag from "
						"%s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_GVG_TE)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set GvG TE and PvP flags for the same map! Removing GvG TE flag "
						"from %s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_GVG_DUNGEON)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set GvG Dungeon and PvP flags for the same map! Removing GvG "
						"Dungeon flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_GVG_CASTLE)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and PvP flags for the same map! Removing GvG Castle "
						"flag from %s.\n",
						mapdata->name);
				}
				...
				if (mapdata->getMapFlag(MF_BATTLEGROUND)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set Battleground and PvP flags for the same map! Removing "
						"Battleground flag from %s.\n",
						mapdata->name);
				}
			}
			break;
		...
		case MF_GVG_TE:
			...
			if (!status) {
				...
				if (mapdata->getMapFlag(MF_BATTLEGROUND)) {
					mapdata->setMapFlag(MF_BATTLEGROUND, false);
					ShowWarning(
						"map_setmapflag: Unable to set Battleground and GvG flags for the same map! Removing "
						"Battleground flag from %s.\n",
						mapdata->name);
				}
			}
		...
		case MF_GVG_TE_CASTLE:
			if (status) {
				if (mapflag == MF_GVG_CASTLE && mapdata->getMapFlag(MF_GVG_TE_CASTLE)) {
					mapdata->setMapFlag(MF_GVG_TE_CASTLE, false);
					ShowWarning(
						"map_setmapflag: Unable to set GvG TE Castle and GvG Castle flags for the same map! Removing "
						"GvG TE Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapflag == MF_GVG_TE_CASTLE && mapdata->getMapFlag(MF_GVG_CASTLE)) {
					mapdata->setMapFlag(MF_GVG_CASTLE, false);
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and GvG TE Castle flags for the same map! Removing "
						"GvG Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_PVP)) {
					mapdata->setMapFlag(MF_PVP, false);
					if (!battle_config.pk_mode) {
						ShowWarning(
							"npc_parse_mapflag: Unable to set PvP and GvG%s Castle flags for the same map! Removing "
							"PvP flag from %s.\n",
							(mapflag == MF_GVG_CASTLE ? nullptr : " TE"),
							mapdata->name);
					}
				}
			}
		...
		case MF_GVG_DUNGEON:
			if (status && mapdata->getMapFlag(MF_PVP)) {
				...
				if (!battle_config.pk_mode) {
					ShowWarning(
						"map_setmapflag: Unable to set PvP and GvG Dungeon flags for the same map! Removing PvP flag "
						"from %s.\n",
						mapdata->name);
				}
			}
		...
		case MF_NOJOBEXP:
			if (status) {
				...
				if (mapflag == MF_NOJOBEXP && mapdata->getMapFlag(MF_JEXP) != 100) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set JEXP and No Job EXP flags for the same map! Removing JEXP flag "
						"from %s.\n",
						mapdata->name);
				}
			}
		...
		case MF_PVP_NIGHTMAREDROP:
			if (status) {
				...
				if (mapdata->drop_list.size() == MAX_DROP_PER_MAP) {
					ShowWarning(
						"map_setmapflag: Reached the maximum number of drop list items for mapflag pvp_nightmaredrop "
						"on %s. Skipping.\n",
						mapdata->name);
					break;
				}
		...
		case MF_BEXP:
			if (status) {
				...
				if (mapflag == MF_BEXP && mapdata->getMapFlag(MF_NOBASEEXP)) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set No Base EXP and BEXP flags for the same map! Removing No Base "
						"EXP flag from %s.\n",
						mapdata->name);
				}
		...
		case MF_BATTLEGROUND:
			if (status) {
				...
				if (mapdata->getMapFlag(MF_PVP)) {
					...
					if (!battle_config.pk_mode) {
						ShowWarning(
							"map_setmapflag: Unable to set PvP and Battleground flags for the same map! Removing PvP "
							"flag from %s.\n",
							mapdata->name);
					}
				}
				if (mapdata->getMapFlag(MF_GVG)) {
					mapdata->setMapFlag(MF_GVG, false);
					ShowWarning(
						"map_setmapflag: Unable to set GvG and Battleground flags for the same map! Removing GvG flag "
						"from %s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_GVG_DUNGEON)) {
					mapdata->setMapFlag(MF_GVG_DUNGEON, false);
					ShowWarning(
						"map_setmapflag: Unable to set GvG Dungeon and Battleground flags for the same map! Removing "
						"GvG Dungeon flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->getMapFlag(MF_GVG_CASTLE)) {
					mapdata->setMapFlag(MF_GVG_CASTLE, false);
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and Battleground flags for the same map! Removing "
						"GvG Castle flag from %s.\n",
						mapdata->name);
				}
