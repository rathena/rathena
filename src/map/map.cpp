 ...
 *------------------------------------------*/
int map_search_freecell(struct block_list *src, int16 m, int16 *x,int16 *y, int16 rx, int16 ry, int flag, int tries)
{
	int spawn=0;
	int bx, by;
	...
	while(tries--) {
		...
		*y = (ry >= 0) ? rnd_value(by - ry, by + ry) : rnd_value<int16>(edge, mapdata->ys - edge - 1);
		...
		// Cells outside the map or within 4-5 cells of the map edge are considered invalid
		if (*x < 5 || *x > mapdata->xs - 5 || *y < 5 || *y > mapdata->ys - 5)
			continue;
