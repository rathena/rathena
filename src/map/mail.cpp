...
bool mail_setattachment(map_session_data *sd, struct mail_message *msg) {
	...
	if (sd->mail.zeny < 0 || (sd->mail.zeny + sd->mail.zeny * battle_config.mail_zeny_fee / 100 +
							  amount * battle_config.mail_attachment_price) > sd->status.zeny) {
		return false;
	}
		...
		if (item[i].nameid > 0 && item[i].amount > 0) {
			...
			if (pet != nullptr && item[i].card[0] == 0) {
				...
				if (check != CHKADDITEM_OVERAMOUNT &&
					pc_additem(sd, &item[i], item[i].amount, LOG_TYPE_MAIL) == ADDITEM_SUCCESS) {
					item_received = true;
...
void mail_deliveryfail(map_session_data *sd, struct mail_message *msg) {
	...
	if (msg->zeny > 0) {
		pc_getzeny(sd,
				   msg->zeny + msg->zeny * battle_config.mail_zeny_fee / 100 + zeny,
				   LOG_TYPE_MAIL); // Zeny receive (due to failure)
	}
...
void mail_send(map_session_data *sd, const char *dest_name, const char *title, const char *body_msg, int32 body_len) {
	...
	if (battle_config.mail_daily_count) {
		...
		if (sd->sc.getSCE(SC_DAILYSENDMAILCNT) == nullptr ||
			sd->sc.getSCE(SC_DAILYSENDMAILCNT)->val2 >= battle_config.mail_daily_count) {
			...
			sc_start2(&sd->bl,
					  ...
					  100,
					  date_get_dayofyear(),
					  sd->sc.getSCE(SC_DAILYSENDMAILCNT)->val2 + 1,
					  INFINITE_TICK);
		}
	}
