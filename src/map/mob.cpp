...
int32 mob_dead(struct mob_data *md, struct block_list *src, int32 type)
{
...
#ifdef RENEWAL
			if (base_exp && tmpsd[i] && tmpsd[i]->hd && static_cast<uint32>(tmpsd[i]->hd->homunculus.level) <= tmpsd[i]->status.base_level)
...
#endif
						pc_gainexp(tmpsd[i], md, base_exp, job_exp, 0, rate);
					}
				}
	...
	if( mvp_sd != nullptr ){
		...
		if( md->db->mexp > 0 && !( map_getmapflag( m, MF_NOBASEEXP ) || type&2 ) ){
			...
			pc_gainexp( mvp_sd, md, log_mvp_exp, 0, 0,100 );
		}
