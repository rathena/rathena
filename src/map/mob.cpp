...
int32 mobdb_checkid(const int32 id) {
	...
	if (mob_is_clone(
			id)) { // checkid is used mostly for random ID based code, therefore clone mobs are out of the question.
		return 0;
	}
	return id;
}
		...
		if (mob_id == MOBID_EMPERIUM) {
			...
			if (gc) {
				...
				if (g) {
					...
					add_timer(gettick() + battle_config.mob_respawn_time,
...
int32 mob_setdelayspawn(struct mob_data *md) {
	...
	else if (status_has_mode(&db->status,
							 MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED | MD_IGNOREMISC)) { // Plant type
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick) {
		...
		if (md->attacked_id == md->target_id) { // Rude attacked check.
			...
			if (md->bl.m != abl->m || abl->prev == nullptr ||
				...
				|| (battle_config.mob_ai & 0x2 &&
					!status_check_skilluse(&md->bl, abl, 0, 0)) // Cannot normal attack back to Attacker
				||
	...
	if (!tbl && can_move && mode & MD_LOOTER && md->lootitems && DIFF_TICK(tick, md->ud.canact_tick) > 0 &&
		(md->lootitem_count < LOOTITEM_SIZE ||
		 battle_config.monster_loot_type !=
			 1)) { // Scan area for items to loot, avoid trying to loot if the mob is full and can't consume the items.
	...
	if ((mode & MD_AGGRESSIVE && (!tbl || slave_lost_target)) || md->state.skillstate == MSS_FOLLOW) {
		...
		// If a monster finds a target through search that is already in attack range it immediately switches to berserk
		// mode This behavior overrides even angry mode and other mode-specific behavior
	...
	// Out of range
	if (!(mode & MD_CANMOVE) ||
		(!can_move && (md->sc.cant.move || DIFF_TICK(tick, md->ud.canmove_tick) >
											   0))) { // Can't chase. Immobile and trapped mobs will use idle skills and
													  // unlock their target after a while
 ...
 *------------------------------------------*/
static void mob_item_drop(struct mob_data *md,
						  std::shared_ptr<s_item_drop_list> &dlist,
						  std::shared_ptr<s_item_drop> &ditem,
						  int32 loot,
						  int32 drop_rate,
						  bool flag) {
	TBL_PC *sd;
	...
	test_autoloot =
		...
		(flag ? ((battle_config.homunculus_autoloot ? (battle_config.hom_idle_no_share == 0 || !pc_isidle_hom(sd))
													: 0) ||
 ...
 */
int32 mob_getdroprate(
	struct block_list *src, std::shared_ptr<s_mob_db> mob, int32 base_rate, int32 drop_modifier, mob_data *md) {
		...
		for (i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (base_exp && tmpsd[i] && tmpsd[i]->hd) {
				hom_gainexp(tmpsd[i]->hd,
							base_exp * battle_config.homunculus_exp_gain / 100); // Homunculus only receive 10% of EXP
			}
#else
...
#ifdef RENEWAL_DROP
		drop_modifier = pc_level_penalty_mod(mvp_sd != nullptr      ? mvp_sd
			...
			for (const auto &it : sd->add_drop) {
					...
					(it.class_ == CLASS_ALL || it.class_ == status->class_)) // Matched class
				{
					...
					if (it.rate < 0) {
						...
						drop_rate = cap_value(drop_rate,
											  max(battle_config.item_drop_adddrop_min, 1),
					...
					if (it.nameid > 0) {
						...
						std::shared_ptr<s_item_group_entry> entry =
							itemdb_group.get_random_entry(it.group, 1, GROUP_ALGORITHM_DROP);
						if (entry == nullptr) {
							continue;
						}
...
void mob_add_spawn(uint16 mob_id, const struct spawn_info &new_spawn) {
	...
	std::sort(
		spawns.begin(), spawns.end(), [](const spawn_info &a, const spawn_info &b) -> bool { return a.qty > b.qty; });
	/** Note
			...
			if (md->state.skillstate != MSS_DEAD &&
				(ms[i]->state == MSS_ANY ||
				 (ms[i]->state == MSS_ANYTARGET && md->target_id &&
				  md->state.skillstate !=
					  MSS_LOOT))) { // ANYTARGET works with any state as long as there's a target. [Skotlex]
				;
			}
			else {
				continue;
		...
		if (ms[i]->cond1 == event) {
			...
			switch (ms[i]->cond1) {
				...
				case MSC_MYSTATUSOFF: // status[num] off
					...
					else if (mob_getstatus_sub(*md,
											   static_cast<e_mob_skill_condition>(ms[i]->cond1),
											   static_cast<sc_type>(ms[i]->cond2))) {
						flag = 1;
					}
					else {
						flag = 0;
				...
				case MSC_RUDEATTACKED:
					...
					if (flag) {
						md->state.attacked_count = 0; // Rude attacked count should be reset after the skill condition
		...
		if (skill_get_casttype(ms[i]->skill_id) == CAST_GROUND) { // Ground skill.
			...
			switch (skill_target) {
				case MST_RANDOM: // Pick a random enemy within skill range.
					bl = battle_getenemy(&md->bl,
										 DEFAULT_ENEMY_TYPE(md),
			...
			if (inf & INF_ATTACK_SKILL) {
				...
				switch (skill_id) { // Certain Special skills that are passive, and thus, never triggered.
					...
					case GS_CHAINACTION:
						...
						ms->cond1 = MSC_ALWAYS;
						ms->permillage =
							skill_id == MO_TRIPLEATTACK ? (3000 - ms->skill_lv * 100) : (ms->skill_lv * 500);
...
bool MobDatabase::parseDropNode(std::string nodeName, const ryml::NodeRef &node, uint8 max, s_mob_drop *drops) {
	...
	for (const auto &dropit : dropNode) {
		...
		if (this->nodeExists(dropit, "Index")) {
			...
			if (index >= max) {
				this->invalidWarning(dropit["Index"],
		...
		if (this->nodeExists(dropit, "RandomOptionGroup")) {
			...
			if (!random_option_group.option_get_id(group_name.c_str(), group)) {
				this->invalidWarning(dropit["RandomOptionGroup"],
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "JapaneseName")) {
		...
		if (name.size() > NAME_LENGTH) {
			this->invalidWarning(node["JapaneseName"],
	...
	for (uint8 fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi) {
		...
		if (SQL_ERROR ==
			Sql_Query(
				...
				"racegroup_illusion_frozen`,`racegroup_illusion_moonlight`,`racegroup_ep16_def`,`racegroup_edda_"
				"arunafeltz`,"
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			ShowStatus("Loading [%" PRIu64 "/%" PRIu64 "] entries in '" CL_WHITE "%s" CL_RESET "'" CL_CLL "\r",
					   ++rows,
					   total_rows,
					   mob_db_name[fi]);
#endif
		...
		for (const auto &mobit : MobNode) {
			...
			if (rate == 0) {
				if (summon->list.erase(mob_id) == 0) {
					this->invalidWarning(mobit["Rate"],
		...
		for (j = 0; j < MAX_MOB_DROP_TOTAL; j++) {
			...
			if (util::vector_exists(mob->race2, RC2_TREASURE)) {
				...
				switch (id->type) {
					case IT_HEALING:
						rate_adjust =
							is_mvp ? battle_config.item_rate_heal_mvp
					...
					case IT_PETARMOR:
						rate_adjust =
							is_mvp ? battle_config.item_rate_equip_mvp
					...
					case IT_CARD:
						rate_adjust =
							is_mvp ? battle_config.item_rate_card_mvp
...
static int32 mob_reload_sub_npc(struct npc_data *nd, va_list args) {
	...
	if (mobdb_checkid(nd->class_)) {
		...
		if (nd->bl.prev) { // If they are spawned right now
			unit_refresh(&nd->bl); // Respawn all NPCs on client side so that they are displayed correctly(if their view
								   // id changed)
		}
	}
