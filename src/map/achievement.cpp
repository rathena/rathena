...
bool achievement_read_db_sub(const YAML::Node &node, const std::string &source)
{
	...
	if (!node["ID"]) {
		YamlDatabase::invalidWarning("achievement_read_db_sub: Missing ID field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
		return false;
	}
	try {
		achievement_id = node["ID"].as<unsigned int>();
	} catch (...) {
		YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid ID field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
		return false;
	}
	...
	if (!node["Group"]) {
		YamlDatabase::invalidWarning("achievement_read_db_sub: Missing group field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
		return false;
	}
	try {
		group_char = node["Group"].as<std::string>();
	} catch (...) {
		YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid group field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
		return false;
	}
	...
	try {
		...
		YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid name field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
		return false;
	}
	...
	if (node["Target"]) {
		try {
			...
			YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid target field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
			return false;
		}
	}
	...
	if (node["Condition"]) {
		try {
			...
			YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid condition field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
			return false;
		}
	...
	if (node["Map"]) {
		try {
			...
			YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid map field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
			return false;
		}
		if (group != AG_CHAT)
	...
	if (node["Dependent"]) {
		try {
			...
			YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid dependent field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
			return false;
		}
	}
	...
	if (node["Score"]) {
		try {
			...
			YamlDatabase::invalidWarning("achievement_read_db_sub: Achievement definition with invalid score field in '" CL_WHITE "%s" CL_RESET "', skipping.\n", node, source);
			return false;
		}
	}
