...
uint64 AchievementDatabase::parseBodyNode(const ryml::NodeRef &node) {
		...
		for (const auto &targetNode : targets) {
			...
			if (this->nodeExists(targetNode, "Mob")) {
				if (achievement->group != AG_BATTLE && achievement->group != AG_TAMING) {
					this->invalidWarning(
						targets,
						"Target Mob is only supported for targets in group AG_BATTLE or AG_TAMING, skipping.\n");
					continue;
				}
	...
	if (this->nodeExists(node, "Condition")) {
		...
		achievement->condition = parse_script(condition.c_str(),
											  this->getCurrentFile().c_str(),
											  this->getLineNumber(node["Condition"]),
	...
	if (this->nodeExists(node, "Rewards")) {
		...
		if (this->nodeExists(rewardNode, "Script")) {
			...
			achievement->rewards.script = parse_script(script.c_str(),
													   this->getCurrentFile().c_str(),
													   this->getLineNumber(rewardNode["Script"]),
													   SCRIPT_IGNORE_EXTERNAL_BRACKETS);
		}
		else {
			if (!exists) {
...
bool achievement_remove(map_session_data *sd, int32 achievement_id) {
	...
	if (i != sd->achievement_data.count - 1) {
		memmove(&sd->achievement_data.achievements[i],
				&sd->achievement_data.achievements[i + 1],
...
bool achievement_update_achievement(map_session_data *sd, int32 achievement_id, bool complete) {
	...
	if (complete) {
		...
		if (i < (--sd->achievement_data.incompleteCount)) { // The achievement needs to be moved to the completed
		...
		ARR_FIND(sd->achievement_data.incompleteCount,
				 sd->achievement_data.count,
				 i,
...
static bool achievement_target_complete(std::shared_ptr<s_achievement_db> ad,
										std::array<int32, MAX_ACHIEVEMENT_OBJECTIVES> current_count) {
	return std::find_if(ad->targets.begin(),
						ad->targets.end(),
 ...
 */
static bool achievement_update_objectives(map_session_data *sd,
										  std::shared_ptr<struct s_achievement_db> ad,
										  enum e_achievement_group group,
										  const std::array<int32, MAX_ACHIEVEMENT_OBJECTIVES> &update_count) {
	if (!ad || !sd) {
		return false;
	}
	...
	if (i == sd->achievement_data.count) { // Achievement isn't in player's log
		if (!achievement_check_dependent(
				sd, ad->achievement_id)) { // Check to see if dependents are complete before adding to player's log
			return false;
		}
