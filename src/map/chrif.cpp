...
static const int32 packet_len_table[0x3d] = {
	...
	6,  -1, 18, 7,  -1, -1, 28 + MAP_NAME_LENGTH_EXT,
...
int32 chrif_changemapserverack(uint32 account_id,
							   int32 login_id1,
							   int32 login_id2,
							   uint32 char_id,
							   const char *map,
...
static int32 chrif_reconnect(DBKey key, DBData *data, va_list ap) {
	...
	switch (node->state) {
		case ST_LOGIN:
			if (node->sd && node->char_dat == nullptr) { // Since there is no way to request the char auth, make it
														 // fail.
				pc_authfail(node->sd);
				chrif_char_offline(node->sd);
 ...
 */
int32 chrif_req_login_operation(
	int32 aid, const char *character_name, enum chrif_req_op operation_type, int32 timediff, int32 val1, int32 val2) {
	chrif_check(-1);
...
int32 chrif_save_scdata(
	map_session_data *sd) { // parses the sc_data of the player and sends it to the char-server for saving. [Skotlex]
#ifdef ENABLE_SC_SAVING
	int32 i, count = 0;
	t_tick tick;
...
int32 chrif_bsdata_received(int32 fd) {
	...
	if ((count = RFIFOB(fd, 8))) {
		...
		for (i = 0; i < count; i++) {
			...
			if (!(entry = pc_bonus_script_add(
					  sd, bs->script_str, bs->tick, (enum efst_type)bs->icon, bs->flag, bs->type))) {
				continue;
			}
	...
	while (RFIFOREST(fd) >= 2) {
		...
		switch (cmd) {
			...
			case 0x2b04:
				...
				break;
			case 0x2b06:
				chrif_changemapserverack(RFIFOL(fd, 2),
										 RFIFOL(fd, 6),
										 RFIFOL(fd, 10),
										 RFIFOL(fd, 14),
										 RFIFOCP(fd, 18),
										 RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT),
										 RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT + 2),
										 RFIFOL(fd, 18 + MAP_NAME_LENGTH_EXT + 4),
										 RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT + 8));
				break;
			case 0x2b09:
...
void do_init_chrif(void) {
	...
	if ((sizeof(struct bonus_script_data) * MAX_PC_BONUS_SCRIPT) > 0xFFFF) {
		ShowError(
			...
			"0xFFFF).\n",
			(sizeof(struct bonus_script_data) * MAX_PC_BONUS_SCRIPT),
			MAX_PC_BONUS_SCRIPT);
		exit(EXIT_FAILURE);
	}
