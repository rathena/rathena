...
void battle_damage(struct block_list *src, struct block_list *target, int64 damage, tick_t delay, uint16 skill_lv, uint16 skill_id, enum damage_lv dmg_lv, unsigned short attack_type, bool additional_effects, tick_t tick, bool spdamage);
int battle_delay_damage (tick_t tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, uint16 skill_id, uint16 skill_lv, int64 damage, enum damage_lv dmg_lv, tick_t ddelay, bool additional_effects, bool spdamage);

int battle_calc_chorusbonus(struct map_session_data *sd);

// Summary normal attack treatment (basic attack)
enum damage_lv battle_weapon_attack( struct block_list *bl,struct block_list *target,tick_t tick,int flag);
