...
int64 battle_calc_return_damage(
	struct block_list *bl, struct block_list *src, int64 *, int32 flag, uint16 skill_id, bool status_reflect);
...
struct Battle_Config {
	...
	int32 item_rate_mvp, item_rate_common, item_rate_common_boss, item_rate_card, item_rate_card_boss, item_rate_equip,
		item_rate_equip_boss, item_rate_heal, item_rate_heal_boss, item_rate_use, item_rate_use_boss,
		item_rate_treasure, item_rate_adddrop, item_group_rate;
	...
	int32 retaliate_to_master; // Whether when a mob is attacked by another mob, it will retaliate versus the mob or the
	...
	int32 skip_teleport_lv1_menu; // possibility to disable (skip) Teleport Lv1 menu, that have only two lines `Random`
								  // and `Cancel` [LuzZza]
	...
	int32 mob_size_influence; // Enable modifications on earned experience, drop rates and monster status depending on
							  // monster size. [mkbu95]
