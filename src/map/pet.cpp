...
uint64 PetDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Script")) {
		...
		pet->pet_bonus_script = parse_script(script.c_str(),
											 this->getCurrentFile().c_str(),
											 this->getLineNumber(node["Script"]),
	...
	if (this->nodeExists(node, "SupportScript")) {
		...
		pet->pet_support_script = parse_script(script.c_str(),
											   this->getCurrentFile().c_str(),
...
int32 pet_attackskill(struct pet_data *pd, int32 target_id) {
	...
	if (rnd_chance((pd->a_skill->rate + pd->pet.intimate * pd->a_skill->bonusrate / 1000),
				   100)) { // Skotlex: Use pet's skill
		int32 inf;
...
int32 pet_select_egg(map_session_data *sd, short egg_index) {
	...
	if (sd->inventory.u.items_inventory[egg_index].card[0] == CARD0_PET) {
		intif_request_petdata(sd->status.account_id,
							  sd->status.char_id,
...
static TIMER_FUNC(pet_delay_item_drop) {
	...
	for (std::shared_ptr<s_item_drop> &ditem : list->items) {
		map_addflooritem(&ditem->item_data,
...
int32 pet_egg_search(map_session_data *sd, int32 pet_id) {
	for (int32 i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].card[0] == CARD0_PET &&
			pet_id ==
				MakeDWord(sd->inventory.u.items_inventory[i].card[1], sd->inventory.u.items_inventory[i].card[2])) {
			return i;
		}
	}
	return -1;
 ...
 */
void pet_exeautobonus(map_session_data &sd,
