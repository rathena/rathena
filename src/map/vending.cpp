	...
	for (i = 0; i < count; i++) {
		...
		if (vsd->vending[vend_list[i]].amount) {
			if (Sql_Query(mmysql_handle,
						  ...
						  vending_items_table,
						  vsd->vending[vend_list[i]].amount,
						  vsd->vender_id,
...
int8 vending_openvending(
	map_session_data &sd, const char *message, const uint8 *data, int32 count, struct s_autotrader *at) {
	int32 i, j;
	...
	for (j = 0; j < count; j++) {
		...
		if (index < 0 || index >= MAX_CART // invalid position
			...
			|| (sd.cart.u.items_cart[index].bound &&
				!pc_can_give_bounded_items(&sd)) // can't trade account bound items and has no permission
			||
			!itemdb_cantrade(
				&sd.cart.u.items_cart[index], pc_get_group_level(&sd), pc_get_group_level(&sd))) { // untradeable item
			continue;
		}
	...
	if (i != j) {
		clif_displaymessage(sd.fd,
							msg_txt(&sd, 266)); //"Some of your items cannot be vended and were removed from the shop."
	...
	if (Sql_Query(mmysql_handle,
				  ...
				  sd.status.char_id,
				  sd.status.sex == SEX_FEMALE ? 'F' : 'M',
				  map_getmapdata(sd.bl.m)->name,
				  sd.bl.x,
				  sd.bl.y,
				  message_sql,
				  sd.state.autotrade,
				  at ? at->dir : sd.ud.dir,
				  at ? at->head_dir : sd.head_dir,
	...
	for (j = 0; j < i; j++) {
		StringBuf_Printf(&buf,
						 ...
						 sd.vender_id,
						 j,
						 sd.cart.u.items_cart[sd.vending[j].index].id,
						 sd.vending[j].amount,
						 sd.vending[j].value);
		if (j < i - 1) {
...
void do_init_vending_autotrade(void) {
	if (battle_config.feature_autotrade) {
		if (Sql_Query(mmysql_handle,
					  ...
					  "ORDER BY `id`;",
					  vendings_table,
					  vending_items_table) != SQL_SUCCESS) {
			Sql_ShowDebug(mmysql_handle);
			return;
		}
		...
		if (Sql_NumRows(mmysql_handle) > 0) {
			...
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				...
				CREATE(at->sd,
					   map_session_data,
					   1); // TODO: Dont use Memory Manager allocation anymore and rely on the C++ container
			...
			ShowStatus("Done loading '" CL_WHITE "%d" CL_RESET "' vending autotraders with '" CL_WHITE "%d" CL_RESET
					   "' items.\n",
					   db_size(vending_autotrader_db),
					   items);
		}
	}
...
void vending_update(map_session_data &sd) {
	if (Sql_Query(mmysql_handle,
				  "UPDATE `%s` SET `map` = '%s', `x` = '%d', `y` = '%d', `body_direction` = '%d', `head_direction` = "
				  "'%d', `sit` = '%d', `autotrade` = '%d' WHERE `id` = '%d'",
				  vendings_table,
				  map_getmapdata(sd.bl.m)->name,
				  sd.bl.x,
				  sd.bl.y,
