...
class Map_Interface {
	...
	public:
		...
		virtual int usercount(void) = 0;

		// blocklist lock
		virtual int freeblock(struct block_list *bl) = 0;
		virtual int freeblock_lock(void) = 0;
		virtual int freeblock_unlock(void) = 0;
		// blocklist manipulation
		virtual int addblock(struct block_list* bl) = 0;
		virtual int delblock(struct block_list* bl) = 0;
		virtual int moveblock(struct block_list *, int, int, t_tick) = 0;
		virtual int foreachinmap(int (*func)(struct block_list*,va_list), int16 m, int type, ...) = 0;
		//blocklist nb in one cell
		virtual int count_oncell(int16 m,int16 x,int16 y,int type,int flag) = 0;
		virtual struct skill_unit *find_skill_unit_oncell(struct block_list *,int16 x,int16 y,uint16 skill_id,struct skill_unit *, int flag) = 0;
		// search and creation
		virtual int get_new_object_id(void) = 0;
		virtual int search_freecell(struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry, int flag) = 0;
		virtual bool closest_freecell(int16 m, int16 *x, int16 *y, int type, int flag) = 0;
		virtual int quit(struct map_session_data *) = 0;
		// npc
		virtual bool addnpc(int16 m,struct npc_data *) = 0;

		// map item
		//TIMER_FUNC(map_clearflooritem_timer) = 0;
		//TIMER_FUNC(map_removemobs_timer) = 0;
		virtual void clearflooritem(struct block_list* bl) = 0;
		virtual int addflooritem(struct item *item, int amount, int16 m, int16 x, int16 y, int first_charid, int second_charid, int third_charid, int flags, unsigned short mob_id, bool canShowEffect = false) = 0;

		// instances
		virtual int addinstancemap(const char *name, unsigned short instance_id) = 0;
		virtual int delinstancemap(int m) = 0;
		virtual void data_copyall(void) = 0;
		virtual void data_copy(struct map_data *dst_map, struct map_data *src_map) = 0;

		// player to map session
		virtual void addnickdb(int charid, const char* nick) = 0;
		virtual void delnickdb(int charid, const char* nick) = 0;
		virtual void reqnickdb(struct map_session_data* sd,int charid) = 0;
		virtual const char* charid2nick(int charid) = 0;
		virtual struct map_session_data* charid2sd(int charid) = 0;

		virtual struct map_session_data * id2sd(int id) = 0;
		virtual struct mob_data * id2md(int id) = 0;
		virtual struct npc_data * id2nd(int id) = 0;
		virtual struct homun_data* id2hd(int id) = 0;
		virtual struct mercenary_data* id2mc(int id) = 0;
		virtual struct pet_data* id2pd(int id) = 0;
		virtual struct elemental_data* id2ed(int id) = 0;
		virtual struct chat_data* id2cd(int id) = 0;
		virtual struct block_list * id2bl(int id) = 0;
		virtual bool blid_exists( int id ) = 0;

		virtual const char* mapid2mapname(int m) = 0;
		virtual int16 mapindex2mapid(unsigned short mapindex) = 0;
		virtual int16 mapname2mapid(const char* name) = 0;
		virtual int mapname2ipport(unsigned short name, uint32* ip, uint16* port) = 0;
		virtual int setipport(unsigned short map, uint32 ip, uint16 port) = 0;
		virtual int eraseipport(unsigned short map, uint32 ip, uint16 port) = 0;
		virtual int eraseallipport(void) = 0;
		virtual void addiddb(struct block_list *) = 0;
		virtual void deliddb(struct block_list *bl) = 0;
		virtual int foreachinrange(int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, ...) = 0;
		virtual int foreachinallarea(int(*func)(struct block_list*, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int type, ...) = 0;
		virtual int foreachinallrange(int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, ...) = 0;
		virtual void foreachpc(int (*func)(struct map_session_data* sd, va_list args), ...) = 0;
		virtual void foreachmob(int (*func)(struct mob_data* md, va_list args), ...) = 0;
		virtual void foreachnpc(int (*func)(struct npc_data* nd, va_list args), ...) = 0;
		virtual void foreachregen(int (*func)(struct block_list* bl, va_list args), ...) = 0;
		virtual void foreachiddb(int (*func)(struct block_list* bl, va_list args), ...) = 0;
		virtual struct map_session_data * nick2sd(const char* nick, bool allow_partial) = 0;
		virtual struct mob_data * getmob_boss(int16 m) = 0;
		virtual struct mob_data * id2boss(int id) = 0;

		// reload config file looking only for npcs
		virtual void reloadnpc(bool clear) = 0;

		virtual struct questinfo *add_questinfo(int m, struct questinfo *qi) = 0;
		virtual bool remove_questinfo(int m, struct npc_data *nd) = 0;
		virtual struct questinfo *has_questinfo(int m, struct npc_data *nd, int quest_id) = 0;

		virtual int check_dir(int s_dir,int t_dir) = 0;
		virtual uint8 calc_dir(struct block_list *src,int16 x,int16 y) = 0;
		virtual uint8 calc_dir_xy(int16 srcx, int16 srcy, int16 x, int16 y, uint8 srcdir) = 0;
		virtual int random_dir(struct block_list *bl, int16 *x, int16 *y) = 0; // [Skotlex]

//		virtual int cleanup_sub(struct block_list *bl, va_list ap) = 0;

		virtual int delmap(char* mapname) = 0;
		virtual void flags_init(void) = 0;

		virtual bool iwall_exist(const char* wall_name) = 0;
		virtual bool iwall_set(int16 m, int16 x, int16 y, int size, int8 dir, bool shootable, const char* wall_name) = 0;
		virtual void iwall_get(struct map_session_data *sd) = 0;
		virtual bool iwall_remove(const char *wall_name) = 0;

		virtual int addmobtolist(unsigned short m, struct spawn_data *spawn) = 0;	// [Wizputer]
		virtual void spawnmobs(int16 m) = 0; // [Wizputer]
		virtual void removemobs(int16 m) = 0; // [Wizputer]
		virtual void addmap2db(struct map_data *m) = 0;
		virtual void removemapdb(struct map_data *m) = 0;

		virtual void skill_damage_add(struct map_data *m, uint16 skill_id, int rate[SKILLDMG_MAX], uint16 caster) = 0;
		virtual void skill_duration_add(struct map_data *mapd, uint16 skill_id, uint16 per) = 0;

		virtual enum e_mapflag getmapflag_by_name(char* name) = 0;
		virtual bool getmapflag_name(enum e_mapflag mapflag, char* output) = 0;
		virtual int getmapflag_sub(int16 m, enum e_mapflag mapflag, union u_mapflag_args *args) = 0;
		virtual bool setmapflag_sub(int16 m, enum e_mapflag mapflag, bool status, union u_mapflag_args *args) = 0;
