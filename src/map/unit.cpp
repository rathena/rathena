...
int32 unit_walktoxy_sub(struct block_list *bl) {
	...
	if (bl->type != BL_NPC // If type is a NPC, please disregard.
		...
		&& !path_search_long(nullptr, bl->m, bl->x, bl->y, ud->to_x, ud->to_y, CELL_CHKNOPASS)) { // Check if there is
																								  // an obstacle between
		return 0;
	}
#endif
...
static TIMER_FUNC(unit_walktoxy_timer) {
	...
	if (tid ==
		INVALID_TIMER) { // A directly invoked timer is from battle_stop_walking, therefore the rest is irrelevant.
		return 0;
	}
...
void unit_stop_walking_soon(struct block_list &bl) {
	...
	if (cell_percent > 0.0 && cell_percent < 1.0) {
		...
		ud->sx =
			static_cast<decltype(ud->sx)>(24.0 + dirx[ud->walkpath.path[ud->walkpath.path_pos]] * 16.0 * cell_percent);
		ud->sy =
			static_cast<decltype(ud->sy)>(24.0 + diry[ud->walkpath.path[ud->walkpath.path_pos]] * 16.0 * cell_percent);
...
int32 unit_skilluse_id2(struct block_list *src,
	...
	if (!ud->state.running) { // Need TK_RUN or WUGDASH handler to be done before that, see bugreport:6026
		unit_stop_walking(src,
						  1); // Even though this is not how official works but this will do the trick. bugreport:6829
	}
...
int32 unit_skilluse_pos2(struct block_list *src,
	...
	if (ud->skilltimer != INVALID_TIMER) { // Normally not needed since clif.cpp checks for it, but at/char/script
...
bool unit_can_reach_bl(struct block_list *bl, struct block_list *tbl, int32 range, int32 easy, short *x, short *y) {
	...
	if (bl->type != BL_NPC // If type is a NPC, please disregard.
		&& wpd.path_len >
			   14 // Official number of walkable cells is 14 if and only if there is an obstacle between. [malufett]
		&& !path_search_long(
			   nullptr, bl->m, bl->x, bl->y, tbl->x - dx, tbl->y - dy, CELL_CHKNOPASS)) { // Check if there is an
																						  // obstacle between
		return false;
	}
#endif
		...
		case BL_MOB: {
			...
			if (md->spawn) {
				...
				if (!md->spawn->state.dynamic) { // Permanently remove the mob
					if (--md->spawn->num ==
						0) { // Last freed mob is responsible for deallocating the group's spawn data.
						aFree(md->spawn);
						md->spawn = nullptr;
					}
				}
