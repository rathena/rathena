...
static TIMER_FUNC(sale_start_timer) {
	...
	// Init sale end
	sale_item->timer_end =
		add_timer(gettick() + (uint32)(sale_item->end - time(nullptr)) * 1000, sale_end_timer, 0, (intptr_t)sale_item);
...
enum e_sale_add_result sale_add_item(t_itemid nameid, int32 count, time_t from, time_t to) {
	...
	sale_item->timer_start =
		add_timer(gettick() + (uint32)(from - time(nullptr)) * 1000, sale_start_timer, 0, (intptr_t)sale_item);
	sale_item->timer_end = INVALID_TIMER;
...
static void cashshop_read_db(void) {
	...
	for (i = 0; i < sale_items.count; i++) {
		...
		if (it->start > now) {
			it->timer_start =
				add_timer(gettick() + (uint32)(it->start - time(nullptr)) * 1000, sale_start_timer, 0, (intptr_t)it);
		}
		else {
		...
		for (uint32 j = 0; j < quantity; j += get_amt) {
			...
			if (tab == CASHSHOP_TAB_SALE) {
				...
				if (new_amount == 0) {
					...
					if (SQL_ERROR == Sql_Query(mmysql_handle,
											   "UPDATE `%s` SET `amount` = '%d' WHERE `nameid` = '%u'",
											   sales_table,
											   new_amount,
											   nameid)) {
						Sql_ShowDebug(mmysql_handle);
