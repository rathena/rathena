	...
	if (val != 0) {
		if ((m = static_cast<mapreg_save *>(i64db_get(regs.vars, uid)))) {
			...
			if (name[1] != '@' && !skip_insert) {// write new variable to database
				...
				if (SQL_ERROR == Sql_Query(mmysql_handle, "INSERT INTO `%s`(`varname`,`index`,`value`) VALUES ('%s','%d','%d')", mapserv_table(mapreg_table), esc_name, i, val))
		...
		if (name[1] != '@') {// Remove from database because it is unused.
			char esc_name[32 * 2 + 1];
			Sql_EscapeStringLen(mmysql_handle, esc_name, name, strnlen(name, 32));
			if (SQL_ERROR == Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `varname`='%s' AND `index`='%d'", mapserv_table(mapreg_table), esc_name, i))
				Sql_ShowDebug(mmysql_handle);
		}
	}
	...
	if (str == NULL || *str == 0) {
		...
		if ((m = static_cast<mapreg_save *>(i64db_get(regs.vars, uid)))) {
			...
			if (name[1] != '@' && !skip_insert) { //put returned null, so we must insert.
				...
				if (SQL_ERROR == Sql_Query(mmysql_handle, "INSERT INTO `%s`(`varname`,`index`,`value`) VALUES ('%s','%d','%s')", mapserv_table(mapreg_table), esc_name, i, esc_str))
					Sql_ShowDebug(mmysql_handle);
			}
			i64db_put(regs.vars, uid, m);
		}
		...
		for (m = static_cast<mapreg_save *>(dbi_first(iter)); dbi_exists(iter); m = static_cast<mapreg_save *>(dbi_next(iter))) {
			if (m->save) {
				...
				if (!m->is_string) {
					char esc_name[32 * 2 + 1];
					Sql_EscapeStringLen(mmysql_handle, esc_name, name, strnlen(name, 32));
					if (SQL_ERROR == Sql_Query(mmysql_handle, "UPDATE `%s` SET `value`='%d' WHERE `varname`='%s' AND `index`='%d' LIMIT 1", mapserv_table(mapreg_table), m->u.i, esc_name, i))
						Sql_ShowDebug(mmysql_handle);
				} else {
					...
					if (SQL_ERROR == Sql_Query(mmysql_handle, "UPDATE `%s` SET `value`='%s' WHERE `varname`='%s' AND `index`='%d' LIMIT 1", mapserv_table(mapreg_table), esc_str, esc_name, i))
						Sql_ShowDebug(mmysql_handle);
				}
				m->save = false;
			}
