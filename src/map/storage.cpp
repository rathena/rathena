...
int32 compare_item(struct item *a, struct item *b) {
	if (a->nameid == b->nameid && a->identify == b->identify && a->refine == b->refine &&
		a->attribute == b->attribute && a->expire_time == b->expire_time && a->bound == b->bound &&
 ...
 **/
static enum e_storage_add storage_canAddItem(
	struct s_storage *stor, int32 idx, struct item items[], int32 amount, int32 max_num) {
		...
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_storage[i], it)) { // existing items found, stack them
				if (amount > MAX_AMOUNT - stor->u.items_storage[i].amount ||
					(data->stack.storage && amount > data->stack.amount - stor->u.items_storage[i].amount)) {
					return 2;
				}
...
void storage_guild_log(map_session_data *sd, struct item *item, int16 amount) {
	...
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`time`, `guild_id`, `char_id`, `name`, `nameid`, `amount`, `identify`, "
					 "`refine`, `attribute`, `unique_id`, `bound`, `enchantgrade`",
					 guild_storage_log_table);
	...
	StringBuf_Printf(&buf,
					 ") VALUES(NOW(),'%u','%u', '%s', '%u', '%d','%d','%d','%d','%" PRIu64 "','%d','%d'",
					 sd->status.guild_id,
					 sd->status.char_id,
					 sd->status.name,
					 item->nameid,
...
enum e_guild_storage_log storage_guild_log_read_sub(map_session_data *sd,
	...
	StringBuf_AppendStr(
		&buf, " , `nameid`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, `unique_id`, `enchantgrade`");
...
bool storage_guild_additem(map_session_data *sd, struct s_storage *stor, struct item *item_data, int32 amount) {
	...
	if (!itemdb_canguildstore(item_data, pc_get_group_level(sd)) ||
		item_data->expire_time) { // Check if item is storable. [Skotlex]
		clif_displaymessage(sd->fd, msg_txt(sd, 264));
		return false;
	}
	...
	if (itemdb_isstackable2(id)) { // Stackable
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_guild[i], item_data)) {
				if (amount > MAX_AMOUNT - stor->u.items_guild[i].amount ||
					(id->stack.guild_storage && amount > id->stack.amount - stor->u.items_guild[i].amount)) {
					return false;
				}
	...
	if (itemdb_isstackable2(id.get())) { // Stackable
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_guild[i], item)) {
				...
				amount =
					min(amount,
						((id->stack.guild_storage) ? id->stack.amount : MAX_AMOUNT) - stor->u.items_guild[i].amount);
				if (amount != item->amount) {
					ShowWarning("storage_guild_additem2: Stack limit reached! Altered amount of item \"" CL_WHITE
								"%s" CL_RESET "\" (%u). '" CL_WHITE "%d" CL_RESET "' -> '" CL_WHITE "%d" CL_RESET
								"'.\n",
								id->name.c_str(),
								id->nameid,
								item->amount,
...
void storage_premiumStorage_open(map_session_data *sd) {
	...
	clif_storagelist(sd,
					 sd->premiumStorage.u.items_storage,
					 ARRAYLENGTH(sd->premiumStorage.u.items_storage),
					 storage_getName(sd->premiumStorage.stor_id));
