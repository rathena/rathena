...
uint64 HomExpDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (level >= MAX_LEVEL) {
		this->invalidWarning(
			node["Level"], "Homunculus level %d exceeds maximum level %d, skipping.\n", level, MAX_LEVEL);
		return 0;
	}
...
void hom_skillup(struct homun_data *hd, uint16 skill_id) {
	...
	if (hd->homunculus.skillpts > 0 && hd->homunculus.hskill[idx].id &&
		hd->homunculus.hskill[idx].flag ==
			SKILL_FLAG_PERMANENT && // Don't allow raising while you have granted skills. [Skotlex]
...
int32 hom_levelup(struct homun_data *hd) {
	...
	if (((m_class & HOM_REG) && hd->homunculus.level >= battle_config.hom_max_level) ||
		((m_class & HOM_S) && hd->homunculus.level >= battle_config.hom_S_max_level) || !hd->exp_next ||
	...
	// Needed to update skill list for mutated homunculus so unlocked skills will appear when the needed level is
	// reached.
	status_calc_homunculus(hd, SCO_NONE);
	clif_hominfo(hd->master, hd, 0);
...
void hom_gainexp(struct homun_data *hd, t_exp exp) {
	...
	if (hd->exp_next == 0 || ((m_class & HOM_REG) && hd->homunculus.level >= battle_config.hom_max_level) ||
...
static TIMER_FUNC(hom_hungry) {
	...
	if (battle_config.feature_homunculus_autofeed && hd->homunculus.autofeed &&
		hd->homunculus.hunger <= battle_config.feature_homunculus_autofeed_rate) {
		hom_food(sd, hd);
	}
	...
	int32 hunger_delay =
		(battle_config.homunculus_starving_rate > 0 && hd->homunculus.hunger <= battle_config.homunculus_starving_rate)
			? battle_config.homunculus_starving_delay
			: hd->homunculusDB->hungryDelay; // Every 20 seconds if hunger <= 10
...
void hom_init_timers(struct homun_data *hd) {
	if (hd->hungry_timer == INVALID_TIMER) {
		int32 hunger_delay = (battle_config.homunculus_starving_rate > 0 &&
							  hd->homunculus.hunger <= battle_config.homunculus_starving_rate)
								 ? battle_config.homunculus_starving_delay
								 : hd->homunculusDB->hungryDelay; // Every 20 seconds if hunger <= 10
...
bool hom_call(map_session_data *sd) {
	...
	if (hd->bl.prev == nullptr) { // Spawn him
		hd->bl.x = sd->bl.x;
		hd->bl.y = sd->bl.y;
		hd->bl.m = sd->bl.m;
 ...
 * @param hd
 */
void hom_reset_stats(
	struct homun_data *hd) { // Resets a homunc stats back to zero (but doesn't touches hunger or intimacy)
...
bool HomunculusDatabase::parseStatusNode(const std::string &nodeName,
...
uint64 HomunculusDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "EvolutionClass")) {
		...
		if (!script_get_constant(evo_class_name_constant.c_str(), &constant)) {
			this->invalidWarning(
				node["EvolutionClass"], "Invalid homunculus Evolution Class %s, skipping.\n", evo_class_name.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "EvolutionSize")) {
		...
		if (!script_get_constant(size_constant.c_str(), &constant)) {
			this->invalidWarning(
				node["EvolutionSize"], "Invalid homunculus EvolutionSize %s, skipping.\n", size.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "Status")) {
		...
		if (hom->gmin.HP > hom->gmax.HP) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum HP %d is greater than GrowthMaximum HP %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.HP,
								 hom->gmax.HP,
								 class_name.c_str());
		...
		if (hom->gmin.SP > hom->gmax.SP) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum SP %d is greater than GrowthMaximum SP %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.SP,
								 hom->gmax.SP,
								 class_name.c_str());
		...
		if (hom->gmin.str > hom->gmax.str) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum STR %d is greater than GrowthMaximum STR %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.str,
								 hom->gmax.str,
								 class_name.c_str());
		...
		if (hom->gmin.agi > hom->gmax.agi) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum AGI %d is greater than GrowthMaximum AGI %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.agi,
								 hom->gmax.agi,
								 class_name.c_str());
		...
		if (hom->gmin.vit > hom->gmax.vit) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum VIT %d is greater than GrowthMaximum VIT %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.vit,
								 hom->gmax.vit,
								 class_name.c_str());
		...
		if (hom->gmin.int_ > hom->gmax.int_) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum INT %d is greater than GrowthMaximum INT %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.int_,
								 hom->gmax.int_,
								 class_name.c_str());
		...
		if (hom->gmin.dex > hom->gmax.dex) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum DEX %d is greater than GrowthMaximum DEX %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.dex,
								 hom->gmax.dex,
								 class_name.c_str());
		...
		if (hom->gmin.luk > hom->gmax.luk) {
			...
			this->invalidWarning(node,
								 "GrowthMinimum LUK %d is greater than GrowthMaximum LUK %d for homunculus %s, capping "
								 "minimum to maximum.\n",
								 hom->gmin.luk,
								 hom->gmax.luk,
								 class_name.c_str());
		...
		if (hom->emin.HP > hom->emax.HP) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum HP %d is greater than EvolutionMaximum HP %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.HP,
								 hom->emax.HP,
								 class_name.c_str());
		}
		if (hom->emin.SP > hom->emax.SP) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum SP %d is greater than EvolutionMaximum SP %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.SP,
								 hom->emax.SP,
								 class_name.c_str());
		...
		if (hom->emin.str > hom->emax.str) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum STR %d is greater than EvolutionMaximum STR %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.str,
								 hom->emax.str,
								 class_name.c_str());
		...
		if (hom->emin.agi > hom->emax.agi) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum AGI %d is greater than EvolutionMaximum AGI %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.agi,
								 hom->emax.agi,
								 class_name.c_str());
		...
		if (hom->emin.vit > hom->emax.vit) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum VIT %d is greater than EvolutionMaximum VIT %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.vit,
								 hom->emax.vit,
								 class_name.c_str());
		...
		if (hom->emin.int_ > hom->emax.int_) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum INT %d is greater than EvolutionMaximum INT %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.int_,
								 hom->emax.int_,
								 class_name.c_str());
		...
		if (hom->emin.dex > hom->emax.dex) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum DEX %d is greater than EvolutionMaximum DEX %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.dex,
								 hom->emax.dex,
								 class_name.c_str());
		...
		if (hom->emin.luk > hom->emax.luk) {
			...
			this->invalidWarning(node,
								 "EvolutionMinimum LUK %d is greater than EvolutionMaximum LUK %d for homunculus %s, "
								 "capping minimum to maximum.\n",
								 hom->emin.luk,
								 hom->emax.luk,
								 class_name.c_str());
	...
	if (this->nodeExists(node, "SkillTree")) {
		...
		for (const ryml::NodeRef &skill : skillsNode) {
			...
			if (this->nodeExists(skill, "Skill")) {
				...
				if (skill_id == 0) {
					this->invalidWarning(
						skill["Skill"], "Invalid homunculus skill %s, skipping.\n", skill_name.c_str());
					return 0;
				}
				...
				if (!SKILL_CHK_HOMUN(skill_id)) {
					this->invalidWarning(
						skill["Skill"],
						"Homunculus skill %s (%u) is out of the homunculus skill range [%u-%u], skipping.\n",
						skill_name.c_str(),
						skill_id,
						HM_SKILLBASE,
						HM_SKILLBASE + MAX_HOMUNSKILL - 1);
					return 0;
				}
			...
			if (this->nodeExists(skill, "Clear")) {
				...
				if (!found) {
					this->invalidWarning(skill["Clear"],
										 "Failed to remove nonexistent skill %s from homunuculus %s.\n",
										 skill_db.find(entry.id)->name,
										 class_name.c_str());
				}
				continue;
			}
			...
			if (this->nodeExists(skill, "MaxLevel")) {
				...
				if (level > db_level) {
					this->invalidWarning(
						skill["MaxLevel"],
						"Skill %s exceeds maximum defined skill level %d from homunuculus %s, capping.\n",
						skill_db.find(entry.id)->name,
						db_level,
						class_name.c_str());
					level = db_level;
			...
			if (this->nodeExists(skill, "RequiredLevel")) {
				...
				if (level > config_max) {
					this->invalidWarning(skill["RequiredLevel"],
										 "Homunculus Required Skill level %u exceeds maximum level %u, capping.\n",
										 level,
										 config_max);
					level = config_max;
				}
			...
			if (this->nodeExists(skill, "RequiredIntimacy")) {
				...
				if (intimacy > 1000) {
					this->invalidWarning(skill["RequiredIntimacy"],
										 "Homunculus Required Intimacy %u exceeds maximum intimacy 1000, capping.\n",
										 intimacy);
					intimacy = 1000;
				}
			...
			if (this->nodeExists(skill, "RequireEvolution")) {
				...
				if (evo && hom->base_class == hom->evo_class) {
					this->invalidWarning(
						skill["RequireEvolution"],
						"Homunculus %s does not have any evolution making skill %s unobtainable, skipping.\n",
						class_name.c_str(),
						skill_db.find(entry.id)->name);
					return 0;
				}
			...
			if (this->nodeExists(skill, "Required")) {
				...
				for (const ryml::NodeRef &prereqskill : required) {
					...
					if (this->nodeExists(prereqskill, "Skill")) {
						...
						if (skill_id == 0) {
							this->invalidWarning(
								prereqskill["Skill"], "Invalid homunculus skill %s, skipping.\n", skill_name.c_str());
							return 0;
						}
