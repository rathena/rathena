		...
		for (const auto &location : node["Locations"]) {
			...
			if (this->nodeExists(location, "StartEvent")) {
				...
				if (map_entry.bgcallscript.length() > EVENT_NAME_LENGTH) {
					this->invalidWarning(location["StartEvent"],
	...
	if (!sd->sc.empty()) {
		if (sd->sc.getSCE(SC_ENTRY_QUEUE_APPLY_DELAY)) { // Exclude any player who's recently left a battleground queue
			...
			sprintf(buf,
					msg_txt(sd, 339),
					static_cast<int32>(
						(get_timer(sd->sc.getSCE(SC_ENTRY_QUEUE_APPLY_DELAY)->timer)->tick - gettick()) /
						1000)); // You can't apply to a battleground queue for %d seconds due to recently leaving one.
			clif_bg_queue_apply_result(BG_APPLY_NONE, name, sd);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], buf, false, SELF);
			return false;
		}

		if (sd->sc.getSCE(SC_ENTRY_QUEUE_NOTIFY_ADMISSION_TIME_OUT)) { // Exclude any player who's recently deserted a
																	   // battleground
			char buf[CHAT_SIZE_MAX];
			...
			sprintf(buf,
					msg_txt(sd, 338),
					status_tick / 60,
					status_tick % 60); // You can't apply to a battleground queue due to recently deserting a
		...
		for (auto &map : pair.second->maps) {
			if (map.mapindex == mapindex) {
				...
				for (auto &queue : bg_queues) {
					if (queue->map == &map) {
						if (ended) { // The ended flag is applied from bg_reserve (bg_unbook clears it for the next
									 // queue)
		...
		if (sd->bg_queue_id == queue->queue_id) {
			if (!bg_queue_leave_sub(sd, queue->teama_members, apply_sc) &&
				...
				if ((queue->state == QUEUE_STATE_SETUP || queue->state == QUEUE_STATE_SETUP_DELAY) &&
					...
					queue->teamb_members
						.empty()) { // If there are no players left in the queue (that hasn't started), discard it
					bg_queue_clear(queue, true);
				}
...
void bg_queue_on_accept_invite(map_session_data *sd) {
	...
	if (queue->state == QUEUE_STATE_ACTIVE) { // Battleground is already active
		...
		if (queue->accepted_players == queue->required_players * 2) {
			...
			queue->tid_start = add_timer(gettick() + battleground_db.find(queue->id)->start_delay * 1000,
										 bg_on_ready_start,
										 0,
