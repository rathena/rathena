 ...
 * @param mes
 */
int32 guild_member_withdraw(
	int32 guild_id, uint32 account_id, uint32 char_id, int32 flag, const char *name, const char *mes) {
	int32 i;
	auto g = guild_search(guild_id);
	...
	if (sd) { // Character is online
		...
		if (j) {
			...
			for (i = 0; i < j; i++) { // Loop the matching items, gstorage_additem takes care of opening storage
				if (stor) {
					storage_guild_additem(sd,
										  stor,
										  &sd->inventory.u.items_inventory[idxlist[i]],
...
int32 guild_recv_memberinfoshort(
	int32 guild_id, uint32 account_id, uint32 char_id, int32 online, int32 lv, int32 class_) { // cleaned up [LuzZza]
...
void guild_guildaura_refresh(map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	if (group) {
		sc_start4(nullptr,
				  ...
				  type,
				  100,
				  (battle_config.guild_aura & 16) ? 0 : skill_lv,
				  0,
				  0,
				  group->group_id,
 ...
 *---------------------------------------------------*/
int32 guild_allianceack(int32 guild_id1,
						int32 guild_id2,
						uint32 account_id1,
						uint32 account_id2,
						int32 flag,
...
int32 guild_gm_changed(int32 guild_id, uint32 account_id, uint32 char_id, time_t time) {
	...
	if (g->guild.member[pos].sd && g->guild.member[pos].sd->fd) {
		clif_displaymessage(g->guild.member[pos].sd->fd,
							msg_txt(g->guild.member[pos].sd, 678)); //"You no longer are the Guild Master."
	...
	if (g->guild.member[0].sd && g->guild.member[0].sd->fd) {
		clif_displaymessage(g->guild.member[0].sd->fd,
							msg_txt(g->guild.member[pos].sd, 679)); //"You have become the Guild Master!"
