...
ACMD_FUNC(servertime)
{
	...
	if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
		...
		if (night_flag == 1 && (timer_data = get_timer(day_timer_tid)) != nullptr) { // we start with night
		...
		if (night_flag == 0 && (timer_data = get_timer(night_timer_tid)) != nullptr) { // we start with day
