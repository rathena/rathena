...
#define ACMD_FUNC(x) \
	static int32 atcommand_##x(const int32 fd, map_session_data* sd, const char* command, const char* message)
...
ACMD_FUNC(mapmove) {
	...
	if (!message || !*message ||
		...
		clif_displaymessage(fd,
							msg_txt(sd, 909)); // Please enter a map (usage: @warp/@rura/@mapmove <mapname> <x> <y>).
		return -1;
	}
...
ACMD_FUNC(jumpto) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 911)); // Please enter a player name (usage: @jumpto/@warpto/@goto <char name/ID>).
		return -1;
	}
...
ACMD_FUNC(whogm) {
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		...
		sprintf(atcmd_output,
				msg_txt(sd, 914), // Name: %s (GM:%d) | Location: %s %d %d
				pl_sd->status.name,
				pl_level,
				mapindex_id2name(pl_sd->mapindex),
				pl_sd->bl.x,
		...
		sprintf(atcmd_output,
				msg_txt(sd, 915), // BLvl: %d | Job: %s (Lvl: %d)
				pl_sd->status.base_level,
		...
		sprintf(atcmd_output,
				msg_txt(sd, 916), // Party: '%s' | Guild: '%s'
				p ? p->party.name : msg_txt(sd, 917),
				g ? g->guild.name : msg_txt(sd, 917)); // None.
...
ACMD_FUNC(resetcooltime) {
	...
	if (!sd->scd.empty()) {
		for (const auto& entry : sd->scd) {
			sprintf(
				atcmd_output, msg_txt(sd, 1537), skill_db.find(entry.first)->name); // Found skill '%s', unblocking...
...
ACMD_FUNC(kami) {
	...
	if (*(command + 5) != 'c' && *(command + 5) != 'C') {
		...
		if (strstr(command, "l") != nullptr) {
			...
			intif_broadcast(atcmd_output,
							strlen(atcmd_output) + 1,
		...
		if (!message || !*message || (sscanf(message, "%20lx %199[^\n]", &color, atcmd_output) < 2)) {
			clif_displaymessage(fd,
								msg_txt(sd, 981)); // Please enter color and message (usage: @kamic <color> <message>).
			return -1;
		}
...
ACMD_FUNC(item) {
	...
	if (!strcmpi(parent_cmd, "itembound")) {
		...
		clif_displaymessage(
			fd, msg_txt(sd, 983)); // Please enter an item name or ID (usage: @item <item name/ID> <quantity>).
		return -1;
	}
...
ACMD_FUNC(item2) {
	...
	if (!strcmpi(parent_cmd + 1, "itembound2")) {
		if (!message || !*message ||
			(sscanf(message,
					"\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d %11d",
					item_name,
					&number,
					&identify,
					&refine,
					...
					&bound) < 10 &&
			 sscanf(message,
					"%99s %11d %11d %11d %11d %11d %11d %11d %11d %11d",
					item_name,
					&number,
					&identify,
					&refine,
					...
					&bound) < 10)) {
			clif_displaymessage(
				fd, msg_txt(sd, 296)); // Please enter all parameters (usage: @item2 <item name/ID> <quantity>
			clif_displaymessage(
				fd,
				msg_txt(sd,
		...
		if (bound <= BOUND_NONE || bound >= BOUND_MAX) {
			...
			return -1;
			 ...
			 (sscanf(message,
					 "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d",
					 item_name,
					 &number,
					 &identify,
					 &refine,
					 ...
					 &c4) < 9 &&
			  sscanf(message,
					 "%99s %11d %11d %11d %11d %11d %11d %11d %11d",
					 item_name,
					 &number,
					 &identify,
					 &refine,
					 ...
					 &c4) < 9)) {
		clif_displaymessage(fd,
							msg_txt(sd, 984)); // Please enter all parameters (usage: @item2 <item name/ID> <quantity>
		clif_displaymessage(
			fd, msg_txt(sd, 985)); //   <identify_flag> <refine> <attribute> <card1> <card2> <card3> <card4>).
		return -1;
	}
...
ACMD_FUNC(baselevelup) {
	...
	if (!message || !*message || !level) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 986)); // Please enter a level adjustment (usage: @lvup/@blevel/@baselvlup <number of levels>).
		return -1;
	}
	...
	if (level > 0) {
		...
		if ((uint32)level > pc_maxbaselv(sd) ||
			(uint32)level > pc_maxbaselv(sd) - sd->status.base_level) { // fix positive overflow
...
ACMD_FUNC(joblevelup) {
	...
	if (level > 0) {
		...
		if ((uint32)level > pc_maxjoblv(sd) ||
			(uint32)level > pc_maxjoblv(sd) - sd->status.job_level) { // fix positive overflow
...
ACMD_FUNC(model) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d %11d", &hair_style, &hair_color, &cloth_color) < 1) {
		sprintf(atcmd_output,
				msg_txt(sd, 991), // Please enter at least one value (usage: @model <hair ID: %d-%d> <hair color: %d-%d>
								  // <clothes color: %d-%d>).
				MIN_HAIR_STYLE,
				MAX_HAIR_STYLE,
				MIN_HAIR_COLOR,
...
ACMD_FUNC(refine) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &refine) < 2) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 996)); // Please enter a position and an amount (usage: @refine <equip position> <+/- amount>).
...
ACMD_FUNC(grade) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &grade) < 2) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1519)); // Please enter a position and an amount (usage: @grade <equip position> <+/- amount>).
		sprintf(atcmd_output, msg_txt(sd, 997), EQP_HEAD_LOW); // %d: Lower Headgear
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, msg_txt(sd, 998), EQP_HAND_R); // %d: Right Hand
		clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(produce) {
	...
	if (!message || !*message ||
		...
		clif_displaymessage(fd, msg_txt(sd, 1007)); // Please enter at least one item name/ID (usage: @produce <equip
													// name/ID> <element> <# of very's>).
		return -1;
	}
	...
	if (itemdb_isequip2(item_data.get())) {
		...
		sprintf(
			atcmd_output, msg_txt(sd, 169), item_id, item_data->name.c_str()); // The item (%u: '%s') is not equipable.
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(param) {
	...
	if (!message || !*message || sscanf(message, "%11d", &value) < 1 || value == 0) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1013)); // Please enter a valid value (usage: @str/@agi/@vit/@int/@dex/@luk <+/-adjustment>).
		return -1;
	}
...
ACMD_FUNC(guildlevelup) {
	...
	if (!message || !*message || sscanf(message, "%11d", &level) < 1 || level == 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1014)); // Please enter a valid level (usage: @guildlvup/@guildlvlup <# of levels>).
		return -1;
	}
	...
	if (level > 0 && (level > MAX_GUILDLEVEL ||
					  added_level > ((short)MAX_GUILDLEVEL - guild_info->guild.guild_lv))) { // fix positive overflow
...
ACMD_FUNC(makeegg) {
	...
	if (pet != nullptr) {
		...
		if (mdb) {
			if (intif_create_pet(sd->status.account_id,
								 sd->status.char_id,
								 pet->class_,
								 mdb->lv,
								 pet->EggID,
...
ACMD_FUNC(recall) {
	...
	if (pc_get_group_level(sd) < pc_get_group_level(pl_sd)) {
		clif_displaymessage(
			fd, msg_txt(sd, 81)); // Your GM level doesn't authorize you to perform this action on the specified player.
		return -1;
	}
...
ACMD_FUNC(char_ban) {
	...
	if (timediff == 0) { // allow negative ?
		safesnprintf(
			output, sizeof(output), msg_txt(sd, 85), command, timediff); // Invalid time for %s command (time=%d)
		clif_displaymessage(fd, output);
...
ACMD_FUNC(spiritball) {
	...
	if (!message || !*message || (number = atoi(message)) < 0 || number > MAX_SPIRITBALL) {
		...
		safesnprintf(msg,
					 sizeof(msg),
					 msg_txt(sd, 1028),
					 MAX_SPIRITBALL); // Please enter an amount (usage: @spiritball <number: 0-%d>).
...
ACMD_FUNC(idsearch) {
	...
	if (!message || !*message || sscanf(message, "%99s", item_name) < 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1031)); // Please enter part of an item name (usage: @idsearch <part_of_item_name>).
		return -1;
	}
...
ACMD_FUNC(guildrecall) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd,
							msg_txt(sd, 1034)); // Please enter a guild name/ID (usage: @guildrecall <guild_name/ID>).
		return -1;
	}
	...
	sprintf(atcmd_output,
			msg_txt(sd, 93),
			g->guild.name); // All online characters of the %s guild have been recalled to your position.
	clif_displaymessage(fd, atcmd_output);
	if (count) {
		sprintf(atcmd_output,
				msg_txt(sd, 1033),
...
ACMD_FUNC(partyrecall) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(fd,
							msg_txt(sd, 1035)); // Please enter a party name/ID (usage: @partyrecall <party_name/ID>).
		return -1;
	}
...
ACMD_FUNC(mapinfo) {
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(
			fd, msg_txt(sd, 1038)); // Please enter at least one valid list number (usage: @mapinfo <0-3> <map>).
		return -1;
	}
	...
	if (mapdata->getMapFlag(MF_SKILL_DAMAGE)) {
		...
		sprintf(atcmd_output,
				msg_txt(sd, 1053), // > [Map] %d%%, %d%%, %d%%, %d%% | Caster:%d
		...
		if (!mapdata->skill_damage.empty()) {
			clif_displaymessage(
				fd, msg_txt(sd, 1054)); // > [Map Skill] Name : Player, Monster, Boss Monster, Other | Caster
	...
	sprintf(atcmd_output,
			msg_txt(sd, 1065), // No Exp Penalty: %s | No Zeny Penalty: %s
			(map_getmapflag(m_id, MF_NOEXPPENALTY)) ? msg_txt(sd, 1066) : msg_txt(sd, 1067),
			(map_getmapflag(m_id, MF_NOZENYPENALTY)) ? msg_txt(sd, 1066) : msg_txt(sd, 1067)); // On / Off
	clif_displaymessage(fd, atcmd_output);
	...
	if (map_getmapflag(m_id, MF_NOSAVE)) {
		if (!mapdata->save.map) {
			...
			sprintf(
				atcmd_output, msg_txt(sd, 1069), mapindex_id2name(mapdata->save.map)); // No Save, Save Point: %s,Random
			clif_displaymessage(fd, atcmd_output);
		}
		else {
			sprintf(atcmd_output,
					msg_txt(sd, 1070), // No Save, Save Point: %s,%d,%d
					mapindex_id2name(mapdata->save.map),
					mapdata->save.x,
					mapdata->save.y);
			clif_displaymessage(fd, atcmd_output);
		}
	...
	switch (list) {
		...
		case 2:
			...
			for (i = 0; i < mapdata->npc_num;) {
				...
				if (strcmp(nd->name, nd->exname) == 0) {
					sprintf(atcmd_output,
							msg_txt(sd, 490), // NPC %d: %s | Direction: %s | Sprite: %d | Location: %d %d
							++i,
							nd->name,
							direction,
							nd->class_,
					...
					sprintf(atcmd_output,
							msg_txt(sd, 489), // NPC %d: %s::%s | Direction: %s | Sprite: %d | Location: %d %d
							++i,
							nd->name,
							nd->exname,
							direction,
		...
		default: // normally impossible to arrive here
			clif_displaymessage(
				fd, msg_txt(sd, 488)); // Please enter at least one valid list number (usage: @mapinfo <0-3> <map>).
			return -1;
			break;
	}
...
ACMD_FUNC(nuke) {
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name, false)) != nullptr) {
		if (pc_get_group_level(sd) >= pc_get_group_level(pl_sd)) { // you can kill only lower or same GM level
			...
			clif_displaymessage(fd,
								msg_txt(sd, 81)); // Your GM level don't authorise you to do this action on this player.
			return -1;
		}
	}
	else {
...
ACMD_FUNC(reloadnpcfile) {
	...
	if (npc_unloadfile(message)) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1386)); // File unloaded. Be aware that mapflags and monsters spawned directly are not removed.
	}
		...
		if (night_flag == 1) { // we start with night
			if ((timer_data = get_timer(day_timer_tid)) != nullptr) {
				sprintf(
					temp,
					msg_txt(sd, 233),
					txt_time(DIFF_TICK(timer_data->tick, gettick()) / 1000)); // Game time: The game is in night for %s.
				clif_displaymessage(fd, temp);
		...
		if (night_flag == 0) {
			if ((timer_data = get_timer(night_timer_tid)) != nullptr &&
				...
				if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0) {
					sprintf(temp,
							msg_txt(sd, 237),
							txt_time(DIFF_TICK(timer_data->interval, DIFF_TICK(timer_data->tick, timer_data2->tick)) /
				...
				sprintf(temp,
						msg_txt(sd, 238),
						txt_time(timer_data->interval / 1000)); // Game time: A day cycle has a normal duration of %s.
				clif_displaymessage(fd, temp);
			}
			else {
				clif_displaymessage(fd, msg_txt(sd, 231)); // Game time: The game is in permanent daylight.
			...
			if ((timer_data = get_timer(day_timer_tid)) != nullptr &&
				...
				if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0) {
					sprintf(temp,
							msg_txt(sd, 239),
							txt_time((timer_data->interval - DIFF_TICK(timer_data->tick, timer_data2->tick)) /
...
ACMD_FUNC(unjail) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd,
							msg_txt(sd, 1135)); // Please enter a player name (usage: @unjail/@discharge <char_name>).
		return -1;
	}
	...
	if (pl_sd->sc.getSCE(SC_JAILED) && pl_sd->sc.getSCE(SC_JAILED)->val1 != INT_MAX) { // Update the player's jail time
		...
		if (jailtime <= 0) {
			...
			sprintf(atcmd_output,
					...
					day,
					hour,
					minute); // This player is now in jail for %d years, %d months, %d days, %d hours and %d minutes
			clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(jailtime) {
	...
	sprintf(atcmd_output,
			...
			day,
			hour,
			minute); // You will remain in jail for %d years, %d months, %d days, %d hours and %d minutes
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(disguiseall) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd,
							msg_txt(sd, 1145)); // Please enter a Monster/NPC name/ID (usage: @disguiseall <name/ID>).
		return -1;
	}
...
ACMD_FUNC(disguiseguild) {
	...
	if (!message || !*message || sscanf(message, "%23[^,], %23[^\r\n]", monster, guild) < 2) {
		clif_displaymessage(fd, msg_txt(sd, 1146)); // Please enter a mob name/ID and guild name/ID (usage:
...
ACMD_FUNC(undisguiseguild) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd,
							msg_txt(sd, 1147)); // Please enter guild name/ID (usage: @undisguiseguild <guild name/ID>).
		return -1;
	}
...
ACMD_FUNC(dropall) {
	...
	if (message[0]) {
		...
		if (type != -1 && type != IT_HEALING && type != IT_USABLE && type != IT_ETC && type != IT_WEAPON &&
			...
			clif_displaymessage(
				fd, msg_txt(sd, 1493)); // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3, armor = 4,
	...
	if (message[0]) {
		...
		switch (type) {
			...
			default:
				...
				clif_displaymessage(
					fd, msg_txt(sd, 1493)); // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3, armor =
											// 4, weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
				return -1;
		}
	}
...
ACMD_FUNC(skillid) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd,
							msg_txt(sd, 1163)); // Please enter a skill name to look up (usage: @skillid <skill name>).
		return -1;
	}
	...
	for (const auto& skill : skill_db) {
		...
		if (strnicmp(name, message, skillen) == 0 || strnicmp(desc, message, skillen) == 0) {
			...
			snprintf(partials[found++],
					 MAX_SKILLID_PARTIAL_RESULTS_LEN,
					 msg_txt(sd, 1164),
					 skill_id,
					 desc,
					 name); // // skill %d: %s (%s)
...
ACMD_FUNC(displayskill) {
	...
	if (!message || !*message || sscanf(message, "%6hu %6hu %6hu", &skill_id, &skill_lv, &type) < 1) {
		...
		clif_displaymessage(
			fd, msg_txt(sd, 825)); // Effect Types: 0: All, 1: Damage, 2: Splash Dmg, 3: No Damage, 4: Ground
		return -1;
	}
...
ACMD_FUNC(displayskillcast) {
	...
	if (!message || !*message ||
		...
		clif_displaymessage(
			fd,
			msg_txt(sd, 824)); // Usage: @displayskillcast <skill ID> {<skill level> <ground target flag> <cast time>}
		return -1;
	}
...
ACMD_FUNC(marry) {
	...
	if (w = itemdb_weight((pl_sd->status.sex) ? WEDDING_RING_M : WEDDING_RING_F) &&
			w + pl_sd->weight > pl_sd->max_weight) {
...
ACMD_FUNC(changelook) {
	int32 i, j = 0, k = 0;
	int32 pos[8] = {
		LOOK_HEAD_TOP, LOOK_HEAD_MID, LOOK_HEAD_BOTTOM, LOOK_WEAPON, LOOK_SHIELD, LOOK_SHOES, LOOK_ROBE, LOOK_BODY2};

	if ((i = sscanf(message, "%11d %11d", &j, &k)) < 1) {
		clif_displaymessage(fd, msg_txt(sd, 1177)); // Usage: @changelook {<position>} <view id>
		clif_displaymessage(
			fd, msg_txt(sd, 1178)); // Position: 1-Top 2-Middle 3-Bottom 4-Weapon 5-Shield 6-Shoes 7-Robe 8-Body
		return -1;
	}
	else if (i == 2) {
		if (j < 1 || j > 8) {
...
ACMD_FUNC(autotrade) {
	...
	if (battle_config.at_timeout) {
		...
		status_change_start(nullptr,
							...
							0,
							0,
							((timeout > 0) ? min(timeout, battle_config.at_timeout) : battle_config.at_timeout) * 60000,
							SCSTART_NONE);
	}
...
ACMD_FUNC(partyoption) {
	...
	if (!message || !*message || sscanf(message, "%15s %15s", w1, w2) < 2) {
		clif_displaymessage(
			fd, msg_txt(sd, 1186)); // Usage: @partyoption <pickup share: yes/no> <item distribution: yes/no>
		return -1;
	}
...
ACMD_FUNC(autolootitem) {
	...
	switch (action) {
		case 1:
			...
			if (i == AUTOLOOTITEM_SIZE) {
				clif_displaymessage(fd, msg_txt(sd, 1191)); // Your autolootitem list is full. Remove some items first
															// with @autolootid -<item name or ID>.
				return -1;
			}
			...
			sprintf(atcmd_output,
					...
					item_db.create_item_link(item_data).c_str(),
		...
		case 3:
			sprintf(
				atcmd_output, msg_txt(sd, 1195), AUTOLOOTITEM_SIZE); // You can have %d items on your autolootitem list.
			clif_displaymessage(fd, atcmd_output);
			clif_displaymessage(fd, msg_txt(sd, 1196)); // To add an item to the list, use "@alootid +<item name or
...
ACMD_FUNC(autoloottype) {
	...
	switch (action) {
		...
		case 2:
			...
			sprintf(atcmd_output,
					msg_txt(sd, 1485),
					itemdb_typename(type),
					type); // Removed item type: '%s' {%u} from your autoloottype list.
			clif_displaymessage(fd, atcmd_output);
			break;
		case 3:
			clif_displaymessage(
				fd, msg_txt(sd, 1486)); // To add an item type to the list, use "@aloottype +<type name or ID>". To
										// remove an item type, use "@aloottype -<type name or ID>".
			clif_displaymessage(fd, msg_txt(sd, 1487)); // Type List: healing = 0, usable = 2, etc = 3, armor = 4,
														// weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
...
ACMD_FUNC(cleanarea) {
	...
	if (!message || !*message || sscanf(message, "%6hd %6hd %6hd %6hd", &x0, &y0, &x1, &y1) < 1) {
		map_foreachinallarea(atcommand_cleanfloor_sub,
							 sd->bl.m,
							 sd->bl.x - (AREA_SIZE * 2),
							 sd->bl.y - (AREA_SIZE * 2),
							 sd->bl.x + (AREA_SIZE * 2),
...
ACMD_FUNC(npctalk) {
	...
	if (!ifcolor) {
		if (!message || !*message || sscanf(message, "%49[^,], %99[^\n]", name, mes) < 2) {
			clif_displaymessage(
				fd, msg_txt(sd, 1222)); // Please enter the correct parameters (usage: @npctalk <npc name>, <message>).
			return -1;
		}
	}
	else {
...
ACMD_FUNC(summon) {
	...
	if (!message || !*message || sscanf(message, "%23s %11d", name, &duration) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1225)); // Please enter a monster name (usage: @summon <monster name> {duration}).
		return -1;
	}
...
ACMD_FUNC(mobinfo) {
	...
	unsigned char mrace[RC_ALL][11] = {
		"Formless", "Undead", "Beast", "Plant", "Insect", "Fish", "Demon", "Demi-Human", "Angel", "Dragon", "Player"};
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1239)); // Please enter a monster name/ID (usage: @mobinfo <monster_name_or_monster_ID>).
		return -1;
	}
	...
	for (uint16 k = 0; k < count; k++) {
		...
		sprintf(atcmd_output,
				msg_txt(sd, 1244), //  ATK:%d~%d  Range:%d~%d~%d  Size:%s  Race: %s  Element: %s (Lv:%d)
				mob->status.batk + mob->status.rhw.atk,
				mob->status.batk + mob->status.rhw.atk2,
				mob->status.rhw.range,
				mob->range2,
				mob->range3,
				msize[mob->status.size],
				mrace[mob->status.race],
				melement[mob->status.def_ele],
				mob->status.ele_lv);
		clif_displaymessage(fd, atcmd_output);
#ifdef RENEWAL
...
ACMD_FUNC(showmobs) {
	...
	snprintf(atcmd_output,
			 sizeof atcmd_output,
			 msg_txt(sd, 1252), // Mob Search... %s %s
			 mob_name,
...
ACMD_FUNC(homlevel) {
	...
	if (!message || !*message || (level = atoi(message)) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1253)); // Please enter a level adjustment (usage: @homlevel <number of levels>).
		return -1;
	}
...
ACMD_FUNC(homfriendly) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1258)); // Please enter a friendly value (usage: @homfriendly <friendly value [0-1000]>).
		return -1;
	}
...
ACMD_FUNC(homhungry) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1259)); // Please enter a hunger value (usage: @homhungry <hunger value [0-100]>).
		return -1;
	}
...
ACMD_FUNC(hominfo) {
	...
	snprintf(atcmd_output,
			 sizeof(atcmd_output),
			 msg_txt(sd, 1263), // ATK: %d - MATK: %d~%d
			 status->rhw.atk2 + status->batk,
			 status->matk_min,
			 status->matk_max);
	clif_displaymessage(fd, atcmd_output);
	...
	snprintf(atcmd_output,
			 sizeof(atcmd_output),
			 msg_txt(sd, 1264), // Hungry: %d - Intimacy: %u
...
ACMD_FUNC(homstats) {
	...
	snprintf(
		atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1266), lv, db->name); // Homunculus growth stats (Lv %d %s):
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(iteminfo) {
	if (!message || !*message) {
		clif_displaymessage(fd,
							msg_txt(sd, 1276)); // Please enter an item name/ID (usage: @ii/@iteminfo <item name/ID>).
		return -1;
	}
	...
	for (const auto& result : item_array) {
		...
		sprintf(atcmd_output,
				msg_txt(sd, 1277), // Item: '%s'/'%s' (%u) Type: %s | Extra Effect: %s
				item_data->name.c_str(),
				item_db.create_item_link(item_data).c_str(),
				item_data->nameid,
		...
		if (item_data->maxchance == -1) {
			...
			if (item_data->maxchance) {
				sprintf(atcmd_output,
						msg_txt(sd, 1282),
						(float)item_data->maxchance / 100); //  - Maximal monsters drop chance: %02.02f%%
			}
			else {
...
ACMD_FUNC(whodrops) {
	...
	for (const auto& result : item_array) {
		...
		sprintf(
			atcmd_output, msg_txt(sd, 1285), item_db.create_item_link(id).c_str(), id->nameid); // Item: '%s' (ID:%u)
		clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(whereis) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1288)); // Please enter a monster name/ID (usage: @whereis <monster_name_or_monster_ID>).
		return -1;
	}
...
ACMD_FUNC(mutearea) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1297)); // Please enter a time in minutes (usage: @mutearea/@stfu <time in minutes>).
		return -1;
	}

	time = atoi(message);

	map_foreachinallarea(atcommand_mutearea_sub,
						 sd->bl.m,
						 sd->bl.x - AREA_SIZE,
						 sd->bl.y - AREA_SIZE,
						 sd->bl.x + AREA_SIZE,
...
ACMD_FUNC(rates) {
	...
	snprintf(buf,
			 CHAT_SIZE_MAX,
			 msg_txt(sd, 1298), // Experience rates: Base %.2fx / Job %.2fx
...
ACMD_FUNC(sizeguild) {
	...
	if (!message || !*message || sscanf(message, "%11d %23[^\n]", &size, guild) < 2) {
		clif_displaymessage(
			fd, msg_txt(sd, 1304)); // Please enter guild name/ID (usage: @sizeguild <size> <guild name/ID>).
		return -1;
	}
	...
	if (strcasecmp(flag_name, "available")) {
		...
		if (mapflag != MF_INVALID) {
			...
			if (flag > 0 && util::vector_exists(disabled_mf, mapflag)) {
				...
				sprintf(atcmd_output,
						"[ @mapflag ] %s flag has been set to %s value = %hd",
						flag_name,
						flag ? "On" : "Off",
						flag);
				clif_displaymessage(sd->fd, atcmd_output);
...
ACMD_FUNC(duel) {
	...
	if (!duel_checktime(sd)) {
		...
		sprintf(output,
				msg_txt(sd, 356),
				battle_config.duel_time_interval); // "Duel: You can take part in duel only one time per %d minutes."
		clif_displaymessage(fd, output);
		return 0;
	}
...
ACMD_FUNC(clone) {
	...
	clif_displaymessage(
		fd,
		msg_txt(sd, 129 + flag * 2)); // Unable to spawn evil clone. Unable to spawn clone. Unable to spawn slave clone.
	return 0;
}
...
ACMD_FUNC(itemlist) {
	...
	for (i = 0; i < size; ++i) {
		...
		StringBuf_Printf(&buf,
						 "%d %s (%s, id: %u)",
						 it->amount,
						 item_db.create_item_link(*it).c_str(),
						 itd->name.c_str(),
						 it->nameid);
		...
		if (it->card[0] == CARD0_PET) { // pet egg
			if (it->card[3] & 1) {
				StringBuf_Printf(&buf,
								 msg_txt(sd, 1348),
								 (uint32)MakeDWord(it->card[1], it->card[2])); //  -> (pet egg, pet id: %u, named)
			}
			else {
				StringBuf_Printf(&buf,
								 msg_txt(sd, 1349),
								 (uint32)MakeDWord(it->card[1], it->card[2])); //  -> (pet egg, pet id: %u, unnamed)
			}
		}
		else if (it->card[0] == CARD0_FORGE) { // forged item
			StringBuf_Printf(&buf,
							 msg_txt(sd, 1350),
							 (uint32)MakeDWord(it->card[2], it->card[3]),
							 it->card[1] >> 8,
							 it->card[1] & 0x0f); //  -> (crafted item, creator id: %u, star crumbs %d, element %d)
		}
		else if (it->card[0] == CARD0_CREATE) { // created item
			StringBuf_Printf(&buf,
							 msg_txt(sd, 1351),
							 (uint32)MakeDWord(it->card[2], it->card[3])); //  -> (produced item, creator id: %u)
		}
		else { // normal item
			int32 counter2 = 0;
...
ACMD_FUNC(delitem) {
	...
	while (amount && (idx = pc_search_inventory(sd, nameid)) != -1) {
		int32 delamount = (amount < sd->inventory.u.items_inventory[idx].amount)
							  ? amount
...
ACMD_FUNC(accinfo) {
	...
	if (!message || !*message || strlen(message) > NAME_LENGTH || (sscanf(message, "%23s %c", query, &type) < 1)) {
		...
		clif_displaymessage(fd,
							msg_txt(sd, 1366)); // You may search partial name by making use of '%' in the search, ex.
...
ACMD_FUNC(set) {
	...
	if (!message || !*message || (toset = sscanf(message, "%45s %127[^\n]s", reg, val)) < 1) {
		...
		clif_displaymessage(
			fd, msg_txt(sd, 1370)); // Usage: ex. "@set PoringCharVarSTR$" outputs its value, Super Duper String.
		return -1;
	}
...
static inline void atcmd_channel_help(map_session_data* sd, const char* command) {
	...
	if (can_create) {
		...
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(fd,
							msg_txt(sd, 1422)); // -- Changes channel text to the specified color (channel owners only).
	}
...
ACMD_FUNC(vip) {
	...
	if (pl_sd->vip.time <= now) {
		...
		sprintf(atcmd_output,
				...
				day,
				hour,
				minute); // Your VIP status is valid for %d years, %d months, %d days, %d hours and %d minutes.
		clif_displaymessage(pl_sd->fd, atcmd_output);
		...
		if (pl_sd != sd) {
			sprintf(atcmd_output,
					...
					day,
					hour,
					minute); // Player '%s' is now VIP for %d years, %d months, %d days, %d hours and %d minutes.
			clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(fullstrip) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 349)); // Please enter a player name (usage: @fullstrip/@warpto/@goto <char name/ID>).
		return -1;
	}
...
ACMD_FUNC(costume) {
	...
	const int32 name2id[] = {SC_WEDDING, SC_XMAS, SC_SUMMER, SC_DRESSUP, SC_HANBOK, SC_OKTOBERFEST};
	...
	for (k = 0; k < len; k++) {
		if (sd->sc.getSCE(name2id[k])) {
			sprintf(atcmd_output,
					msg_txt(sd, 724),
					names[k]); // You're already wearing a(n) '%s' costume, type '@costume' to remove it.
			clif_displaymessage(sd->fd, atcmd_output);
			return -1;
		}
	}
...
#define ACMD_DEF(x) \
	{ \
		#x, atcommand_##x, nullptr, nullptr, 0 \
	}
#define ACMD_DEF2(x2, x) \
	{ x2, atcommand_##x, nullptr, nullptr, 0 }
// Define with restriction
#define ACMD_DEFR(x, r) \
	{ \
		#x, atcommand_##x, nullptr, nullptr, r \
	}
#define ACMD_DEF2R(x2, x, r) \
	{ x2, atcommand_##x, nullptr, nullptr, r }
...
bool is_atcommand(const int32 fd, map_session_data* sd, const char* message, int32 type) {
	...
	if (is_atcommand) { // @command
		...
		if ((n = sscanf(message, "%255s \"%23[^\"]\" %255[^\n]", command, charname, params)) < 2 &&
			...
			sprintf(output,
					msg_txt(sd, 1388),
					charcommand_symbol); // Charcommand failed (usage: %c<command> <char name> <parameters>).
			clif_displaymessage(fd, output);
			return true;
		}
