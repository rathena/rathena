...
ACMD_FUNC(send)
{
			...
			|| sscanf(message, "%8x", &type)==1) )
	{
		int i;
		for (i = 900; i <= 903; ++i)
			clif_displaymessage(*sd, msg_txt(sd,i));
		return -1;
	}

#define PARSE_ERROR(error,p) \
	{\
		clif_displaymessage(*sd, (error));\
...
#define CHECK_EOS(p) \
	if(*(p) == 0){\
		clif_displaymessage(*sd, "Unexpected end of string");\
		return -1;\
	}
//define CHECK_EOS
...
//define GET_VALUE
	if (type > 0 && type < MAX_PACKET_DB) {
		...
		clif_displaymessage(*sd, msg_txt(sd,259)); // Invalid packet
		return -1;
	}
...
ACMD_FUNC(mapmove)
{
	...
	if (!message || !*message ||
		(sscanf(message, "%15s %6hd %6hd", map_name, &x, &y) < 3 &&
		 sscanf(message, "%15[^,],%6hd,%6hd", map_name, &x, &y) < 1)) {
			clif_displaymessage(*sd, msg_txt(sd,909)); // Please enter a map (usage: @warp/@rura/@mapmove <mapname> <x> <y>).
			return -1;
	}
	...
	if (m < 0) { // m < 0 means on different server! [Kevin]
		clif_displaymessage(*sd, msg_txt(sd,1)); // Map not found.
	...
	{	//This is to prevent the pc_setpos call from printing an error.
		clif_displaymessage(*sd, msg_txt(sd,2)); // Invalid coordinates, using random target cell.
	...
	if ((map_getmapflag(m, MF_NOWARPTO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) || !pc_job_can_entermap((enum e_job)sd->status.class_, m, pc_get_group_level(sd))) {
		clif_displaymessage(*sd, msg_txt(sd,247)); // You are not authorized to warp to this map.
		return -1;
	}
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARP) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,248)); // You are not authorized to warp from your current map.
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,0)); // Warped.
	return 0;
}
...
ACMD_FUNC(where)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,910)); // Please enter a player name (usage: @where <char name>).
		return -1;
	}
...
ACMD_FUNC(jumpto)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,911)); // Please enter a player name (usage: @jumpto/@warpto/@goto <char name/ID>).
		return -1;
	}
	...
	if( pc_isdead(sd) )
	{
		clif_displaymessage(*sd, msg_txt(sd,664)); // You cannot use this command when dead.
		return -1;
	}
...
ACMD_FUNC(who) {
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter))	{
		if (!((pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) || pc_isinvisible(pl_sd)) && pc_get_group_level(pl_sd) > level)) { // you can look only lower or same level
			...
			clif_displaymessage(*sd, StringBuf_Value(&buf));
			StringBuf_Clear(&buf);
			count++;
		}
	}
...
ACMD_FUNC(whogm)
{
	...
	if (count == 0)
		clif_displaymessage(*sd, msg_txt(sd,150)); // No GM found.
	else if (count == 1)
		clif_displaymessage(*sd, msg_txt(sd,151)); // 1 GM found.
	else {
...
ACMD_FUNC(save)
{
	...
	if( map_getmapdata(sd->bl.m)->instance_id ) {
		clif_displaymessage(*sd, msg_txt(sd,383)); // You cannot create a savepoint in an instance.
		return 1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,6)); // Your save point has been changed.
...
ACMD_FUNC(load){
	...
	if (m >= 0 && map_getmapflag(m, MF_NOWARPTO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,249));	// You are not authorized to warp to your save map.
		return -1;
	}
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARP) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,248));	// You are not authorized to warp from your current map.
	...
	clif_displaymessage(*sd, msg_txt(sd,7)); // Warping to save point..
...
ACMD_FUNC(speed)
{
	...
	if (sd->base_status.speed != DEFAULT_WALK_SPEED) {
		...
		clif_displaymessage(*sd, msg_txt(sd,8)); // Speed changed.
	} else
		clif_displaymessage(*sd, msg_txt(sd,389)); // Speed returned to normal.
...
ACMD_FUNC(storage)
{
	...
	{	//Already open.
		clif_displaymessage(*sd, msg_txt(sd,250)); // You have already opened your storage. Close it first.
		return -1;
	}

	clif_displaymessage(*sd, msg_txt(sd,919)); // Storage opened.
...
ACMD_FUNC(guildstorage)
{
	...
	switch (storage_guild_storageopen(sd)) {
		case GSTORAGE_OPEN:
			clif_displaymessage(*sd, msg_txt(sd, 920)); // Guild storage opened.
			break;
		case GSTORAGE_STORAGE_ALREADY_OPEN:
			clif_displaymessage(*sd, msg_txt(sd, 250)); // You have already opened your storage. Close it first.
			return -1;
		case GSTORAGE_ALREADY_OPEN:
			clif_displaymessage(*sd, msg_txt(sd, 251)); // You have already opened your guild storage. Close it first.
			return -1;
		case GSTORAGE_NO_GUILD:
			clif_displaymessage(*sd, msg_txt(sd, 252)); // You are not in a guild.
			return -1;
		case GSTORAGE_NO_STORAGE:
			clif_displaymessage(*sd, msg_txt(sd, 786)); // The guild does not have a guild storage.
			return -1;
		case GSTORAGE_NO_PERMISSION:
			clif_displaymessage(*sd, msg_txt(sd, 787)); // You do not have permission to use the guild storage.
			return -1;
	}
	return 0;
}
...
ACMD_FUNC(option)
{
	...
	{// failed to match the parameters so inform the user of the options
		...
		clif_displaymessage(*sd, msg_txt(sd,921)); // Please enter at least one option.

		if( text )
		{// send the help text associated with this command
			clif_displaymessage(*sd, text );
		}
	...
	clif_displaymessage(*sd, msg_txt(sd,9)); // Options changed.
...
ACMD_FUNC(hide)
{
	...
	if (pc_isinvisible(sd)) {
		...
		clif_displaymessage(*sd, msg_txt(sd,10)); // Invisible: Off
...
ACMD_FUNC(jobchange)
{
	...
	{ // Deny direct transformation into dummy jobs
		clif_displaymessage(*sd, msg_txt(sd,923)); //"You can not change to this job by command."
		return 0;
	}
	...
	if (pcdb_checkid(job))
	{
		if (pc_jobchange(sd, job, upper))
			clif_displaymessage(*sd, msg_txt(sd,12)); // Your job has been changed.
		else {
			clif_displaymessage(*sd, msg_txt(sd,155)); // You are unable to change your job.
			return -1;
		}
	} else {
		text = atcommand_help_string(command);
...
ACMD_FUNC(kill)
{
	nullpo_retr(-1, sd);
	status_kill(&sd->bl);
	clif_displaymessage(*sd, msg_txt(sd,13)); // A pity! You've died.
	if (fd != sd->fd)
		clif_displaymessage(*sd, msg_txt(sd,14)); // Character killed.
	return 0;
}
...
ACMD_FUNC(alive)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,667)); // You're not dead.
		return -1;
	}
	clif_skill_nodamage(&sd->bl,&sd->bl,ALL_RESURRECTION,4,1);
	clif_displaymessage(*sd, msg_txt(sd,16)); // You've been revived! It's a miracle!
	return 0;
}
...
ACMD_FUNC(kami)
{
	...
	if(*(command + 5) != 'c' && *(command + 5) != 'C') {
		if (!message || !*message) {
			clif_displaymessage(*sd, msg_txt(sd,980)); // Please enter a message (usage: @kami <message>).
			return -1;
		}
		...
		if(!message || !*message || (sscanf(message, "%20lx %199[^\n]", &color, atcmd_output) < 2)) {
			clif_displaymessage(*sd, msg_txt(sd,981)); // Please enter color and message (usage: @kamic <color> <message>).
			return -1;
		}

		if(color > 0xFFFFFF) {
			clif_displaymessage(*sd, msg_txt(sd,982)); // Invalid color.
			return -1;
		}
...
ACMD_FUNC(heal)
{
	...
	if ( hp == 0 && sp == 0 ) {
		if (!status_percent_heal(&sd->bl, 100, 100))
			clif_displaymessage(*sd, msg_txt(sd,157)); // HP and SP have already been recovered.
		else
			clif_displaymessage(*sd, msg_txt(sd,17)); // HP, SP recovered.
		return 0;
	}

	if ( hp > 0 && sp >= 0 ) {
		if(!status_heal(&sd->bl, hp, sp, 0))
			clif_displaymessage(*sd, msg_txt(sd,157)); // HP and SP are already with the good value.
		else
			clif_displaymessage(*sd, msg_txt(sd,17)); // HP, SP recovered.
		return 0;
	}
	...
	if ( hp < 0 && sp <= 0 ) {
		...
		clif_displaymessage(*sd, msg_txt(sd,156)); // HP or/and SP modified.
		return 0;
	}
...
ACMD_FUNC(healap)
{
	...
	if (ap == 0) {
		if (!status_percent_heal(&sd->bl, 0, 0, 100))
			clif_displaymessage(*sd, msg_txt(sd, 823));// AP have already been recovered.
		else
			clif_displaymessage(*sd, msg_txt(sd, 821));// AP recovered.
		return 0;
	}else if (ap > 0) {
		...
		clif_displaymessage(*sd, msg_txt(sd, 822));// AP modified.
		return 0;
	}
}
...
ACMD_FUNC(item)
{
	...
	if (!strcmpi(parent_cmd,"itembound")) {
		...
		{
			clif_displaymessage(*sd, msg_txt(sd,295)); // Please enter an item name or ID (usage: @item <item name/ID> <quantity> <bound_type>).
			clif_displaymessage(*sd, msg_txt(sd,298)); // Invalid bound type
			return -1;
		}
		...
		clif_displaymessage(*sd, msg_txt(sd,983)); // Please enter an item name or ID (usage: @item <item name/ID> <quantity>).
		return -1;
	}
	...
	while( itemlist != nullptr ){
		...
		if( item == nullptr ){
			clif_displaymessage(*sd, msg_txt(sd,19)); // Invalid item ID or name.
			return -1;
		}
	...
	if (flag == 0)
		clif_displaymessage(*sd, msg_txt(sd,18)); // Item created.
	return 0;
}
...
ACMD_FUNC(item2)
{
	...
	if (!strcmpi(parent_cmd+1,"itembound2")) {
		...
		{
			clif_displaymessage(*sd, msg_txt(sd,296)); // Please enter all parameters (usage: @item2 <item name/ID> <quantity>
			clif_displaymessage(*sd, msg_txt(sd,297)); //   <identify_flag> <refine> <attribute> <card1> <card2> <card3> <card4> <bound_type>).
			clif_displaymessage(*sd, msg_txt(sd,298)); // Invalid bound type
			return -1;
		}
		if( bound <= BOUND_NONE || bound >= BOUND_MAX ) {
		...
		clif_displaymessage(*sd, msg_txt(sd,984)); // Please enter all parameters (usage: @item2 <item name/ID> <quantity>
		clif_displaymessage(*sd, msg_txt(sd,985)); //   <identify_flag> <refine> <attribute> <card1> <card2> <card3> <card4>).
		return -1;
	}
...
ACMD_FUNC(itemreset)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,20)); // All of your items have been removed.
...
ACMD_FUNC(baselevelup)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(*sd, msg_txt(sd,986)); // Please enter a level adjustment (usage: @lvup/@blevel/@baselvlup <number of levels>).
		return -1;
	}

	if (level > 0) {
		if (sd->status.base_level >= pc_maxbaselv(sd)) { // check for max level by Valaris
			clif_displaymessage(*sd, msg_txt(sd,47)); // Base level can't go any higher.
			return -1;
		} // End Addition
		...
		clif_displaymessage(*sd, msg_txt(sd,21)); // Base level raised.
	} else {
		if (sd->status.base_level == 1) {
			clif_displaymessage(*sd, msg_txt(sd,158)); // Base level can't go any lower.
			return -1;
		}
		level*=-1;
		...
		if (sd->status.trait_point < trait_point)
			...
			sd->status.trait_point -= trait_point;
		sd->status.base_level -= (unsigned int)level;
		clif_displaymessage(*sd, msg_txt(sd,22)); // Base level lowered.
		status_calc_pc(sd, SCO_FORCE);
		level*=-1;
	}
...
ACMD_FUNC(joblevelup)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(*sd, msg_txt(sd,987)); // Please enter a level adjustment (usage: @joblvup/@jlevel/@joblvlup <number of levels>).
		return -1;
	}
	if (level > 0) {
		if (sd->status.job_level >= pc_maxjoblv(sd)) {
			clif_displaymessage(*sd, msg_txt(sd,23)); // Job level can't go any higher.
			return -1;
		}
		...
		clif_displaymessage(*sd, msg_txt(sd,24)); // Job level raised.
	} else {
		if (sd->status.job_level == 1) {
			clif_displaymessage(*sd, msg_txt(sd,159)); // Job level can't go any lower.
			return -1;
		}
		level *=-1;
		...
		if (sd->status.skill_point < level)
			...
			sd->status.skill_point -= level;
		clif_displaymessage(*sd, msg_txt(sd,25)); // Job level lowered.
		level *=-1;
	}
	sd->status.job_exp = 0;
...
ACMD_FUNC(pvpoff)
{
	...
	if (!map_getmapflag(sd->bl.m, MF_PVP)) {
		clif_displaymessage(*sd, msg_txt(sd,160)); // PvP is already Off.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_PVP, false);

	clif_displaymessage(*sd, msg_txt(sd,31)); // PvP: Off.
	return 0;
}
...
ACMD_FUNC(pvpon)
{
	...
	if (map_getmapflag(sd->bl.m, MF_PVP)) {
		clif_displaymessage(*sd, msg_txt(sd,161)); // PvP is already On.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_PVP, true);

	clif_displaymessage(*sd, msg_txt(sd,32)); // PvP: On.
...
ACMD_FUNC(gvgoff)
{
	...
	if (!map_getmapflag(sd->bl.m, MF_GVG)) {
		clif_displaymessage(*sd, msg_txt(sd,162)); // GvG is already Off.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_GVG, false);
	clif_displaymessage(*sd, msg_txt(sd,33)); // GvG: Off.
...
ACMD_FUNC(gvgon)
{
	...
	if (map_getmapflag(sd->bl.m, MF_GVG)) {
		clif_displaymessage(*sd, msg_txt(sd,163)); // GvG is already On.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_GVG, true);
	clif_displaymessage(*sd, msg_txt(sd,34)); // GvG: On.
...
ACMD_FUNC(model)
{
	...
	if (hair_style >= MIN_HAIR_STYLE && hair_style <= MAX_HAIR_STYLE &&
		...
		cloth_color >= MIN_CLOTH_COLOR && cloth_color <= MAX_CLOTH_COLOR) {
			...
			clif_displaymessage(*sd, msg_txt(sd,36)); // Appearance changed.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,37)); // An invalid number was specified.
		return -1;
	}
...
ACMD_FUNC(bodystyle)
{
	...
	if ( (sd->class_ & JOBL_FOURTH) || !(sd->class_ & JOBL_THIRD) || (sd->class_ & MAPID_THIRDMASK) == MAPID_SUPER_NOVICE_E || (sd->class_ & MAPID_THIRDMASK) == MAPID_STAR_EMPEROR || (sd->class_ & MAPID_THIRDMASK) == MAPID_SOUL_REAPER) {
		clif_displaymessage(*sd, msg_txt(sd,740));	// This job has no alternate body styles.
		return -1;
	}
	...
	if (body_style >= MIN_BODY_STYLE && body_style <= MAX_BODY_STYLE) {
		...
		clif_displaymessage(*sd, msg_txt(sd,36)); // Appearence changed.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,37)); // An invalid number was specified.
		return -1;
	}
...
ACMD_FUNC(go)
{
	...
	if( map_getmapflag(sd->bl.m, MF_NOGO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE) ) {
		clif_displaymessage(*sd,msg_txt(sd,995)); // You cannot use @go on this map.
		return 0;
	}
	...
	{// no value matched so send the list of locations
		...
		clif_displaymessage(*sd, msg_txt(sd,38)); // Invalid location number, or name.
...
ACMD_FUNC(monster)
{
	...
	if (!message || !*message) {
			clif_displaymessage(*sd, msg_txt(sd,80)); // Give the display name or monster name/id please.
			return -1;
	}
	if (sscanf(message, "\"%23[^\"]\" %23s %11d", name, monster, &number) > 1 ||
		...
		clif_displaymessage(*sd, msg_txt(sd,80)); // Give a display name and monster name/id please.
		return -1;
	}
	...
	if (mob_id == 0) {
		clif_displaymessage(*sd, msg_txt(sd,40)); // Invalid monster ID or name.
		return -1;
	}

	if (mob_id == MOBID_EMPERIUM) {
		clif_displaymessage(*sd, msg_txt(sd,83)); // Monster 'Emperium' cannot be spawned.
		return -1;
	}
	...
	if (count != 0)
		if (number == count)
			clif_displaymessage(*sd, msg_txt(sd,39)); // All monster summoned!
		else {
...
ACMD_FUNC(killmonster)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,165)); // All monsters killed!
...
ACMD_FUNC(refine)
{
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &refine) < 2) {
		clif_displaymessage(*sd, msg_txt(sd,996)); // Please enter a position and an amount (usage: @refine <equip position> <+/- amount>).
	...
	if (count == 0)
		clif_displaymessage(*sd, msg_txt(sd,166)); // No item has been refined.
	else if (count == 1)
		clif_displaymessage(*sd, msg_txt(sd,167)); // 1 item has been refined.
	else {
...
ACMD_FUNC(grade)
{
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &grade) < 2) {
		clif_displaymessage(*sd, msg_txt(sd,1519)); // Please enter a position and an amount (usage: @grade <equip position> <+/- amount>).
		sprintf(atcmd_output, msg_txt(sd,997), EQP_HEAD_LOW); // %d: Lower Headgear
		clif_displaymessage(*sd, atcmd_output);
		sprintf(atcmd_output, msg_txt(sd,998), EQP_HAND_R); // %d: Right Hand
		clif_displaymessage(*sd, atcmd_output);
...
ACMD_FUNC(produce)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(*sd, msg_txt(sd,1007)); // Please enter at least one item name/ID (usage: @produce <equip name/ID> <element> <# of very's>).
		return -1;
	}
	...
	if( item_data == nullptr ){
		clif_displaymessage(*sd, msg_txt(sd,170)); //This item is not an equipment.
		return -1;
	}
...
ACMD_FUNC(memo)
{
	...
	{
		int i;
		clif_displaymessage(*sd,  msg_txt(sd,668)); // Your actual memo positions are:
...
ACMD_FUNC(displaystatus)
{
	...
	if (!message || !*message || (i = sscanf(message, "%11d %11d %11d %11d %11d %11d", &type, &flag, &tick, &val1, &val2, &val3)) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1009)); // Please enter a status type/flag (usage: @displaystatus <status type> <flag> <tick> {<val1> {<val2> {<val3>}}}).
		return -1;
	}
	if (i < 2) flag = 1;
	if (i < 3) tick = 0;
...
ACMD_FUNC(statuspoint)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1010)); // Please enter a number (usage: @stpoint <number of points>).
		return -1;
	}
	...
	if (new_status_point != sd->status.status_point) {
		...
		clif_updatestatus(*sd, SP_STATUSPOINT);
		clif_displaymessage(*sd, msg_txt(sd,174)); // Number of status points changed.
	} else {
		if (point < 0)
			clif_displaymessage(*sd, msg_txt(sd,41)); // Unable to decrease the number/value.
		else
			clif_displaymessage(*sd, msg_txt(sd,149)); // Unable to increase the number/value.
		return -1;
	}
...
ACMD_FUNC(traitpoint)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(*sd, msg_txt(sd, 820)); // Please enter a number (usage: @trpoint <number of points>).
		return -1;
	}
...
ACMD_FUNC(skillpoint)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1011)); // Please enter a number (usage: @skpoint <number of points>).
		return -1;
	}
	...
	if (new_skill_point != sd->status.skill_point) {
		...
		clif_updatestatus(*sd, SP_SKILLPOINT);
		clif_displaymessage(*sd, msg_txt(sd,175)); // Number of skill points changed.
	} else {
		if (point < 0)
			clif_displaymessage(*sd, msg_txt(sd,41)); // Unable to decrease the number/value.
		else
...
ACMD_FUNC(zeny)
{
	...
	if (!message || !*message || (zeny = atoi(message)) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1012)); // Please enter an amount (usage: @zeny <amount>).
		return -1;
	}
	...
	if(!ret) clif_displaymessage(*sd, msg_txt(sd,176)); //ret=0 mean cmd success
	return 0;
}
...
ACMD_FUNC(param)
{
	...
	if (!message || !*message || sscanf(message, "%11d", &value) < 1 || value == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1013)); // Please enter a valid value (usage: @str/@agi/@vit/@int/@dex/@luk <+/-adjustment>).
		return -1;
	}
	...
	if( stat < PARAM_POW ){
		...
		if( !( sd->class_ & JOBL_FOURTH ) ){
			clif_displaymessage(*sd, msg_txt(sd, 797)); // This command is unavailable to non - 4th class.
			return -1;
		}
	...
	if( new_value != status ){
		...
		status_calc_pc(sd, SCO_FORCE);
		clif_displaymessage(*sd, msg_txt(sd,42)); // Stat changed.
...
ACMD_FUNC(stat_all)
{
	...
	if (count > 0) { // if at least 1 stat modified
		status_calc_pc(sd, SCO_FORCE);
		clif_displaymessage(*sd, msg_txt(sd,84)); // All stats changed!

		achievement_update_objective(sd, AG_GOAL_STATUS, 0);
	} else {
		if (value < 0)
			clif_displaymessage(*sd, msg_txt(sd,177)); // You cannot decrease that stat anymore.
		else
			clif_displaymessage(*sd, msg_txt(sd,178)); // You cannot increase that stat anymore.
		return -1;
	}
...
ACMD_FUNC(guildlevelup) {
	...
	if (!message || !*message || sscanf(message, "%11d", &level) < 1 || level == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1014)); // Please enter a valid level (usage: @guildlvup/@guildlvlup <# of levels>).
		return -1;
	}
	...
	if (sd->status.guild_id <= 0 || guild_info == nullptr) {
		clif_displaymessage(*sd, msg_txt(sd,43)); // You're not in a guild.
		return -1;
	}
	//if (strcmp(sd->status.name, guild_info->master) != 0) {
	//	clif_displaymessage(*sd, msg_txt(sd,44)); // You're not the master of your guild.
	//	return -1;
	//}
	...
	if (added_level != 0) {
		...
		clif_displaymessage(*sd, msg_txt(sd,179)); // Guild level changed.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,45)); // Guild level change failed.
		return -1;
	}
...
ACMD_FUNC(makeegg) {
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1015)); // Please enter a monster/egg name/ID (usage: @makeegg <pet>).
		return -1;
	}
	...
	switch(res){
		case -1:		
			clif_displaymessage(*sd, msg_txt(sd,180)); // The monster/egg name/id doesn't exist.
			break;
		case -2:
			clif_displaymessage(*sd, msg_txt(sd,407)); //charserver link broken
			break;
	}
...
ACMD_FUNC(hatch) {
	...
	if (sd->status.pet_id <= 0)
		...
		clif_displaymessage(*sd, msg_txt(sd,181)); // You already have a pet.
		return -1;
	}
...
ACMD_FUNC(petfriendly) {
	...
	if (!message || !*message || (friendly = atoi(message)) < 0) {
		clif_displaymessage(*sd, msg_txt(sd,1016)); // Please enter a valid value (usage: @petfriendly <0-1000>).
		return -1;
	}

	pd = sd->pd;
	if (!pd) {
		clif_displaymessage(*sd, msg_txt(sd,184)); // Sorry, but you have no pet.
		return -1;
	}
	...
	if (friendly == pd->pet.intimate) {
		clif_displaymessage(*sd, msg_txt(sd,183)); // Pet intimacy is already at maximum.
		return -1;
	}

	pet_set_intimate(pd, friendly);
	clif_send_petstatus(sd);
	clif_displaymessage(*sd, msg_txt(sd,182)); // Pet intimacy changed.
	return 0;
}
...
ACMD_FUNC(pethungry)
{
	...
	if (!message || !*message || (hungry = atoi(message)) < 0) {
		clif_displaymessage(*sd, msg_txt(sd,1017)); // Please enter a valid number (usage: @pethungry <0-100>).
		return -1;
	}
	...
	if (hungry == pd->pet.hungry) {
		clif_displaymessage(*sd, msg_txt(sd,186)); // Pet hunger is already at maximum.
		return -1;
	}

	pd->pet.hungry = hungry;
	clif_send_petstatus(sd);
	clif_displaymessage(*sd, msg_txt(sd,185)); // Pet hunger changed.
...
ACMD_FUNC(petrename)
{
	...
	if (!pd->pet.rename_flag) {
		clif_displaymessage(*sd, msg_txt(sd,188)); // You can already rename your pet.
		return -1;
	}
	...
	clif_send_petstatus(sd);
	clif_displaymessage(*sd, msg_txt(sd,187)); // You can now rename your pet.
...
ACMD_FUNC(recall) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1018)); // Please enter a player name (usage: @recall <char name/ID>).
		return -1;
	}
	...
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARPTO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,1019)); // You are not authorized to warp someone to this map.
		return -1;
	}
	if (pl_sd->bl.m >= 0 && map_getmapflag(pl_sd->bl.m, MF_NOWARP) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,1020)); // You are not authorized to warp this player from their map.
		return -1;
	}
	...
	if( pc_setpos(pl_sd, sd->mapindex, sd->bl.x, sd->bl.y, CLR_RESPAWN) == SETPOS_AUTOTRADE ){
		clif_displaymessage(*sd, msg_txt(sd,1025)); // The player is currently autotrading and cannot be recalled.
		return -1;
	}
...
ACMD_FUNC(char_ban)
{
	...
	if (timediff == 0) { //allow negative ?
		...
		clif_displaymessage(*sd, output);
		clif_displaymessage(*sd, msg_txt(sd,702)); // Time parameter format is +/-<value> to alter. y/a = Year, m = Month, d/j = Day, h = Hour, n/mn = Minute, s = Second.
		return -1;
	}
	...
	{
		clif_displaymessage(*sd,msg_txt(sd,1023)); // You are not allowed to alter the time of a ban.
		return -1;
	}
...
ACMD_FUNC(night)
{
	...
	if (night_flag != 1) {
		...
		clif_displaymessage(*sd, msg_txt(sd,89)); // Night mode is already enabled.
		return -1;
	}
...
ACMD_FUNC(day)
{
	...
	if (night_flag != 0) {
		...
		clif_displaymessage(*sd, msg_txt(sd,90)); // Day mode is already enabled.
		return -1;
	}
...
ACMD_FUNC(doom)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if (pl_sd->fd != fd && pc_get_group_level(sd) >= pc_get_group_level(pl_sd))
		{
			...
			clif_displaymessage(*pl_sd, msg_txt(sd,61)); // The holy messenger has given judgement.
		}
	}
	mapit_free(iter);

	clif_displaymessage(*sd, msg_txt(sd,62)); // Judgement was made.
...
static void atcommand_raise_sub(map_session_data* sd) {
	...
	clif_skill_nodamage(&sd->bl,&sd->bl,ALL_RESURRECTION,4,1);
	clif_displaymessage(*sd, msg_txt(sd,63)); // Mercy has been shown.
}
...
ACMD_FUNC(raise)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,64)); // Mercy has been granted.
...
ACMD_FUNC(kick)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1026)); // Please enter a player name (usage: @kick <char name/ID>).
		return -1;
	}
	...
	if ( pc_get_group_level(sd) < pc_get_group_level(pl_sd) )
	{
		clif_displaymessage(*sd, msg_txt(sd,81)); // Your GM level don't authorise you to do this action on this player.
		return -1;
	}
...
ACMD_FUNC(kickall)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,195)); // All players have been kicked!
...
ACMD_FUNC(allskill)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,76)); // All skills have been added to your skill tree.
...
ACMD_FUNC(questskill)
{
	...
	{// also send a list of skills applicable to this command
		...
		clif_displaymessage(*sd, msg_txt(sd,1027)); // Please enter a quest skill number.
	...
	if (skill_id >= MAX_SKILL_ID) {
		clif_displaymessage(*sd, msg_txt(sd,198)); // This skill number doesn't exist.
		return -1;
	}
	if (!skill_get_inf2(skill_id, INF2_ISQUEST)) {
		clif_displaymessage(*sd, msg_txt(sd,197)); // This skill number doesn't exist or isn't a quest skill.
		return -1;
	}
	if (pc_checkskill(sd, skill_id) > 0) {
		clif_displaymessage(*sd, msg_txt(sd,196)); // You already have this quest skill.
		return -1;
	}

	pc_skill(sd, skill_id, 1, ADDSKILL_PERMANENT);
	clif_displaymessage(*sd, msg_txt(sd,70)); // You have learned the skill.
...
ACMD_FUNC(lostskill)
{
	...
	if (pc_checkskill(sd, skill_id) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,201)); // You don't have this quest skill.
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,71)); // You have forgotten the skill.
...
ACMD_FUNC(party)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1029)); // Please enter a party name (usage: @party <party_name>).
		return -1;
	}
...
ACMD_FUNC(guild)
{
	...
	if (sd->clan) {
		clif_displaymessage(*sd, msg_txt(sd, 1498)); // You cannot create a guild because you are in a clan.
		return -1;
	}

	if (!message || !*message || sscanf(message, "%23[^\n]", guild) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1030)); // Please enter a guild name (usage: @guild <guild_name>).
		return -1;
	}
	...
	if (sd->status.guild_id) { // Check if the player has a guild
		if (sd->guild) { // Check if guild was found
			if (sd->state.gmaster_flag) { // Check if player is guild master
				...
				clif_displaymessage(*sd, msg_txt(sd,1181)); // You need to be a Guild Master to use this command.
				return -1;
			}
...
ACMD_FUNC(agitstart)
{
	...
	if( guild_agit_start() ){
		clif_displaymessage(*sd, msg_txt(sd,72)); // War of Emperium has been initiated.
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,73)); // War of Emperium is currently in progress.
		return -1;
	}
}
...
ACMD_FUNC(agitstart2)
{
	...
	if( guild_agit2_start() ){
		clif_displaymessage(*sd, msg_txt(sd,403)); // "War of Emperium SE has been initiated."
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,404)); // "War of Emperium SE is currently in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitstart3)
{
	...
	if( guild_agit3_start() ){
		clif_displaymessage(*sd, msg_txt(sd,749)); // "War of Emperium TE has been initiated."
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,750)); // "War of Emperium TE is currently in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitend)
{
	...
	if( guild_agit_end() ){
		clif_displaymessage(*sd, msg_txt(sd,74)); // War of Emperium has been ended.
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,75)); // War of Emperium is currently not in progress.
		return -1;
	}
}
...
ACMD_FUNC(agitend2)
{
	...
	if( guild_agit2_end() ){
		clif_displaymessage(*sd, msg_txt(sd,405)); // "War of Emperium SE has been ended."
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,406)); // "War of Emperium SE is currently not in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitend3)
{
	...
	if( guild_agit3_end() ){
		clif_displaymessage(*sd, msg_txt(sd,751));// War of Emperium TE has been ended.
		return 0;
	}else{
		clif_displaymessage(*sd, msg_txt(sd,752));// War of Emperium TE is currently not in progress.
		return -1;
	}
}
...
ACMD_FUNC(idsearch)
{
	...
	if (!message || !*message || sscanf(message, "%99s", item_name) < 0) {
		clif_displaymessage(*sd, msg_txt(sd,1031)); // Please enter part of an item name (usage: @idsearch <part_of_item_name>).
		return -1;
	}
...
ACMD_FUNC(recallall)
{
	...
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARPTO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(*sd, msg_txt(sd,1032)); // You are not authorized to warp someone to your current map.
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,92)); // All characters recalled!
	if (count) {
...
ACMD_FUNC(guildrecall)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1034)); // Please enter a guild name/ID (usage: @guildrecall <guild_name/ID>).
		return -1;
	}
	...
	if (!g) {
		clif_displaymessage(*sd, msg_txt(sd,94)); // Incorrect name/ID, or no one from the guild is online.
		return -1;
	}
...
ACMD_FUNC(partyrecall)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1035)); // Please enter a party name/ID (usage: @partyrecall <party_name/ID>).
		return -1;
	}
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,96)); // Incorrect name or ID, or no one from the party is online.
		return -1;
	}
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		itemdb_reload();
		clif_displaymessage(*sd, msg_txt(sd,97)); // Item database has been reloaded.
	} else if (strstr(command, "cashdb") || strncmp(message, "cashdb", 4) == 0) {
		cashshop_reloaddb();
		clif_displaymessage(*sd, msg_txt( sd, 832 ) ); // Cash shop database has been reloaded.
		...
		mercenary_db.reload();
		elemental_db.reload();
		clif_displaymessage(*sd, msg_txt(sd,98)); // Monster database has been reloaded.
	} else if (strstr(command, "skilldb") || strncmp(message, "skilldb", 4) == 0) {
		skill_reload();
		homunculus_db.reload();
		clif_displaymessage(*sd, msg_txt(sd,99)); // Skill database has been reloaded.
	} else if (strstr(command, "atcommand") || strncmp(message, "atcommand", 4) == 0) {
		atcommand_doload();
		pc_groups_reload();
		clif_displaymessage(*sd, msg_txt(sd,254)); // GM command configuration has been reloaded.
		...
		clif_displaymessage(*sd, msg_txt(sd,255)); // Battle configuration has been reloaded.
	} else if (strstr(command, "statusdb") || strncmp(message, "statusdb", 3) == 0) {
		status_readdb( true );
		clif_displaymessage(*sd, msg_txt(sd,256)); // Status database has been reloaded.
	} else if (strstr(command, "pcdb") || strncmp(message, "pcdb", 2) == 0) {
		pc_readdb();
		clif_displaymessage(*sd, msg_txt(sd,257)); // Player database has been reloaded.
	} else if (strstr(command, "motd") || strncmp(message, "motd", 4) == 0) {
		pc_read_motd();
		clif_displaymessage(*sd, msg_txt(sd,268)); // Reloaded the Message of the Day.
		...
		clif_displaymessage(*sd, msg_txt(sd,100)); // Scripts have been reloaded.
	} else if (strstr(command, "msgconf") || strncmp(message, "msgconf", 3) == 0) {
		map_msg_reload();
		clif_displaymessage(*sd, msg_txt(sd,463)); // Message configuration has been reloaded.
	} else if (strstr(command, "questdb") || strncmp(message, "questdb", 3) == 0) {
		if (quest_db.reload())
			clif_displaymessage(*sd, msg_txt(sd,1377)); // Quest database has been reloaded.
	} else if (strstr(command, "instancedb") || strncmp(message, "instancedb", 4) == 0) {
		if (instance_db.reload())
			clif_displaymessage(*sd, msg_txt(sd,516)); // Instance database has been reloaded.
	} else if (strstr(command, "achievementdb") || strncmp(message, "achievementdb", 4) == 0) {
		achievement_db_reload();
		clif_displaymessage(*sd, msg_txt(sd,771)); // Achievement database has been reloaded.
	} else if (strstr(command, "attendancedb") || strncmp(message, "attendancedb", 4) == 0) {
		attendance_db.reload();
		clif_displaymessage(*sd, msg_txt(sd, 795)); // Attendance database has been reloaded.
	}else if( strstr( command, "barterdb" ) || strncmp( message, "barterdb", 4 ) == 0 ){
		barter_db.reload();
		clif_displaymessage(*sd, msg_txt(sd, 830)); // Barter database has been reloaded.
	} else if (strstr(command, "logconf") || strncmp(message, "logconf", 3) == 0) {
		log_config_read(LOG_CONF_NAME);
    clif_displaymessage(*sd, msg_txt(sd, 1536)); // Log configuration has been reloaded.
	}
...
ACMD_FUNC(partysharelvl) {
	...
	if(!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1322)); // Please enter an amount.
		return -1;
	} else {
	...
	if(intif_party_sharelvlupdate(share_lvl)) //Successfully updated
		clif_displaymessage(*sd, msg_txt(sd,1478)); // Party share level range has been changed successfully.
	else //Char server offline
		clif_displaymessage(*sd, msg_txt(sd,1479)); // Failed to update configuration. Character server is offline.
...
ACMD_FUNC(mapinfo) {
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(*sd, msg_txt(sd,1038)); // Please enter at least one valid list number (usage: @mapinfo <0-3> <map>).
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,1039)); // ------ Map Info ------
	...
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1041)); // ------ Map Flags ------
	if (map_getmapflag(m_id, MF_TOWN))
		clif_displaymessage(*sd, msg_txt(sd,1042)); // Town Map
	...
	if (battle_config.autotrade_mapflag == map_getmapflag(m_id, MF_AUTOTRADE))
		clif_displaymessage(*sd, msg_txt(sd,1043)); // Autotrade Enabled
	else
		clif_displaymessage(*sd, msg_txt(sd,1044)); // Autotrade Disabled
	...
	if (mapdata->getMapFlag(MF_SKILL_DAMAGE)) {
		clif_displaymessage(*sd,msg_txt(sd,1052)); // Skill Damage Adjustments:
		...
		if (!mapdata->skill_damage.empty()) {
			clif_displaymessage(*sd, msg_txt(sd, 1054)); // > [Map Skill] Name : Player, Monster, Boss Monster, Other | Caster
	...
	if (map_getmapflag(m_id, MF_SKILL_DURATION)) {
		clif_displaymessage(*sd, msg_txt(sd, 1055)); // Skill Duration Adjustments:
	...
	if (map_getmapflag(m_id, MF_NOSAVE)) {
		if (!mapdata->save.map)
			clif_displaymessage(*sd, msg_txt(sd,1068)); // No Save (Return to last Save Point)
	...
	case 0:
		...
		break;
	case 1:
		clif_displaymessage(*sd, msg_txt(sd,480)); // ----- Players in Map -----
		iter = mapit_getallusers();
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			if (pl_sd->mapindex == m_index) {
		...
		break;
	case 2:
		clif_displaymessage(*sd, msg_txt(sd,482)); // ----- NPCs in Map -----
		...
		break;
	case 3:
		clif_displaymessage(*sd, msg_txt(sd,483)); // ----- Chats in Map -----
		iter = mapit_getallusers();
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
	...
	default: // normally impossible to arrive here
		clif_displaymessage(*sd, msg_txt(sd,488)); // Please enter at least one valid list number (usage: @mapinfo <0-3> <map>).
		return -1;
		break;
	}
...
ACMD_FUNC(mount_peco)
{
	...
	if (sd->disguise) {
		clif_displaymessage(*sd, msg_txt(sd,212)); // Cannot mount while in disguise.
		return -1;
	}
	...
	if( (sd->class_&MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT && pc_checkskill(sd,RK_DRAGONTRAINING) > 0 ) {
		if( !(sd->sc.option&OPTION_DRAGON) ) {
			...
			clif_displaymessage(*sd,msg_txt(sd,1119)); // You have mounted your Dragon.
			pc_setoption(sd, sd->sc.option|option);
		} else {
			clif_displaymessage(*sd,msg_txt(sd,1120)); // You have released your Dragon.
	...
	if( (sd->class_&MAPID_THIRDMASK) == MAPID_RANGER && pc_checkskill(sd,RA_WUGRIDER) > 0 && (!pc_isfalcon(sd) || (pc_checkskill(sd, WH_HAWK_M) || battle_config.warg_can_falcon)) ) {
		if( !pc_isridingwug(sd) ) {
			clif_displaymessage(*sd,msg_txt(sd,1121)); // You have mounted your Warg.
			pc_setoption(sd, sd->sc.option|OPTION_WUGRIDER);
		} else {
			clif_displaymessage(*sd,msg_txt(sd,1122)); // You have released your Warg.
			pc_setoption(sd, sd->sc.option&~OPTION_WUGRIDER);
		}
		return 0;
	}
	if( (sd->class_&MAPID_THIRDMASK) == MAPID_MECHANIC ) {
		if( !pc_ismadogear(sd) ) {
			...
			clif_displaymessage(*sd,msg_txt(sd,1123)); // You have mounted your Mado Gear.
			pc_setmadogear(sd, true, type);
		} else {
			clif_displaymessage(*sd,msg_txt(sd,1124)); // You have released your Mado Gear.
	...
	if (!pc_isriding(sd)) { // if actually no peco
		...
		if (!pc_checkskill(sd, KN_RIDING)) {
			clif_displaymessage(*sd, msg_txt(sd,213)); // You can not mount a Peco Peco with your current job.
			return -1;
		}

		pc_setoption(sd, sd->sc.option | OPTION_RIDING);
		clif_displaymessage(*sd, msg_txt(sd,102)); // You have mounted a Peco Peco.
	} else {//Dismount
		pc_setoption(sd, sd->sc.option & ~OPTION_RIDING);
		clif_displaymessage(*sd, msg_txt(sd,214)); // You have released your Peco Peco.
	}
...
ACMD_FUNC(guildspy)
{
	...
	if (!enable_spy)
	{
		clif_displaymessage(*sd, msg_txt(sd,1125)); // The mapserver has spy command support disabled.
		return -1;
	}
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1126)); // Please enter a guild name/ID (usage: @guildspy <guild_name/ID>).
		return -1;
	}

	auto g = guild_searchnameid(guild_name);
	if (!g) {
		clif_displaymessage(*sd, msg_txt(sd,94)); // Incorrect name/ID, or no one from the specified guild is online.
		return -1;
	}
...
ACMD_FUNC(partyspy)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1127)); // Please enter a party name/ID (usage: @partyspy <party_name/ID>).
		return -1;
	}
	...
	if ((p = party_searchname(party_name)) != nullptr || // name first to avoid error when name begin with a number
		...
		clif_displaymessage(*sd, msg_txt(sd,96)); // Incorrect name/ID, or no one from the specified party is online.
		return -1;
	}
...
ACMD_FUNC(clanspy){
	...
	if( !message || !*message || sscanf( message, "%23[^\n]", clan_name ) < 1 ){
		clif_displaymessage(*sd, msg_txt(sd, 1499)); // Please enter a clan name/ID (usage: @clanspy <clan_name/ID>).
		return -1;
	}
	...
	if ((c = clan_searchname(clan_name)) != nullptr || // name first to avoid error when name begin with a number
		...
		clif_displaymessage(*sd, msg_txt(sd, 1502)); // Incorrect clan name/ID.
		return -1;
	}
...
ACMD_FUNC(repairall)
{
	...
	if (count > 0) {
		clif_misceffect( sd->bl, NOTIFYEFFECT_REFINE_SUCCESS );
		clif_equiplist(sd);
		clif_displaymessage(*sd, msg_txt(sd,107)); // All items have been repaired.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,108)); // No item need to be repaired.
		return -1;
	}
...
ACMD_FUNC(nuke)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1128)); // Please enter a player name (usage: @nuke <char name>).
		return -1;
	}
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name,false)) != nullptr) {
		if (pc_get_group_level(sd) >= pc_get_group_level(pl_sd)) { // you can kill only lower or same GM level
			...
			clif_displaymessage(*sd, msg_txt(sd,109)); // Player has been nuked!
		} else {
			clif_displaymessage(*sd, msg_txt(sd,81)); // Your GM level don't authorise you to do this action on this player.
			return -1;
		}
...
ACMD_FUNC(tonpc)
{
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", npcname) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1129)); // Please enter a NPC name (usage: @tonpc <NPC_name>).
		return -1;
	}
	...
	if ((nd = npc_name2id(npcname)) != nullptr) {
		...
		clif_displaymessage(*sd, msg_txt(sd,111)); // This NPC doesn't exist.
		return -1;
	}
...
ACMD_FUNC(shownpc)
{
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1130)); // Please enter a NPC name (usage: @enablenpc <NPC_name>).
		return -1;
	}
	...
	if (nd) {
		...
		clif_displaymessage(*sd, msg_txt(sd,110)); // Npc Enabled.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,111)); // This NPC doesn't exist.
		return -1;
	}
...
ACMD_FUNC(hidenpc)
{
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1131)); // Please enter a NPC name (usage: @hidenpc <NPC_name>).
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,112)); // Npc Disabled.
	return 0;
}
...
{
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1132)); // Please enter a script file name (usage: @loadnpc <file name>).
		return -1;
	}
	
	if (!npc_addsrcfile(message, true)) {
		clif_displaymessage(*sd, msg_txt(sd,261)); // Script could not be loaded.
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,262)); // Script loaded.
	return 0;
}
...
ACMD_FUNC(unloadnpc)
{
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1133)); // Please enter a NPC name (usage: @unloadnpc <NPC_name>).
		return -1;
	}
...
ACMD_FUNC(reloadnpcfile) {
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,733)); // Please enter a NPC file name (usage: @reloadnpcfile <file name>).
		return -1;
	}

	if (npc_unloadfile(message))
		clif_displaymessage(*sd, msg_txt(sd,1386)); // File unloaded. Be aware that mapflags and monsters spawned directly are not removed.
...
ACMD_FUNC(servertime)
{
	...
	if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
		if (night_flag == 0)
			clif_displaymessage(*sd, msg_txt(sd,231)); // Game time: The game is in permanent daylight.
		else
			clif_displaymessage(*sd, msg_txt(sd,232)); // Game time: The game is in permanent night.
		...
		if (night_flag == 1) { // we start with night
			if ((timer_data = get_timer(day_timer_tid)) != nullptr) {
				...
				clif_displaymessage(*sd, temp);
				clif_displaymessage(*sd, msg_txt(sd,234)); // Game time: After, the game will be in permanent daylight.
			} else
				clif_displaymessage(*sd, msg_txt(sd,232)); // Game time: The game is in permanent night.
		} else
			clif_displaymessage(*sd, msg_txt(sd,231)); // Game time: The game is in permanent daylight.
		...
		if (night_flag == 0) { // we start with day
			if ((timer_data = get_timer(night_timer_tid)) != nullptr) {
				...
				clif_displaymessage(*sd, temp);
				clif_displaymessage(*sd, msg_txt(sd,236)); // Game time: After, the game will be in permanent night.
			} else
				clif_displaymessage(*sd, msg_txt(sd,231)); // Game time: The game is in permanent daylight.
		} else
			clif_displaymessage(*sd, msg_txt(sd,232)); // Game time: The game is in permanent night.
...
ACMD_FUNC(jail)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1134)); // Please enter a player name (usage: @jail <char_name>).
		return -1;
	}
	...
	if (pl_sd->sc.getSCE(SC_JAILED)) {
		clif_displaymessage(*sd, msg_txt(sd,118)); // Player warped in jails.
		return -1;
	}

	pc_jail(*pl_sd);
	clif_displaymessage(*pl_sd, msg_txt(sd,117)); // GM has send you in jails.
	clif_displaymessage(*sd, msg_txt(sd,118)); // Player warped in jails.
	return 0;
}
...
ACMD_FUNC(unjail)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1135)); // Please enter a player name (usage: @unjail/@discharge <char_name>).
		return -1;
	}
	...
	if (!pl_sd->sc.getSCE(SC_JAILED)) {
		clif_displaymessage(*sd, msg_txt(sd,119)); // This player is not in jails.
		return -1;
	}

	//Reset jail time to 1 sec.
	pc_jail(*pl_sd, 0);
	clif_displaymessage(*pl_sd, msg_txt(sd,120)); // A GM has discharged you from jail.
	clif_displaymessage(*sd, msg_txt(sd,121)); // Player unjailed.
	return 0;
}
...
ACMD_FUNC(jailfor) {
	...
	if (!message || !*message || sscanf(message, "%255s %23[^\n]",atcmd_output,atcmd_player_name) < 2) {
		clif_displaymessage(*sd, msg_txt(sd,400));	//Usage: @jailfor <time> <character name>
		return -1;
	}
	...
	if (jailtime == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1136)); // Invalid time for jail command.
		clif_displaymessage(*sd, msg_txt(sd,702)); // Time parameter format is +/-<value> to alter. y/a = Year, m = Month, d/j = Day, h = Hour, n/mn = Minute, s = Second.
		return -1;
	}
	...
	if(pl_sd->sc.getSCE(SC_JAILED) && pl_sd->sc.getSCE(SC_JAILED)->val1 != INT_MAX) { // Update the player's jail time
		...
		if (jailtime <= 0) {
			jailtime = 0;
			clif_displaymessage(*pl_sd, msg_txt(sd,120)); // GM has discharge you.
			clif_displaymessage(*sd, msg_txt(sd,121)); // Player unjailed
		} else {
...
ACMD_FUNC(jailtime){
	...
	if (!sd->sc.getSCE(SC_JAILED)) {
		clif_displaymessage(*sd, msg_txt(sd,1139)); // You are not in jail.
		return -1;
	}

	if (sd->sc.getSCE(SC_JAILED)->val1 == INT_MAX) {
		clif_displaymessage(*sd, msg_txt(sd,1140)); // You have been jailed indefinitely.
		return 0;
	}

	if (sd->sc.getSCE(SC_JAILED)->val1 <= 0) { // Was not jailed with @jailfor (maybe @jail? or warped there? or got recalled?)
		clif_displaymessage(*sd, msg_txt(sd,1141)); // You have been jailed for an unknown amount of time.
		return -1;
	}
...
ACMD_FUNC(disguise)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1143)); // Please enter a Monster/NPC name/ID (usage: @disguise <name/ID>).
		return -1;
	}
	...
	if (id == 0)
	{
		clif_displaymessage(*sd, msg_txt(sd,123));	// Invalid Monster/NPC name/ID specified.
		return -1;
	}

	if(pc_isriding(sd))
	{
		clif_displaymessage(*sd, msg_txt(sd,1144)); // Character cannot be disguised while mounted.
		return -1;
	}

	if (sd->sc.getSCE(SC_MONSTER_TRANSFORM) || sd->sc.getSCE(SC_ACTIVE_MONSTER_TRANSFORM)) {
		clif_displaymessage(*sd, msg_txt(sd,730)); // Character cannot be disguised while in monster transform.
		return -1;
	}

	pc_disguise(sd, id);
	clif_displaymessage(*sd, msg_txt(sd,122)); // Disguise applied.
...
ACMD_FUNC(disguiseall)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1145)); // Please enter a Monster/NPC name/ID (usage: @disguiseall <name/ID>).
		return -1;
	}
	...
	if (!mobdb_checkid(mob_id) && !npcdb_checkid(mob_id)) { //if mob or npc...
		clif_displaymessage(*sd, msg_txt(sd,123)); // Monster/NPC name/id not found.
		return -1;
	}
...
ACMD_FUNC(disguiseguild)
{
	...
	if( !message || !*message || sscanf(message, "%23[^,], %23[^\r\n]", monster, guild) < 2 ) {
		clif_displaymessage(*sd, msg_txt(sd,1146)); // Please enter a mob name/ID and guild name/ID (usage: @disguiseguild <mob name/ID>, <guild name/ID>).
		return -1;
	}
	...
	if( id == 0 ) {
		clif_displaymessage(*sd, msg_txt(sd,123));	// Monster/NPC name/id hasn't been found.
		return -1;
	}
...
ACMD_FUNC(undisguise)
{
	...
	if (sd->disguise) {
		pc_disguise(sd, 0);
		clif_displaymessage(*sd, msg_txt(sd,124)); // Undisguise applied.
	} else {
		clif_displaymessage(*sd, msg_txt(sd,125)); // You're not disguised.
		return -1;
	}
...
ACMD_FUNC(undisguiseguild)
{
	...
	if(!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1147)); // Please enter guild name/ID (usage: @undisguiseguild <guild name/ID>).
		return -1;
	}
...
ACMD_FUNC(broadcast)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1149)); // Please enter a message (usage: @broadcast <message>).
		return -1;
	}
...
ACMD_FUNC(localbroadcast)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1150)); // Please enter a message (usage: @localbroadcast <message>).
		return -1;
	}
...
ACMD_FUNC(email)
{
	...
	if (!message || !*message || sscanf(message, "%99s %99s", actual_email, new_email) < 2) {
		clif_displaymessage(*sd, msg_txt(sd,1151)); // Please enter 2 emails (usage: @email <actual@email> <new@email>).
		return -1;
	}

	if (e_mail_check(actual_email) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,144)); // Invalid actual email. If you have default e-mail, give a@a.com.
		return -1;
	} else if (e_mail_check(new_email) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,145)); // Invalid new email. Please enter a real e-mail.
		return -1;
	} else if (strcmpi(new_email, "a@a.com") == 0) {
		clif_displaymessage(*sd, msg_txt(sd,146)); // New email must be a real e-mail.
		return -1;
	} else if (strcmpi(actual_email, new_email) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,147)); // New email must be different of the actual e-mail.
		return -1;
	}

	chrif_changeemail(sd->status.account_id, actual_email, new_email);
	clif_displaymessage(*sd, msg_txt(sd,148)); // Information sent to login-server via char-server.
	return 0;
}
...
ACMD_FUNC(effect)
{
	...
	if (!message || !*message || sscanf(message, "%11d", &type) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1152)); // Please enter an effect number (usage: @effect <effect number>).
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,229)); // Your effect has changed.
	return 0;
}
...
ACMD_FUNC(killer)
{
	...
	if(sd->state.killer)
		clif_displaymessage(*sd, msg_txt(sd,241)); // You can now attack and kill players freely.
	else {
		clif_displaymessage(*sd, msg_txt(sd,292)); // Killer state reset.
		pc_stop_attack(sd);
	}
	return 0;
}
...
ACMD_FUNC(killable)
{
	...
	if(sd->state.killable)
		clif_displaymessage(*sd, msg_txt(sd,242)); // You can now be attacked and killed by players.
	else {
		clif_displaymessage(*sd, msg_txt(sd,288)); // You are no longer killable.
...
ACMD_FUNC(skillon)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,244)); // Skills have been enabled on this map.
	return 0;
}
...
ACMD_FUNC(skilloff)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,243)); // Skills have been disabled on this map.
	return 0;
}
...
ACMD_FUNC(npcmove)
{
	...
	if (!message || !*message || sscanf(message, "%6hd %6hd %49[^\n]", &x, &y, npc_name) < 3) {
		clif_displaymessage(*sd, msg_txt(sd,1153)); // Usage: @npcmove <X> <Y> <npc_name>
		return -1;
	}
	...
	{ //actually failed to move
		clif_displaymessage(*sd, msg_txt(sd,1154)); // NPC is not on this map.
		return -1;	//Not on a map.
	} else
		clif_displaymessage(*sd, msg_txt(sd,1155)); // NPC moved
...
ACMD_FUNC(addwarp)
{
	...
	if (!message || !*message || sscanf(message, "%15s %6hd %6hd %49[^\n]", mapname, &x, &y, warpname) < 4) {
		clif_displaymessage(*sd, msg_txt(sd,1156)); // Usage: @addwarp <mapname> <X> <Y> <npc name>
		return -1;
	}
...
ACMD_FUNC(follow)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1159)); // Follow mode OFF.
		return 0;
	}
	...
	if (sd->followtarget == pl_sd->bl.id) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1160)); // Follow mode ON.
	}
...
ACMD_FUNC(dropall)
{
	...
	if( message[0] ) {
		...
		{
			clif_displaymessage(*sd, msg_txt(sd,1492)); // Usage: @dropall {<type>}
			clif_displaymessage(*sd, msg_txt(sd,1493)); // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3, armor = 4, weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
			return -1;
		}
	}
...
ACMD_FUNC(stockall)
{
	...
	if (!pc_iscarton(sd)) {
		clif_displaymessage(*sd, msg_txt(sd,1533)); // You do not have a cart.
		return -1;
	}
	...
	if ( message[0] ) {
		...
		switch (type) {
			...
			default:
				clif_displaymessage(*sd, msg_txt(sd, 1534)); // Usage: @stockall {<type>}
				clif_displaymessage(*sd, msg_txt(sd, 1493)); // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3, armor = 4, weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
				return -1;
		}
	}
...
ACMD_FUNC(storeall)
{
	...
	{	//Open storage.
		if( storage_storageopen(sd) == 1 ) {
			clif_displaymessage(*sd, msg_txt(sd,1161)); // You currently cannot open your storage.
			return -1;
		}
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,1162)); // All items stored.
	return 0;
}
...
ACMD_FUNC(clearstorage)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1394)); // Your storage was cleaned.
	return 0;
}
...
ACMD_FUNC(cleargstorage)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1395)); // Your guild storage was cleaned.
	return 0;
}
...
ACMD_FUNC(clearcart)
{
	...
	if (pc_iscarton(sd) == 0) {
		clif_displaymessage(*sd, msg_txt(sd,1396)); // You do not have a cart to be cleaned.
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,1397)); // Your cart was cleaned.
	return 0;
}
...
ACMD_FUNC(skillid) {
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1163)); // Please enter a skill name to look up (usage: @skillid <skill name>).
		return -1;
	}
	...
	for(i = 0; i < found; i++) { /* partials */
		clif_displaymessage(*sd, partials[i]);
	}
...
ACMD_FUNC(useskill)
{
	...
	if(!message || !*message || sscanf(message, "%6hu %6hu %23[^\n]", &skill_id, &skill_lv, atcmd_player_name) != 3) {
		clif_displaymessage(*sd, msg_txt(sd,1165)); // Usage: @useskill <skill ID> <skill level> <char name>
		return -1;
	}
...
ACMD_FUNC(displayskill)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1166));// Usage: @displayskill <skill ID> {<skill level> <type>}
		clif_displaymessage(*sd, msg_txt(sd,825));// Effect Types: 0: All, 1: Damage, 2: Splash Dmg, 3: No Damage, 4: Ground
		return -1;
	}
...
ACMD_FUNC(displayskillcast)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd, 824));// Usage: @displayskillcast <skill ID> {<skill level> <ground target flag> <cast time>}
		return -1;
	}
...
ACMD_FUNC(displayskillunit)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd, 826));// Usage: @displayskillunit <unit ID> {<skill level> <range>}
		return -1;
	}
...
ACMD_FUNC(skilltree)
{
	...
	if(!message || !*message || sscanf(message, "%6hu %23[^\n]", &skill_id, atcmd_player_name) != 2) {
		clif_displaymessage(*sd, msg_txt(sd,1167)); // Usage: @skilltree <skill ID> <char name>
		return -1;
	}
	...
	if (entry == nullptr) {
		clif_displaymessage(*sd, msg_txt(sd,1169)); // The player cannot use that skill.
		return 0;
	}
	...
	if (meets) {
		clif_displaymessage(*sd, msg_txt(sd,1171)); // The player meets all the requirements for that skill.
	}
...
ACMD_FUNC(marry)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1172)); // Usage: @marry <char name>
		return -1;
	}
	...
	if (pc_marriage(sd, pl_sd)) {
		clif_displaymessage(*sd, msg_txt(sd,1173)); // They are married... wish them well.
	...
	clif_displaymessage(*sd, msg_txt(sd,1174)); // The two cannot wed because one is either a baby or already married.
	return -1;
}
...
ACMD_FUNC(changelook)
{
	...
	if((i = sscanf(message, "%11d %11d", &j, &k)) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1177)); // Usage: @changelook {<position>} <view id>
		clif_displaymessage(*sd, msg_txt(sd,1178)); // Position: 1-Top 2-Middle 3-Bottom 4-Weapon 5-Shield 6-Shoes 7-Robe 8-Body
		return -1;
	} else if ( i == 2 ) {
		if (j < 1 || j > 8)
			j = 1;
...
ACMD_FUNC(autotrade) {
	...
	if( map_getmapflag(sd->bl.m, MF_AUTOTRADE) != battle_config.autotrade_mapflag ) {
		clif_displaymessage(*sd, msg_txt(sd,1179)); // Autotrade is not allowed on this map.
		return -1;
	}

	if( pc_isdead(sd) ) {
		clif_displaymessage(*sd, msg_txt(sd,1180)); // You cannot autotrade when dead.
		return -1;
	}

	if( !sd->state.vending && !sd->state.buyingstore ) { //check if player is vending or buying
		clif_displaymessage(*sd, msg_txt(sd,549)); // "You should have a shop open to use @autotrade."
		return -1;
	}
...
ACMD_FUNC(changegm)
{
	...
	if( map_getmapflag(sd->bl.m, MF_GUILDLOCK) || map_getmapflag(sd->bl.m, MF_GVG_CASTLE) ) {
		clif_displaymessage(*sd, msg_txt(sd,1182)); // You cannot change guild leaders on this map.
		return -1;
	}

	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1183)); // Usage: @changegm <guild_member_name>
		return -1;
	}

	if((pl_sd=map_nick2sd(atcmd_player_name,false)) == nullptr || pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(*sd, msg_txt(sd,1184)); // Target character must be online and be a guild member.
		return -1;
	}
...
#else
		clif_displaymessage(*sd, msg_txt(sd,1513)); // Currently in WoE hours, unable to delegate Guild leader
#endif
		return -1;
	}
...
#else
		clif_displaymessage(*sd, msg_txt(sd,1514)); // You have to wait for a while before delegating a new Guild leader
#endif
		return -1;
	}
...
ACMD_FUNC(changeleader)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1185)); // Usage: @changeleader <party_member_name>
		return -1;
	}
...
ACMD_FUNC(partyoption)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,282)); // You need to be a party leader to use this command.
		return -1;
	}
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1186)); // Usage: @partyoption <pickup share: yes/no> <item distribution: yes/no>
		return -1;
	}
	...
	if (option != p->party.item)
		...
		clif_displaymessage(*sd, msg_txt(sd,286)); // There's been no change in the setting.
...
ACMD_FUNC(autoloot)
{
	...
	if (sd->state.autoloot) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1188)); // Autoloot is now off.
...
ACMD_FUNC(autolootitem)
{
	...
	if (action < 3) // add or remove
	{
		...
		if( item_data == nullptr ){
			...
			clif_displaymessage(*sd, msg_txt(sd,1189)); // Item not found.
			return -1;
		}
	}
	...
	case 1:
		...
		if (i != AUTOLOOTITEM_SIZE) {
			clif_displaymessage(*sd, msg_txt(sd,1190)); // You're already autolooting this item.
			return -1;
		}
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] == 0);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(*sd, msg_txt(sd,1191)); // Your autolootitem list is full. Remove some items first with @autolootid -<item name or ID>.
			return -1;
		}
	...
	case 2:
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] == item_data->nameid);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(*sd, msg_txt(sd,1193)); // You're currently not autolooting this item.
			return -1;
		}
	...
	case 3:
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1196)); // To add an item to the list, use "@alootid +<item name or ID>". To remove an item, use "@alootid -<item name or ID>".
		clif_displaymessage(*sd, msg_txt(sd,1197)); // "@alootid reset" will clear your autolootitem list.
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(*sd, msg_txt(sd,1198)); // Your autolootitem list is empty.
		} else {
			clif_displaymessage(*sd, msg_txt(sd,1199)); // Items on your autolootitem list:
	...
	case 4:
		...
		clif_displaymessage(*sd, msg_txt(sd,1200)); // Your autolootitem list has been reset.
		sd->state.autolooting = 0;
		break;
	}
	return 0;
...
ACMD_FUNC(autoloottype)
{
	...
	if (action < 3) { // add or remove
		if ((strncmp(message, "healing", 3) == 0) || (atoi(message) == 0))
			...
			clif_displaymessage(*sd, msg_txt(sd,1480)); // Item type not found.
			return -1;
		}
	}
	...
	switch (action) {
		case 1:
			if (sd->state.autoloottype&(1<<type)) {
				clif_displaymessage(*sd, msg_txt(sd,1481)); // You're already autolooting this item type.
				return -1;
			}
			if (sd->state.autoloottype == ITEM_MAX) {
				clif_displaymessage(*sd, msg_txt(sd,1482)); // Your autoloottype list has all item types. You can remove some items with @autoloottype -<type name or ID>.
				return -1;
			}
		...
		case 2:
			if (!(sd->state.autoloottype&(1<<type))) {
				clif_displaymessage(*sd, msg_txt(sd,1484)); // You're currently not autolooting this item type.
				return -1;
			}
			...
			break;
		case 3:
			clif_displaymessage(*sd, msg_txt(sd,1486)); // To add an item type to the list, use "@aloottype +<type name or ID>". To remove an item type, use "@aloottype -<type name or ID>".
			clif_displaymessage(*sd, msg_txt(sd,1487)); // Type List: healing = 0, usable = 2, etc = 3, armor = 4, weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
			clif_displaymessage(*sd, msg_txt(sd,1488)); // "@aloottype reset" will clear your autoloottype list.
			if (sd->state.autoloottype == 0)
				clif_displaymessage(*sd, msg_txt(sd,1489)); // Your autoloottype list is empty.
			else {
				uint8 i = 0;
				clif_displaymessage(*sd, msg_txt(sd,1490)); // Item types on your autoloottype list:
				while (i < IT_MAX) {
		...
		case 4:
			...
			clif_displaymessage(*sd, msg_txt(sd,1491)); // Your autoloottype list has been reset.
			break;
	}
	return 0;
}
...
//		clif_displaymessage(*sd, msg_txt(sd,1201)); // The rain has stopped.
//	} else {
//		map_setmapflag(sd->bl.m, MF_RAIN, true);
//		clif_weather(sd->bl.m);
//		clif_displaymessage(*sd, msg_txt(sd,1202)); // It has started to rain.
//	}
//	return 0;
//}
...
ACMD_FUNC(snow)
{
	...
	if (map_getmapflag(sd->bl.m, MF_SNOW)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1203)); // Snow has stopped falling.
	} else {
		map_setmapflag(sd->bl.m, MF_SNOW, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1204)); // It has started to snow.
	}
...
ACMD_FUNC(sakura)
{
	...
	if (map_getmapflag(sd->bl.m, MF_SAKURA)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1205)); // Cherry tree leaves no longer fall.
	} else {
		map_setmapflag(sd->bl.m, MF_SAKURA, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1206)); // Cherry tree leaves have begun to fall.
	}
	return 0;
}
...
ACMD_FUNC(clouds)
{
	...
	if (map_getmapflag(sd->bl.m, MF_CLOUDS)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1207)); // The clouds has disappear.
	} else {
		map_setmapflag(sd->bl.m, MF_CLOUDS, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1208)); // Clouds appear.
	}
...
ACMD_FUNC(clouds2)
{
	...
	if (map_getmapflag(sd->bl.m, MF_CLOUDS2)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1209)); // The alternative clouds disappear.
	} else {
		map_setmapflag(sd->bl.m, MF_CLOUDS2, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1210)); // Alternative clouds appear.
	}
...
ACMD_FUNC(fog)
{
	...
	if (map_getmapflag(sd->bl.m, MF_FOG)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1211)); // The fog has gone.
	} else {
		map_setmapflag(sd->bl.m, MF_FOG, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1212)); // Fog hangs over.
	}
		return 0;
}
...
ACMD_FUNC(leaves)
{
	...
	if (map_getmapflag(sd->bl.m, MF_LEAVES)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1213)); // Leaves no longer fall.
	} else {
		map_setmapflag(sd->bl.m, MF_LEAVES, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1214)); // Fallen leaves fall.
	}
...
ACMD_FUNC(fireworks)
{
	...
	if (map_getmapflag(sd->bl.m, MF_FIREWORKS)) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1215)); // Fireworks have ended.
	} else {
		map_setmapflag(sd->bl.m, MF_FIREWORKS, true);
		clif_weather(sd->bl.m);
		clif_displaymessage(*sd, msg_txt(sd,1216)); // Fireworks have launched.
	}
...
ACMD_FUNC(clearweather)
{
	...
	map_setmapflag(sd->bl.m, MF_LEAVES, false);
	clif_weather(sd->bl.m);
	clif_displaymessage(*sd, msg_txt(sd,291)); // Weather effects will dispell on warp/refresh
...
ACMD_FUNC(sound)
{
	...
	memset(sound_file, '\0', sizeof(sound_file));
		...
		clif_displaymessage(*sd, msg_txt(sd,1217)); // Please enter a sound filename (usage: @sound <filename>).
		return -1;
	}
...
ACMD_FUNC(mobsearch)
{
	...
	char mob_name[100]={0};
	if (!message || !*message || sscanf(message, "%99[^\n]", mob_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1218)); // Please enter a monster name (usage: @mobsearch <monster name>).
		return -1;
	}
...
ACMD_FUNC(cleanmap)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1221)); // All dropped items have been cleaned up.
	return 0;
}
...
ACMD_FUNC(npctalk)
{
	...
	if(!ifcolor) {
		if (!message || !*message || sscanf(message, "%49[^,], %99[^\n]", name, mes) < 2) {
			clif_displaymessage(*sd, msg_txt(sd,1222)); // Please enter the correct parameters (usage: @npctalk <npc name>, <message>).
			return -1;
		}
	}
	else {
		if (!message || !*message || sscanf(message, "%16lx %23[^,], %99[^\n]", &color, name, mes) < 3) {
			clif_displaymessage(*sd, msg_txt(sd,1223)); // Please enter the correct parameters (usage: @npctalkc <color> <npc name>, <message>).
			return -1;
		}
	}
...
ACMD_FUNC(pettalk)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1224)); // Please enter a message (usage: @pettalk <message>).
		return -1;
	}
...
ACMD_FUNC(summon)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1225)); // Please enter a monster name (usage: @summon <monster name> {duration}).
		return -1;
	}
...
ACMD_FUNC(adjgroup)
{
	...
	if (!message || !*message || sscanf(message, "%11d", &new_group) != 1) {
		clif_displaymessage(*sd, msg_txt(sd,1226)); // Usage: @adjgroup <group_id>
		return -1;
	}

	if (!player_group_db.exists(new_group)) {
		clif_displaymessage(*sd, msg_txt(sd,1227)); // Specified group does not exist.
		return -1;
	}

	sd->group_id = new_group;
	pc_group_pc_load(sd);/* update cache */
	clif_displaymessage(*sd, msg_txt(sd,1228)); // Group changed successfully.
	clif_displaymessage(*sd, msg_txt(sd,1229)); // Your group has changed.
	return 0;
}
...
ACMD_FUNC(trade)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1230)); // Please enter a player name (usage: @trade <char name>).
		return -1;
	}
...
ACMD_FUNC(setbattleflag)
{
	...
	if (!message || !*message || sscanf(message, "%127s %127s %11d", flag, value, &reload) < 2) {
        	clif_displaymessage(*sd, msg_txt(sd,1231)); // Usage: @setbattleflag <flag> <value> {<reload>}
        	return -1;
    	}

	if (battle_set_value(flag, value) == 0)
	{
		clif_displaymessage(*sd, msg_txt(sd,1232)); // Unknown battle_config flag.
		return -1;
	}

	clif_displaymessage(*sd, msg_txt(sd,1233)); // Set battle_config as requested.
...
ACMD_FUNC(unmute)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1234)); // Please enter a player name (usage: @unmute <char name>).
		return -1;
	}
	...
	if(!pl_sd->sc.getSCE(SC_NOCHAT)) {
		clif_displaymessage(*sd,msg_txt(sd,1235)); // Player is not muted.
		return -1;
	}

	pl_sd->status.manner = 0;
	status_change_end(&pl_sd->bl, SC_NOCHAT);
	clif_displaymessage(*sd,msg_txt(sd,1236)); // Player unmuted.
...
ACMD_FUNC(mute)
{
	...
	if (!message || !*message || sscanf(message, "%11d %23[^\n]", &manner, atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1237)); // Usage: @mute <time> <char name>
		return -1;
	}
...
ACMD_FUNC(identify)
{
	...
	if (num > 0) {
		...
		clif_displaymessage(*sd,msg_txt(sd,1238)); // There are no items to appraise.
	}
	return 0;
}
...
ACMD_FUNC(mobinfo)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1239)); // Please enter a monster name/ID (usage: @mobinfo <monster_name_or_monster_ID>).
		return -1;
	}
	...
	for (uint16 k = 0; k < count; k++) {
...
#endif
		// drops
		clif_displaymessage(*sd, msg_txt(sd,1245)); //  Drops:
		strcpy(atcmd_output, " ");
		unsigned int j = 0;
		...
		if (j == 0)
			clif_displaymessage(*sd, msg_txt(sd,1246)); // This monster has no drops.
		else if (j % 3 != 0)
			clif_displaymessage(*sd, atcmd_output);
		// mvp
		if( mob->get_bosstype() == BOSSTYPE_MVP ){
			...
			if (j == 0)
				clif_displaymessage(*sd, msg_txt(sd,1249)); // This monster has no MVP prizes.
			else
				clif_displaymessage(*sd, atcmd_output);
		}
	}
...
ACMD_FUNC(showmobs)
{
	...
	if(status_has_mode(&mob->status,MD_STATUSIMMUNE) && !pc_has_permission(sd, PC_PERM_SHOW_BOSS)){	// If player group does not have access to boss mobs.
		clif_displaymessage(*sd, msg_txt(sd,1251)); // Can't show boss mobs!
		return 0;
	}
...
ACMD_FUNC(homlevel)
{
	...
	if ( !message || !*message || ( level = atoi(message) ) < 1 ) {
		clif_displaymessage(*sd, msg_txt(sd,1253)); // Please enter a level adjustment (usage: @homlevel <number of levels>).
		return -1;
	}

	if ( !hom_is_active(sd->hd) ) {
		clif_displaymessage(*sd, msg_txt(sd,1254)); // You do not have a homunculus.
		return -1;
	}
...
ACMD_FUNC(homevolution)
{
	...
	if ( !hom_evolution(sd->hd) ) {
		clif_displaymessage(*sd, msg_txt(sd,1255)); // Your homunculus doesn't evolve.
		return -1;
	}
...
ACMD_FUNC(makehomun)
{
	...
	if ( sd->status.hom_id ) {
		clif_displaymessage(*sd, msg_txt(sd,450)); // You already have a homunculus
		return -1;
	}

	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1256)); // Please enter a homunculus ID (usage: @makehomun <homunculus id>).
		return -1;
	}
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1257)); // Invalid Homunculus ID.
		return -1;
	}
...
ACMD_FUNC(homfriendly)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1258)); // Please enter a friendly value (usage: @homfriendly <friendly value [0-1000]>).
		return -1;
	}
...
ACMD_FUNC(homhungry)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1259)); // Please enter a hunger value (usage: @homhungry <hunger value [0-100]>).
		return -1;
	}
...
ACMD_FUNC(homtalk)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,1260)); // Please enter a message (usage: @homtalk <message>).
		return -1;
	}
...
ACMD_FUNC(hominfo)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1261)); // Homunculus stats:
...
ACMD_FUNC(homshuffle)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1275)); // Homunculus stats altered.
...
{
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1276)); // Please enter an item name/ID (usage: @ii/@iteminfo <item name/ID>).
		return -1;
	}
...
{
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1284)); // Please enter item name/ID (usage: @whodrops <item name/ID>).
		return -1;
	}
...
ACMD_FUNC(whereis)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1288)); // Please enter a monster name/ID (usage: @whereis <monster_name_or_monster_ID>).
		return -1;
	}
	...
	for (int i = 0; i < count; i++) {
		...
		if (spawns.size() <= 0) {
			...
			clif_displaymessage(*sd, msg_txt(sd,1290));
		} else {
...
ACMD_FUNC(mutearea)
{
	...
	if (!message || !*message) {
		clif_displaymessage(*sd, msg_txt(sd,1297)); // Please enter a time in minutes (usage: @mutearea/@stfu <time in minutes>).
		return -1;
	}
...
ACMD_FUNC(me)
{
	...
	if (!message || !*message || sscanf(message, "%255[^\n]", tempmes) < 0) {
		clif_displaymessage(*sd, msg_txt(sd,1302)); // Please enter a message (usage: @me <message>).
		return -1;
	}
...
ACMD_FUNC(size)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,1303)); // Size change applied.
	return 0;
}
...
ACMD_FUNC(sizeguild)
{
	...
	if( !message || !*message || sscanf(message, "%11d %23[^\n]", &size, guild) < 2 ) {
		clif_displaymessage(*sd, msg_txt(sd,1304)); // Please enter guild name/ID (usage: @sizeguild <size> <guild name/ID>).
		return -1;
	}
...
ACMD_FUNC(monsterignore)
{
	...
	if (sd->state.block_action & PCBLOCK_IMMUNE) {
		sd->state.block_action &= ~PCBLOCK_IMMUNE;
		clif_displaymessage(*sd, msg_txt(sd,1306)); // Returned to normal state.
	} else {
		sd->state.block_action |= PCBLOCK_IMMUNE;
		clif_displaymessage(*sd, msg_txt(sd,1305)); // You are now immune to attacks.
	}
...
ACMD_FUNC(fakename)
{
	...
	if( !message || !*message )
	{
		if( sd->fakename[0] )
		{
			...
			clif_displaymessage(*sd, msg_txt(sd,1307)); // Returned to real name.
			return 0;
		}

		clif_displaymessage(*sd, msg_txt(sd,1308)); // You must enter a name.
		return -1;
	}

	if( strlen(message) < 2 )
	{
		clif_displaymessage(*sd, msg_txt(sd,1309)); // Fake name must be at least two characters.
		return -1;
	}
	...
	if (sd->disguise) // Another packet should be sent so the client updates the name for sd
		clif_name_self(&sd->bl);
	clif_displaymessage(*sd, msg_txt(sd,1310)); // Fake name enabled.
...
ACMD_FUNC(mapflag) {
	...
	if (!message || !*message || (sscanf(message, "%49s %6hd", flag_name, &flag) < 1)) {
		clif_displaymessage(*sd,msg_txt(sd,1311)); // Enabled Mapflags in this map:
		clif_displaymessage(*sd,"----------------------------------");
		for( i = MF_MIN; i < MF_MAX; i++ ){
			...
			if( map_getmapflag_name(static_cast<e_mapflag>(i), flag_name) && map_getmapflag_sub( sd->bl.m, static_cast<e_mapflag>(i), &args ) ){
				clif_displaymessage(*sd, flag_name);
			}
		}

		clif_displaymessage(*sd," ");
		clif_displaymessage(*sd,msg_txt(sd,1312)); // Usage: "@mapflag monster_noteleport 1" (0=Off | 1=On)
		clif_displaymessage(*sd,msg_txt(sd,1313)); // Type "@mapflag available" to list the available mapflags.
		return 1;
	}
	...
	if( strcasecmp(flag_name,"available") ){
		...
		if( mapflag != MF_INVALID ){
			...
			clif_displaymessage(*sd, msg_txt(sd, 1314)); // Invalid flag name or flag.
			clif_displaymessage(*sd, msg_txt(sd, 1313)); // Type "@mapflag available" to list the available mapflags.
			return 1;
		}
	}

	clif_displaymessage(*sd,msg_txt(sd,1315)); // Available Flags:
	clif_displaymessage(*sd,"----------------------------------");
	for( i = MF_MIN, j = 0; i < MF_MAX; i++ ){
		...
		if( i > MF_MIN && ( j == 6 || ( i + 1 ) == MF_MAX ) ){
			clif_displaymessage(*sd, buf.c_str() );
			buf.clear();
			j = 0;
		}
	}
...
ACMD_FUNC(showexp)
{
	if (sd->state.showexp) {
		sd->state.showexp = 0;
		clif_displaymessage(*sd, msg_txt(sd,1316)); // Gained exp will not be shown.
		return 0;
	}

	sd->state.showexp = 1;
	clif_displaymessage(*sd, msg_txt(sd,1317)); // Gained exp is now shown.
	return 0;
}
...
ACMD_FUNC(showzeny)
{
	if (sd->state.showzeny) {
		sd->state.showzeny = 0;
		clif_displaymessage(*sd, msg_txt(sd,1318)); // Gained zeny will not be shown.
		return 0;
	}

	sd->state.showzeny = 1;
	clif_displaymessage(*sd, msg_txt(sd,1319)); // Gained zeny is now shown.
	return 0;
}
...
ACMD_FUNC(showdelay)
{
	if (sd->state.showdelay) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1320)); // Skill delay failures will not be shown.
		return 0;
	}

	sd->state.showdelay = 1;
	clif_displaymessage(*sd, msg_txt(sd,1321)); // Skill delay failures are now shown.
	return 0;
}
...
ACMD_FUNC(invite)
{
	...
	if(did == 0 || !duel_exist(did) ) {
		clif_displaymessage(*sd, msg_txt(sd,350)); // "Duel: @invite without @duel."
		return 0;
	}

	if( !duel_check_player_limit( duel_get_duelid(did) ) ){
		clif_displaymessage(*sd, msg_txt(sd,351)); // "Duel: Limit of players is reached."
		return 0;
	}

	if((target_sd = map_nick2sd(atcmd_player_name, true)) == nullptr) {
		clif_displaymessage(*sd, msg_txt(sd,352)); // "Duel: Player not found."
		return 0;
	}

	if(target_sd->duel_group > 0 || target_sd->duel_invite > 0) {
		clif_displaymessage(*sd, msg_txt(sd,353)); // "Duel: Player already in duel."
		return 0;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,354)); // "Duel: Invitation has been sent."
	return 0;
}
...
ACMD_FUNC(duel)
{
	...
	if(sd->duel_invite > 0) {
		clif_displaymessage(*sd, msg_txt(sd,355)); // "Duel: @duel without @reject."
		return 0;
	}
	...
	if( message[0] ) {
		if(sscanf(message, "%11u", &maxpl) >= 1) {
			if(maxpl < 2 || maxpl > 65535) {
				clif_displaymessage(*sd, msg_txt(sd,357)); // "Duel: Invalid value."
				return 0;
			}
...
{
	if(sd->duel_group <= 0) {
		clif_displaymessage(*sd, msg_txt(sd,358)); // "Duel: @leave without @duel."
		return 0;
	}

	duel_leave(sd->duel_group, sd);
	clif_displaymessage(*sd, msg_txt(sd,359)); // "Duel: You left the duel."
	return 0;
}
...
ACMD_FUNC(accept)
{
	...
	if(sd->duel_invite <= 0 || !duel_exist(sd->duel_invite) ) {
		clif_displaymessage(*sd, msg_txt(sd,360)); // "Duel: @accept without invititation."
		return 0;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,361)); // "Duel: Invitation has been accepted."
	return 0;
}
...
{
	if(sd->duel_invite <= 0) {
		clif_displaymessage(*sd, msg_txt(sd,362)); // "Duel: @reject without invititation."
		return 0;
	}

	duel_reject(sd->duel_invite, sd);
	clif_displaymessage(*sd, msg_txt(sd,363)); // "Duel: Invitation has been rejected."
	return 0;
}
...
ACMD_FUNC(cash)
{
	...
	if( sd->state.cashshop_open ){
		clif_displaymessage(*sd, msg_txt(sd, 1376)); // Please close the cashshop before using this command.
		return -1;
	}
	...
	if( !strcmpi(parent_cmd,"cash") )
	{
		if( value > 0 ) {
			...
			else clif_displaymessage(*sd, msg_txt(sd,149)); // Impossible to increase the number/value.
		} else {
			...
			else clif_displaymessage(*sd, msg_txt(sd,41)); // Unable to decrease the number/value.
		}
	}
	else
	{ // @points
...
ACMD_FUNC(clone)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd,msg_txt(sd,1323)); // You must enter a player name or ID.
		return 0;
	}
	...
	if(pc_get_group_level(pl_sd) > pc_get_group_level(sd)) {
		clif_displaymessage(*sd, msg_txt(sd,126));	// Cannot clone a player of higher GM level than yourself.
		return 0;
	}
	...
	if (strcmpi(parent_cmd, "clone") == 0)
		...
		flag = 2;
		if(pc_isdead(sd)){
		    clif_displaymessage(*sd, msg_txt(sd,129+flag*2));
		    return 0;
		}
		master = sd->bl.id;
		if (battle_config.atc_slave_clone_limit
			&& mob_countslave(&sd->bl) >= battle_config.atc_slave_clone_limit) {
			clif_displaymessage(*sd, msg_txt(sd,127));	// You've reached your slave clones limit.
			return 0;
		}
	}
	...
	if((x = mob_clone_spawn(pl_sd, sd->bl.m, x, y, "", master, MD_NONE, flag?1:0, 0)) > 0) {
		clif_displaymessage(*sd, msg_txt(sd,128+flag*2));	// Evil Clone spawned. Clone spawned. Slave clone spawned.
		return 0;
	}
	clif_displaymessage(*sd, msg_txt(sd,129+flag*2));	// Unable to spawn evil clone. Unable to spawn clone. Unable to spawn slave clone.
	return 0;
}
...
{
	if(sd->state.noask) {
		clif_displaymessage(*sd, msg_txt(sd,391)); // Autorejecting is deactivated.
		sd->state.noask = 0;
	} else {
		clif_displaymessage(*sd, msg_txt(sd,390)); // Autorejecting is activated.
		sd->state.noask = 1;
	}
...
{
	if (!message || !*message) {
		clif_displaymessage(*sd,msg_txt(sd,277));	// Usage: @request <petition/message to online GMs>.
		return -1;
	}
	...
	clif_displaymessage(*sd,msg_txt(sd,279));	// @request sent.
	return 0;
}
...
ACMD_FUNC(feelreset)
{
	if ((sd->class_&MAPID_UPPERMASK) != MAPID_STAR_GLADIATOR) {
		clif_displaymessage(*sd,msg_txt(sd,35));	// You can't use this command with this class.
		return -1;
	}

	pc_resetfeel(sd);
	clif_displaymessage(*sd, msg_txt(sd,1324)); // Reset 'Feeling' maps.
...
ACMD_FUNC(hatereset)
{
	...
	pc_resethate(sd);
	clif_displaymessage(*sd, msg_txt(sd,1515)); // Reset 'Hatred' monsters.
...
ACMD_FUNC(ksprotection)
{
	...
	if( sd->state.noks ) {
		sd->state.noks = 0;
		clif_displaymessage(*sd, msg_txt(sd,1325)); // [ K.S Protection Inactive ]
	}
	else
	{
		...
		{ // Default is Party
			sd->state.noks = 2;
			clif_displaymessage(*sd, msg_txt(sd,1326)); // [ K.S Protection Active - Option: Party ]
		}
		else if( !strcmpi(message, "self") )
		{
			sd->state.noks = 1;
			clif_displaymessage(*sd, msg_txt(sd,1327)); // [ K.S Protection Active - Option: Self ]
		}
		else if( !strcmpi(message, "guild") )
		{
			sd->state.noks = 3;
			clif_displaymessage(*sd, msg_txt(sd,1328)); // [ K.S Protection Active - Option: Guild ]
		}
		else
			clif_displaymessage(*sd, msg_txt(sd,1329)); // Usage: @noks <self|party|guild>
	}
	return 0;
}
/*==========================================
...
ACMD_FUNC(allowks)
{
	...
	if( map_getmapflag(sd->bl.m, MF_ALLOWKS) ) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1330)); // [ Map K.S Protection Active ]
	} else {
		map_setmapflag(sd->bl.m, MF_ALLOWKS, true);
		clif_displaymessage(*sd, msg_txt(sd,1331)); // [ Map K.S Protection Inactive ]
	}
	return 0;
}
...
ACMD_FUNC(delitem)
{
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1355)); // Please enter an item name/ID, a quantity, and a player name (usage: #delitem <player> <item_name_or_ID> <quantity>).
		return -1;
	}
	...
	if( amount == total )
	{
		clif_displaymessage(*sd, msg_txt(sd,116)); // Character does not have the item.
	}
	else if( amount )
	{
...
ACMD_FUNC(font)
{
	...
	if( font_id == 0 ) {
		if( sd->status.font ) {
			sd->status.font = 0;
			clif_displaymessage(*sd, msg_txt(sd,1356)); // Returning to normal font.
			clif_font(sd);
		} else {
			clif_displaymessage(*sd, msg_txt(sd,1357)); // Use @font <1-9> to change your message font.
			clif_displaymessage(*sd, msg_txt(sd,1358)); // Use 0 or no parameter to return to normal font.
		}
	} else if( font_id < 0 || font_id > 9 )
		clif_displaymessage(*sd, msg_txt(sd,1359)); // Invalid font. Use a value from 0 to 9.
	else if( font_id != sd->status.font ) {
		sd->status.font = font_id;
		clif_font(sd);
		clif_displaymessage(*sd, msg_txt(sd,1360)); // Font changed.
	} else
		clif_displaymessage(*sd, msg_txt(sd,1361)); // Already using this font.
...
static void atcommand_commands_sub(map_session_data* sd, const int fd, AtCommandType type)
{
	...
	clif_displaymessage(*sd, msg_txt(sd,273)); // "Commands available:"
	...
	for (cmd = (AtCommandInfo*)dbi_first(iter); dbi_exists(iter); cmd = (AtCommandInfo*)dbi_next(iter)) {
		...
		if (slen + cur - line_buff >= CHATBOX_SIZE) {
			clif_displaymessage(*sd,line_buff);
			cur = line_buff;
			memset(line_buff,' ',CHATBOX_SIZE);
			line_buff[CHATBOX_SIZE-1] = 0;
		}
	...
	if ( atcmd_binding_count ) {
		...
		for( i = count_bind = 0; i < atcmd_binding_count; i++ ) {
			if ( gm_lvl >= ( (type - 1) ? atcmd_binding[i]->level2 : atcmd_binding[i]->level ) ) {
				...
				if ( count_bind == 0 ) {
					...
					line_buff[CHATBOX_SIZE-1] = 0;
					clif_displaymessage(*sd, "-----------------");
					clif_displaymessage(*sd, msg_txt(sd,509)); // Script-bound commands:
				}
				if (slen + cur - line_buff >= CHATBOX_SIZE) {
					clif_displaymessage(*sd,line_buff);
					cur = line_buff;
		...
		if ( count_bind )
			clif_displaymessage(*sd,line_buff);// last one
		count += count_bind;
...
/* for new mounts */
ACMD_FUNC(mount2) {
	clif_displaymessage(*sd,msg_txt(sd,1362)); // NOTICE: If you crash with mount your LUA is outdated.
	if (!sd->sc.getSCE(SC_ALL_RIDING)) {
		clif_displaymessage(*sd,msg_txt(sd,1363)); // You have mounted.
		sc_start(nullptr, &sd->bl, SC_ALL_RIDING, 10000, 1, INFINITE_TICK);
	} else {
		clif_displaymessage(*sd,msg_txt(sd,1364)); // You have released your mount.
...
ACMD_FUNC(accinfo) {
	...
	{
		clif_displaymessage(*sd, msg_txt(sd,1365)); // Usage: @accinfo/@accountinfo <account_id/char name>
		clif_displaymessage(*sd, msg_txt(sd,1366)); // You may search partial name by making use of '%' in the search, ex. "@accinfo %Mario%" lists all characters whose name contains "Mario".
		return -1;
	} else if (type != 0) {
		type = type-'0'; //make it int
		if (type != 1) {
			clif_displaymessage(*sd, "accinfo : Unknow type specified\n");
			return -1;
		}
 	}
...
ACMD_FUNC(set) {
	...
	if( !message || !*message || (toset = sscanf(message, "%45s %127[^\n]s", reg, val)) < 1  ) {
		clif_displaymessage(*sd, msg_txt(sd,1367)); // Usage: @set <variable name> <value>
		clif_displaymessage(*sd, msg_txt(sd,1368)); // Usage: ex. "@set PoringCharVar 50"
		clif_displaymessage(*sd, msg_txt(sd,1369)); // Usage: ex. "@set PoringCharVarSTR$ Super Duper String"
		clif_displaymessage(*sd, msg_txt(sd,1370)); // Usage: ex. "@set PoringCharVarSTR$" outputs its value, Super Duper String.
		return -1;
	}

	/* disabled variable types (they require a proper script state to function, so allowing them would crash the server) */
	if( reg[0] == '.' ) {
		clif_displaymessage(*sd, msg_txt(sd,1371)); // NPC variables may not be used with @set.
		return -1;
	} else if( reg[0] == '\'' ) {
		clif_displaymessage(*sd, msg_txt(sd,1372)); // Instance variables may not be used with @set.
		return -1;
	}
...
ACMD_FUNC(addperm) {
	...
	if( !message || !*message ) {
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1379)); // -- Permission List
...
ACMD_FUNC(unloadnpcfile) {
	...
	if( !message || !*message ) {
		clif_displaymessage(*sd, msg_txt(sd,1385)); // Usage: @unloadnpcfile <file name>
		return -1;
	}
	...
	if( npc_unloadfile(message) )
		...
		clif_displaymessage(*sd, msg_txt(sd,1387)); // File not found.
		return -1;
	}
	return 0;
}
...
#define MC_CART_MDFY(idx, x) \
	...
	if( val == 0 && !pc_iscarton(sd) ) {
		clif_displaymessage(*sd, msg_txt(sd,1391)); // You do not possess a cart to be removed
		return -1;
	}
	...
	clif_displaymessage(*sd, msg_txt(sd,1392)); // Cart Added
...
static inline void atcmd_channel_help(map_session_data *sd, const char *command)
{
	...
	if( can_create ) {
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1416));// -- Creates a new channel.
	}
	...
	if(can_delete){
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1470)); // -- Destroys the specified channel.
	}

	//option list
	sprintf(atcmd_output, msg_txt(sd,1417),command);// * %s list
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1418));// -- Lists all public channels.
	sprintf(atcmd_output, msg_txt(sd,1471),command);// * %s list mine
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1472));// -- Lists all channels you have joined.
	if( can_create ) {
		sprintf(atcmd_output, msg_txt(sd,1419),command);// * %s list colors
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1420));// -- Lists all available colors for custom channels.
	}
	...
	if(can_create){
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1422));// -- Changes channel text to the specified color (channel owners only).
	}

	//option join
	sprintf(atcmd_output, msg_txt(sd,1473),command);// * %s join <#channel_name> <channel_password>
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1474));// -- Joins the specified channel.

	//option leave
	sprintf(atcmd_output, msg_txt(sd,1423),command);// * %s leave <#channel_name>
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1424));// -- Leaves the specified channel.

	//option bindto
	sprintf(atcmd_output, msg_txt(sd,1427),command);// * %s bindto <#channel_name>
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1428));// -- Binds your global chat to the specified channel, sending all global messages to that channel.

	//option unbind
	sprintf(atcmd_output, msg_txt(sd,1429),command);// * %s unbind
	clif_displaymessage(*sd, atcmd_output);
	clif_displaymessage(*sd, msg_txt(sd,1430));// -- Unbinds your global chat from the attached channel, if any.

	//option ban/unban/banlist
	if( can_create ) {
		sprintf(atcmd_output, msg_txt(sd,1456),command);// * %s ban <#channel_name> <player>
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1457));// -- Bans the specified player from the channel.
		sprintf(atcmd_output, msg_txt(sd,1458),command);// * %s banlist <#channel_name>
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1459));// -- Lists all players banned from the specified channel.
		sprintf(atcmd_output, msg_txt(sd,1460),command);// * %s unban <#channel_name> <player>
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1461));// -- Unbans the specified player from the channel.
		sprintf(atcmd_output, msg_txt(sd,1467),command);// * %s unbanall <#channel_name>
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1468));// -- Clears all bans from the specified channel.
	}
	...
	if(can_create){
		...
		clif_displaymessage(*sd, atcmd_output);
		clif_displaymessage(*sd, msg_txt(sd,1463));// -- Sets an option and value for the specified channel.
	}
...
ACMD_FUNC(channel) {
	...
	if( strcmpi(key,"delete") == 0 && pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) {
		...
		if( strcmpi(key,"create") == 0 && ( channel_config.private_channel.allow || pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) ) {
			if (sub4[0] != '\0') {
				clif_displaymessage(*sd, msg_txt(sd, 1408)); // Channel password may not contain spaces.
				return -1;
			}
...
ACMD_FUNC(langtype)
{
	...
	if(sscanf(message, "%3s", langstr) >= 1){
		...
		if( msg_checklangtype(lang,false) == 1 ){ //Verify it's enabled and set it
			...
			clif_displaymessage(*sd,msg_txt(sd,462)); // This langage is currently disabled.
			return -1;
		}
	}

	//wrong or no entry
	clif_displaymessage(*sd,msg_txt(sd,460)); // Please enter a valid language (usage: @langtype <language>).
	clif_displaymessage(*sd,msg_txt(sd,464)); // ---- Available languages:
	while(fail != -1){ //out of range
		fail = msg_checklangtype(i,false);
		if(fail == 1)
			clif_displaymessage(*sd,msg_langtype2langstr(i));
		i++;
	}
	return -1;
}
...
ACMD_FUNC(vip) {
	...
	if (!message || !*message || sscanf(message, "%255s %23[^\n]",atcmd_output,atcmd_player_name) < 2) {
		clif_displaymessage(*sd, msg_txt(sd,700));	//Usage: @vip <timef> <character name>
		return -1;
	}
	...
	if (vipdifftime == 0) {
		clif_displaymessage(*sd, msg_txt(sd,701)); // Invalid time for vip command.
		clif_displaymessage(*sd, msg_txt(sd,702)); // Time parameter format is +/-<value> to alter. y/a = Year, m = Month, d/j = Day, h = Hour, n/mn = Minute, s = Second.
		return -1;
	}
...
ACMD_FUNC(fullstrip) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(*sd, msg_txt(sd,349)); // Please enter a player name (usage: @fullstrip/@warpto/@goto <char name/ID>).
		return -1;
	}
...
ACMD_FUNC(costume) {
	...
	if( !message || !*message ) {
		...
		clif_displaymessage(*sd, msg_txt(sd, 726)); // Available Costumes
		for( k = 0; k < len; k++ ) {
...
ACMD_FUNC(adopt)
{
	...
	if (response < ADOPT_MORE_CHILDREN) // No displaymessage for client-type responses
		clif_displaymessage(*sd, msg_txt(sd, 744 + response - 1));
	return -1;
}
...
ACMD_FUNC(camerainfo){
	...
	if( sscanf( message, "%f %f %f", &range, &rotation, &latitude ) < 3 ){
		clif_displaymessage(*sd, msg_txt( sd, 793 ) ); // Usage @camerainfo range rotation latitude
		return -1;
	}
...
ACMD_FUNC(refineui)
{
	...
	clif_displaymessage(*sd, msg_txt(sd, 773)); // This command requires packet version 2016-10-12 or newer.
	return -1;
#else
	if( !battle_config.feature_refineui ){
		clif_displaymessage(*sd, msg_txt(sd, 774)); // This command is disabled via configuration.
		return -1;
	}

	if( sd->state.refineui_open ){
		clif_displaymessage(*sd, msg_txt(sd, 775)); // You have already opened the refine UI.
		return -1;
	}
...
ACMD_FUNC( stylist ){
	...
	clif_displaymessage(*sd, msg_txt( sd, 798 ) ); // This command requires packet version 2015-11-04 or newer.
	return -1;
#else
	...
	if( sd->state.stylist_open ){
		clif_displaymessage(*sd, msg_txt( sd, 799 ) ); // You have already opened the stylist UI.
		return -1;
	}
...
ACMD_FUNC( roulette ){
	...
	if( !battle_config.feature_roulette ){
		clif_displaymessage(*sd, msg_txt( sd, 1497 ) ); // Roulette is disabled
		return -1;
	}
...
ACMD_FUNC(setcard)
{
	...
	if (!message || !*message || sscanf(message, "%11d %11d %11d", &position, &slot, &card_id) < 3) {
		...
		clif_displaymessage(*sd, msg_txt(sd,1527)); // Please enter the position, the slot number and the card ID (usage: @setcard <equip position> <slot> <card_id>).
		sprintf(atcmd_output, msg_txt(sd,997), EQP_HEAD_LOW); // %d: Lower Headgear
		clif_displaymessage(*sd, atcmd_output);
		sprintf(atcmd_output, msg_txt(sd,998), EQP_HAND_R); // %d: Right Hand
		clif_displaymessage(*sd, atcmd_output);
	...
	if (position < EQP_HEAD_LOW || position > EQP_HEAD_MID) {
		clif_displaymessage(*sd, msg_txt(sd,1531)); // Invalid position.
		return -1;
	}
	if (slot < 0 || slot >= MAX_SLOTS) {
		clif_displaymessage(*sd, msg_txt(sd,1532)); // Invalid slot number.
		return -1;
	}
	if (card_id != 0) {
		...
		if (item_data == nullptr) {
			clif_displaymessage(*sd, msg_txt(sd,1530)); // Invalid card ID.
			return -1;
		}
		if (item_data->type != IT_CARD) {
			clif_displaymessage(*sd, msg_txt(sd,1529)); // The item type must be a card or enchant.
			return -1;
		}
	}
	...
	if (i < 0) {
		clif_displaymessage(*sd, msg_txt(sd,1528)); // You are not wearing any equipment in this position.
		return -1;
	}
...
bool is_atcommand(const int fd, map_session_data* sd, const char* message, int type)
{
	...
	if ( type == 1 || type == 3) {
		...
		if ( pc_get_group_level(sd) < map_getmapflag(sd->bl.m, MF_NOCOMMAND) ) {
			clif_displaymessage(*sd, msg_txt(sd,143)); // Commands are disabled on this map.
			return false;
		}
	}
	...
	if (type == 1) {
		...
		if( pc_isdead(sd) && pc_has_permission(sd,PC_PERM_DISABLE_CMD_DEAD) ) {
			clif_displaymessage(*sd, msg_txt(sd,1393)); // You can't use commands while dead
			return true;
		}
	}
