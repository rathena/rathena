	...
	for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
		...
		if (sd && mapdata->instance_id > 0) {
			...
			switch (idata->mode) {
				...
				case IM_CHAR:
					if (sd->instance_id !=
						mapdata->instance_id) { // Someone who is not instance owner is on instance map
						continue;
					}
					instance_id = sd->instance_id;
					break;
				case IM_PARTY:
					if ((!(pd = party_search(sd->status.party_id)) ||
						 pd->instance_id != mapdata->instance_id)) { // Someone not in party is on instance map
						continue;
					}
					instance_id = pd->instance_id;
					break;
