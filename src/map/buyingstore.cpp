...
int8 buyingstore_create(map_session_data *sd,
	...
	if (Sql_Query(mmysql_handle,
				  "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, `title`, `limit`, "
				  "`autotrade`, `body_direction`, `head_direction`, `sit`) "
...
void buyingstore_trade(map_session_data *sd,
	...
	for (int32 i = 0; i < count; i++) {
		...
		if (pl_sd->buyingstore.items[listidx].amount > 0) {
			if (Sql_Query(mmysql_handle,
						  ...
						  pl_sd->buyingstore.items[listidx].amount,
						  pl_sd->buyer_id,
						  listidx) != SQL_SUCCESS) {
				Sql_ShowDebug(mmysql_handle);
			}
	...
	if (i == pl_sd->buyingstore.slots) { // everything was bought
		...
		if (Sql_Query(mmysql_handle,
					  "UPDATE `%s` SET `limit` = %d WHERE `id` = %d;",
					  buyingstores_table,
					  pl_sd->buyingstore.zenylimit,
...
void buyingstore_reopen(map_session_data *sd) {
	...
	if ((at = (struct s_autotrader *)uidb_get(buyingstore_autotrader_db, sd->status.char_id)) && at->count &&
		...
		if ((fail = buyingstore_setup(sd, (unsigned char)at->count)) == 0 &&
			...
			ShowInfo("Buyingstore loaded for '" CL_WHITE "%s" CL_RESET "' with '" CL_WHITE "%d" CL_RESET
	...
	if (fail != 0) {
		ShowError("buyingstore_reopen: (Error:%d) Load failed for autotrader '" CL_WHITE "%s" CL_RESET
				  "' (CID=%/AID=%d)\n",
				  fail,
				  sd->status.name,
				  sd->status.char_id,
...
void do_init_buyingstore_autotrade(void) {
	if (battle_config.feature_autotrade) {
		if (Sql_Query(mmysql_handle,
					  "SELECT `id`, `account_id`, `char_id`, `sex`, `title`, `limit`, `body_direction`, "
					  "`head_direction`, `sit` "
					  "FROM `%s` "
		...
		if (Sql_NumRows(mmysql_handle) > 0) {
			...
			ShowStatus("Done loading '" CL_WHITE "%d" CL_RESET "' buyingstore autotraders with '" CL_WHITE "%d" CL_RESET
					   "' items.\n",
					   db_size(buyingstore_autotrader_db),
					   items);
		}
	}
