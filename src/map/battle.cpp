...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case LG_OVERBRAND_BRANDISH:
			skillratio += -100 + 300 * skill_lv + ((sstatus->str + sstatus->dex) / 6); // !TOD: What's the STR/DEX bonus?
			RE_LVL_DMOD(100);
			break;
		...
		case GN_CART_TORNADO: { // ATK [( Skill Level x 200 ) + ( Cart Weight / ( 150 - Caster Base STR ))] + ( Cart Remodeling Skill Level x 50 )] %
			...
			break;
		case GN_CARTCANNON:
			// ATK [(20 * skill_lv) + {( INT / (3 - ( Cart Remodeling Skill Level ) )} + ( Cart Cannon Skill Level x 250 )] %
			skillratio += -100 + 250 * skill_lv + 20 * skill_lv + sstatus->int_ / (3 - (sd ? pc_checkskill(sd, GN_REMODELING_CART) : 1));
			RE_LVL_DMOD(100);
			break;
