...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag)
{
	...
	if(!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
		...
		if ((wd.damage + wd.damage2) && tstatus->res > 0 && !nk[NK_SIMPLEDEFENSE]) {
			...
			res = (int16)(res - (double)(ignore_res * res) / 100.0);

			// Apply damage reduction.
			wd.damage = (int64)(wd.damage - (double)(res / (res + 400.0)) * 80.0 / 100.0 * wd.damage);
			wd.damage2 = (int64)(wd.damage2 - (double)(res / (res + 400.0)) * 80.0 / 100.0 * wd.damage2);
		}
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		if (ad.damage != 0 && tstatus->mres > 0) {
			...
			mres = (int16)(mres - (double)(ignore_mres) * mres / 100.0);

			ad.damage = (int64)(ad.damage - (double)(mres) / (mres + 400.0) * 80.0 / 100.0 * ad.damage);
		}
#endif
