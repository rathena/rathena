...
#define APPLY_CARDFIX(damage, fix)                                     \
  {                                                                    \
    (damage) = (damage) - (int64)(((damage) * (1000 - (fix))) / 1000); \
  }
  ...
  case BF_MAGIC:
    ...
    if (sd && !nk[NK_IGNOREATKCARD])
    {
      ...
      {
        // Affected by Element modifier bonuses
    ...
    if (sd && !nk[NK_IGNOREATKCARD] && (left & 2))
    {
      ...
      else
      {
        ...
        else
        {
          ...
          int ele_fix = sd->right_weapon.addele[tstatus->def_ele] + sd->left_weapon.addele[tstatus->def_ele] + sd->right_weapon.addele[ELE_ALL] + sd->left_weapon.addele[ELE_ALL];
...
bool battle_status_block_damage(struct block_list *src, struct block_list *target, struct status_change *sc, struct Damage *d, int64 &damage, uint16 skill_id, uint16 skill_lv)
{
  ...
  if ((sce = sc->data[SC_KAUPE]) && (skill_id != NPC_EARTHQUAKE || (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) && rnd() % 100 < sce->val2)
  {
...
int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
...
#ifdef RENEWAL
    // Steel body reduces all incoming damage to 1/10 [helvetica]
  ...
  if (bl->type == BL_MOB)
  {
    // Reduces damage received for Green Aura MVP
...
{
  // Traps are always short range. [Akinari, Xynvaroth]
...
static void battle_calc_element_damage(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (!nk[NK_IGNOREELEMENT] && (wd->damage > 0 || wd->damage2 > 0))
  {
    ...
    default:
      ...
      if (sd == nullptr)
      {
        // Renewal player's elemental damage calculation is already done before this point, only calculate for everything else
#endif
        wd->damage = battle_attr_fix(src, target, wd->damage, right_element, tstatus->def_ele, tstatus->ele_lv);
        if (is_attack_left_handed(src, skill_id))
          wd->damage2 = battle_attr_fix(src, target, wd->damage2, left_element, tstatus->def_ele, tstatus->ele_lv);
    ...
    if (sd == nullptr)
    {
      // Only monsters have a single ATK for element, in pre-renewal we also apply element to entire ATK on players [helvetica]
#endif
...
static void battle_calc_attack_masteries(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
      ...
      skill_id != CR_GRANDCROSS)
  {
    ...
    {
      // !TODO: Confirm new mastery formula
    ...
    if (sc)
    {
      // Status change considered as masteries
#ifdef RENEWAL
...
static void battle_calc_multi_attack(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (sd && !skill_id)
  {
    ...
    short i;
    if (((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER) || (sd->bonus.double_rate > 0 && sd->weapontype1 != W_FIST) // Will fail bare-handed
...
static int battle_calc_attack_skill_ratio(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (sc && skill_id != PA_SACRIFICE)
  {
    ...
    if (!skill_id || skill_id == KN_AUTOCOUNTER)
    {
      ...
      {
        if (sc->data[SC_GIANTGROWTH] && (sd->class_ & MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT)
        {                          // Increase damage again if Crush Strike is not active
  ...
  case LK_JOINTBEAT:
    ...
    break;
#ifdef RENEWAL
  // Skill ratio applies to entire damage [helvetica]
  case LK_SPIRALPIERCE:
  case ML_SPIRALPIERCE:
    skillratio += 50 + 50 * skill_lv;
    RE_LVL_DMOD(100);
...
static void battle_calc_attack_plant(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (attack_hits && target->type == BL_MOB)
  {
    ...
    if (sc && !battle_status_block_damage(src, target, sc, wd, damage_dummy, skill_id, skill_lv))
    {
      // Statuses that reduce damage to 0.
...
static void battle_calc_attack_left_right_hands(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (sd)
  {
    ...
    else if (is_attack_right_handed(src, skill_id) && is_attack_left_handed(src, skill_id))
    {
      // Dual-wield
      if (wd->damage)
      {
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
  ...
  if (wd->damage && tsc && tsc->data[SC_REJECTSWORD] &&
      (src->type != BL_PC || (((TBL_PC *)src)->weapontype1 == W_DAGGER ||
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
  ...
  if (!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
    wd.dmg_lv = ATK_FLEE;
  else if (!(infdef = is_infinite_defense(target, wd.flag)))
  {
    // no need for math against plants
    int64 ratio = 0;
    int i = 0;
    ...
    {
      // Add weapon attack for MATK onto Magic Crasher
      struct status_data *sstatus = status_get_status_data(src);
  ...
  if (sd)
  {
    ...
    case RK_DRAGONBREATH_WATER:
      if (wd.flag & BF_LONG)
      {
        // Add check here, because we want to apply the same behavior in pre-renewal [exneval]
...
#define MATK_ADD(a) \
  {                 \
    ...
    if (sd)
    {
      ...
      // Ignore Defense?
      if (!flag.imdef && (sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) || sd->bonus.ignore_mdef_ele & (1 << ELE_ALL) ||
...
int64 battle_calc_return_damage(struct block_list *bl, struct block_list *src, int64 *dmg, int flag, uint16 skill_id, bool status_reflect)
{
  ...
  if (sc)
  {
    // These statuses do not reflect any damage (off the target)
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, t_tick tick, int flag)
{
  ...
  if (tsc)
  {
    if (tsc->data[SC_DEVOTION])
    {
      ...
      if (d_bl && ((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master && ((TBL_MER *)d_bl)->master->bl.id == target->id) || (d_bl->type == BL_PC && ((TBL_PC *)d_bl)->devotion[sce->val2] == target->id)) && check_distance_bl(target, d_bl, sce->val3))
      {
...
int battle_check_target(struct block_list *src, struct block_list *target, int flag)
{
  ...
  case BL_SKILL:
  {
    ...
    if (skill_get_inf2(su->group->skill_id, INF2_ISTRAP) && su->group->unit_id != UNT_USED_TRAPS)
    {
      ...
      else if (skill_get_inf2(skill_id, INF2_TARGETTRAP))
      {
...
static const struct _battle_data
{
  ...
  int max;
    ...
    {
        "finding_ore_rate",
        &battle_config.finding_ore_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "berserk_cancels_buffs",
        &battle_config.berserk_cancels_buffs,
        0,
        0,
        1,
    },
    ...
    {
        "emblem_transparency_limit",
        &battle_config.emblem_transparency_limit,
        80,
        0,
        100,
    },
