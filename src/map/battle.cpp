...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case DK_SERVANTWEAPON_ATK:
			skillratio += -100 + 450 + 800 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		...
		case DK_MADNESS_CRUSHER:
			skillratio += -100 + 1000 + 3800 * skill_lv + 10 * sstatus->pow;
			if( sd != nullptr ){
		...
		case DK_DRAGONIC_BREATH:
			...
			if (sc && sc->getSCE(SC_DRAGONIC_AURA)) {
				...
				skillratio += (skill_lv * (sstatus->max_hp / 4) * 5) / 100;	// Skill level x 0.05 x ((MaxHP / 4) + (MaxSP / 2))
				skillratio += (skill_lv * (status_get_max_sp(src) / 2) * 5) / 100;
			} else {
				skillratio += (skill_lv * (sstatus->max_hp / 4) * 7) / 100;	// Skill level x 0.07 x ((MaxHP / 4) + (MaxSP / 2))
				skillratio += (skill_lv * (status_get_max_sp(src) / 2) * 7) / 100;
			}
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, t_tick tick, int flag) {
	...
	if (sd) {
		...
		if( sc ){
			if( sc->getSCE( SC_SERVANTWEAPON ) && sd->servantball > 0 && rnd() % 100 < ( 5 * sc->getSCE( SC_SERVANTWEAPON )->val1 ) ){
