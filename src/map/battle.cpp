...
// Early declaration
int32 battle_get_weapon_element(struct Damage *wd,
								struct block_list *src,
								struct block_list *target,
								uint16 skill_id,
								uint16 skill_lv,
								short weapon_position,
								bool calc_for_damage_only);
int32 battle_get_magic_element(
	struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 mflag);
int32 battle_get_misc_element(
	struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 mflag);
static void battle_calc_defense_reduction(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv);
...
int32 battle_gettarget(struct block_list *bl) {
	switch (bl->type) {
		case BL_PC:
			return ((map_session_data *)bl)->ud.target;
		case BL_MOB:
		...
		case BL_MER:
			return ((s_mercenary_data *)bl)->ud.target;
		case BL_ELEM:
			return ((s_elemental_data *)bl)->ud.target;
	}
 ...
 * @return Target list
 */
struct block_list *battle_getenemyarea(
	struct block_list *src, int32 x, int32 y, int32 range, int32 type, int32 ignore_id) {
	struct block_list *bl_list[24];
	int32 c = 0;

	memset(bl_list, 0, sizeof(bl_list));
	map_foreachinallarea(
		battle_getenemyarea_sub, src->m, x - range, y - range, x + range, y + range, type, bl_list, &c, src, ignore_id);
 ...
 * All other instances of battle damage should also go through this function (i.e. anything that displays a damage
 * number) Consider calling this function or battle_fix_damage instead status_fix_damage directly
 ...
 */
int32 battle_damage(struct block_list *src,
					struct block_list *target,
					int64 damage,
					t_tick delay,
					uint16 skill_lv,
	...
	// This is the last place where we have access to the actual damage type, so any monster events depending on type
	// must be placed here
		...
		if (target && !status_isdead(*target)) {
				...
				check_distance_bl(src, target, dat->distance)) // Check to see if you haven't teleported. [Skotlex]
			{
				...
				battle_damage(src,
							  ...
							  dat->dmg_lv,
							  dat->attack_type,
							  dat->additional_effects,
							  tick,
							  dat->isspdamage);
			}
			else if (!src &&
					 dat->skill_id == CR_REFLECTSHIELD) { // it was monster reflected damage, and the monster died, we
														  // pass the damage to the character as expected
...
int32 battle_delay_damage(t_tick tick,
						  int32 amotion,
						  struct block_list *src,
						  struct block_list *target,
						  int32 attack_type,
	...
	if (((d_tbl && check_distance_bl(target, d_tbl, sc->getSCE(SC_DEVOTION)->val3)) || e_tbl) && damage > 0 &&
...
int32 battle_fix_damage(
	struct block_list *src, struct block_list *target, int64 damage, t_tick walkdelay, uint16 skill_id) {
 ...
 * @return damage
 */
int64 battle_attr_fix(struct block_list *src,
					  struct block_list *target,
					  int64 damage,
					  int32 atk_elem,
					  int32 def_type,
	...
	if (!CHK_ELEMENT(def_type) || !CHK_ELEMENT_LEVEL(def_lv)) {
		ShowError(
			"battle_attr_fix: unknown attribute type: atk=%d def_type=%d def_lv=%d\n", atk_elem, def_type, def_lv);
		return damage;
	}
 ...
 */
int32 battle_calc_cardfix(int32 attack_type,
						  struct block_list *src,
						  struct block_list *target,
...
#define APPLY_CARDFIX(damage, fix) \
	...
	#define APPLY_CARDFIX_RE(damage, fix) \
		{ (damage) = (damage) - (int64)(((damage) * (100 - max(0, 100 + (fix)))) / 100); }
				// On (at least) BF_MAGIC, damages are calculated consecutively and rounded down in the following order
				// to match official damage : size, race2, ele, atk_ele, race, class
				APPLY_CARDFIX_RE(
					damage, sd->indexed_bonus.magic_addsize[tstatus->size] + sd->indexed_bonus.magic_addsize[SZ_ALL]);
				...
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					APPLY_CARDFIX_RE(damage,
									 sd->indexed_bonus.magic_addele[tstatus->def_ele] +
										 sd->indexed_bonus.magic_addele[ELE_ALL] +
										 sd->indexed_bonus.magic_addele_script[tstatus->def_ele] +
										 sd->indexed_bonus.magic_addele_script[ELE_ALL]);
				}
			}
			// Statuses that affect the target's element and should be calculated right after magic_addele,
			// independently of it
			...
			if (sd && !nk[NK_IGNOREATKCARD]) {
				if (!nk[NK_IGNOREELEMENT]) {
					APPLY_CARDFIX_RE(
						damage, sd->indexed_bonus.magic_atk_ele[rh_ele] + sd->indexed_bonus.magic_atk_ele[ELE_ALL]);
				}
				APPLY_CARDFIX_RE(
					damage, sd->indexed_bonus.magic_addrace[tstatus->race] + sd->indexed_bonus.magic_addrace[RC_ALL]);
				APPLY_CARDFIX_RE(
					damage,
					sd->indexed_bonus.magic_addclass[tstatus->class_] + sd->indexed_bonus.magic_addclass[CLASS_ALL]);
...
// Pre-renewal / old renewal behaviour
				...
				cardfix = cardfix *
						  (100 + sd->indexed_bonus.magic_addrace[tstatus->race] +
						   sd->indexed_bonus.magic_addrace[RC_ALL] + race2_val) /
						  100;
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					cardfix = cardfix *
							  (100 + sd->indexed_bonus.magic_addele[tstatus->def_ele] +
							   sd->indexed_bonus.magic_addele[ELE_ALL] +
							   sd->indexed_bonus.magic_addele_script[tstatus->def_ele] +
							   sd->indexed_bonus.magic_addele_script[ELE_ALL]) /
							  100;
					cardfix =
						cardfix *
						(100 + sd->indexed_bonus.magic_atk_ele[rh_ele] + sd->indexed_bonus.magic_atk_ele[ELE_ALL]) /
						100;
				}
				cardfix =
					cardfix *
					(100 + sd->indexed_bonus.magic_addsize[tstatus->size] + sd->indexed_bonus.magic_addsize[SZ_ALL]) /
					100;
				cardfix = cardfix *
						  (100 + sd->indexed_bonus.magic_addclass[tstatus->class_] +
						   sd->indexed_bonus.magic_addclass[CLASS_ALL]) /
						  100;
				for (const auto &it : sd->add_mdmg) {
					if (it.id == t_class) {
			...
			if (tsd && !nk[NK_IGNOREDEFCARD]) {
				...
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					int32 ele_fix = tsd->indexed_bonus.subele[rh_ele] + tsd->indexed_bonus.subele[ELE_ALL] +
									tsd->indexed_bonus.subele_script[rh_ele] +
					...
					for (const auto &it : tsd->subele2) {
						...
						if (!(((it.flag) & flag) & BF_WEAPONMASK && ((it.flag) & flag) & BF_RANGEMASK &&
					...
					if (s_defele != ELE_NONE) {
						ele_fix +=
							tsd->indexed_bonus.magic_subdefele[s_defele] + tsd->indexed_bonus.magic_subdefele[ELE_ALL];
					}
#ifndef RENEWAL
				...
				cardfix = cardfix *
						  (100 - tsd->indexed_bonus.subsize[sstatus->size] - tsd->indexed_bonus.subsize[SZ_ALL]) / 100;
				cardfix =
					cardfix *
					(100 - tsd->indexed_bonus.magic_subsize[sstatus->size] - tsd->indexed_bonus.magic_subsize[SZ_ALL]) /
					100;
				...
				cardfix =
					cardfix *
					(100 - tsd->indexed_bonus.subclass[sstatus->class_] - tsd->indexed_bonus.subclass[CLASS_ALL]) / 100;
		...
		case BF_WEAPON:
			...
			if (sd && !nk[NK_IGNOREATKCARD] && (left & 2)) {
				...
				if (sd->state.arrow_atk) { // Ranged attack
					cardfix = cardfix *
							  (100 + sd->right_weapon.addrace[tstatus->race] +
							   sd->indexed_bonus.arrow_addrace[tstatus->race] + sd->right_weapon.addrace[RC_ALL] +
							   sd->indexed_bonus.arrow_addrace[RC_ALL]) /
							  100;
					if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
						int32 ele_fix = sd->right_weapon.addele[tstatus->def_ele] +
										sd->indexed_bonus.arrow_addele[tstatus->def_ele] +
					...
					cardfix = cardfix *
							  (100 + sd->right_weapon.addsize[tstatus->size] +
							   sd->indexed_bonus.arrow_addsize[tstatus->size] + sd->right_weapon.addsize[SZ_ALL] +
							   sd->indexed_bonus.arrow_addsize[SZ_ALL]) /
							  100;
					...
					cardfix = cardfix * (100 + race_fix) / 100;
					cardfix = cardfix *
							  (100 + sd->right_weapon.addclass[tstatus->class_] +
							   sd->indexed_bonus.arrow_addclass[tstatus->class_] +
							   sd->right_weapon.addclass[CLASS_ALL] + sd->indexed_bonus.arrow_addclass[CLASS_ALL]) /
							  100;
				}
				else { // Melee attack
					int32 skill = 0;
					...
					if (!battle_config.left_cardfix_to_right) {
						// Right-handed weapon
						cardfix = cardfix *
								  (100 + sd->right_weapon.addrace[tstatus->race] + sd->right_weapon.addrace[RC_ALL]) /
								  100;
						if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
							int32 ele_fix =
								sd->right_weapon.addele[tstatus->def_ele] + sd->right_weapon.addele[ELE_ALL];
						...
						cardfix = cardfix *
								  (100 + sd->right_weapon.addsize[tstatus->size] + sd->right_weapon.addsize[SZ_ALL]) /
								  100;
						for (const auto &raceit : t_race2) {
						...
						cardfix =
							cardfix *
							(100 + sd->right_weapon.addclass[tstatus->class_] + sd->right_weapon.addclass[CLASS_ALL]) /
							100;

						if (left & 1) { // Left-handed weapon
							cardfix_ =
								cardfix_ *
								(100 + sd->left_weapon.addrace[tstatus->race] + sd->left_weapon.addrace[RC_ALL]) / 100;
							if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
								int32 ele_fix_lh =
									sd->left_weapon.addele[tstatus->def_ele] + sd->left_weapon.addele[ELE_ALL];
							...
							cardfix_ =
								cardfix_ *
								(100 + sd->left_weapon.addsize[tstatus->size] + sd->left_weapon.addsize[SZ_ALL]) / 100;
							for (const auto &raceit : t_race2) {
								cardfix_ = cardfix_ * (100 + sd->left_weapon.addrace2[raceit]) / 100;
							}
							cardfix_ = cardfix_ *
									   (100 + sd->left_weapon.addclass[tstatus->class_] +
										sd->left_weapon.addclass[CLASS_ALL]) /
									   100;
						}
					}
					// Calculates right & left hand weapon as unity
						...
						int32 ele_fix = sd->right_weapon.addele[tstatus->def_ele] +
										sd->left_weapon.addele[tstatus->def_ele] + sd->right_weapon.addele[ELE_ALL] +
										sd->left_weapon.addele[ELE_ALL];
						...
						cardfix =
							cardfix *
							(100 + sd->right_weapon.addrace[tstatus->race] + sd->left_weapon.addrace[tstatus->race] +
							 sd->right_weapon.addrace[RC_ALL] + sd->left_weapon.addrace[RC_ALL]) /
							100;
						cardfix =
							cardfix *
							(100 + sd->right_weapon.addsize[tstatus->size] + sd->left_weapon.addsize[tstatus->size] +
							 sd->right_weapon.addsize[SZ_ALL] + sd->left_weapon.addsize[SZ_ALL]) /
							100;
						for (const auto &raceit : t_race2) {
							cardfix = cardfix *
									  (100 + sd->right_weapon.addrace2[raceit] + sd->left_weapon.addrace2[raceit]) /
									  100;
						}
						cardfix = cardfix *
								  (100 + sd->right_weapon.addclass[tstatus->class_] +
								   sd->left_weapon.addclass[tstatus->class_] + sd->right_weapon.addclass[CLASS_ALL] +
								   sd->left_weapon.addclass[CLASS_ALL]) /
								  100;
					}
#ifndef RENEWAL
					if (sd->status.weapon == W_KATAR &&
						(skill = pc_checkskill(sd, ASC_KATAR)) >
							0) { // Adv. Katar Mastery functions similar to a +%ATK card on official [helvetica]
			...
			// Affected by target DEF bonuses
				...
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					...
					if (left & 1 && lh_ele != rh_ele) {
						int32 ele_fix_lh = tsd->indexed_bonus.subele[lh_ele] + tsd->indexed_bonus.subele[ELE_ALL] +
										   tsd->indexed_bonus.subele_script[lh_ele] +
										   tsd->indexed_bonus.subele_script[ELE_ALL];
					...
					cardfix = cardfix *
							  (100 - tsd->indexed_bonus.subdefele[s_defele] - tsd->indexed_bonus.subdefele[ELE_ALL]) /
							  100;
				}
				...
				cardfix = cardfix *
						  (100 - tsd->indexed_bonus.subsize[sstatus->size] - tsd->indexed_bonus.subsize[SZ_ALL]) / 100;
				cardfix = cardfix *
						  (100 - tsd->indexed_bonus.weapon_subsize[sstatus->size] -
						   tsd->indexed_bonus.weapon_subsize[SZ_ALL]) /
						  100;
				for (const auto &raceit : s_race2) {
					race_fix += tsd->indexed_bonus.subrace2[raceit];
				}
 ...
 **/
bool battle_status_block_damage(struct block_list *src,
								struct block_list *target,
								status_change *sc,
								struct Damage *d,
								int64 &damage,
	...
	if ((sce = sc->getSCE(SC_GUARDIAN_S)) && damage > 0) {
		clif_specialeffect(
			target, EF_GUARD3, AREA); // Not official but we gotta show some way the barrier is working. [Rytech]
		sce->val2 -= static_cast<int>(cap_value(damage, INT_MIN, INT_MAX));
		if (flag & BF_WEAPON) {
			if (sce->val2 >= 0) {
				damage = 0;
	...
	if ((sc->getSCE(SC_PNEUMA) && (flag & (BF_MAGIC | BF_LONG)) == BF_LONG) ||
		...
		(sc->getSCE(SC_ZEPHYR) && !(flag & BF_MAGIC && skill_id) &&
		 !(skill_get_inf(skill_id) & (INF_GROUND_SKILL | INF_SELF_SKILL))) ||
		sc->getSCE(SC__MANHOLE) || sc->getSCE(SC_KINGS_GRACE) || sc->getSCE(SC_GRAVITYCONTROL)) {
		d->dmg_lv = ATK_BLOCK;
		return false;
	}

	if (sc->getSCE(SC_WHITEIMPRISON)) { // Gravitation and Pressure do damage without removing the effect
		if (skill_id == MG_NAPALMBEAT || skill_id == MG_SOULSTRIKE || skill_id == WL_SOULEXPANSION ||
	...
	// ATK_MISS Type
	if ((sce = sc->getSCE(SC_AUTOGUARD)) && flag & BF_WEAPON && rnd() % 100 < sce->val2 &&
		!skill_get_inf2(skill_id, INF2_IGNOREAUTOGUARD)) {
		...
		if (sce_d && (d_bl = map_id2bl(sce_d->val1)) &&
			...
			check_distance_bl(target, d_bl, sce_d->val3)) { // If player is target of devotion, show guard effect on the
															// devotion caster rather than the target
	...
	if ((sce = sc->getSCE(SC_PARRYING)) && flag & BF_WEAPON && skill_id != WS_CARTTERMINATION &&
		rnd() % 100 < sce->val2) {
	...
	if (sc->getSCE(SC_DODGE) && (flag & BF_LONG || sc->getSCE(SC_SPURT)) &&
		(skill_id != NPC_EARTHQUAKE || (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) &&
		rnd() % 100 < 20) {
		map_session_data *sd = map_id2sd(target->id);
	...
	if (((sce = sc->getSCE(SC_UTSUSEMI)) || sc->getSCE(SC_BUNSINJYUTSU)) && flag & BF_WEAPON &&
		!skill_get_inf2(skill_id, INF2_IGNORECICADA)) {
 ...
 * @return damage
 */
int64 battle_calc_damage(
	struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv) {
	...
	if (tsc != nullptr && !tsc->empty()) {
		...
		if (tsc->getSCE(SC_DEFENDER) && skill_id != NJ_ZENYNAGE && skill_id != KO_MUCHANAGE &&
#ifdef RENEWAL
		...
		if (tsc->getSCE(SC_FOGWALL) && skill_id != RK_DRAGONBREATH && skill_id != RK_DRAGONBREATH_WATER &&
			skill_id != NPC_DRAGONBREATH) {
		...
		if (skill_id == RA_AIMEDBOLT &&
			(tsc->getSCE(SC_BITE) || tsc->getSCE(SC_ANKLE) || tsc->getSCE(SC_ELECTRICSHOCKER))) {
		...
		if (tsc->getSCE(SC__DEADLYINFECT) && (flag & (BF_SHORT | BF_MAGIC)) == BF_SHORT && damage > 0 &&
			rnd() % 100 < 30 + 10 * tsc->getSCE(SC__DEADLYINFECT)->val1) {
		...
		if (sc->getSCE(SC_POISONINGWEAPON) && flag & BF_SHORT && damage > 0) {
			...
			if (rnd() % 100 < sc->getSCE(SC_POISONINGWEAPON)->val3) {
				sc_start4(
					src,
					bl,
					(sc_type)sc->getSCE(SC_POISONINGWEAPON)->val2,
					100,
					sc->getSCE(SC_POISONINGWEAPON)->val1,
					0,
					1,
					0,
					(sc->getSCE(SC_POISONINGWEAPON)->val2 == SC_VENOMBLEED ? skill_get_time2(GC_POISONINGWEAPON, 1)
																		   : skill_get_time2(GC_POISONINGWEAPON, 2)));
			}
		}

		if (sc->getSCE(SC__DEADLYINFECT) && (flag & (BF_SHORT | BF_MAGIC)) == BF_SHORT && damage > 0 &&
			rnd() % 100 < 30 + 10 * sc->getSCE(SC__DEADLYINFECT)->val1) {
	...
	if (sd && pc_ismadogear(sd)) {
		pc_overheat(*sd,
					(battle_get_weapon_element(d, src, bl, skill_id, skill_lv, EQI_HAND_R, false) == ELE_FIRE ? 3 : 1));
	}

	// Target status (again), required for RELIEVE
	sc = status_get_sc(bl);

	// !TODO: Should RELIEVE needed to be down here or after some other check? Should the skill be independent of
	// damagetaken from mob_db.yml?
	if (sc != nullptr && !sc->empty()) {
	...
	if (tsc != nullptr && !tsc->empty()) {
		if (!battle_status_block_damage(
				src, bl, tsc, d, damage, skill_id, skill_lv)) { // Statuses that reduce damage to 0.
			return 0;
		}
	}
...
bool battle_can_hit_gvg_target(struct block_list *src, struct block_list *bl, uint16 skill_id, int32 flag) {
	...
	if (md && (md->guardian_data || md->special_state.ai == AI_GUILD)) {
		if ((status_bl_has_mode(bl, MD_SKILLIMMUNE) ||
			 (class_ == MOBID_EMPERIUM && !skill_get_inf2(skill_id, INF2_TARGETEMPERIUM))) &&
			flag & BF_SKILL) { // Skill immunity.
			return false;
		}
		if (src->type != BL_MOB || mob_is_clone(((struct mob_data *)src)->mob_id)) {
			...
			if (g != nullptr) {
				if (battle_config.guild_max_castles &&
					guild_checkcastles(g->guild) >= battle_config.guild_max_castles) {
...
int64 battle_addmastery(map_session_data *sd, struct block_list *target, int64 dmg, int32 type) {
	...
	if ((skill = pc_checkskill(sd, RA_RANGERMAIN)) > 0 &&
		(status->race == RC_BRUTE || status->race == RC_PLAYER_DORAM || status->race == RC_PLANT ||
		 status->race == RC_FISH)) {
		damage += (skill * 5);
	}
	if ((skill = pc_checkskill(sd, NC_RESEARCHFE)) > 0 &&
		(status->def_ele == ELE_FIRE || status->def_ele == ELE_EARTH)) {
		damage += (skill * 10);
	}
	...
	if ((skill = pc_checkskill(sd, HT_BEASTBANE)) > 0 &&
		(status->race == RC_INSECT || status->race == RC_BRUTE || status->race == RC_PLAYER_DORAM)) {
		damage += (skill * 4);
...
#ifdef RENEWAL
static int32 battle_calc_sizefix(
	int64 damage, map_session_data *sd, unsigned char t_size, unsigned char weapon_type, short flag) {
	if (sd && !sd->special_state.no_sizefix && !flag) { // Size fix only for players
		damage = damage *
				 (weapon_type == EQI_HAND_L ? sd->left_weapon.atkmods[t_size] : sd->right_weapon.atkmods[t_size]) / 100;
	}
 ...
 */
static int32 battle_calc_base_weapon_attack(
	struct block_list *src, struct status_data *tstatus, struct weapon_atk *wa, map_session_data *sd, bool critical) {
 ...
 * &16: Arrow attack but BOW, REVOLVER, RIFLE, SHOTGUN, GATLING or GRENADE type weapon not equipped (i.e. shuriken,
 *kunai and venom knives not affected by DEX)
 *
 * Credits:
 *	Original coder Skotlex
 *	Initial refactoring by Baalberith
 *	Refined and optimized by helvetica
 */
static int64 battle_calc_base_damage(struct block_list *src,
	...
	if (!sd) { // Mobs/Pets
		...
		if (!(flag & BDMG_CRIT) || (flag & BDMG_ARROW)) { // Normal attacks
			...
			if (sd->equip_index[type] >= 0 && sd->inventory_data[sd->equip_index[type]] &&
				sd->inventory_data[sd->equip_index[type]]->type == IT_WEAPON) {
	...
	if (sd) {
		...
		if (!(sd->special_state.no_sizefix || (flag & BDMG_NOSIZE))) {
			damage = damage *
					 (type == EQI_HAND_L ? sd->left_weapon.atkmods[t_size] : sd->right_weapon.atkmods[t_size]) / 100;
		}
	}
...
static int32 battle_skill_damage_skill(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	if ((damage->map & 1 &&
		 (!mapdata->getMapFlag(MF_PVP) && !mapdata_flag_gvg2(mapdata) && !mapdata->getMapFlag(MF_BATTLEGROUND) &&
		  !mapdata->getMapFlag(MF_SKILL_DAMAGE) && !mapdata->getMapFlag(MF_RESTRICTED))) ||
		(damage->map & 2 && mapdata->getMapFlag(MF_PVP)) || (damage->map & 4 && mapdata_flag_gvg2(mapdata)) ||
...
static int32 battle_skill_damage_map(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	if (mapdata->skill_damage.find(skill_id) != mapdata->skill_damage.end() &&
		mapdata->skill_damage[skill_id].caster & src->type) {
...
bool is_infinite_defense(struct block_list *target, int32 flag) {
	...
	if (target->type == BL_SKILL) {
		...
		if (su && su->group &&
			(su->group->skill_id == NPC_REVERBERATION || su->group->skill_id == WM_POEMOFNETHERWORLD)) {
			return true;
		}
	}
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_critical(struct Damage *wd,
							   struct block_list *src,
							   struct block_list *target,
							   int32 skill_id,
							   int32 skill_lv,
 ...
 *	Refined and optimized by helvetica
 */
static int32 is_attack_piercing(struct Damage *wd,
								struct block_list *src,
								struct block_list *target,
								int32 skill_id,
								int32 skill_lv,
	...
	if (src != nullptr) {
		...
		if (skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS &&
			...
			if (sd && (sd->left_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) ||
					   sd->left_weapon.def_ratio_atk_ele & (1 << ELE_ALL) ||
					   sd->left_weapon.def_ratio_atk_race & (1 << tstatus->race) ||
					   sd->left_weapon.def_ratio_atk_race & (1 << RC_ALL) ||
					   sd->left_weapon.def_ratio_atk_class & (1 << tstatus->class_) ||
					   sd->left_weapon.def_ratio_atk_class &
						   (1 << CLASS_ALL))) { // Pass effect onto right hand if configured so. [Skotlex]
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_hitting(struct Damage *wd,
							  struct block_list *src,
							  struct block_list *target,
							  int32 skill_id,
							  int32 skill_lv,
	...
	else if (skill_id == CR_SHIELDBOOMERANG && sc && sc->getSCE(SC_SPIRIT) &&
			 sc->getSCE(SC_SPIRIT)->val2 == SL_CRUSADER) {
		return true;
	}
	...
	if (battle_config.agi_penalty_type && battle_config.agi_penalty_target & target->type) {
		...
		if (attacker_count >= battle_config.agi_penalty_count) {
			if (battle_config.agi_penalty_type == 1) {
				flee = (flee * (100 - (attacker_count - (battle_config.agi_penalty_count - 1)) *
	...
	else if (sd && wd->type & DMG_MULTI_HIT &&
			 wd->div_ == 2) { // +1 hit per level of Double Attack on a successful double attack (making sure other
							  // multi attack skills do not trigger this) [helvetica]
 ...
 *	Refined and optimized by helvetica
 */
static bool attack_ignores_def(struct Damage *wd,
							   struct block_list *src,
							   struct block_list *target,
							   uint16 skill_id,
							   uint16 skill_lv,
	...
	if (skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS) { // Ignore Defense?
		if (sd && (sd->right_weapon.ignore_def_ele & (1 << tstatus->def_ele) ||
				   sd->right_weapon.ignore_def_ele & (1 << ELE_ALL) ||
				   sd->right_weapon.ignore_def_race & (1 << tstatus->race) ||
				   sd->right_weapon.ignore_def_race & (1 << RC_ALL) ||
				   sd->right_weapon.ignore_def_class & (1 << tstatus->class_) ||
				   sd->right_weapon.ignore_def_class & (1 << CLASS_ALL))) {
			if (weapon_position == EQI_HAND_R) {
				return true;
			}
		}
		...
		if (sd && (sd->left_weapon.ignore_def_ele & (1 << tstatus->def_ele) ||
				   sd->left_weapon.ignore_def_ele & (1 << ELE_ALL) ||
				   sd->left_weapon.ignore_def_race & (1 << tstatus->race) ||
				   sd->left_weapon.ignore_def_race & (1 << RC_ALL) ||
				   sd->left_weapon.ignore_def_class & (1 << tstatus->class_) ||
				   sd->left_weapon.ignore_def_class & (1 << CLASS_ALL))) {
			if (battle_config.left_cardfix_to_right &&
				is_attack_right_handed(src, skill_id)) { // Move effect to right hand. [Skotlex]
				if (weapon_position == EQI_HAND_R) {
					return true;
				}
			}
...
static int32 battle_calc_equip_attack(struct block_list *src, int32 skill_id) {
	if (src != nullptr) {
		...
		if (sd != nullptr && is_skill_using_arrow(src, skill_id)) {
			...
			if (ammo_idx >= 0 && sd->inventory_data[ammo_idx] != nullptr &&
				sd->inventory_data[ammo_idx]->subtype != AMMO_CANNONBALL) {
...
int32 battle_get_weapon_element(struct Damage *wd,
	...
	if (sc && sc->getSCE(SC_GOLDENE_FERSE) &&
		((!skill_id && (rnd() % 100 < sc->getSCE(SC_GOLDENE_FERSE)->val4)) || skill_id == MH_STAHL_HORN)) {
		element = ELE_HOLY;
	}
...
int32 battle_get_magic_element(
	...
	switch (skill_id) {
		...
		case SO_PSYCHIC_WAVE:
			if (sc != nullptr && !sc->empty()) {
				static const std::vector<sc_type> types = {SC_HEATER_OPTION,
														   SC_COOLER_OPTION,
														   SC_BLAST_OPTION,
...
int32 battle_get_misc_element(
	...
	if (element == ELE_WEAPON ||
		element == ELE_ENDOWED) { // Attack that takes weapon's element for misc attacks? Make it neutral [Skotlex]
		element = ELE_NEUTRAL;
	}
	else if (element == ELE_RANDOM) { // Use random element
		element = rnd() % ELE_ALL;
 ...
 * As we delete the spheres before calculating the damage, we need some kind of logic to figure out how many were
 * available Each skill handles this in its own way, this function handles the different cases
 * @param wd: Weapon damage structure
 * @param src: Source of the attack
 * @param skill_id: Skill ID of the skill used by source
 * @param min: Minimum value to which damage should be capped
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_element_damage(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	std::bitset<NK_MAX> nk = battle_skill_get_damage_properties(skill_id, wd->miscflag);
	map_session_data *sd = BL_CAST(BL_PC, src);
	status_change *sc = status_get_sc(src);
	...
	if (!nk[NK_IGNOREELEMENT] && (wd->damage > 0 || wd->damage2 > 0)) {
		...
		switch (skill_id) {
			...
			case HFLI_SBR44:
				wd->damage =
					battle_attr_fix(src, target, wd->damage, right_element, tstatus->def_ele, tstatus->ele_lv, 1);
				if (is_attack_left_handed(src, skill_id)) {
					wd->damage2 =
						battle_attr_fix(src, target, wd->damage2, left_element, tstatus->def_ele, tstatus->ele_lv, 1);
				}
				break;
			default:
...
#ifdef RENEWAL
				if (sd == nullptr) { // Renewal player's elemental damage calculation is already done before this point,
									 // only calculate for everything else
#endif
					wd->damage =
						battle_attr_fix(src, target, wd->damage, right_element, tstatus->def_ele, tstatus->ele_lv, 1);
					if (is_attack_left_handed(src, skill_id)) {
						wd->damage2 = battle_attr_fix(
							src, target, wd->damage2, left_element, tstatus->def_ele, tstatus->ele_lv, 1);
					}
#ifdef RENEWAL
				}
#endif
		...
		// Skills forced to neutral gain benefits from weapon element but final damage is considered "neutral" and
		// resistances are applied again
		switch (skill_id) {
#ifdef RENEWAL
			case MO_INVESTIGATE:
			case MO_EXTREMITYFIST:
			...
			case GN_FIRE_EXPANSION_ACID:
				wd->damage =
					battle_attr_fix(src, target, wd->damage, ELE_NEUTRAL, tstatus->def_ele, tstatus->ele_lv, 1);
				if (is_attack_left_handed(src, skill_id)) {
					wd->damage2 =
						battle_attr_fix(src, target, wd->damage2, ELE_NEUTRAL, tstatus->def_ele, tstatus->ele_lv, 1);
				}
				break;
		}
	}
	...
	// If damage was reduced below 0 and was not increased again to a positive value through mastery bonuses, these
	// bonuses are ignored Any of these are only applied to your right hand weapon in pre-renewal
	if (!nk[NK_IGNOREELEMENT] && !nk[NK_IGNOREATKCARD] && (wd->damage > 0 || wd->damage2 > 0) && sc) {
		...
		if (sd && sc->getSCE(SC_WATK_ELEMENT)) {
			...
			wd->basedamage = battle_attr_fix(
				src, target, wd->basedamage, sc->getSCE(SC_WATK_ELEMENT)->val1, tstatus->def_ele, tstatus->ele_lv, 1);
			// Star Crumb bonus damage
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_masteries(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	status_change *sc = status_get_sc(src);
	status_data *sstatus = status_get_status_data(*src);
			...
			case NC_ARMSCANNON:
				...
				if (sd != nullptr) {
					...
					ATK_ADD(
						...
						wd->masteryAtk2,
						battle_attr_fix(
							src, target, sd->bonus.arrow_atk, sd->bonus.arrow_ele, tstatus->def_ele, tstatus->ele_lv));
				}
				break;
#endif
		}
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_damage_parts(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	status_data *sstatus = status_get_status_data(*src);
	status_data *tstatus = status_get_status_data(*target);
	map_session_data *sd = BL_CAST(BL_PC, src);
	...
	if (sd &&
		sd->sc.getSCE(SC_SEVENWIND)) { // Mild Wind applies element to status ATK as well as weapon ATK [helvetica]
		wd->statusAtk =
			battle_attr_fix(src, target, wd->statusAtk, sstatus->rhw.ele, tstatus->def_ele, tstatus->ele_lv);
		wd->statusAtk2 =
			battle_attr_fix(src, target, wd->statusAtk2, sstatus->lhw.ele, tstatus->def_ele, tstatus->ele_lv);
	}
	...
	if (sd && sd->sc.getSCE(SC_SUB_WEAPONPROPERTY)) {
		...
		bonus_atk = battle_attr_fix(
			src, target, bonus_atk, sd->sc.getSCE(SC_SUB_WEAPONPROPERTY)->val1, tstatus->def_ele, tstatus->ele_lv);
		bonus_atk2 = battle_attr_fix(
			src, target, bonus_atk2, sd->sc.getSCE(SC_SUB_WEAPONPROPERTY)->val1, tstatus->def_ele, tstatus->ele_lv);
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_skill_base_damage(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	status_change *sc = status_get_sc(src);
	status_data *sstatus = status_get_status_data(*src);
	status_data *tstatus = status_get_status_data(*target);
	...
	switch (skill_id) { // Calc base damage according to skill
		...
		case ML_SPIRALPIERCE:
			if (sd) {
				...
				wd->damage = battle_calc_base_damage(
					src, sstatus, &sstatus->rhw, sc, tstatus->size, 0); // Monsters have no weight and use ATK instead
			}
#else
		case NJ_ISSEN:
		...
		case RK_DRAGONBREATH_WATER: {
			...
			if (sd) {
				if (pc_checkskill(sd, DK_DRAGONIC_AURA) >= 1) {
					damagevalue =
						damagevalue * (90 + 10 * pc_checkskill(sd, RK_DRAGONTRAINING) + sstatus->pow / 5) / 100;
				}
				else {
		...
		case NC_SELFDESTRUCTION: {
			int32 damagevalue =
				(skill_lv + 1) * ((sd ? pc_checkskill(sd, NC_MAINFRAME) : 0) + 8) * (status_get_sp(src) + sstatus->vit);
				...
				int32 damagevalue =
					...
					(
						...
						2 *
							...
							sstatus->batk +
						sstatus->rhw.atk +
						(index >= 0 && sd->inventory_data[index] ? sd->inventory_data[index]->atk : 0)) *
					(skill_lv + 5) / 5;
		...
		default:
			...
			if (is_skill_using_arrow(src, skill_id) && sd) {
				switch (sd->status.weapon) {
					...
					case W_GRENADE:
						...
						break;
					default:
						// Attacks that use ammo are calculated like melee attacks as long as no ranged weapon is
						// equipped Some skills manually add arrow_atk as mastery bonus later (e.g. Throw Shuriken,
						// Throw Kunai)
						break;
				}
			}
			...
			if (nk[NK_SPLASHSPLIT]) { // Divide ATK among targets
				if (wd->miscflag > 0) {
					...
					ShowError(
						"0 enemies targeted by %d:%s, divide per 0 avoided!\n", skill_id, skill_get_name(skill_id));
				}
			}
			...
			if (sd) {
				...
				if (sd->bonus.crit_atk_rate &&
					(bflag & BDMG_CRIT)) { // add +crit damage bonuses here in pre-renewal mode [helvetica]
					ATK_ADDRATE(wd->damage, wd->damage2, sd->bonus.crit_atk_rate);
				}
				if (sd->status.party_id && (skill = pc_checkskill(sd, TK_POWER)) > 0) {
					if ((i = party_foreachsamemap(party_sub_count, sd, 0)) >
						1) { // exclude the player himself [Inkfish]
...
static void battle_apply_div_fix(struct Damage *d, uint16 skill_id) {
	if (d->damage) {
		...
		// Min damage
		if (d->damage < d->div_ &&
			(skill_id == SU_LUNATICCARROTBEAT || skill_id == SU_LUNATICCARROTBEAT2 || skill_id == SU_CN_METEOR ||
			 skill_id == SU_CN_METEOR2 || (battle_config.skill_min_damage & d->flag))) {
			d->damage = d->div_;
		}
	}
	else if (d->div_ < 0) {
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_multi_attack(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	status_change *sc = status_get_sc(src);
	status_change *tsc = status_get_sc(target);
	...
	if (sd && !skill_id) { // if no skill_id passed, check for double attack [helvetica]
		short i;
		if (sc && sc->getSCE(SC_FEARBREEZE) && sd->weapontype1 == W_BOW && (i = sd->equip_index[EQI_AMMO]) >= 0 &&
			sd->inventory_data[i] && sd->inventory.u.items_inventory[i].amount > 1) {
		...
		if (wd->div_ == 1 &&
			((sd->weapontype1 == W_REVOLVER &&
			  (skill_lv = pc_checkskill(sd, GS_CHAINACTION)) > 0) // Normal Chain Action effect
			 || (sc != nullptr && !sc->empty() && sc->getSCE(SC_E_CHAIN) &&
				 (skill_lv = sc->getSCE(SC_E_CHAIN)->val1) > 0)) // Chain Action of ETERNAL_CHAIN
 ...
 * This bonus works as a separate unit to the rest (e.g., if one of these is not applied to a skill, then we know none
 * are) Do not add additional status changes here unless they are confirmed to use ATKpercent
 ...
 *	Refined and optimized by helvetica
 */
static int32 battle_calc_attack_skill_ratio(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	map_session_data *tsd = BL_CAST(BL_PC, target);
	status_change *sc = status_get_sc(src);
			...
			if (sc->getSCE(SC_CRUSHSTRIKE)) {
				if (sd) { // ATK [{Weapon Level * (Weapon Upgrade Level + 6) * 100} + (Weapon ATK) + (Weapon Weight)]%
					...
					if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON) {
						skillratio += -100 + sd->inventory_data[index]->weight / 10 + sd->inventory_data[index]->atk +
									  100 * sd->inventory_data[index]->weapon_level *
										  (sd->inventory.u.items_inventory[index].refine + 6);
					}
				}
				...
				if (sc->getSCE(SC_GIANTGROWTH) &&
					(sd->class_ & MAPID_THIRDMASK) ==
						MAPID_RUNE_KNIGHT) { // Increase damage again if Crush Strike is not active
	...
	switch (skill_id) {
		...
		case LK_JOINTBEAT:
			...
			if (wd->miscflag & BREAK_NECK ||
				(tsc && tsc->getSCE(SC_JOINTBEAT) &&
				 tsc->getSCE(SC_JOINTBEAT)->val2 & BREAK_NECK)) { // The 2x damage is only for the BREAK_NECK ailment.
				skillratio *= 2;
			}
			break;
#ifdef RENEWAL
		...
		case PA_SHIELDCHAIN:
			...
			if (sd != nullptr) {
				...
				if (index >= 0 && sd->inventory_data[index] != nullptr && sd->inventory_data[index]->type == IT_ARMOR) {
					skillratio +=
						sd->inventory_data[index]->weight / 10 + 4 * sd->inventory.u.items_inventory[index].refine;
				}
		...
		case GS_BULLSEYE:
			...
			if ((tstatus->race == RC_BRUTE || tstatus->race == RC_DEMIHUMAN || tstatus->race == RC_PLAYER_HUMAN ||
				 tstatus->race == RC_PLAYER_DORAM) &&
		...
		case NPC_IGNITIONBREAK:
			...
			break;
		case RK_STORMBLAST:
			skillratio += -100 + (((sd) ? pc_checkskill(sd, RK_RUNEMASTERY) : 0) + sstatus->str / 6) *
									 100; // ATK = [{Rune Mastery Skill Level + (Caster's STR / 6)} x 100] %
			RE_LVL_DMOD(100);
			break;
		...
		case GC_COUNTERSLASH:
			// ATK [{(Skill Level x 150) + 300} x Caster's Base Level / 120]% + ATK [(AGI x 2) + (Caster's Job Level x
			// 4)]%
		...
		case NPC_DRAGONBREATH:
			if (skill_lv > 5) {
				skillratio +=
					500 + 500 * (skill_lv - 5); // Level 6-10 is using water element, like RK_DRAGONBREATH_WATER
			}
			else {
		...
		case NC_AXEBOOMERANG:
			...
			if (sd) {
				...
				if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON) {
					skillratio +=
						sd->inventory_data[index]->weight /
						10; // Weight is divided by 10 since 10 weight in coding make 1 whole actual weight. [Rytech]
				}
			}
			RE_LVL_DMOD(100);
			break;
		...
		case LG_HESPERUSLIT:
			...
			break;
		case SR_EARTHSHAKER:
			if (tsc && ((tsc->option & (OPTION_HIDE | OPTION_CLOAK | OPTION_CHASEWALK)) || tsc->getSCE(SC_CAMOUFLAGE) ||
						tsc->getSCE(SC_STEALTHFIELD) || tsc->getSCE(SC__SHADOWFORM))) {
		...
		case SR_FALLENEMPIRE:
			...
			break;
		case SR_TIGERCANNON: {
			uint32 hp = sstatus->max_hp * (10 + (skill_lv * 2)) / 100, sp = sstatus->max_sp * (5 + skill_lv) / 100;
		...
		case SR_KNUCKLEARROW:
			if (wd->miscflag & 4) { // ATK [(Skill Level x 150) + (1000 x Target current weight / Maximum weight) +
									// (Target Base Level x 5) x (Caster Base Level / 150)] %
		...
		case WM_GREAT_ECHO:
			...
			RE_LVL_DMOD(100);
			break;
		case GN_CART_TORNADO: { // ATK [( Skill Level x 200 ) + ( Cart Weight / ( 150 - Caster Base STR ))] + ( Cart
								// Remodeling Skill Level x 50 )] %
			skillratio += -100 + 200 * skill_lv;
			if (sd && sd->cart_weight) {
				skillratio += sd->cart_weight / 10 / (150 - min(sd->status.str, 120)) +
							  pc_checkskill(sd, GN_REMODELING_CART) * 50;
			}
		...
		case GN_CARTCANNON:
			skillratio += -100 + (250 + 20 * pc_checkskill(sd, GN_REMODELING_CART)) * skill_lv +
						  2 * sstatus->int_ / (6 - pc_checkskill(sd, GN_REMODELING_CART));
			RE_LVL_DMOD(100);
			break;
		...
		case GN_HELLS_PLANT_ATK:
			skillratio += -100 + 100 * skill_lv +
						  sstatus->int_ *
							  (sd ? pc_checkskill(sd, AM_CANNIBALIZE) : 5); // !TODO: Confirm INT and Cannibalize bonus
			RE_LVL_DMOD(100);
			break;
		...
		case KO_SETSUDAN:
			...
			if (tsc) {
				...
				if ((sce = tsc->getSCE(SC_SPIRIT)) || (sce = tsc->getSCE(SC_SOULGOLEM)) ||
					(sce = tsc->getSCE(SC_SOULSHADOW)) || (sce = tsc->getSCE(SC_SOULFALCON)) ||
					(sce = tsc->getSCE(SC_SOULFAIRY))) { // Bonus damage added when target is soul linked.
		...
		case KO_BAKURETSU:
			skillratio +=
				-100 + (sd ? pc_checkskill(sd, NJ_TOBIDOUGU) : 1) * (50 + sstatus->dex / 4) * skill_lv * 4 / 10;
			RE_LVL_DMOD(120);
		...
		case MH_NEEDLE_OF_PARALYZE:
			skillratio += -100 + 450 * skill_lv * status_get_lv(src) / 100 +
						  sstatus->dex; // !TODO: Confirm Base Level and DEX bonus
			break;
		case MH_TOXIN_OF_MANDARA:
			skillratio += -100 + 400 + 450 * skill_lv * status_get_lv(src) / 100 +
						  sstatus->dex; // !TODO: Confirm Base Level and DEX bonus
			break;
		case MH_NEEDLE_STINGER:
			skillratio += -100 + 200 + 500 * skill_lv * status_get_lv(src) / 100 +
						  sstatus->dex; // !TODO: Confirm Base Level and DEX bonus
			break;
		case MH_STAHL_HORN:
			skillratio +=
				-100 + 1000 + 300 * skill_lv * status_get_lv(src) / 150 + sstatus->vit; // !TODO: Confirm VIT bonus
			break;
		case MH_GLANZEN_SPIES:
			skillratio +=
				-100 + 300 + 450 * skill_lv * status_get_lv(src) / 100 + sstatus->vit; // !TODO: Confirm VIT bonus
			break;
		case MH_LAVA_SLIDE:
			skillratio += -100 + 50 * skill_lv;
			break;
		...
		case DK_DRAGONIC_BREATH:
			...
			if (sc && sc->getSCE(SC_DRAGONIC_AURA)) {
				...
				skillratio += (skill_lv * (sstatus->max_hp * 25 / 100) * 7) /
							  100; // Skill level x 0.07 x ((MaxHP / 4) + (MaxSP / 2))
				skillratio += (skill_lv * (sstatus->max_sp * 50 / 100) * 7) / 100;
			}
			else {
				skillratio += (skill_lv * (sstatus->max_hp * 25 / 100) * 5) /
							  100; // Skill level x 0.05 x ((MaxHP / 4) + (MaxSP / 2))
 ...
 *	Refined and optimized by helvetica
 */
static int64 battle_calc_skill_constant_addition(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	map_session_data *tsd = BL_CAST(BL_PC, target);
	status_data *sstatus = status_get_status_data(*src);
		...
		case PA_SHIELDCHAIN:
			if (sd) {
				...
				if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_ARMOR) {
					...
					bonus +=
						static_cast<decltype(bonus)>(pow(skill_lv + sd->inventory.u.items_inventory[index].refine, 2));
 ...
 *	Refined and optimized by helvetica
 */
static void battle_attack_sc_bonus(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	status_change *sc = status_get_sc(src);
	status_data *sstatus = status_get_status_data(*src);
	...
	if (sc) {
		...
		if (sc->getSCE(SC_MAGICALBULLET)) {
			...
			if (sstatus->matk_min > tmdef && sstatus->matk_max > sstatus->matk_min) {
				ATK_ADD(wd->weaponAtk,
						wd->weaponAtk2,
						i64max((sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min)) - tmdef, 0));
			}
			else {
		...
		if (sc->getSCE(SC_SPIRIT)) {
			if (skill_id == AS_SONICBLOW && sc->getSCE(SC_SPIRIT)->val2 == SL_ASSASIN) {
				ATK_ADDRATE(
					wd->damage, wd->damage2, map_flag_gvg2(src->m) ? 25 : 100); //+25% dmg on woe/+100% dmg on nonwoe
		...
		if (sc->getSCE(SC_DANCEWITHWUG)) {
			if (skill_get_inf2(skill_id, INF2_INCREASEDANCEWITHWUGDAMAGE)) {
				ATK_ADDRATE(
					wd->damage, wd->damage2, sc->getSCE(SC_DANCEWITHWUG)->val1 * 10 * battle_calc_chorusbonus(sd));
...
#ifdef RENEWAL
			ATK_ADDRATE(
				wd->equipAtk, wd->equipAtk2, sc->getSCE(SC_DANCEWITHWUG)->val1 * 2 * battle_calc_chorusbonus(sd));
#endif
		}
...
static void battle_calc_defense_reduction(
	...
	if (battle_config.vit_penalty_type && battle_config.vit_penalty_target & target->type) {
		...
		target_count = min(unit_counttargeted(target),
						   (100 / battle_config.vit_penalty_num) + (battle_config.vit_penalty_count - 1));
		if (target_count >= battle_config.vit_penalty_count) {
			if (battle_config.vit_penalty_type == 1) {
				if (!tsc || !tsc->getSCE(SC_STEELBODY)) {
					def1 = (def1 * (100 - (target_count - (battle_config.vit_penalty_count - 1)) *
											  battle_config.vit_penalty_num)) /
						   100;
				}
				def2 = (def2 * (100 - (target_count - (battle_config.vit_penalty_count - 1)) *
										  battle_config.vit_penalty_num)) /
					   100;
			}
			else { // Assume type 2
...
#endif
		if (src->type == BL_MOB &&
			(battle_check_undead(sstatus->race, sstatus->def_ele) ||
			 sstatus->race == RC_DEMON) && // This bonus already doesn't work vs players
			(skill = pc_checkskill(tsd, AL_DP)) > 0) {
			vit_def += (int32)(((float)tsd->status.base_level / 25.0 + 3.0) * skill + 0.5);
		}
		if (src->type == BL_MOB && (skill = pc_checkskill(tsd, RA_RANGERMAIN)) > 0 &&
			(sstatus->race == RC_BRUTE || sstatus->race == RC_PLAYER_DORAM || sstatus->race == RC_FISH ||
			 sstatus->race == RC_PLANT)) {
			vit_def += skill * 5;
		}
	...
	if (nk[NK_SIMPLEDEFENSE]) {
		...
		ATK_ADD2(wd->damage,
				 ...
				 is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
					 ? (def1 * battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv)) / 200
					 : 0,
	...
	ATK_RATE2(wd->damage,
			  ...
			  attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
				  ? 100
				  : (is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
						 ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R) * (def1 + vit_def)
						 : (100 - def1)),
			  attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
				  ? 100
				  : (is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
						 ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) * (def1 + vit_def)
						 : (100 - def1)));
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_post_defense(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	status_change *sc = status_get_sc(src);
	status_data *sstatus = status_get_status_data(*src);
	...
	if (sc) { // SC skill damages
		if (sc->getSCE(SC_AURABLADE)
			...
			ATK_ADD(wd->damage,
					wd->damage2,
					(3 + sc->getSCE(SC_AURABLADE)->val1) * status_get_lv(src)); // !TODO: Confirm formula
#else
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_plant(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	status_data *tstatus = status_get_status_data(*target);
	...
	if (attack_hits && target->type == BL_MOB) {
		...
		if (sc && !battle_status_block_damage(
					  src, target, sc, wd, damage_dummy, skill_id, skill_lv)) { // Statuses that reduce damage to 0.
	...
	if (attack_hits && status_get_class(target) == MOBID_EMPERIUM) {
		if (target && !battle_can_hit_gvg_target(src, target, skill_id, (skill_id) ? BF_SKILL : 0) &&
			map_flag_gvg2(target->m)) {
			wd->damage = wd->damage2 = 0;
			return;
		}
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_left_right_hands(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	...
	if (sd) {
		...
		if (sd->status.weapon == W_KATAR &&
			!skill_id) { // Katars (offhand damage only applies to normal attacks, tested on Aegis 10.2)
...
#endif
			// If you only have a weapon in the left hand, then your main hand damage will be identical to an unarmed
			// attack
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_gvg_bg(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	if (wd->damage + wd->damage2) { // There is a total damage value
		if (src != target && // Don't reflect your own damage (Grand Cross)
			...
			if (rdamage > 0) { // Item reflect gets calculated before any mapflag reducing is applicated
				...
				rdelay = clif_damage(*src,
									 (d_bl == nullptr) ? *src : *d_bl,
									 tick,
									 wd->amotion,
									 sstatus->dmotion,
									 rdamage,
				...
				skill_additional_effect(
					target, (!d_bl) ? src : d_bl, CR_REFLECTSHIELD, 1, BF_WEAPON | BF_SHORT | BF_NORMAL, ATK_DEF, tick);
			}
		}
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_weapon_final_atk_modifiers(
	struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	map_session_data *sd = BL_CAST(BL_PC, src);
	map_session_data *tsd = BL_CAST(BL_PC, target);
	status_change *sc = status_get_sc(src);
	...
	if (wd->damage && tsc && tsc->getSCE(SC_REJECTSWORD) &&
		(src->type != BL_PC || (((TBL_PC *)src)->weapontype1 == W_DAGGER || ((TBL_PC *)src)->weapontype1 == W_1HSWORD ||
	...
	if (tsc && tsc->getSCE(SC_CRESCENTELBOW) && wd->flag & BF_SHORT &&
		rnd() % 100 < tsc->getSCE(SC_CRESCENTELBOW)->val2) {
		// ATK [{(Target HP / 100) x Skill Level} x Caster Base Level / 125] % + [Received damage x {1 + (Skill Level x
		// 0.2)}]
		int64 rdamage = 0;
		int32 ratio =
			(int64)(status_get_hp(src) / 100) * tsc->getSCE(SC_CRESCENTELBOW)->val1 * status_get_lv(target) / 125;
		if (ratio > 5000) {
		...
		skill_blown(target,
					src,
					skill_get_blewcount(SR_CRESCENTELBOW_AUTOSPELL, tsc->getSCE(SC_CRESCENTELBOW)->val1),
					unit_getdir(src),
					BLOWN_NONE);
		clif_skill_damage(target,
						  src,
						  gettick(),
						  ...
						  tsc->getSCE(SC_CRESCENTELBOW)->val1,
						  DMG_SINGLE); // This is how official does
 ...
 *	Refined and optimized by helvetica
 */
static struct Damage initialize_weapon_data(
	struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag) {
	status_data *sstatus = status_get_status_data(*src);
	status_data *tstatus = status_get_status_data(*target);
	status_change *sc = status_get_sc(src);
	map_session_data *sd = BL_CAST(BL_PC, src);
	...
	wd.amotion = (skill_id && skill_get_inf(skill_id) & INF_GROUND_SKILL)
					 ? 0
					 : sstatus->amotion; // Amotion should be 0 for ground skills.
	...
	wd.flag |= (skill_id || wd.miscflag) ? BF_SKILL
										 : BF_NORMAL; // Baphomet card's splash damage is counted as a skill. [Inkfish]
	wd.isspdamage = false;
	wd.damage = wd.damage2 =
#ifdef RENEWAL
		wd.statusAtk = wd.statusAtk2 = wd.equipAtk = wd.equipAtk2 = wd.weaponAtk = wd.weaponAtk2 = wd.masteryAtk =
			wd.masteryAtk2 = wd.percentAtk = wd.percentAtk2 =
#else
		wd.basedamage =
#endif
				0;
 ...
 */
void battle_do_reflect(int32 attack_type,
					   struct Damage *wd,
					   struct block_list *src,
					   struct block_list *target,
					   uint16 skill_id,
	...
	if ((wd->damage + wd->damage2) && src && target && src != target &&
		(src->type != BL_SKILL || (src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM ||
															 skill_id == SG_STAR_WARM)))) {
		int64 damage = wd->damage + wd->damage2, rdamage = 0;
		map_session_data *tsd = BL_CAST(BL_PC, target);
		status_change *tsc = status_get_sc(target);
		status_data *sstatus = status_get_status_data(*src);
		...
		if (sce) {
			...
			if (!tsc->getSCE(SC_KYOMU) &&
				!(tsc->getSCE(SC_DARKCROW) &&
				  (wd->flag & BF_SHORT))) { // SC_KYOMU invalidates reflecting ability. SC_DARKCROW also does, but only
											// for short weapon attack.
				skill_castend_damage_id(
					target, src, NPC_MAXPAIN_ATK, sce->val1, tick, ((wd->flag & 1) ? wd->flag - 1 : wd->flag));
			}
		}
		...
		if (rdamage > 0) {
			...
			if (attack_type == BF_WEAPON &&
				tsc->getSCE(SC_REFLECTDAMAGE)) { // Don't reflect your own damage (Grand Cross)
				map_foreachinshootrange(battle_damage_area,
										target,
										skill_get_splash(LG_REFLECTDAMAGE, 1),
										BL_CHAR,
										tick,
										target,
										wd->amotion,
...
static struct Damage battle_calc_weapon_attack(
	...
	if (!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true)) {
		...
		if (skill_id == HW_MAGICCRASHER) { // Add weapon attack for MATK onto Magic Crasher
			...
			if (sstatus->matk_max > sstatus->matk_min) {
				ATK_ADD(
					wd.weaponAtk, wd.weaponAtk2, sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min));
			}
			else {
		...
		if (sd) {
			wd.weaponAtk +=
				battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.weaponAtk, 2, wd.flag);
			wd.equipAtk +=
				battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.equipAtk, 2, wd.flag);
			if (is_attack_left_handed(src, skill_id)) {
				wd.weaponAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, nk, right_element, left_element, wd.weaponAtk2, 3, wd.flag);
				wd.equipAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, nk, right_element, left_element, wd.equipAtk2, 3, wd.flag);
			}
		}

		// Card Fix for target (tsd), 2 is not added to the "left" flag meaning "target cards only"
		if (tsd && skill_id != NJ_ISSEN &&
			skill_id != GN_FIRE_EXPANSION_ACID) { // These skills will do a card fix later
			std::bitset<NK_MAX> ignoreele_nk = nk;

			ignoreele_nk.set(NK_IGNOREELEMENT);
			wd.statusAtk += battle_calc_cardfix(
				BF_WEAPON, src, target, ignoreele_nk, right_element, left_element, wd.statusAtk, 0, wd.flag);
			wd.weaponAtk +=
				battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.weaponAtk, 0, wd.flag);
			wd.equipAtk +=
				battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.equipAtk, 0, wd.flag);
			wd.masteryAtk += battle_calc_cardfix(
				BF_WEAPON, src, target, ignoreele_nk, right_element, left_element, wd.masteryAtk, 0, wd.flag);
			if (is_attack_left_handed(src, skill_id)) {
				wd.statusAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, ignoreele_nk, right_element, left_element, wd.statusAtk2, 1, wd.flag);
				wd.weaponAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, nk, right_element, left_element, wd.weaponAtk2, 1, wd.flag);
				wd.equipAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, nk, right_element, left_element, wd.equipAtk2, 1, wd.flag);
				wd.masteryAtk2 += battle_calc_cardfix(
					BF_WEAPON, src, target, ignoreele_nk, right_element, left_element, wd.masteryAtk2, 1, wd.flag);
			}
		}
		...
		if (sd) { // monsters, homuns and pets have their damage computed directly
			...
			if ((skill_id != RK_DRAGONBREATH && skill_id != RK_DRAGONBREATH_WATER) ||
				pc_checkskill(sd, DK_DRAGONIC_AURA) > 0) {
			...
			if (wd.flag & BF_LONG &&
				(skill_id != RA_WUGBITE &&
				 skill_id != RA_WUGSTRIKE)) { // Long damage rate addition doesn't use weapon + equip attack
		...
		if (sd) {
			...
			if (sd->status.weapon == W_KATAR && (katar_skill = pc_checkskill(sd, ASC_KATAR)) >
													0) { // Adv. Katar Mastery applied after calculate with skillratio.
		...
		if ((wd.damage + wd.damage2) && tstatus->res > 0 && !nk[NK_SIMPLEDEFENSE]) {
			...
			if (sd) {
				ignore_res +=
					sd->indexed_bonus.ignore_res_by_race[tstatus->race] + sd->indexed_bonus.ignore_res_by_race[RC_ALL];
			}
...
#ifdef RENEWAL
			// Check if attack ignores DEF (in pre-renewal we need to update base damage even when the skill ignores
			// DEF)
	...
	if (sd) {
		...
		switch (skill_id) {
			...
			case RK_DRAGONBREATH_WATER:
				if (wd.flag &
					BF_LONG) { // Add check here, because we want to apply the same behavior in pre-renewal [exneval]
			...
			default:
				wd.damage +=
					battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.damage, 2, wd.flag);
				if (is_attack_left_handed(src, skill_id)) {
					wd.damage2 += battle_calc_cardfix(
						BF_WEAPON, src, target, nk, right_element, left_element, wd.damage2, 3, wd.flag);
				}
				break;
		}
#endif
	...
	if (tsd) {
		...
		// Card Fix for target (tsd), 2 is not added to the "left" flag meaning "target cards only"
		wd.damage +=
			battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.damage, 0, wd.flag);
		if (is_attack_left_handed(src, skill_id)) {
			wd.damage2 +=
				battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element, wd.damage2, 1, wd.flag);
		}
	}
...
struct Damage battle_calc_magic_attack(
	...
	ad.amotion =
		(skill_get_inf(skill_id) & INF_GROUND_SKILL ? 0 : sstatus->amotion); // Amotion should be 0 for ground skills.
...
#define MATK_ADD(a) \
	{ ad.damage += a; }
		...
		switch (skill_id) {
...
#ifdef RENEWAL
				i = 10 * skill_lv + sstatus->luk + sstatus->int_ + status_get_lv(src) + 300 -
					300 * tstatus->hp / tstatus->max_hp;
#else
				i = 20 * skill_lv + sstatus->luk + sstatus->int_ + status_get_lv(src) + 200 -
					200 * tstatus->hp / tstatus->max_hp;
#endif
				if (i > 700) {
					i = 700;
				}
			...
			case NPC_EARTHQUAKE:
				...
				if (src->type == BL_PC) {
					ad.damage =
						sstatus->str * 2 + battle_calc_weapon_attack(src, target, skill_id, skill_lv, mflag).damage;
				}
				else {
			...
			case NPC_FLAMECROSS:
				ad.damage =
					static_cast<int64>(sstatus->rhw.atk) * static_cast<int64>(20) * static_cast<int64>(skill_lv);
				break;
			default: {
				if (sstatus->matk_max > sstatus->matk_min) {
					MATK_ADD(sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min));
				...
				switch (skill_id) {
					...
					case MG_LIGHTNINGBOLT:
						if (sc) {
							...
							if (sc->getSCE(SC_SPELLFIST) && mflag & BF_SHORT) {
								skillratio += (sc->getSCE(SC_SPELLFIST)->val3 * 100) +
											  (sc->getSCE(SC_SPELLFIST)->val1 * 50 - 50) -
											  100; // val3 = used bolt level, val1 = used spellfist level. [Rytech]
					...
					case NPC_ELECTRICWALK:
						...
						break;
					case SO_EARTHGRAVE:
						skillratio += -100 + 2 * sstatus->int_ + 300 * pc_checkskill(sd, SA_SEISMICWEAPON) +
									  sstatus->int_ * skill_lv;
						RE_LVL_DMOD(100);
						...
						break;
					case SO_DIAMONDDUST:
						skillratio += -100 + 2 * sstatus->int_ + 300 * pc_checkskill(sd, SA_FROSTWEAPON) +
									  sstatus->int_ * skill_lv;
						RE_LVL_DMOD(100);
					...
					case SO_VARETYR_SPEAR:
						skillratio +=
							-100 + (2 * sstatus->int_ +
									150 * (pc_checkskill(sd, SO_STRIKING) + pc_checkskill(sd, SA_LIGHTNINGLOADER)) +
									sstatus->int_ * skill_lv / 2) /
									   3;
						RE_LVL_DMOD(100);
						if (sc && sc->getSCE(SC_BLAST_OPTION)) {
							skillratio += (sd ? sd->status.job_level * 5 : 0);
					...
					case GN_DEMONIC_FIRE:
						if (skill_lv > 20) { // Fire expansion Lv.2
							...
							skillratio +=
								10 + 20 * (skill_lv - 10) + status_get_int(src) + ((sd) ? sd->status.job_level : 50);
							RE_LVL_DMOD(100);
						}
						else {
					...
					case MH_XENO_SLASHER:
						skillratio += -100 + 450 * skill_lv * status_get_lv(src) / 100 +
									  sstatus->int_; // !TODO: Confirm Base Level and INT bonus
						break;
					case MH_TWISTER_CUTTER:
						skillratio += -100 + 480 * skill_lv * status_get_lv(src) / 100 +
									  sstatus->int_; // !TODO: Confirm Base Level and INT bonus
						break;
					case MH_ABSOLUTE_ZEPHYR:
						skillratio += -100 + 1000 + 450 * skill_lv * status_get_lv(src) / 100 +
									  sstatus->int_; // !TODO: Confirm Base Level and INT bonus
						break;
					case MH_HEILIGE_STANGE:
						skillratio += -100 + 1500 + 250 * skill_lv * status_get_lv(src) / 150 +
									  sstatus->vit; // !TODO: Confirm VIT bonus
						break;
					case MH_HEILIGE_PFERD:
						skillratio += -100 + 1200 + 350 * skill_lv * status_get_lv(src) / 100 +
									  sstatus->vit; // !TODO: Confirm VIT bonus
						break;
					case MH_POISON_MIST:
						skillratio += -100 + 200 * skill_lv * status_get_lv(src) / 100 +
									  sstatus->dex; // ! TODO: Confirm DEX bonus
						break;
					...
					case HN_NAPALM_VULCAN_STRIKE:
						...
						RE_LVL_DMOD(100);
						// After RE_LVL_DMOD calculation, HN_SELFSTUDY_SOCERY amplifies the skill ratio of
						// HN_NAPALM_VULCAN_STRIKE by (2x skill level)%
					...
					case HN_JUPITEL_THUNDER_STORM:
						...
						RE_LVL_DMOD(100);
						// After RE_LVL_DMOD calculation, HN_SELFSTUDY_SOCERY amplifies the skill ratio of
						// HN_JUPITEL_THUNDER_STORM by (skill level)%
					...
					case HN_GROUND_GRAVITATION:
						...
						RE_LVL_DMOD(100);
						// After RE_LVL_DMOD calculation, HN_SELFSTUDY_SOCERY amplifies the skill ratio of
						// HN_GROUND_GRAVITATION (gravity field damage) by (skill level)%
						if (!(mflag & SKILL_ALTDMG_FLAG)) {
							skillratio += skillratio * pc_checkskill(sd, HN_SELFSTUDY_SOCERY) / 100;
						}
						// SC_RULEBREAK increases the skill ratio after HN_SELFSTUDY_SOCERY
					...
					case HN_JACK_FROST_NOVA:
						...
						RE_LVL_DMOD(100);
						// After RE_LVL_DMOD calculation, HN_SELFSTUDY_SOCERY amplifies the skill ratio of
						// HN_JACK_FROST_NOVA (explosion damage) by (skill level)%
						if (!(mflag & SKILL_ALTDMG_FLAG)) {
							skillratio += skillratio * pc_checkskill(sd, HN_SELFSTUDY_SOCERY) / 100;
						}
						// SC_RULEBREAK increases the skill ratio after HN_SELFSTUDY_SOCERY
					...
					case HN_METEOR_STORM_BUSTER:
						...
						RE_LVL_DMOD(100);
						// After RE_LVL_DMOD calculation, HN_SELFSTUDY_SOCERY amplifies the skill ratio of
						// HN_METEOR_STORM_BUSTER (fall damage) by (skill level)%
						if (mflag & SKILL_ALTDMG_FLAG) {
							skillratio += skillratio * pc_checkskill(sd, HN_SELFSTUDY_SOCERY) / 100;
						}
						// SC_RULEBREAK increases the skill ratio after HN_SELFSTUDY_SOCERY
				...
				if (sc) { // Insignia's increases the damage of offensive magic by a fixed percentage depending on the
						  // element.
					if ((sc->getSCE(SC_FIRE_INSIGNIA) && sc->getSCE(SC_FIRE_INSIGNIA)->val1 == 3 &&
						 s_ele == ELE_FIRE) ||
						(sc->getSCE(SC_WATER_INSIGNIA) && sc->getSCE(SC_WATER_INSIGNIA)->val1 == 3 &&
						 s_ele == ELE_WATER) ||
						(sc->getSCE(SC_WIND_INSIGNIA) && sc->getSCE(SC_WIND_INSIGNIA)->val1 == 3 &&
						 s_ele == ELE_WIND) ||
						(sc->getSCE(SC_EARTH_INSIGNIA) && sc->getSCE(SC_EARTH_INSIGNIA)->val1 == 3 &&
						 s_ele == ELE_EARTH)) {
						skillratio += 25;
					}
				}
		...
		if (sd) {
			...
			if (!flag.imdef &&
				(sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) || sd->bonus.ignore_mdef_ele & (1 << ELE_ALL) ||
				 ...
				 sd->bonus.ignore_mdef_class & (1 << CLASS_ALL))) {
				flag.imdef = 1;
			}
		}
		...
		if (ad.damage && tstatus->mres > 0) {
			...
			if (sd) {
				ignore_mres += sd->indexed_bonus.ignore_mres_by_race[tstatus->race] +
		...
		if (!flag.imdef) {
			...
			if (sd != nullptr) {
				i += sd->indexed_bonus.ignore_mdef_by_race[tstatus->race] +
					 sd->indexed_bonus.ignore_mdef_by_race[RC_ALL] +
					 sd->indexed_bonus.ignore_mdef_by_class[tstatus->class_] +
					 sd->indexed_bonus.ignore_mdef_by_class[CLASS_ALL];
...
#ifdef RENEWAL
		// pc_skillatk_bonus must be after def reduction and before the damages applied when the target is hit (at least
		// on renewal)
		if (sd != nullptr) {
	...
	// battle_do_reflect(BF_MAGIC,&ad, src, target, skill_id, skill_lv); //WIP [lighta] Magic skill has own handler at
	// skill_attack
	return ad;
}
...
struct Damage battle_calc_misc_attack(
	...
	switch (skill_id) {
		...
		case HT_CLAYMORETRAP:
			md.damage = (int64)(skill_lv * sstatus->dex * (3.0 + (float)status_get_lv(src) / 100.0) *
								(1.0 + (float)sstatus->int_ / 35.0));
		...
		case MA_LANDMINE:
			md.damage =
				static_cast<decltype(md.damage)>(skill_lv * (sstatus->dex + 75.0) * (100.0 + sstatus->int_) / 100.0);
			break;
		case HT_BLASTMINE:
			md.damage = static_cast<decltype(md.damage)>(skill_lv * (sstatus->dex / 2.0 + 50.0) *
														 (100.0 + sstatus->int_) / 100.0);
			break;
		case HT_CLAYMORETRAP:
			md.damage = static_cast<decltype(md.damage)>(skill_lv * (sstatus->dex / 2.0 + 75.0) *
														 (100.0 + sstatus->int_) / 100.0);
			break;
#endif
		case HT_BLITZBEAT:
		...
		case CR_ACIDDEMONSTRATION:
			if (tstatus->vit + sstatus->int_) { // crash fix
				md.damage = (int32)((int64)7 * tstatus->vit * sstatus->int_ * sstatus->int_ /
									(10 * (tstatus->vit + sstatus->int_)));
			}
			else {
				md.damage = 0;
			}
		...
		case NJ_ISSEN:
			...
			// modified def formula
			{
				...
				if (sc && sc->getSCE(SC_BUNSINJYUTSU) &&
					(i = sc->getSCE(SC_BUNSINJYUTSU)->val2) > 0) { // mirror image bonus only occurs if active
			...
			if (battle_config.agi_penalty_type && battle_config.agi_penalty_target & target->type) {
				...
				if (attacker_count >= battle_config.agi_penalty_count) {
					if (battle_config.agi_penalty_type == 1) {
						...
						flee -=
							(attacker_count - (battle_config.agi_penalty_count - 1)) * battle_config.agi_penalty_num;
					}
					if (flee < 1) {
						flee = 1;
					}
...
struct Damage battle_calc_attack(
	int32 attack_type, struct block_list *bl, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 flag) {
	struct Damage d;
	...
	if (d.damage + d.damage2 < 1) { // Miss/Absorbed
		...
		if (d.dmg_lv ==
			ATK_DEF /*&& attack_type&(BF_MAGIC|BF_MISC)*/) { // Isn't it that additional effects don't apply if miss?
			d.dmg_lv = ATK_MISS;
		}
		d.dmotion = 0;
		if (bl->type == BL_PC) {
 ...
 *	Refined and optimized by helvetica
 */
int64 battle_calc_return_damage(
	struct block_list *tbl, struct block_list *src, int64 *dmg, int32 flag, uint16 skill_id, bool status_reflect) {
	...
	if (flag & BF_SHORT) { // Bounces back part of the damage.
		if ((skill_get_inf2(skill_id, INF2_ISTRAP) || !status_reflect) && tsd &&
			tsd->bonus.short_weapon_damage_return) {
			...
			if (tsc->getSCE(SC_REFLECTSHIELD)) {
				...
				if ((sce_d = tsc->getSCE(SC_DEVOTION)) && (d_bl = map_id2bl(sce_d->val1)) &&
					((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master &&
					  ((TBL_MER *)d_bl)->master->bl.id == tbl->id) ||
					 (d_bl->type == BL_PC && ((TBL_PC *)d_bl)->devotion[sce_d->val2] ==
												 tbl->id))) { // Don't reflect non-skill attack if has SC_REFLECTSHIELD
															  // from Devotion bonus inheritance
					if ((!skill_id && battle_config.devotion_rdamage_skill_only &&
						 tsc->getSCE(SC_REFLECTSHIELD)->val4) ||
			...
			if (tsc->getSCE(SC_DEATHBOUND) && skill_id != WS_CARTTERMINATION && skill_id != GN_HELLS_PLANT_ATK &&
				...
				if (distance_bl(src, tbl) <= 0 || !map_check_dir(map_calc_dir(tbl, src->x, src->y), unit_getdir(tbl))) {
					int64 rd1 = i64min(damage, status_get_max_hp(tbl)) * tsc->getSCE(SC_DEATHBOUND)->val2 /
								100; // Amplify damage.

					*dmg = rd1 * 30 / 100; // Received damage = 30% of amplified damage.
					clif_skill_damage(src,
									  tbl,
									  gettick(),
									  status_get_amotion(src),
									  0,
									  ...
									  1,
									  RK_DEATHBOUND,
									  tsc->getSCE(SC_DEATHBOUND)->val1,
									  DMG_SINGLE);
					skill_blown(tbl,
								src,
								skill_get_blewcount(RK_DEATHBOUND, tsc->getSCE(SC_DEATHBOUND)->val1),
								unit_getdir(src),
								BLOWN_NONE);
...
bool battle_check_coma(map_session_data &sd, struct block_list &target, e_battle_flag attack_type) {
	...
	// Coma
	if (sd.special_state.bonus_coma && (!dstmd || (!util::vector_exists(status_get_race2(&dstmd->bl), RC2_GVG) &&
												   status_get_class(&dstmd->bl) != CLASS_BATTLEFIELD))) {
		int32 rate = 0;
...
bool battle_vellum_damage(map_session_data *sd, struct block_list *target, struct Damage *wd) {
	...
	status_data *tstatus = status_get_status_data(*target);
	// bHPVanishRaceRate
	int16 vellum_rate_hp =
		cap_value(sd->hp_vanish_race[tstatus->race].rate + sd->hp_vanish_race[RC_ALL].rate, 0, INT16_MAX);
	int8 vellum_hp =
		cap_value(sd->hp_vanish_race[tstatus->race].per + sd->hp_vanish_race[RC_ALL].per, INT8_MIN, INT8_MAX);
	// bSPVanishRaceRate
	int16 vellum_rate_sp =
		cap_value(sd->sp_vanish_race[tstatus->race].rate + sd->sp_vanish_race[RC_ALL].rate, 0, INT16_MAX);
	int8 vellum_sp =
		cap_value(sd->sp_vanish_race[tstatus->race].per + sd->sp_vanish_race[RC_ALL].per, INT8_MIN, INT8_MAX);
 ...
 *-------------------------------------------*/
void battle_drain(
	map_session_data *sd, struct block_list *tbl, int64 rdamage, int64 ldamage, int32 race, int32 class_) {
 ...
 */
void battle_autocast_elembuff_skill(
	map_session_data *sd, struct block_list *target, uint16 skill_id, t_tick tick, int32 flag) {
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, t_tick tick, int32 flag) {
	...
	if (tsc && tsc->getSCE(SC_BLADESTOP_WAIT) &&
		...
		if (sc_start4(src,
					  ...
					  SC_BLADESTOP,
					  100,
					  sd ? pc_checkskill(sd, MO_BLADESTOP) : 5,
					  0,
					  0,
					  target->id,
					  duration)) { // Target locked.
			clif_damage(*src, *target, tick, sstatus->amotion, 1, 0, 1, DMG_NORMAL, 0, false); // Display MISS.
	...
	if (sc) {
		...
		if (sc->getSCE(SC_MAGICALATTACK)) {
			if (skill_attack(
					BF_MAGIC, src, src, target, NPC_MAGICALATTACK, sc->getSCE(SC_MAGICALATTACK)->val1, tick, 0)) {
				return ATK_DEF;
			}
			return ATK_MISS;
		}
		if (sc->getSCE(SC_GT_ENERGYGAIN)) {
			...
			if (sd && rnd() % 100 < sc->getSCE(SC_GT_ENERGYGAIN)->val2) {
				pc_addspiritball(
					sd, skill_get_time2(SR_GENTLETOUCH_ENERGYGAIN, sc->getSCE(SC_GT_ENERGYGAIN)->val1), spheres);
			}
		}
	}
	...
	if (tsc && tsc->getSCE(SC_GT_ENERGYGAIN)) {
		...
		if (tsd && rnd() % 100 < tsc->getSCE(SC_GT_ENERGYGAIN)->val2) {
			pc_addspiritball(
				tsd, skill_get_time2(SR_GENTLETOUCH_ENERGYGAIN, tsc->getSCE(SC_GT_ENERGYGAIN)->val1), spheres);
		}
	}

	if (tsc && tsc->getSCE(SC_KAAHI) && tstatus->hp < tstatus->max_hp &&
		status_charge(target, 0, tsc->getSCE(SC_KAAHI)->val3)) {
	...
	if (sc != nullptr && !sc->empty()) {
		...
		if (sc->getSCE(SC_SPELLFIST) && !vellum_damage) {
			if (status_charge(src, 0, 20)) {
				if (!is_infinite_defense(target, wd.flag)) {
					struct Damage ad = battle_calc_attack(BF_MAGIC,
														  src,
														  target,
														  sc->getSCE(SC_SPELLFIST)->val2,
														  sc->getSCE(SC_SPELLFIST)->val3,
														  flag | BF_SHORT);
		...
		if (sc->getSCE(SC_GIANTGROWTH) && (wd.flag & BF_SHORT) && rnd() % 100 < sc->getSCE(SC_GIANTGROWTH)->val2 &&
			!is_infinite_defense(target, wd.flag) && !vellum_damage) {
	...
	if (damage > 0 && src != target) {
		if (sc && sc->getSCE(SC_DUPLELIGHT) &&
			(wd.flag & BF_SHORT)) { // Activates only from regular melee damage. Success chance is seperate for both
									// duple light attacks.
			uint16 duple_rate = 10 + 2 * sc->getSCE(SC_DUPLELIGHT)->val1;

			if (rand() % 100 < duple_rate) {
				skill_castend_damage_id(
					src, target, AB_DUPLELIGHT_MELEE, sc->getSCE(SC_DUPLELIGHT)->val1, tick, flag | SD_LEVEL);
			}

			if (rand() % 100 < duple_rate) {
				skill_castend_damage_id(
					src, target, AB_DUPLELIGHT_MAGIC, sc->getSCE(SC_DUPLELIGHT)->val1, tick, flag | SD_LEVEL);
			}
		}
	}

	wd.dmotion = clif_damage(
		*src, *target, tick, wd.amotion, wd.dmotion, wd.damage, wd.div_, wd.type, wd.damage2, wd.isspdamage);
	...
	if (!(tsc && tsc->getSCE(SC_DEVOTION)) && !vellum_damage && skill_check_shadowform(target, damage, wd.div_)) {
		...
		battle_delay_damage(
			tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.dmotion, true, wd.isspdamage);
	}
	if (tsc) {
		if (tsc->getSCE(SC_DEVOTION)) {
			...
			if (d_bl &&
				((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master &&
				  ((TBL_MER *)d_bl)->master->bl.id == target->id) ||
				...
				if (damage > 0) {
					...
					clif_damage(
						*d_bl, *d_bl, gettick(), wd.amotion, wd.dmotion, devotion_damage, 1, DMG_NORMAL, 0, false);
		...
		if (target->type == BL_PC && (wd.flag & BF_SHORT) && tsc->getSCE(SC_CIRCLE_OF_FIRE_OPTION)) {
			...
			if (ed) {
				clif_skill_damage(&ed->bl,
								  ...
								  1,
								  EL_CIRCLE_OF_FIRE,
								  tsc->getSCE(SC_CIRCLE_OF_FIRE_OPTION)->val1,
								  DMG_SINGLE);
				skill_attack(BF_WEAPON,
							 &ed->bl,
							 &ed->bl,
							 src,
							 EL_CIRCLE_OF_FIRE,
	...
	if (sd) {
		...
		if (wd.flag & BF_WEAPON && sc && sc->getSCE(SC__AUTOSHADOWSPELL) &&
			rnd() % 100 < sc->getSCE(SC__AUTOSHADOWSPELL)->val3 &&
			...
			if (r_skill != AL_HOLYLIGHT && r_skill != PR_MAGNUS) {
				...
				if ((type = skill_get_casttype(r_skill)) == CAST_GROUND) {
					...
					if (BL_PC & battle_config.land_skill_limit && (maxcount = skill_get_maxcount(r_skill, r_lv)) > 0) {
		...
		if (wd.flag & BF_WEAPON && sc && sc->getSCE(SC_FALLINGSTAR) &&
			rand() % 100 < sc->getSCE(SC_FALLINGSTAR)->val2) {
			if (sd) {
				sd->state.autocast = 1;
			}
		...
		if (sc) {
			if (sc->getSCE(SC_SERVANTWEAPON) && sd->servantball > 0 &&
				rnd_chance(5 * sc->getSCE(SC_SERVANTWEAPON)->val1, 100)) {
	...
	if (tsc) {
		if (damage > 0 && tsc->getSCE(SC_POISONREACT) &&
			(rnd() % 100 < tsc->getSCE(SC_POISONREACT)->val3 || sstatus->def_ele == ELE_POISON) &&
...
int32 battle_check_target(struct block_list *src, struct block_list *target, int32 flag) {
	...
	switch (target->type) { // Checks on actual target
		...
		case BL_MOB: {
			...
			if (((md->special_state.ai == AI_SPHERE || // Marine Spheres
				  (md->special_state.ai == AI_FLORA && battle_config.summon_flora & 1)) &&
				 s_bl->type == BL_PC && src->type != BL_MOB) || // Floras
				(md->special_state.ai == AI_ZANZOU && t_bl->id != s_bl->id) || // Zanzou
				(md->special_state.ai == AI_FAW &&
				 (t_bl->id != s_bl->id || (s_bl->type == BL_PC && src->type != BL_MOB)))) { // Targettable by players
				state |= BCT_ENEMY;
				strip_enemy = 0;
			}
			break;
		...
		case BL_SKILL: {
			...
			if (skill_get_inf2(su->group->skill_id, INF2_ISTRAP) && su->group->unit_id != UNT_USED_TRAPS) {
				if (!skill_id || su->group->skill_id == NPC_REVERBERATION ||
					su->group->skill_id == WM_POEMOFNETHERWORLD) {
					;
				}
				...
				strip_enemy = 0;
					 ...
					 (su->group->skill_id == GN_WALLOFTHORN && skill_id != GN_CARTCANNON)) {
				switch (skill_id) {
					case RK_DRAGONBREATH:
					case RK_DRAGONBREATH_WATER:
					case NC_SELFDESTRUCTION:
					...
					case HT_CLAYMORETRAP:
						...
						return 0;
					default:
						// Usually BCT_ALL stands for only hitting chars, but skills specifically set to hit traps also
						// hit icewall
	...
	switch (t_bl->type) { // Checks on target master
		case BL_PC: {
			...
			if (((sd->state.block_action & PCBLOCK_IMMUNE) || (sc->getSCE(SC_KINGS_GRACE) && s_bl->type != BL_PC)) &&
				flag & BCT_ENEMY) {
	...
	switch (src->type) { // Checks on actual src type
		...
		case BL_SKILL: {
			...
			if (sc != nullptr && !sc->empty() && inf2[INF2_ISTRAP]) {
				if (sc->getSCE(SC_SIGHTBLASTER) && sc->getSCE(SC_SIGHTBLASTER)->val2 > 0 &&
					sc->getSCE(SC_SIGHTBLASTER)->val4 % 2 == 0) {
					return -1;
				}
			}
		} break;
	...
	switch (s_bl->type) { // Checks on source master
		case BL_PC: {
			...
			if (s_bl != t_bl) {
				...
				else if (sd->duel_group && !((!battle_config.duel_allow_pvp && mapdata->getMapFlag(MF_PVP)) ||
											 (!battle_config.duel_allow_gvg && mapdata_flag_gvg(mapdata)))) {
			...
			if (!sd->status.guild_id && t_bl->type == BL_MOB && ((TBL_MOB *)t_bl)->mob_id == MOBID_EMPERIUM &&
				mapdata_flag_gvg(mapdata)) {
		...
		case BL_MOB: {
			...
			if (!md->special_state.ai) { // Normal mobs
				if ((target->type == BL_MOB && t_bl->type == BL_PC && !battle_get_exception_ai(*target)) ||
					(t_bl->type == BL_MOB && (((TBL_MOB *)t_bl)->special_state.ai == AI_NONE ||
											  ((TBL_MOB *)t_bl)->special_state.ai == AI_WAVEMODE))) {
	...
	if (mapdata_flag_vs(mapdata)) { // Check rivalry settings.
		...
		if (flag & (BCT_PARTY | BCT_ENEMY)) {
			...
			if (s_party && s_party == status_get_party_id(t_bl) &&
				!(mapdata->getMapFlag(MF_PVP) && mapdata->getMapFlag(MF_PVP_NOPARTY)) &&
				!(mapdata_flag_gvg(mapdata) && mapdata->getMapFlag(MF_GVG_NOPARTY)) &&
				(!mapdata->getMapFlag(MF_BATTLEGROUND) || sbg_id == tbg_id)) {
				state |= BCT_PARTY;
			}
			else {
				state |= BCT_ENEMY;
		...
		if (flag & (BCT_GUILD | BCT_ENEMY)) {
			...
			if (!(mapdata->getMapFlag(MF_PVP) && mapdata->getMapFlag(MF_PVP_NOGUILD)) && s_guild && t_guild &&
				(s_guild == t_guild || (!(flag & BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) &&
				(!mapdata->getMapFlag(MF_BATTLEGROUND) || sbg_id == tbg_id)) {
				state |= BCT_GUILD;
			}
			else {
		...
		if (state & BCT_ENEMY && battle_config.pk_mode && !mapdata_flag_gvg(mapdata) && s_bl->type == BL_PC &&
			t_bl->type == BL_PC) { // Prevent novice engagement on pk_mode (feature by Valaris)
			TBL_PC *sd = (TBL_PC *)s_bl, *sd2 = (TBL_PC *)t_bl;
			if ((sd->class_ & MAPID_UPPERMASK) == MAPID_NOVICE || (sd2->class_ & MAPID_UPPERMASK) == MAPID_NOVICE ||
				...
				(battle_config.pk_level_range &&
				 abs((int32)sd->status.base_level - (int32)sd2->status.base_level) > battle_config.pk_level_range)) {
				state &= ~BCT_ENEMY;
			}
		}
...
#endif
	{
		"summon_flora_setting",
		&battle_config.summon_flora,
		1 | 2,
		0,
		1 | 2,
	},
...
void battle_adjust_conf() {
	...
	if (battle_config.feature_mesitemlink) {
		ShowWarning(
			"conf/battle/feature.conf:mesitemlink is enabled but it requires PACKETVER 2010-01-01 or newer, "
			"disabling...\n");
		battle_config.feature_mesitemlink = 0;
	}
#elif PACKETVER == 20151029 || PACKETVER == 20151104
	// The feature is broken on those two clients or maybe even more. For more details check
	// ItemDatabase::create_item_link_for_mes [Lemongrass]
	if (battle_config.feature_mesitemlink) {
		ShowWarning(
			"conf/battle/feature.conf:mesitemlink is enabled but it is broken on this specific PACKETVER, "
			"disabling...\n");
		battle_config.feature_mesitemlink = 0;
	}
#endif
	...
	if (battle_config.feature_buying_store) {
		ShowWarning(
			"conf/battle/feature.conf:buying_store is enabled but it requires PACKETVER 2010-04-27 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_search_stores) {
		ShowWarning(
			"conf/battle/feature.conf:search_stores is enabled but it requires PACKETVER 2010-08-03 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_bgqueue) {
		ShowWarning(
			"conf/battle/feature.conf:bgqueue is enabled but it requires PACKETVER 2012-01-01 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_auction) {
		ShowWarning(
			"conf/battle/feature.conf:feature.auction is enabled but it is not stable on PACKETVER " EXPAND_AND_QUOTE(
				PACKETVER) ", disabling...\n");
		ShowWarning(
			"conf/battle/feature.conf:feature.auction change value to '2' to silence this warning and maintain it "
			"enabled\n");
	...
	if (battle_config.feature_auction) {
		ShowWarning(
			"conf/battle/feature.conf:feature.auction is enabled but it is not available for clients from 2014-11-12 "
			"on, disabling...\n");
		ShowWarning(
			"conf/battle/feature.conf:feature.auction change value to '2' to silence this warning and maintain it "
			"enabled\n");
	...
	if (battle_config.feature_banking) {
		ShowWarning(
			"conf/battle/feature.conf banking is enabled but it requires PACKETVER 2013-07-24 or newer, "
			"disabling...\n");
	...
	if (battle_config.mvp_exp_reward_message) {
		ShowWarning(
			"conf/battle/client.conf MVP EXP reward message is enabled but it requires PACKETVER 2013-12-23 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_roulette) {
		ShowWarning(
			"conf/battle/feature.conf roulette is enabled but it requires PACKETVER 2014-10-22 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_achievement) {
		ShowWarning(
			"conf/battle/feature.conf achievement is enabled but it requires PACKETVER 2015-05-13 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_itemlink) {
		ShowWarning(
			"conf/battle/feature.conf:itemlink is enabled but it requires PACKETVER 2015-11-04 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_stylist) {
		ShowWarning(
			"conf/battle/feature.conf stylist is enabled but it requires PACKETVER 2015-11-04 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_petevolution) {
		ShowWarning(
			"conf/battle/feature.conf petevolution is enabled but it requires PACKETVER 2014-10-08 or newer, "
			"disabling...\n");
		battle_config.feature_petevolution = 0;
	}
	if (battle_config.feature_pet_autofeed) {
		ShowWarning(
			"conf/battle/feature.conf pet auto feed is enabled but it requires PACKETVER 2014-10-08 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_refineui) {
		ShowWarning(
			"conf/battle/feature.conf refine UI is enabled but it requires PACKETVER 2016-10-12 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_equipswitch) {
		ShowWarning(
			"conf/battle/feature.conf equip switch is enabled but it requires PACKETVER 2017-02-08 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_homunculus_autofeed) {
		ShowWarning(
			"conf/battle/feature.conf homunculus autofeeding is enabled but it requires PACKETVER 2017-09-20 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_attendance) {
		ShowWarning(
			"conf/battle/feature.conf attendance system is enabled but it requires PACKETVER 2018-03-07 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_privateairship) {
		ShowWarning(
			"conf/battle/feature.conf private airship system is enabled but it requires PACKETVER 2018-03-21 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_barter) {
		ShowWarning(
			"conf/battle/feature.conf barter shop system is enabled but it requires PACKETVER 2019-01-16 or newer, "
			"disabling...\n");
	...
	if (battle_config.feature_barter_extended) {
		ShowWarning(
			"conf/battle/feature.conf extended barter shop system is enabled but it requires PACKETVER 2019-11-06 or "
			"newer, disabling...\n");
	...
	if (battle_config.custom_cell_stack_limit != 1) {
		ShowWarning(
			"Battle setting 'custom_cell_stack_limit' takes no effect as this server was compiled without Cell Stack "
			"Limit support.\n");
	}
#endif
