		...
		case BF_MAGIC:
			...
			if( sd && !nk[NK_IGNOREATKCARD] ) {
				...
				for (const auto &raceit : t_race2)
					race2_val += sd->magic_addrace2[raceit];
				cardfix = cardfix * (100 + sd->magic_addrace[tstatus->race] + sd->magic_addrace[RC_ALL] + race2_val) / 100;
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
				...
				for (const auto &raceit : s_race2)
					cardfix = cardfix * (100 - tsd->subrace2[raceit]) / 100;
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		if(!flag.imdef){
			...
			if(sd) {
				...
				for (const auto &raceit : race2)
					i += sd->ignore_mdef_by_race2[raceit];
				if (i)
				{
					if (i > 100) i = 100;
					mdef -= mdef * i/100;
