...
static bool status_change_isDisabledOnMap_(
	sc_type type, bool mapIsVS, bool mapIsPVP, bool mapIsGVG, bool mapIsBG, uint32 mapZone, bool mapIsTE);
#define status_change_isDisabledOnMap(type, m) \
	(status_change_isDisabledOnMap_((type), \
									mapdata_flag_vs2((m)), \
									m->getMapFlag(MF_PVP) != 0, \
									mapdata_flag_gvg2_no_te((m)), \
									m->getMapFlag(MF_BATTLEGROUND) != 0, \
									(m->zone << 3) != 0, \
									mapdata_flag_gvg2_te((m))))
				...
				for (const auto &refineLevelNode : refineLevelsNode) {
					...
					if (this->nodeExists(refineLevelNode, "BlacksmithBlessingAmount")) {
						...
						if (amount > MAX_AMOUNT) {
							this->invalidWarning(refineLevelNode["BlacksmithBlessingAmount"],
						...
						for (const auto &chanceNode : chancesNode) {
							...
							if (this->nodeExists(chanceNode, "DowngradeAmount")) {
								...
								if (downgrade_amount > MAX_REFINE) {
									this->invalidWarning(chanceNode["DowngradeAmount"],
...
std::shared_ptr<s_refine_level_info> RefineDatabase::findCurrentLevelInfo(const struct item_data &data,
																		  struct item &item) {
	if (item.refine > 0) {
...
std::shared_ptr<s_enchantgradelevel> EnchantgradeDatabase::findCurrentLevelInfo(const struct item_data &data,
																				struct item &item) {
			...
			if (!flag &&
				( // Blocked only from using the skill (stuff like autospell may still go through
					...
					(sc->getSCE(SC_BASILICA) &&
					 (sc->getSCE(SC_BASILICA)->val4 != src->id ||
					  skill_id != HP_BASILICA)) || // Only Basilica caster that can cast, and only Basilica to cancel it
#endif
					(sc->getSCE(SC_MARIONETTE) &&
					 skill_id != CG_MARIONETTE) || // Only skill you can use is marionette again to cancel it
					(sc->getSCE(SC_MARIONETTE2) &&
					 skill_id == CG_MARIONETTE) || // Cannot use marionette if you are being buffed by another
...
int32 status_base_amotion_pc(map_session_data *sd, struct status_data *status) {
	...
	amotion = ((int32)(temp_aspd + ((float)(status_calc_aspd(&sd->bl, &sd->sc, true) + val) * status->agi / 200)) -
			   min(amotion, 200));
#else
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int32 level) {
	...
	if (bl->type == BL_HOM) {
		...
		stat += level + status->dex +
				(bl->type == BL_PC ? status->luk / 3 + 175
		...
		if (bl->type == BL_MER) {
			...
			stat = status->def2;
			stat += (int32)(((float)level + status->vit) / 2 +
							(bl->type == BL_PC ? ((float)status->agi / 5)
		...
		if (bl->type == BL_MER) {
			...
			stat = status->mdef2;
			stat += (int32)(bl->type == BL_PC
								? (status->int_ + ((float)level / 4) + ((float)(status->dex + status->vit) / 5))
								: ((float)(status->int_ + level) /
								   4)); //(every 4 base level = +1 mdef) + (every 1 int32 = +1 mdef) + (every 5 dex = +1
										// mdef) + (every 5 vit = +1 mdef)
		}
		status->mdef2 = cap_value(stat, 0, SHRT_MAX);
		// PAtk
		stat = status->patk;
...
static uint32 status_calc_maxhpsp_pc(map_session_data *sd, uint32 stat, bool isHP) {
	...
	if (isHP) { // Calculates MaxHP
		...
		item_bonus = (dmax * status_get_hpbonus_item(&sd->bl) /
					  100); // !FIXME: using bMaxHP* for usable items may cause rounding issue
		...
		item_bonus = (dmax * status_get_spbonus_item(&sd->bl) /
					  100); // !FIXME: using bMaxSP* for usable items may cause rounding issue
		dmax += equip_bonus + item_bonus;
...
int32 status_calc_pc_sub(map_session_data *sd, uint8 opt) {
	...
	base_status->mode =
		static_cast<e_mode>(MD_MASK & ~(MD_STATUSIMMUNE | MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED |
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory.u.items_inventory[index].enchantgrade > 0) {
			enchantgrade_info = enchantgrade_db.findCurrentLevelInfo(*sd->inventory_data[index],
																	 sd->inventory.u.items_inventory[index]);
		}
#endif
	...
	if (sd->equip_index[EQI_AMMO] >= 0) {
		...
		if (sd->inventory_data[index]) { // Arrows
			...
			if (!itemdb_group.item_exists(IG_THROWABLE,
	...
	if (!sc->empty()) {
		...
		for (sc_type type = SC_NONE; type < SC_MAX; type = static_cast<sc_type>(type + 1)) {
			if (status_change_entry *sce = sc->getSCE(type); sce != nullptr) {
				if (std::shared_ptr<s_status_change_db> scdb = status_db.find(type);
					scdb != nullptr && scdb->script != nullptr) {
	...
	if ((pc_isriding(sd) || pc_isridingdragon(sd)) &&
		(sd->status.weapon == W_1HSPEAR ||
		 sd->status.weapon == W_2HSPEAR)) { // When Riding with spear, damage modifier to mid-class becomes
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, status_change *sc) {
	...
	if ((sc->getSCE(SC_POISON) && !sc->getSCE(SC_SLOWPOISON)) ||
		...
		sc->getSCE(SC_SATURDAYNIGHTFEVER) || sc->getSCE(SC_REBOUND) || sc->getSCE(SC_NORECOVER_STATE)) {
		regen->flag = RGN_NONE;
	}
...
void status_calc_state(struct block_list *bl, status_change *sc, std::bitset<SCS_MAX> flag, bool start) {
	...
	if (flag[SCS_NOMOVE]) {
		...
		else if ((sc->getSCE(SC_GOSPEL) &&
				  sc->getSCE(SC_GOSPEL)->val4 == BCT_SELF) // cannot move while gospel is in effect
#ifndef RENEWAL
...
void status_calc_bl_main(struct block_list &bl, std::bitset<SCB_MAX> flag) {
	...
	if (flag[SCB_DEF2]) {
		if (status->vit == b_status->vit
			...
			status->def2 = status_calc_def2(&bl,
											sc,
											b_status->def2
#ifdef RENEWAL
												+ (int32)(((float)status->vit / 2 - (float)b_status->vit / 2) +
	...
	if (flag[SCB_MDEF2]) {
		if (status->int_ == b_status->int_ && status->vit == b_status->vit
			...
			status->mdef2 = status_calc_mdef2(&bl,
											  sc,
											  b_status->mdef2 + (status->int_ - b_status->int_)
#ifdef RENEWAL
												  + (int32)(((float)status->dex / 5 - (float)b_status->dex / 5) +
		...
		switch (bl.type) {
			case BL_PC:
				...
				if (hom_is_active(sd->hd)) {
					...
					sd->hd->battle_status.speed =
						status_calc_speed(&sd->hd->bl, &sd->hd->sc, sd->hd->battle_status.speed);
				}
#endif
				break;
	...
	if (flag[SCB_ASPD]) {
		...
		if (bl.type == BL_HOM) {
			...
			amotion = (1000 - 4 * status->agi - status->dex) *
					  (reinterpret_cast<homun_data *>(&bl))->homunculusDB->baseASPD / 1000;
...
static defType status_calc_mdef(struct block_list *bl, status_change *sc, int32 mdef) {
	...
	if (sc->getSCE(SC_ENDURE) && !sc->getSCE(SC_ENDURE)->val3) { // It has been confirmed that Eddga card grants 1 MDEF,
																 // not 0, not 10, but 1.
	...
	if (sd && sd->ud.skilltimer != INVALID_TIMER &&
		...
		if (sd && sc->getSCE(SC_HIDING) && pc_checkskill(sd, RG_TUNNELDRIVE) > 0) {
			...
			if (sc->getSCE(SC_LONGING)) {
				...
				val = max(val,
						  500 - (40 + 10 * (sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_BARDDANCER)) *
			...
			if (sc->getSCE(SC_JOINTBEAT) && sc->getSCE(SC_JOINTBEAT)->val2 & (BREAK_ANKLE | BREAK_KNEE)) {
				val = max(val,
						  (sc->getSCE(SC_JOINTBEAT)->val2 & BREAK_ANKLE ? 50 : 0) +
...
void status_calc_slave_mode(mob_data &md) {
	switch (battle_config.slaves_inherit_mode) {
		...
		case 4: // Overwrite with slave mode
			...
			break;
		default: // Copy master
			if (block_list *mbl = map_id2bl(md.master_id);
				mbl != nullptr && status_has_mode(status_get_status_data(*mbl), MD_AGGRESSIVE)) {
				sc_start4(nullptr, &md.bl, SC_MODECHANGE, 100, 1, 0, MD_AGGRESSIVE, 0, 0);
			}
			else {
				sc_start4(nullptr, &md.bl, SC_MODECHANGE, 100, 1, 0, 0, MD_AGGRESSIVE, 0);
...
int32 status_get_class(struct block_list *bl) {
	...
	switch (bl->type) {
		...
		case BL_MOB:
			return ((TBL_MOB *)bl)->vd->class_; // Class used on all code should be the
												// view class of the mob.
		case BL_PET:
...
unsigned short status_get_speed(struct block_list *bl) {
	// TODO: is the statement of Skotlex still true? And would it not be better to check
	// for dummy_status instead? [Lemongrass]
 ...
 * @param flag: Value which determines what parts to calculate. See
 * e_status_change_start_flags
...
t_tick status_get_sc_def(
	...
	nullpo_ret(bl);
	if (src == nullptr) {
		return tick ? tick : 1; // This should not happen in current implementation, but
								// leave it anyway
	}
	...
	if (status_isimmune(bl)) {
		...
		if (skill == nullptr) { // Check for ground-type skills using the status when a
								// player moves through units
		...
		if (skill != nullptr && skill->skill_type == BF_MAGIC && // Basic magic skill
			...
			((skill->inf == INF_ATTACK_SKILL || skill->inf == INF_GROUND_SKILL ||
			  skill->inf == INF_SUPPORT_SKILL) || // Target skills should get blocked
												  // even when cast on self
	...
	switch (type) {
		...
		case SC_DEEPSLEEP:
			...
			break;
		case SC_NETHERWORLD:
			// Resistance: {(Target's Base Level / 50) + (Target's Job Level / 10)}
			// seconds
		...
		case SC_VOICEOFSIREN:
			// Resistance: {(Target's Base Level / 10) + (Target's Job Level / 5)}
			// seconds
			tick_def2 = status_get_lv(bl) * 100 + (sd ? sd->status.job_level : 1) * 200;
			break;
		case SC_B_TRAP:
			tick_def =
				(sd ? sd->status.str : status_get_base_status(bl)->str) * 50; //! TODO: Figure out reduction formula
			break;
...
int32 status_change_start(struct block_list *src,
	...
	if (scdb->script != nullptr && bl->type != BL_PC) {
		ShowError(
			"status_change_start: Failed to start the scripted status change %d on a "
			"non player.\n",
			type);
		return 0;
	}
	...
	if (bl->type != BL_NPC && status_isdead(*bl) &&
		(type != SC_NOCHAT && type != SC_JAILED)) { // SC_NOCHAT and SC_JAILED should
													// work even on dead characters
		return 0;
	}
	...
	if (!scdb->fail.empty()) {
		for (const auto &it : scdb->fail) {
			// Don't let OPT1 that have RemoveOnDamaged start a new effect in the same
			// attack.
	...
	switch (type) {
		case SC_VACUUM_EXTREME:
			if (sc && sc->getSCE(SC_VACUUM_EXTREME_POSTDELAY) &&
				sc->getSCE(SC_VACUUM_EXTREME_POSTDELAY)->val2 == val2) { // Ignore post delay from other vacuum (this
		...
		case SC_C_MARKER:
			if (src == bl) {
				...
				// Failed if the target is already marked and the new marker that isn't
				// same marker
	...
	if (type == SC_BERSERK && val3 == SC__BLOODYLUST) { // There is some reasons that using SC_BERSERK first
														// before SC__BLOODYLUST itself on Akinari's fix
	...
	switch (type) {
		...
		case SC_IMPOSITIO:
			if (sc->getSCE(SC_IMPOSITIO) &&
				sc->getSCE(SC_IMPOSITIO)->val1 > val1) { // Replace higher level effect for lower.
		...
		switch (type) {
			...
			case SC_JOINTBEAT:
				if (sc && sc->getSCE(type)->val2 & BREAK_NECK) {
					return 0; // BREAK_NECK cannot be stacked with new breaks until the
							  // status is over.
				}
		...
		switch (type) {
			...
			case SC_ADORAMUS:
				if (type == SC_ADORAMUS) {
					...
					if (sc->getSCE(SC_ADORAMUS)) {
						return 0; // Adoramus can't refresh itself, but it can cause
								  // blind again
					}
				}
			...
			case SC_ELEMENTALCHANGE:
				// val1 : Element Lvl (if called by skill lvl 1, takes random value
				// between 1 and 4) val2 : Element (When no element, random one is
				// picked) val3 : 0 = called by skill 1 = called by script (fixed level)
				if (!val2) {
					val2 = rnd() % ELE_ALL;
				}
			...
			case SC_REFLECTSHIELD:
				...
				// val4 used to mark if reflect shield is an inheritance bonus from
				// Devotion
			...
			case SC_SUITON:
				...
				// Suiton is a special case, stop effect is forced and only happens when
				// target enters it
			...
			case SC_NOCHAT:
				// A hardcoded interval of 60 seconds is expected, as the time that
				// SC_NOCHAT uses is defined by mmocharstatus.manner, each negative
				// point results in 1 minute with this status activated. This is done
				// this way because the message that the client displays is hardcoded,
				// and only shows how many minutes are remaining. [Panikon]
				tick = 60000;
			...
			case SC_PYREXIA:
				if (val3 == 1) { // Target
					// Causes blind for duration of pyrexia, unreducable and
					// unavoidable, but can be healed with e.g. green potion
					status_change_start(src,
										bl,
										SC_BLIND,
										10000,
			...
			case SC_DAMAGE_HEAL:
				switch (val1) {
					...
					case 2:
						...
						break;
					case 3:
						// TODO: Absorb MISC damage? Both WEAPON & MAGIC damage? Which
						// is correct on level 3?
						val2 = BF_MISC;
						break;
				}
				break;
			...
			case SC_CHASEWALK:
				...
				break;
			case SC_CLOAKING:
				if (!sd) { // Monsters should be able to walk with no penalties.
						   // [Skotlex]
					val1 = 10;
				}
				...
				if (bl->type == BL_PC ||
					(bl->type == BL_MOB && ((TBL_MOB *)bl)->special_state.clone)) { // Standard cloaking.
			...
			case SC_DEFENDER:
				if (!(flag & SCSTART_NOAVOID)) {
					...
					if (sd) {
						map_session_data *tsd;
						int32 i;
						for (i = 0; i < MAX_DEVOTION; i++) { // See if there are devoted characters, and pass
															 // the status to them. [Skotlex]
							if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i]))) {
								status_change_start(
			...
			case SC_GOSPEL:
				if (val4 == BCT_SELF) { // Self effect
					...
					status_change_clear_buffs(bl,
											  SCCB_BUFFS | SCCB_DEBUFFS | SCCB_CHEM_PROTECT); // Remove buffs/debuffs
				}
				break;
			...
			case SC_CLOSECONFINE2: {
				...
				if (src2 && sc2) {
					if (!sce2) { // Start lock on caster.
						...
						sce2->timer =
							add_timer(gettick() + tick + 1000, status_change_timer, src2->id, SC_CLOSECONFINE);
					}
				}
				else { // Status failed.
					return 0;
			...
			case SC_KAUPE:
				switch (val1) {
					...
					case 2: // 33, 66%
						...
						break;
					default: // Custom. For high level mob usage, higher level means
							 // more blocks. [Skotlex]
						val2 = 100;
						val3 = val1 - 2;
						break;
			...
			case SC_RUN: {
				...
				// Note: this int64 value is stored in two separate int32 variables
				// (FIXME)
			...
			case SC_WEAPONPERFECTION: {
				...
				if (type == SC_OVERTHRUST) {
					// val2 holds if it was casted on self, or is bonus received from
					// others
#ifdef RENEWAL
			...
			case SC_BLOODLUST:
				...
				val2 = 20 + (10 * val1);
				// Leech chance
				// It's actually 9 * level on both pre-re and renewal, despite the
				// description
				val3 = 9 * val1;
				// Leech percent
				val4 = 20;
			...
			case SC__GROOMY:
				...
				if (sd) { // Removes Animals
					...
					// if( sd->md ) mercenary_delete(sd->md,3); // Are Mercenaries
					// removed? [aleos]
				}
				break;
			case SC__LAZINESS:
			...
			case SC_GLOOMYDAY_SK:
				// Random number between [15 ~ (Voice Lesson Skill Level x 5) + (Skill
				// Level x 10)] %.
			...
			case SC_GT_REVITALIZE:
				// Take note there is no vit, aspd, speed increase as skill desc says.
				// [malufett]
				val2 = 2 * val1; // MaxHP: [(Skill Level * 2)]%
				val3 = val1 * 30 + 50; // Natural HP recovery increase: [(Skill Level x 30) + 50] %
				// The stat def is not shown in the status window and it is processed
				// differently
			...
			case SC_PROMOTE_HEALTH_RESERCH:
				...
				else if (val1 == 2) { // If potion was thrown at someone, take the
									  // thrower's BaseLv
			...
			case SC_APPLEIDUN: {
				...
				val2 = (5 + 2 * val1) + (status_get_vit(src) / 10); // HP Rate: (5 + 2 * skill_lv) + (vit/10)
																	// + (BA_MUSICALLESSON level)
				if (s_sd) {
			...
			case SC_STEALTHFIELD_MASTER:
				...
				break;
			case SC_VACUUM_EXTREME:
				// Suck target at n second, only if the n second is lower than the
				// duration Does not suck targets on no-knockback maps
			...
			case SC_RELIEVE_ON:
				val2 = min(10 * val1,
						   99); // % damage received reduced from 10 * skill lvl up to 99%
				break;
			case SC_VIGOR:
			...
			case SC_PRE_ACIES:
				...
				break;
			case SC_COMPETENTIA:
				val2 = 10 * val1; // PAtk/SMatk Increase - Unconfirmed if this is official
								  // formula but its 50 at Lv 5. [Rytech]
				break;
			case SC_RELIGIO:
			case SC_BENEDICTUM:
			...
			default:
				if (calc_flag.none() && scdb->skill_id == 0 && scdb->icon == EFST_BLANK && scdb->opt1 == OPT1_NONE &&
					...
					ShowWarning(
						"status_change_start: Status %s (%d) is bare. Add the NoWarning "
						"flag to suppress this "
						"message.\n",
						script_get_constant_str("SC_", type),
						type);
					return 0;
				}
		}
	...
	// On Aegis, when turning on a status change, first goes the option packet, then the sc
	// packet.
	...
	if (!(flag & SCSTART_NOICON) &&
		...
		clif_status_change(bl,
						   status_icon,
						   1,
						   tick,
						   scdb->flag[SCF_SENDVAL1] ? val1 : 1,
						   scdb->flag[SCF_SENDVAL2] ? val2 : 0,
						   scdb->flag[SCF_SENDVAL3] ? val3 : 0);
	}

	// Used as temporary storage for scs with interval ticks, so that the actual duration is
	// sent to the client first.
	if (tick_time) {
		tick = tick_time;
	}

	// Don't trust the previous sce assignment, in case the SC ended somewhere between there
	// and here.
	...
	switch (type) {
		case SC_BERSERK:
			if (!(sce->val2)) { // Don't heal if already set
				status_heal(bl, status->max_hp, 0, 1); // Do not use percent_heal as this healing
													   // must override BERSERK's block.
	...
	for (const auto &it : status_db) {
		...
		if (type == 0) { // Type 0: PC killed
			if (it.second->flag[SCF_NOREMOVEONDEAD]) {
				switch (status) {
					case SC_ELEMENTALCHANGE: // Only when its Holy or Dark that it doesn't
											 // dispell on death
		...
		if (type == 1 && sc->getSCE(status)) { // If for some reason status_change_end decides to still
											   // keep the status when quitting. [Skotlex]
			(sc->count)--;
...
int32 status_change_end(struct block_list *bl, enum sc_type type, int32 tid) {
	...
	switch (type) {
		...
		case SC_CLOSECONFINE:
			if (sce->val2 > 0) {
				...
				int32 range = 1 + skill_get_range2(bl, scdb->skill_id, sce->val1, true) +
							  skill_get_range2(bl,
											   TF_BACKSLIDING,
											   1,
		...
		case SC_BERSERK:
			...
			else if (status->hp > 100 && sce->val2) { // If val2 is removed, no HP penalty
													  // (dispelled?) [Skotlex]
		...
		case SC_HAWKEYES:
			if (sce->val4) { // Clear the group.
				...
				sce->val4 = 0;
				if (group) { // Might have been cleared before status ended, e.g. land
							 // protector
					skill_delunitgroup(group);
				}
			}
		...
		case SC_SATURDAYNIGHTFEVER: // Sit down force of Saturday Night Fever has the
		...
		case SC_CURSEDCIRCLE_ATKER:
			if (sce->val2) { // Used the default area size cause there is a chance the
							 // caster could knock back and can't clear the target.
		...
		case SC_H_MINE:
			...
			break;
		case SC_VACUUM_EXTREME:
			///< !CHECKME: Seems on official, there's delay before same target can be
			///< vacuumed in same area again [Cydh]
			sc_start2(bl,
					  bl,
		...
		case SC_SOULENERGY:
			if (sd != nullptr && sd->soulball > 0) {
				// When SC_SOULENERGY ends, removes all soulball if SC_SOULCOLLECT is active
				// otherwise 1
	...
	// On Aegis, when turning off a status change, first goes the sc packet, then the option
	// packet.
	int32 status_icon = scdb->icon;
	...
	// Needed to be here to make sure OPT1_STONEWAIT has been cleared from the target (only
	// on natural expiration of the stone wait timer)
	...
	switch (type) {
		...
		case SC_CHASEWALK:
			...
			if (!sc->getSCE(SC_CHASEWALK2)) {
				sc_start(bl,
						 ...
						 100,
						 1 << (sce->val1 - 1),
						 (t_tick)(sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_ROGUE
									  ? 10
		...
		case SC_BURNING:
			if (sce->val4 >= 0) {
				...
				dounlock = true;
				status_fix_damage(
					bl, bl, damage, clif_damage(*bl, *bl, tick, 0, 1, damage, 1, DMG_NORMAL, 0, false), 0);
			}
			break;
		...
		case SC_TOXIN:
			if (sce->val4 >= 0) { // Damage is every 10 seconds including 3%sp drain.
				if (sce->val3 == 1) { // Target
					...
					interval = 1000; // Assign here since status_get_sc_internval() contains
									 // the target interval.
			...
			if (sce->val4 >= 0) {
				...
				if (!flag) { // Random Skill Cast
					if (magic_mushroom_db.size() > 0 && sd && !pc_issit(sd)) { // Can't cast if sit
						...
						switch (skill_get_casttype(mushroom_skill_id)) { // Magic Mushroom skills are buffs or area
																		 // damage
							case CAST_GROUND:
		...
		case SC_LEECHESEND:
			if (sce->val4 >= 0) {
				int64 damage =
					status->vit * (sce->val1 - 3) + (int32)status->max_hp / 100; // {Target VIT x (New Poison Research
				...
				status_fix_damage(
					...
					damage,
					clif_damage(
						*bl, *bl, tick, status->amotion, status->dmotion + 500, damage, 1, DMG_NORMAL, 0, false),
					0);
				unit_skillcastcancel(bl, 2);
			}
			break;
		...
		case SC_NOCHAT:
			if (sd) {
				...
				clif_changemanner(*sd);
				clif_updatestatus(*sd, SP_MANNER);
				if (sd->status.manner < 0) { // Every 60 seconds your manner goes up by 1
											 // until it gets back to 0.
		...
		case SC_OVERHEAT_LIMITPOINT:
			if (--(sce->val1) >= 0) { // Cooling
				if (sce->val2 == 0) { // Flag the overheat limit once it has been met.
					...
					uint16 skill_lv =
						(sd ? cap_value(pc_checkskill(sd, NC_MAINFRAME), 0, (uint16)(limit.size() - 1)) : 0);
		...
		case SC_INSPIRATION:
			if (--(sce->val4) >= 0) {
				if (!status_charge(bl,
								   status->max_hp * (35 - 5 * sce->val1) / 1000,
		...
		case SC_CREATINGSTAR:
			if (--(sce->val4) >= 0) { // Needed to check who the caster is and what AoE is giving the status.
				...
				if (star_caster == nullptr || status_isdead(*star_caster) || star_caster->m != bl->m ||
					star_aoe == nullptr) {
					break;
				}
		...
		case SC_TALISMAN_OF_PROTECTION:
			if (--(sce->val4) >= 0) {
				...
				if (ssd == nullptr || status_isdead(ssd->bl) || ssd->bl.m != bl->m || sd == nullptr ||
			...
			if (battle_check_target(src, bl, BCT_ENEMY) > 0 && status_check_skilluse(src, bl, WZ_SIGHTBLASTER, 2)) {
				if (sce) {
					...
					if (skill_attack(BF_MAGIC, src, src, bl, WZ_SIGHTBLASTER, sce->val1, tick, 0x1000000) &&
						(!su || !su->group ||
						 !skill_get_inf2(su->group->skill_id,
										 INF2_ISTRAP))) { // The hit is not counted if it's
														  // against a trap
						sce->val2 = 0; // This signals it to end.
					}
					else if ((bl->type & BL_SKILL) && sce->val4 % 2 == 0) {
						// Remove trap immunity temporarily so it triggers if you still
						// stand on it
						sce->val4++;
					}
				}
...
uint64 StatusDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Script")) {
		...
		status->script = parse_script(script.c_str(),
