 ...
 */
int32 intif_broadcast2(const char *mes,
					   size_t len,
...
int32 intif_parse_WisEnd(int32 fd) {
	if (battle_config.etc_log) {
		ShowInfo(
			...
			RFIFOP(fd, 2),
			RFIFOB(fd,
				   26)); // flag: 0: success to send wisper, 1: target character is not loged in?, 2: ignored by target
	}
...
int32 intif_parse_LoadGuildStorage(int32 fd) {
	...
	if (gstor->status) { // Already open.. lets ignore this update
		ShowWarning("intif_parse_LoadGuildStorage: storage received for a client already open (User %d:%d)\n",
					flag ? sd->status.account_id : 1,
					flag ? sd->status.char_id : 1);
		return 0;
	}
...
int32 intif_parse_PartyMemberWithdraw(int32 fd) {
	if (battle_config.etc_log) {
		ShowInfo("intif: party member withdraw: Type(%d) Party(%d), Account(%d), Char(%d), Name(%s)\n",
				 RFIFOB(fd, 14 + NAME_LENGTH),
				 RFIFOL(fd, 2),
				 RFIFOL(fd, 6),
				 RFIFOL(fd, 10),
				 RFIFOCP(fd, 14));
	}
	party_member_withdraw(RFIFOL(fd, 2),
						  RFIFOL(fd, 6),
						  RFIFOL(fd, 10),
						  RFIFOCP(fd, 14),
 ...
 * @return
 **/
int32 intif_broadcast_obtain_special_item(map_session_data *sd,
										  t_itemid nameid,
										  t_itemid sourceid,
										  unsigned char type) {
	nullpo_retr(0, sd);
...
void intif_parse_itembound_store2gstorage(int32 fd) {
	...
	ShowInfo("Retrieved '" CL_WHITE "%d" CL_RESET "' (failed: %d) guild bound item(s) for Guild ID = " CL_WHITE
			 "%d" CL_RESET ".\n",
			 count,
			 failed,
			 guild_id);
	...
	switch (type) {
		case TABLE_INVENTORY: {
			...
			if (sd->status.party_id == 0 &&
				...
				for (i = 0; i < j; i++) {
					pc_delitem(
						sd, idxlist[i], sd->inventory.u.items_inventory[idxlist[i]].amount, 4, 1, LOG_TYPE_OTHER);
				}
			}
#endif
