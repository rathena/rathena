...
enum e_setpos pc_setpos(map_session_data* sd, uint16 mapindex, int32 x, int32 y, clr_type clrtype)
{
	...
	// ------------------------------------------------------------
	// @ltp: store last position BEFORE any teleport/warp
	// Works with skills, items (Fly Wing), scripts, commands, etc.
	// ------------------------------------------------------------
	if (sd->m >= 0 && sd->x > 0 && sd->y > 0) {
		bool will_move = true;

		// Avoid updating if destination is exactly the same place
		if (m >= 0 && m == sd->m && x == sd->x && y == sd->y) {
			will_move = false;
		}

		if (will_move) {
			pc_setreg(sd, add_str("#LTP_X"), sd->x);
			pc_setreg(sd, add_str("#LTP_Y"), sd->y);
			pc_setregstr(sd, add_str("#LTP_MAP$"), map_mapid2mapname(sd->m));
		}
	}
