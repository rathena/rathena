		...
		for (std::pair<const uint32, std::shared_ptr<s_attendance_period>> &pair : *this) {
			...
			if (period->start <= attendance_period->start && start <= period->end) {
				this->invalidWarning(node,
									 "Node \"Start\" period %u intersects with period %u-%u, skipping.\n",
			...
			if (period->start <= attendance_period->end && attendance_period->end <= period->end) {
				this->invalidWarning(node,
									 "Node \"End\" period %u intersects with period %u-%u.\n",
									 attendance_period->start,
									 period->start,
									 period->end);
				collision = true;
...
void pc_addsoulball(map_session_data &sd, int32 number, int32 max) {
	...
	// Refresh the SC_SOULENERGY timer (which is 10 minutes by default) if the number has changed. The soul is removed
	...
	if (sd.soulball <= 0) {
		...
		if (sd.soulball == 0) {
			...
			sc_start(&sd.bl,
					 ...
					 100,
					 sd.soulball,
					 skill_get_time2(SP_SOULCOLLECT, 1)); // refresh val1 and status MATK
		}
	}
...
static bool pc_isItemClass(map_session_data *sd, struct item_data *item) {
	while (1) {
		if (item->class_upper & ITEMJ_NORMAL &&
			!(sd->class_ & (JOBL_UPPER | JOBL_BABY | JOBL_THIRD |
...
bool pc_authok(map_session_data *sd,
	...
	// warp player
	enum e_setpos setpos_result = pc_setpos(sd,
											mapindex_name2id(sd->status.last_point.map),
											sd->status.last_point.x,
											sd->status.last_point.y,
											CLR_OUTSIGHT);
...
void pc_reg_received(map_session_data *sd) {
	...
	chrif_req_login_operation(
		sd->status.account_id, sd->status.name, CHRIF_OP_LOGIN_VIP, 0, 1 | 8, 0); // request VIP information
#endif
 ...
 */
static void pc_bonus_autospell(std::vector<s_autospell> &spell,
							   uint16 id,
							   uint16 lv,
							   short rate,
							   short battle_flag,
 ...
 */
static void pc_bonus_addeff(std::vector<s_addeffect> &effect,
							enum sc_type sc,
							int32 rate,
							short arrow_rate,
							unsigned char flag,
 ...
 * @param duration: Duration. If 0 use default duration lookup for associated skill with level 7
 */
static void pc_bonus_addeff_onskill(std::vector<s_addeffectonskill> &effect,
									enum sc_type sc,
									int32 rate,
									short skill_id,
 ...
 */
static void pc_bonus_item_drop(
	std::vector<s_add_drop> &drop, t_itemid nameid, uint16 group, int32 class_, short race, int32 rate) {
	if (!nameid && !group) {
...
void pc_bonus2(map_session_data *sd, int32 type, int32 type2, int32 val) {
	...
	switch (type) {
		...
		case SP_ADDEFF: // bonus2 bAddEff,eff,n;
			...
			pc_bonus_addeff(sd->addeff,
							(sc_type)type2,
							sd->state.lr_flag != LR_FLAG_ARROW ? val : 0,
							sd->state.lr_flag == LR_FLAG_ARROW ? val : 0,
							0,
							0);
			break;
		...
		case SP_ADD_DAMAGE_CLASS: { // bonus2 bAddDamageClass,mid,x;
			...
			if (wd->add_dmg.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_DAMAGE_CLASS: Reached max (%d) number of add class damage bonuses per "
					"character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_MDEF_MONSTER: // bonus2 bAddMDefMonster,mid,x;
			...
			if (sd->add_mdef.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_MDEF_MONSTER: Reached max (%d) number of add class mdef bonuses per "
					"character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_SKILL_HEAL2: // bonus2 bSkillHeal2,sk,n;
			...
			if (sd->skillheal2.size() ==
				MAX_PC_BONUS) { // Better mention this so the array length can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: SP_SKILL_HEAL2: Reached max (%d) number of skills per character, bonus skill %d "
					"(+%d%%) lost.\n",
					MAX_PC_BONUS,
					type2,
					val);
		...
		case SP_ADD_SKILL_BLOW: // bonus2 bAddSkillBlow,sk,n;
			...
			if (sd->skillblown.size() ==
				MAX_PC_BONUS) { // Better mention this so the array length can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: SP_ADD_SKILL_BLOW: Reached max (%d) number of skills per character, bonus skill %d "
					"(%d) lost.\n",
					MAX_PC_BONUS,
					type2,
					val);
		...
		case SP_ADD_ITEMGROUP_HEAL_RATE: // bonus2 bAddItemGroupHealRate,ig,n;
			...
			if (sd->itemgrouphealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEMGROUP_HEAL_RATE: Reached max (%d) number of item heal bonuses per "
					"character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_SKILL_COOLDOWN: // bonus2 bSkillCooldown,sk,t;
			...
			if (sd->skillcooldown.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_COOLDOWN: Reached max (%d) number of skills per character, bonus skill %d "
					"(%d) lost.\n",
					MAX_PC_BONUS,
					type2,
					val);
		...
		case SP_SKILL_FIXEDCAST: // bonus2 bSkillFixedCast,sk,t;
			...
			if (sd->skillfixcast.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_FIXEDCAST: Reached max (%d) number of skills per character, bonus skill %d "
					"(%d) lost.\n",
					MAX_PC_BONUS,
					type2,
					val);
		...
		case SP_CASTRATE: // bonus2 bCastrate,sk,n;
			...
			if (sd->skillcastrate.size() ==
				...
				ShowWarning(
					...
					(type == SP_CASTRATE) ? "SP_CASTRATE" : "SP_VARCASTRATE",
					MAX_PC_BONUS,
					type2,
					val);
				break;
		...
		case SP_SUB_SKILL: // bonus2 bSubSkill,sk,n;
			if (sd->subskill.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SUB_SKILL: Reached max (%d) number of skills per character, bonus skill %d (%d) "
					"lost.\n",
					MAX_PC_BONUS,
					type2,
					val);
		...
		case SP_ADD_ITEM_SPHEAL_RATE: // bonus2 bAddItemSPHealRate,iid,n;
			...
			if (sd->itemsphealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEM_SPHEAL_RATE: Reached max (%d) number of item SP heal bonuses per "
					"character!\n",
					MAX_PC_BONUS);
				break;
			}
	...
	switch (type) {
		...
		case SP_AUTOSPELL: // bonus3 bAutoSpell,sk,y,n;
			if (sd->state.lr_flag != LR_FLAG_ARROW) {
				...
				pc_bonus_autospell(sd->autospell,
								   type2,
								   type3,
								   val,
								   0,
		...
		case SP_AUTOSPELL_WHENHIT: // bonus3 bAutoSpellWhenHit,sk,y,n;
			if (sd->state.lr_flag != LR_FLAG_ARROW) {
				...
				target = target & INF_SUPPORT_SKILL ||
						 (target & INF_SELF_SKILL && !skill_get_inf2(type2, INF2_NOTARGETSELF));
				pc_bonus_autospell(sd->autospell2,
								   type2,
								   type3,
								   val,
								   BF_NORMAL | BF_SKILL,
		...
		case SP_ADDEFF: // bonus3 bAddEff,eff,n,y;
			...
			pc_bonus_addeff(sd->addeff,
							(sc_type)type2,
							sd->state.lr_flag != LR_FLAG_ARROW ? type3 : 0,
							sd->state.lr_flag == LR_FLAG_ARROW ? type3 : 0,
							val,
							0);
			break;
...
void pc_bonus4(map_session_data *sd, int32 type, int32 type2, int32 type3, int32 type4, int32 val) {
	...
	switch (type) {
		case SP_AUTOSPELL: // bonus4 bAutoSpell,sk,y,n,i;
			if (sd->state.lr_flag != LR_FLAG_ARROW) {
				pc_bonus_autospell(
					sd->autospell, type2, type3, type4, 0, current_equip_card_id, val & AUTOSPELL_FORCE_ALL);
			}
			break;
		...
		case SP_AUTOSPELL_ONSKILL: // bonus4 bAutoSpellOnSkill,sk,x,y,n;
			if (sd->state.lr_flag != LR_FLAG_ARROW) {
				...
				pc_bonus_autospell_onskill(sd->autospell3,
										   type2,
										   type3,
										   type4,
										   val,
...
int32 pc_paycash(map_session_data *sd, int32 price, int32 points, e_log_pick_type type) {
	...
	if (battle_config.cashshop_show_points) {
		...
		sprintf(output,
				...
				cash,
				sd->kafraPoints,
				sd->cashPoints); // Used %d kafra points and %d cash points. %d kafra and %d cash points remaining.
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
	}
	return cash + points;
}
...
bool pc_dropitem(map_session_data *sd, int32 n, int32 amount) {
	...
	if (!map_addflooritem(&sd->inventory.u.items_inventory[n],
						  amount,
						  sd->bl.m,
						  sd->bl.x,
						  sd->bl.y,
						  ...
						  false,
						  DIR_MAX,
						  battle_config.item_stacking ? BL_NUL : BL_ITEM)) {
		return false;
	}
...
bool pc_isUseitem(map_session_data *sd, int32 n) {
	...
	if (itemdb_group.item_exists(IG_GIANT_FLY_WING, nameid)) {
		...
		if (pd) {
			...
			ARR_FIND(0,
					 MAX_PARTY,
					 i,
					 pd->data[i].sd && pd->data[i].sd != sd && pd->data[i].sd->bl.m == sd->bl.m &&
						 !pc_isdead(pd->data[i].sd));
	...
	if (item->flag.group || item->type == IT_CASH) {
		...
		// TODO: Count the items the player will get and check for the actual inventory space required std::max<size_t>(
		// count, 10 )
...
bool pc_steal_item(map_session_data *sd, struct block_list *bl, uint16 skill_lv) {
	...
	if (md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
		...
		sprintf(message,
				msg_txt(sd, 542),
				(sd->status.name[0]) ? sd->status.name : "GM",
				md->db->jname.c_str(),
				i_data->ename.c_str(),
...
enum e_setpos pc_setpos(map_session_data *sd, unsigned short mapindex, int32 x, int32 y, clr_type clrtype) {
	...
	if (sd->state.changemap) { // Misc map-changing settings
		int32 curr_map_instance_id = map_getmapdata(sd->bl.m)->instance_id,
		...
		if (sd->bg_id && mapdata &&
			!mapdata->getMapFlag(MF_BATTLEGROUND)) { // Moving to a map that isn't a Battlegrounds
			...
			for (const auto &it : status_db) {
				if (sc->getSCE(it.first)) {
					...
					if (it.second->flag[SCF_RESTARTONMAPWARP] && it.second->skill_id > 0) {
						...
						sce->timer = add_timer(gettick() + skill_get_time(it.second->skill_id, sce->val1),
											   status_change_timer,
											   sd->bl.id,
											   it.first);
					}
...
bool pc_memo(map_session_data *sd, int32 pos) {
	...
	if (pos == -1) {
		...
		memmove(&sd->status.memo_point[1],
				&sd->status.memo_point[0],
		...
		for (const auto &status : status_db) {
			if (status.second->flag[SCF_SUPERNOVICEANGEL]) {
				sc_start(&sd->bl,
						 ...
						 100,
						 1,
						 120000); // All buffs with super novice flag for 2 minutes
			}
		}
	...
	for (i = 0; i < EQI_MAX; i++) {
		if (sd->equip_index[i] >= 0 && sd->inventory_data[sd->equip_index[i]]) {
			if (sd->inventory_data[sd->equip_index[i]]->elvmax &&
				sd->status.base_level > (uint32)sd->inventory_data[sd->equip_index[i]]->elvmax) {
...
static void pc_calcexp(map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src) {
	...
	if (src) {
		...
		if (battle_config.pk_mode && (int32)(status_get_lv(src) - sd->status.base_level) >= 20) {
...
void pc_gainexp_disp(
	...
	sprintf(output,
			msg_txt(sd, 743), // Experience %s Base:%ld (%0.2f%%) Job:%ld (%0.2f%%)
			(lost) ? msg_txt(sd, 742) : msg_txt(sd, 741),
			(long)base_exp * (lost ? -1 : 1),
...
void pc_skillup(map_session_data *sd, uint16 skill_id) {
	...
	// Level up homunculus skill
		...
		if (sd->status.skill_point > 0 && sd->status.skill[idx].id &&
			...
			if (!pc_has_permission(sd, PC_PERM_ALL_SKILL)) { // may skill everything at any time anyways, and this would
															 // cause a huge slowdown
...
int32 pc_dead(map_session_data *sd, struct block_list *src) {
	...
	if (src) {
		switch (src->type) {
			case BL_MOB: {
				...
				if (battle_config.mobs_level_up && md->status.hp && (uint32)md->level < pc_maxbaselv(sd) &&
	...
	if (mapdata->getMapFlag(MF_PVP_NIGHTMAREDROP)) { // Moved this outside so it works when PVP isn't enabled and during
													 // pk mode [Ancyker]
			...
			if (id == -1) {
				...
				for (i = 0; i < MAX_INVENTORY; i++) {
					if (sd->inventory.u.items_inventory[i].nameid == id &&
						pc_candrop(sd, &sd->inventory.u.items_inventory[i]) && rnd() % 10000 < per &&
...
bool pc_setparam(map_session_data *sd, int64 type, int64 val_tmp) {
	...
	switch (type) {
		...
		case SP_KAFRAPOINTS:
			...
			if (!sd->state.connect_new) {
				log_cash(
					sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_KAFRA, -(sd->kafraPoints - cap_value(val, 0, MAX_KAFRAPOINT)));
			}
 ...
 *------------------------------------------*/
void pc_heal(map_session_data *sd,
			 uint32 hp,
			 uint32 sp,
			 uint32 ap,
			 int32 type) { // Is there going to be a effect for gaining AP soon??? [Rytech]
	nullpo_retv(sd);
...
bool pc_jobchange(map_session_data *sd, int32 job, char upper) {
	...
	clif_changelook(
		&sd->bl,
		LOOK_BASE,
		sd->vd.class_); // move sprite update to prevent client crashes with incompatible equipment [Valaris]
#if PACKETVER >= 20151001
	clif_changelook(
		&sd->bl, LOOK_HAIR, sd->vd.hair_style); // Update player's head (only matters when switching to or from Doram)
#endif
		...
		case LOOK_HAIR: // Use the battle_config limits! [Skotlex]
			...
			if (sd->status.hair != val) {
				...
				if (sd->status.guild_id) { // Update Guild Window. [Skotlex]
					intif_guild_change_memberinfo(sd->status.guild_id,
												  sd->status.account_id,
												  sd->status.char_id,
												  GMI_HAIR,
												  &sd->status.hair,
	...
	switch (type) {
		...
		default: /* everything else is an allowed ID so we can move on */
			if (!sd->sc.getSCE(SC_PUSH_CART)) { /* first time, so fill cart data */
				clif_cartlist(sd);
				status_calc_cart_weight(sd,
										(e_status_calc_weight_opt)(CALCWT_ITEM | CALCWT_MAXBONUS | CALCWT_CARTSTATE));
			}
			clif_updatestatus(*sd, SP_CARTINFO);
		...
		for (i = 0; i < sd->storage.max_amount; i++) {
			...
			if (!itemdb_available(nameid)) {
				...
				ShowWarning("Removed invalid/disabled item (ID: %u, amount: %d) from storage (char_id: %d).\n",
							nameid,
							sd->storage.u.items_storage[i].amount,
							sd->status.char_id);
		...
		for (const auto &it : node["Tree"]) {
			...
			if (max_lv > skill_lv_max) {
				this->invalidWarning(it["MaxLevel"],
									 "Skill %s's level %hu exceeds the skill's max level %hu. Capping skill level.\n",
									 skill_name.c_str(),
									 max_lv,
									 skill_lv_max);
				max_lv = skill_lv_max;
			...
			if (this->nodeExists(it, "BaseLevel")) {
				...
				if (baselv > baselv_max) {
					this->invalidWarning(it["BaseLevel"],
										 "Skill %hu's base level requirement %hu exceeds job %s's max base level %d. "
										 "Capping skill base level.\n",
										 skill_id,
										 baselv,
										 job_name.c_str(),
										 baselv_max);
			...
			if (this->nodeExists(it, "JobLevel")) {
				...
				if (joblv > joblv_max) {
					this->invalidWarning(it["JobLevel"],
										 "Skill %hu's job level requirement %hu exceeds job %s's max job level %d. "
										 "Capping skill job level.\n",
										 skill_id,
										 joblv,
										 job_name.c_str(),
										 joblv_max);
			...
			if (this->nodeExists(it, "Requires")) {
				for (const auto &Requiresit : it["Requires"]) {
					...
					if (lv_req > MAX_SKILL_LEVEL) {
						this->invalidWarning(Requiresit["Level"],
		...
		for (const auto &jobit : jobsNode) {
			...
			if (this->nodeExists(node, "MaxBaseLevel")) {
				...
				if (level == 0 || level > MAX_LEVEL) {
					this->invalidWarning(node["MaxBaseLevel"],
			...
			if (this->nodeExists(node, "MaxJobLevel")) {
				...
				if (level == 0 || level > MAX_LEVEL) {
					this->invalidWarning(node["MaxJobLevel"],
		...
		while (entries < MOTD_LINE_SIZE && fgets(buf, CHAT_SIZE_MAX, fp)) {
			...
			if (len) {
				...
				if ((ptr = strstr(buf, " :")) != nullptr && ptr - buf >= NAME_LENGTH) { // crashes newer clients
					ShowWarning("Found sequence '" CL_WHITE " :" CL_RESET "' on line '" CL_WHITE "%u" CL_RESET
								"' in '" CL_WHITE "%s" CL_RESET "'. This can cause newer clients to crash.\n",
								lines,
								motd_txt);
				}
			}
			...
			if (DIFF_TICK(sd->item_delay[i].tick, tick) > 0) {
				...
				if (e_tick > 99) {
					sprintf(e_msg,
							...
							itemdb_ename(sd->item_delay[i].nameid),
							(double)e_tick / 60);
				}
				else {
					sprintf(e_msg,
...
void pc_scdata_received(map_session_data *sd) {
	pc_inventory_rentals(
		sd); // Needed here to remove rentals that have Status Changes after chrif_load_scdata has finished
...
void pc_expire_check(map_session_data *sd) {
	...
	sd->expiration_tid =
		add_timer(gettick() + (uint32)(sd->expiration_time - time(nullptr)) * 1000, pc_expiration_timer, sd->bl.id, 0);
}
...
enum e_BANKING_WITHDRAW_ACK pc_bank_withdraw(map_session_data *sd, int32 money) {
	...
	if (money <= 0) {
		...
		clif_messagecolor(
			&sd->bl, color_table[COLOR_RED], msg_txt(sd, 1495), false, SELF); // You can't withdraw that much money
		return BWA_UNKNOWN_ERROR;
	}
...
static void pc_macro_punishment(map_session_data &sd, e_macro_detect_status stype) {
	...
	if (battle_config.macro_detection_punishment == 0) { // Ban
		...
		chrif_req_login_operation(sd.macro_detect.reporter_aid,
								  sd.status.name,
								  (duration == 0 ? CHRIF_OP_LOGIN_BLOCK : CHRIF_OP_LOGIN_BAN),
								  duration,
								  0,
								  0);
	}
 ...
 */
void pc_macro_captcha_register(map_session_data &sd,
							   uint16 image_size,
							   const char captcha_answer[CAPTCHA_ANSWER_SIZE]) {
...
uint64 CaptchaDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Bonus")) {
		...
		cd->bonus_script = parse_script(script.c_str(),
										this->getCurrentFile().c_str(),
										this->getLineNumber(node["Bonus"]),
...
void do_init_pc(void) {
	...
	if (battle_config.day_duration > 0 && battle_config.night_duration > 0) {
		...
		day_timer_tid = add_timer_interval(gettick() + (night_flag ? 0 : day_duration) + night_duration,
										   map_day_timer,
										   0,
										   0,
										   day_duration + night_duration);
		night_timer_tid = add_timer_interval(gettick() + day_duration + (night_flag ? night_duration : 0),
											 map_night_timer,
											 0,
											 0,
											 day_duration + night_duration);
