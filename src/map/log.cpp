...
void log_branch(map_session_data* sd) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS != stmt.Prepare(LOG_QUERY " INTO `%s` (`branch_date`, `account_id`, `char_id`, `char_name`, "
												  "`map`) VALUES (NOW(), '%d', '%d', ?, '%s')",
										log_config.log_branch,
...
void log_pick(int32 id, int16 m, e_log_pick_type type, int32 amount, struct item* itm) {
	...
	if (log_config.sql_logs) {
		...
		StringBuf_Printf(&buf,
						 "%s INTO `%s` (`time`, `char_id`, `type`, `nameid`, `amount`, `refine`, `map`, `unique_id`, "
						 "`bound`, `enchantgrade`",
						 LOG_QUERY,
						 log_config.log_pick);
		...
		StringBuf_Printf(&buf,
						 ") VALUES(NOW(),'%u','%c','%u','%d','%d','%s','%" PRIu64 "','%d','%d'",
						 id,
		...
		strftime(timestring, sizeof(timestring), log_timestamp_format, localtime(&curtime));
		fprintf(logfp,
				"%s - %d\t%c\t%u,%d,%d,%u,%u,%u,%u,%s,'%" PRIu64 "',%d,%d\n",
				timestring,
				id,
				log_picktype2char(type),
				itm->nameid,
...
void log_atcommand(map_session_data* sd, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS != stmt.Prepare(LOG_QUERY " INTO `%s` (`atcommand_date`, `account_id`, `char_id`, `char_name`, "
												  "`map`, `command`) VALUES (NOW(), '%d', '%d', ?, '%s', ?)",
										log_config.log_gm,
										sd->status.account_id,
										sd->status.char_id,
										sd->mapindex == 0 ? "" : mapindex_id2name(sd->mapindex)) ||
			SQL_SUCCESS != stmt.BindParam(0, SQLDT_STRING, sd->status.name, strnlen(sd->status.name, NAME_LENGTH)) ||
...
void log_npc(struct npc_data* nd, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS != stmt.Prepare(LOG_QUERY
										" INTO `%s` (`npc_date`, `char_name`, `map`, `mes`) VALUES (NOW(), ?, '%s', ?)",
										log_config.log_npc,
										map_mapid2mapname(nd->bl.m)) ||
...
void log_npc(map_session_data* sd, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS != stmt.Prepare(LOG_QUERY " INTO `%s` (`npc_date`, `account_id`, `char_id`, `char_name`, "
...
void log_chat(e_log_chat_type type,
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS != stmt.Prepare(LOG_QUERY
										...
										y) ||
			SQL_SUCCESS !=
				stmt.BindParam(0, SQLDT_STRING, (char*)dst_charname, safestrnlen(dst_charname, NAME_LENGTH)) ||
...
void log_cash(map_session_data* sd, e_log_pick_type type, e_log_cash_type cash_type, int32 amount) {
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR == Sql_Query(logmysql_handle,
								   LOG_QUERY " INTO `%s` ( `time`, `char_id`, `type`, `cash_type`, `amount`, `map` ) "
											 "VALUES ( NOW(), '%d', '%c', '%c', '%d', '%s' )",
								   log_config.log_cash,
								   sd->status.char_id,
								   log_picktype2char(type),
...
void log_feeding(map_session_data* sd, e_log_feeding_type type, t_itemid nameid) {
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR == Sql_Query(logmysql_handle,
								   ...
								   log_config.log_feeding,
								   sd->status.char_id,
								   target_id,
								   target_class,
								   log_feedingtype2char(type),
								   intimacy,
								   nameid,
								   mapindex_id2name(sd->mapindex),
								   sd->bl.x,
