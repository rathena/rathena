...
int32 intif_broadcast2(
	const char *mes, size_t len, unsigned long fontColor, short fontType, short fontSize, short fontAlign, short fontY);
...
int32 intif_party_leave(
	int32 party_id, uint32 account_id, uint32 char_id, const char *name, enum e_party_member_withdraw type);
...
int32 intif_guild_memberinfoshort(
	int32 guild_id, uint32 account_id, uint32 char_id, int32 online, int32 lv, int32 class_);
...
int32 intif_guild_change_memberinfo(
...
int32 intif_create_pet(uint32 account_id,
					   uint32 char_id,
					   short pet_type,
					   short pet_lv,
					   t_itemid pet_egg_id,
