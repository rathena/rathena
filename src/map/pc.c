...
int pc_calcweapontype(struct map_session_data *sd)
{
	...
	else if(sd->weapontype1 == W_FIST && sd->weapontype2 != W_FIST)// ç∂éËïêäÌ Only
		sd->status.weapon = sd->weapontype2;
	else if(sd->weapontype1 == W_DAGGER && sd->weapontype2 == W_DAGGER)// ?íZ?
		sd->status.weapon = MAX_WEAPON_TYPE+1;
	else if(sd->weapontype1 == W_1HSWORD && sd->weapontype2 == W_1HSWORD)// ??éË?
		sd->status.weapon = MAX_WEAPON_TYPE+2;
	else if(sd->weapontype1 == W_1HAXE && sd->weapontype2 == W_1HAXE)// ??éËïÄ
		sd->status.weapon = MAX_WEAPON_TYPE+3;
	else if( (sd->weapontype1 == W_DAGGER && sd->weapontype2 == W_1HSWORD) ||
		(sd->weapontype1 == W_1HSWORD && sd->weapontype2 == W_DAGGER) ) // íZ? - ?éË?
		sd->status.weapon = MAX_WEAPON_TYPE+4;
	else if( (sd->weapontype1 == W_DAGGER && sd->weapontype2 == W_1HAXE) ||
		(sd->weapontype1 == W_1HAXE && sd->weapontype2 == W_DAGGER) ) // íZ? - ïÄ
		sd->status.weapon = MAX_WEAPON_TYPE+5;
	else if( (sd->weapontype1 == W_1HSWORD && sd->weapontype2 == W_1HAXE) ||
		(sd->weapontype1 == W_1HAXE && sd->weapontype2 == W_1HSWORD) ) // ?éË? - ïÄ
...
int pc_skill(struct map_session_data *sd,int id,int level,int flag)
{
	...
	if(!flag && (sd->status.skill[id].id == id || level == 0)){	// ÉNÉGÉXÉgèäìæÇ»ÇÁÇ±Ç±Ç≈?åèÇämîFÇµÇƒëóêMÇ∑ÇÈ
		sd->status.skill[id].lv=level;
		if (!skill_get_inf(id)) //Only recalculate for passive skills.
			status_calc_pc(sd,0);
		clif_skillinfoblock(sd);
	}
	else if(flag==2 && (sd->status.skill[id].id == id || level == 0)){	// ÉNÉGÉXÉgèäìæÇ»ÇÁÇ±Ç±Ç≈?åèÇämîFÇµÇƒëóêMÇ∑ÇÈ
		sd->status.skill[id].lv+=level;
		if (!skill_get_inf(id)) //Only recalculate for passive skills.
			status_calc_pc(sd,0);
		clif_skillinfoblock(sd);
	}
	else if(sd->status.skill[id].lv < level){	// ?Ç¶ÇÁÇÍÇÈÇ™lvÇ™è¨Ç≥Ç¢Ç»ÇÁ
...
int pc_insert_card(struct map_session_data *sd,int idx_card,int idx_equip)
{
	...
	if( nameid <= 0 || cardid <= 0 ||
		(sd->inventory_data[idx_equip]->type!=4 && sd->inventory_data[idx_equip]->type!=5)||	// ? îıÇ∂Ç·Ç»Ç¢
		sd->inventory_data[idx_card]->type!=6 || // Prevent Hack [Ancyker]
		sd->status.inventory[idx_equip].identify==0 ||		// ñ¢ä”íË
		sd->status.inventory[idx_equip].card[0]==0x00ff ||		// êªë¢ïêäÌ
		sd->status.inventory[idx_equip].card[0]==0x00fe ||
		sd->status.inventory[idx_equip].card[0]==(short)0xff00 ||
		!(sd->inventory_data[idx_equip]->equip&ep) ||					// ? îıå¬èäà·Ç¢
		(sd->inventory_data[idx_equip]->type==4 && ep==32) ||			// ? éËïêäÌÇ∆èÇÉJ?Éh
	...
	for(i=0;i<sd->inventory_data[idx_equip]->slot;i++){
		...
		// ãÛÇ´ÉXÉçÉbÉgÇ™Ç Ç¡ÇΩÇÃÇ≈ç∑Çµ?Çﬁ
...
int pc_cart_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	i=MAX_CART;
	if(!itemdb_isequip2(data)){
		// ëï îıïiÇ≈ÇÕÇ»Ç¢ÇÃÇ≈ÅAä˘èäóLïiÇ»ÇÁå¬êîÇÃÇ›ïœâªÇ≥ÇπÇÈ
	...
	if(i >= MAX_CART){
		// ëï îıïiÇ©ñ¢èäóLïiÇæÇ¡ÇΩÇÃÇ≈ãÛÇ´óìÇ÷í«â¡
		for(i=0;i<MAX_CART;i++){
...
int pc_allskillup(struct map_session_data *sd)
{
	...
	for(i=0;i<MAX_SKILL;i++){
		sd->status.skill[i].id=0;
		if (sd->status.skill[i].flag && sd->status.skill[i].flag != 13){	// cardÉXÉLÉãÇ»ÇÁÅA
			sd->status.skill[i].lv=(sd->status.skill[i].flag==1)?0:sd->status.skill[i].flag-2;	// ñ{?ÇÃlvÇ…
...
int pc_setcart(struct map_session_data *sd,int type)
{
	...
	if(pc_checkskill(sd,MC_PUSHCART)>0){ // ÉvÉbÉVÉÖÉJ?ÉgÉXÉLÉãèäéù
		option = sd->sc.option;
		//This should preserve the current option, only modifying the cart bit.
		option&=~OPTION_CART;
		option|=cart[type];
		if(!pc_iscarton(sd)){ // ÉJ?ÉgÇïtÇØÇƒÇ¢Ç»Ç¢
...
//
	...
	if(pos==0x88){ // ÉAÉNÉZÉTÉäópó·äO?óù
		int epor=0;
	...
	// ìÒìÅó¨?óù
	if ((pos==0x22) // àÍ?ÅA?îıóvãÅâ”èäÇ™ìÒìÅó¨ïêäÌÇ©É`ÉFÉbÉNÇ∑ÇÈ
	 &&	(id->equip==2)	// ? éËïêäÌ
	 &&	(pc_checkskill(sd, AS_LEFT) > 0 || (sd->class_&MAPID_UPPERMASK) == MAPID_ASSASSIN) ) // ç∂éËèC?óL
	{
		int tpos=0;
	...
	if(pos==0x8000){
		...
		clif_arrow_fail(sd,3);	// 3=ñÓÇ™?îıÇ≈Ç´Ç‹ÇµÇΩ
	}
	else
	...
	pc_checkallowskill(sd);	// ?îıïiÇ≈ÉXÉLÉãÇ©âèúÇ≥ÇÍÇÈÇ©É`ÉFÉbÉN
...
/*==========================================
 * sdÇ™ó£ç•(ëäéËÇÕsd->status.partner_idÇ…àÀÇÈ)(ëäéËÇ‡ìØéb…ó£ç•?åãç•éwó÷é©ìÆ?íD)
 *------------------------------------------
 */
...
static int pc_spheal(struct map_session_data *sd)
{
	...
	if(pc_issit(sd))
		a += a;
	if (sd->sc.count) {
		if (sd->sc.data[SC_MAGNIFICAT].timer!=-1)	// É}ÉOÉjÉtÉBÉJ?Ég
			a += a;
 ...
 * ê›íËÉtÉ@ÉCÉã?Ç›?Çﬁ
 * exp.txt ïKóv??íl
 * job_db1.txt èdó ,hp,sp,çU?ë¨ìx
 * job_db2.txt jobî\óÕílÉ{?ÉiÉX
 * skill_tree.txt äeêE?ÇÃÉXÉLÉãÉcÉä?
 * attr_fix.txt ?ê´èCê≥Ée?ÉuÉã
 * size_fix.txt ÉTÉCÉYï‚ê≥Ée?ÉuÉã
 * refine_db.txt ê∏?Éf?É^Ée?ÉuÉã
 *------------------------------------------
 */
int pc_readdb(void)
{
