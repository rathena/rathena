...
int32 channel_join(struct Channel *channel, map_session_data *sd) {
	...
	if (channel->type == CHAN_TYPE_PRIVATE && db_size(channel->users) >= channel_config.private_channel.max_member) {
		...
		sprintf(output,
				msg_txt(sd, 760),
				channel->name,
				channel_config.private_channel.max_member); // You cannot join channel '%s'. Limit of %d has been met.
		clif_displaymessage(sd->fd, output);
		return -4;
	}
			...
			if (strcmp(channel->pass, pass) != 0) { // wrong pass entry
				if (pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
					...
					sprintf(output,
							msg_txt(sd, 1401),
							chname,
							"@join"); // Channel '%s' is password-protected (usage: %s <#channel_name> <password>).
					clif_displaymessage(sd->fd, output);
					return -1;
				}
			}
...
int32 channel_pcban(map_session_data *sd, char *chname, char *pname, int32 flag) {
	...
	switch (flag) {
		case 0: {
			...
			sprintf(
				output, msg_txt(sd, 1437), tsd->status.name, chname); // Player '%s' is banned from the '%s' channel.
			break;
		}
		case 1:
			if (!tsd) {
			...
			sprintf(
				output, msg_txt(sd, 1441), tsd->status.name, chname); // Player '%s' is unbanned from the '%s' channel.
			break;
		case 2:
...
int32 channel_pckick(map_session_data *sd, char *chname, char *pname) {
	...
	if (!channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE)) {
		safesnprintf(
			output, CHAT_SIZE_MAX, msg_txt(sd, 768), channel->alias, tsd->status.name); // %s %s has been kicked.
		clif_channel_msg(channel, output, channel->color);
	}
	...
	if (val[0] == '\0') {
		if (opt == CHAN_OPT_MSG_DELAY) {
			...
			sprintf(output,
					msg_txt(sd, 1449),
					opt_str[k],
					opt_str[k]); // Option '%s' is already enabled (use '@channel setopt %s 0' to disable).
			clif_displaymessage(sd->fd, output);
			return -1;
		}
		else {
...
bool channel_read_sub(config_setting_t *chan, struct Channel *tmp_chan, uint8 i) {
	...
	int32 delay = 1000, autojoin = 0, leave = 1, chat = 1, color_override = 0, self_notif = 1, join_notif = 0,
		  leave_notif = 0;
...
void channel_read_config(void) {
	...
	if (chan_setting != nullptr) {
		...
		if (private_channel != nullptr) {
			int32 allow = 1, ban = 1, kick = 1, color_override = 0, change_delay = 0, self_notif = 1, join_notif = 0,
