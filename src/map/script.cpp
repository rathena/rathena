...
void script_free_state(struct script_state* st)
{
	if (idb_exists(st_db, st->id)) {
		struct map_session_data *sd = st->rid ? map_id2sd(st->rid) : nullptr;
...
BUILDIN_FUNC(callfunc)
{
	...
	struct script_code* scr = (struct script_code*)strdb_get(userfunc_db, str);
...
BUILDIN_FUNC(callsub)
{
	...
	int i,j;
	struct script_retinfo* ri;
	struct reg_db *ref = (struct reg_db *)aCalloc(sizeof(struct reg_db), 1);
...
BUILDIN_FUNC(getarg)
{
	...
	struct script_retinfo* ri = st->stack->stack_data[st->stack->defsp - 1].u.ri;
...
BUILDIN_FUNC(cleararray)
{
	...
	uint32 end = start + script_getnum(st, 4);
	if( end > SCRIPT_MAX_ARRAYSIZE )
		end = SCRIPT_MAX_ARRAYSIZE;
...
BUILDIN_FUNC(getguildname)
{
	...
	if( ( g = guild_search(guild_id) ) != nullptr )
...
BUILDIN_FUNC(getmobdrops)
{
	...
	for( i = 0; i < MAX_MOB_DROP_TOTAL; i++ )
	{
		if( mob->dropitem[i].nameid == 0 )
			continue;
		if( itemdb_exists(mob->dropitem[i].nameid) == nullptr )
			continue;
...
BUILDIN_FUNC(clone)
{
	map_session_data* sd, *msd=nullptr;
...
BUILDIN_FUNC(warpwaitingpc)
{
	...
	{
		map_session_data* sd = cd->usersd[0];
		...
		{// fee set
			...
			pc_payzeny(sd, cd->zeny, LOG_TYPE_NPC, nullptr);
		}
...
BUILDIN_FUNC(addrid)
{
	...
	switch(script_getnum(st,2)) {
		...
		default:
			mapit_free(iter);
			if((map_id2sd(script_getnum(st,2))) == nullptr) { // Player not found.
				script_pushint(st,0);
				return SCRIPT_CMD_SUCCESS;
			}
...
BUILDIN_FUNC(ispartneron)
{
	...
	if ( !pc_ismarried(sd) || map_charid2sd(sd->status.partner_id) == nullptr)
		script_pushint(st,0);
	else
		script_pushint(st,1);
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(warppartner)
{
	...
	if (!pc_ismarried(sd) || (p_sd=map_charid2sd(sd->status.partner_id)) == nullptr) {
		script_pushint(st,0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(recovery)
{
	...
	switch(type) {
		case 0:
			if(!script_charid2sd(3,sd))
				return SCRIPT_CMD_FAILURE; //If we don't have sd by now, bail out
...
BUILDIN_FUNC(autoequip)
{
	...
	if( ( item_data = itemdb_exists(nameid) ) == nullptr )
	{
...
BUILDIN_FUNC(unitattack)
{
	...
	if (script_isstring(st, 3)) {
		map_session_data* sd = map_nick2sd(script_getstr(st, 3),false);
		if( sd != nullptr )
			target_bl = &sd->bl;
	} else
...
BUILDIN_FUNC(party_delmember)
{
	...
	if( sd || script_charid2sd(2,sd) )	// todo
...
BUILDIN_FUNC(party_destroy)
{
	...
	if( i >= MAX_PARTY || !party->data[i].sd ) { //leader not online
		...
		for( j = 0; j < MAX_PARTY; j++ ) {
			map_session_data* sd = party->data[j].sd;
			if(sd)
		...
		for (i = 0; i < MAX_GUILD; i++) {
			if (g->member[i].account_id) {
				switch (type) {
					case 2:
						if (data)
							setd_sub_num( st, nullptr, varname, j, g->member[i].account_id, data->ref );
						else
							mapreg_setreg(reference_uid(add_str("$@guildmemberaid"), j),g->member[i].account_id);
						break;
					case 1:
						if (data)
							setd_sub_num( st, nullptr, varname, j, g->member[i].char_id, data->ref );
						else
							mapreg_setreg(reference_uid(add_str("$@guildmembercid"), j), g->member[i].char_id);
						break;
					default:
						if (data)
							setd_sub_str( st, nullptr, varname, j, g->member[i].name, data->ref );
						else
