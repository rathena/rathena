...
BUILDIN_FUNC(checkweight2)
{
	...
	for(i=0; i<nb_it; i++) {
		nameid_t nameid = (nameid_t)get_val2_num( st, reference_uid( id_it, idx_it + i ), reference_getref( data_it ) );
...
{
	nameid_t nameid = (unsigned short)va_arg(ap, int);
...
BUILDIN_FUNC(npcshopadditem)
{
	...
	if (nd->subtype == NPCTYPE_MARKETSHOP) {
		for (n = 0, i = 3; n < amount; n++, i += offs) {
			nameid_t nameid = script_getnum(st,i), j;
...
BUILDIN_FUNC(npcshopdelitem)
{
	...
	for( i = 3; i < 3 + amount; i++ ) {
		nameid_t nameid = script_getnum(st,i);
...
BUILDIN_FUNC(consumeitem)
{
	...
	if( script_isstring(st, 2) ){
		...
		nameid_t nameid = script_getnum(st, 2);
...
BUILDIN_FUNC(npcshopupdate) {
	const char* npcname = script_getstr(st, 2);
	struct npc_data* nd = npc_name2id(npcname);
	nameid_t nameid = script_getnum(st, 3);
...
BUILDIN_FUNC(mail){
	...
	while( script_hasdata(st,7) ){
		...
		for( i = 0; i < num_items && start < end; i++, start++ ){
			msg.item[i].nameid = (nameid_t)get_val2_num( st, reference_uid( id, start ), reference_getref( data ) );
