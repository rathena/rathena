...
int32 run_func(struct script_state *st)
{
	...
	if(str_data[func].func) {
		...
		if (str_data[func].func(st) == SCRIPT_CMD_FAILURE) {	// Report error and terminate the script
...
BUILDIN_FUNC(getcharid)
{
	...
	if (!script_nick2sd(3, sd)){
		script_pushint(st,0); // !TODO: remove this logic
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(getunitname)
{
	...
	if (!script_rid2bl(2, bl)) {	// !TODO: need logic update to not throw an error
