		...
		default:
			if (bl->m >= 0) {
				ShowDebug("Source (Non-NPC type %d): name %s at %s (%d,%d)\n",
						  bl->type,
						  status_get_name(*bl),
						  map_mapid2mapname(bl->m),
						  bl->x,
						  bl->y);
			}
			else {
				ShowDebug(
					"Source (Non-NPC type %d): name %s (invisible/not on a map)\n", bl->type, status_get_name(*bl));
			}
			break;
	}
}
	...
	switch (*p) {
		...
		case 'C':
			if (p2 - p == 4 && !strncasecmp(p, "case", 4)) {
				...
				if (pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
					...
					sprintf(label,
							"if(%" PRId64 " != $@__SW%x_VAL) goto __SW%x_%x;",
							v,
							syntax.curly[pos].index,
							syntax.curly[pos].index,
		...
		case 'I':
			if (p2 - p == 2 && !strncasecmp(p, "if", 2)) {
				...
				syntax.curly[syntax.curly_count].flag = 0;
				sprintf(
					label, "__IF%x_%x", syntax.curly[syntax.curly_count].index, syntax.curly[syntax.curly_count].count);
				syntax.curly_count++;
				add_scriptl(add_str("jump_zero"));
				add_scriptc(C_ARG);
				p = parse_expr(p);
...
void script_set_constant_(const char *name, int64 value, const char *constant_name, bool isparameter, bool deprecated) {
	...
	if (str_data[n].type == C_NOP) { // new
		...
		ShowError("script_set_constant: Attempted to overwrite existing %s '%s' (old value=%" PRId64
				  ", new value=%" PRId64 ").\n",
				  (str_data[n].type == C_PARAM) ? "parameter" : "constant",
				  name,
				  str_data[n].val,
				  value);
	}
		...
		ShowError("script_set_constant: Invalid name for %s '%s' (already defined as %s).\n",
				  isparameter ? "parameter" : "constant",
				  name,
				  script_op2name(str_data[n].type));
	}
}
...
void script_errorwarning_sub(StringBuf *buf,
							 const char *src,
							 const char *file,
							 int32 start_line,
							 const char *error_msg_cur,
...
void script_error(
	const char *src, const char *file, int32 start_line, const char *error_msg_cur, const char *error_pos_cur) {
	StringBuf buf;
...
bool set_reg_str(
	...
	if (!script_check_RegistryVariableLength(0, name, &vlen)) {
		ShowError("set_reg: Variable name length is too long (aid: %d, cid: %d): '%s' sz=%" PRIuPTR "\n",
				  sd ? sd->status.account_id : -1,
				  sd ? sd->status.char_id : -1,
				  name,
				  vlen);
		return false;
	}
...
int64 conv_num_(struct script_state *st, struct script_data *data, map_session_data *sd) {
	...
	if (data_isint(data)) { // nothing to convert
		...
		errno = 0;
		num = strtoll(
			data->u.str, nullptr, 10); // change radix to 0 to support octal numbers "o377" and hex numbers "0xFF"
		error = (errno == ERANGE);
...
struct script_data *push_copy(struct script_stack *stack, int32 pos) {
	switch (stack->stack_data[pos].type) {
		...
		default:
			return push_val2(
				stack, stack->stack_data[pos].type, stack->stack_data[pos].u.num, stack->stack_data[pos].ref);
			break;
	}
}
					...
					if (!data_isint(data) &&
						!(data_isreference(data) &&
						  (reference_toparam(data) || reference_toconstant(data) ||
						   !is_string_variable(name)))) { // int32 ( params and constants are always int32 )
...
TIMER_FUNC(run_script_timer) {
	...
	while (node && st->sleep.timer != INVALID_TIMER) {
		if ((int32)__64BPRTSIZE(node->key) == st->oid &&
...
/// @param refcache Pointer to an int32 variable, which keeps a copy of the reference to varname and must be initialized
...
#define BUILDIN_DEF(x, args) \
	{ buildin_##x, #x, args, nullptr }
#define BUILDIN_DEF2(x, x2, args) \
	{ buildin_##x, x2, args, nullptr }
...
BUILDIN_FUNC(warp) {
	...
	if (strcmp(str, "Random") == 0) {
		...
		ret = pc_setpos(sd,
						mapindex_name2id(sd->status.save_point.map),
						sd->status.save_point.x,
		...
		switch (type) {
			...
			case WARPPARTY_SAVEPOINTALL:
				if (!mapdata->getMapFlag(MF_NORETURN)) {
					ret = pc_setpos(pl_sd,
									mapindex_name2id(pl_sd->status.save_point.map),
									pl_sd->status.save_point.x,
									pl_sd->status.save_point.y,
									CLR_TELEPORT);
				}
				break;
			case WARPPARTY_SAVEPOINT:
			...
			case WARPPARTY_RANDOMALLAREA:
				if (!mapdata->getMapFlag(MF_NORETURN) && !mapdata->getMapFlag(MF_NOWARP) &&
					...
					if (rx || ry) {
						...
						if (attempts ==
							0) { // Keep the original coordinates if fails to find a valid cell within the range
							nx = x;
							ny = y;
						}
					}
...
BUILDIN_FUNC(setr) {
	...
	if (!strcmp(command, "setr") &&
		script_hasdata(
			st, 4)) { // Optional argument used by post-increment/post-decrement constructs to return the previous value
		if (is_string_variable(name)) {
...
BUILDIN_FUNC(viewpointmap) {
	...
	map_foreachinmap(buildin_viewpointmap_sub,
					 mapid,
					 BL_PC,
					 st->oid,
					 script_getnum(st, 3),
...
static bool script_getitem_randomoption(
	struct script_state *st, map_session_data *sd, struct item *it, const char *funcname, int32 x) {
	int32 i, opt_id_n;
 ...
 */
static int32 script_countitem_sub(struct item *items,
 ...
 *
 * getitembound2 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>{,<account ID>};
 * getitembound2 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>{,<account ID>};
 *
 * getitem3 <item
 ...
 * getitem3 "<item
 ...
 ID>};
 *
 * getitembound3 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound3 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 *
 * getitem4 <item
 ...
 * getitem4 "<item
 ...
 ID>};
 *
 * getitembound4 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound4 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 ...
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<canShowEffect>}; makeitem2 "<item
 * name>",<amount>,"<map
...
/// storagedelitem2 "<Item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account
/// ID>} guildstoragedelitem2 <item
/// id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>} guildstoragedelitem2
/// "<Item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>} delitem3 <item
/// id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
/// ID>}; delitem3 "<item
/// name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
/// ID>}; delitem4 <item
...
BUILDIN_FUNC(successrefitem) {
	...
	if (i >= 0) {
		...
		if (sd->inventory_data[i]->type == IT_WEAPON) {
			achievement_update_objective(sd,
										 AG_ENCHANT_SUCCESS,
										 2,
		...
		if (sd->inventory.u.items_inventory[i].refine == battle_config.blacksmith_fame_refine_threshold &&
			...
			sd->status.char_id == (int32)MakeDWord(sd->inventory.u.items_inventory[i].card[2],
			...
			switch (sd->inventory_data[i]->weapon_level) {
				case 1:
					pc_addfame(*sd, battle_config.fame_refine_lv1); // Success to refine to +10 a lv1 weapon you forged
																	// = +1 fame point
					break;
				case 2:
					pc_addfame(*sd, battle_config.fame_refine_lv2); // Success to refine to +10 a lv2 weapon you forged
																	// = +25 fame point
					break;
				case 3:
					pc_addfame(*sd, battle_config.fame_refine_lv3); // Success to refine to +10 a lv3 weapon you forged
																	// = +1000 fame point
					break;
			}
		}
...
BUILDIN_FUNC(savepoint) {
	...
	if (cid_pos == 7) {
		int32 dx = script_getnum(st, 5), dy = script_getnum(st, 6), x1 = x + dx, y1 = y + dy, x0 = x - dx, y0 = y - dy;
		uint8 n = 10;
		do {
			x = rnd_value(x0, x1);
			y = rnd_value(y0, y1);
...
BUILDIN_FUNC(makepet) {
	...
	intif_create_pet(sd->status.account_id,
					 sd->status.char_id,
					 pet->class_,
					 mdb->lv,
					 pet->EggID,
 ...
 *------------------------------------------*/
static int32 buildin_killmonster_sub_strip(
	struct block_list *bl, va_list ap) { // same fix but with killmonster instead - stripping events from mobs.
...
static int32 buildin_killmonsterall_sub_strip(
	struct block_list *bl, va_list ap) { // Strips the event from the mob if it's killed the old method.
	...
	if (flag & (BC_TARGET_MASK | BC_SOURCE_MASK)) // Broadcast source or broadcast region defined
	{
		...
		if (fontColor) {
			clif_broadcast2(bl,
							mes,
							(int32)strlen(mes) + 1,
							strtol(fontColor, (char **)nullptr, 0),
							fontType,
							fontSize,
							fontAlign,
							fontY,
...
BUILDIN_FUNC(mapannounce) {
	...
	map_foreachinmap(buildin_announce_sub,
					 m,
					 BL_PC,
					 mes,
					 strlen(mes) + 1,
...
BUILDIN_FUNC(areaannounce) {
	...
	map_foreachinallarea(buildin_announce_sub,
						 m,
						 x0,
						 y0,
						 x1,
	...
	if (!nd) {
		if (script_hasdata(st, 2)) {
			ShowError("buildin_%s: Attempted to %s a non-existing NPC '%s' (flag=%d).\n",
					  (flag & NPCVIEW_VISIBLE) ? "show" : "hide",
					  command,
					  script_getstr(st, 2),
					  flag);
		}
...
BUILDIN_FUNC(addrid) {
	...
	switch (type) {
		...
		case 5:
			...
			map_foreachinmap(
				buildin_addrid_sub, map_mapname2mapid(script_getstr(st, 4)), BL_PC, st, script_getnum(st, 3));
			break;
		case 6:
			if (script_getnum(st, 4) == 0) {
				script_pushint(st, 0);
...
BUILDIN_FUNC(getinventorylist) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory.u.items_inventory[i].amount > 0) {
			...
			pc_setreg(sd,
					  reference_uid(add_str("@inventorylist_attribute"), j),
			...
			pc_setreg(sd,
					  reference_uid(add_str("@inventorylist_enchantgrade"), j),
...
static inline void script_skill_effect(block_list &bl, uint16 skill_id, uint16 skill_lv, int16 x, int16 y) {
	switch (skill_get_casttype(skill_id)) {
		...
		case CAST_NODAMAGE:
			...
			break;
		case CAST_DAMAGE:
			clif_skill_damage(&bl,
							  &bl,
							  gettick(),
							  status_get_amotion(&bl),
							  status_get_dmotion(&bl),
							  0,
							  1,
							  skill_id,
							  skill_lv,
...
BUILDIN_FUNC(getmapxy) {
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_getmapxy: variable '%s' for mapname is not a server variable, but no player is attached!",
				name);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError("buildin_getmapxy: variable '%s' for mapX is not a server variable, but no player is attached!",
					  name);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError("buildin_getmapxy: variable '%s' for mapY is not a server variable, but no player is attached!",
					  name);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	*buf = 0;
	...
	while ((p = strchr(q, '%'))) {
		...
		if (buf_p[strlen(buf_p) - 1] == '$') { // String
			...
			set_reg_str(st,
						sd,
						reference_uid(reference_getid(data), reference_getindex(data)),
						buf_p,
						ref_str,
...
BUILDIN_FUNC(npcshopdelitem) {
	...
	for (i = 3; i < 3 + amount; i++) {
		...
		if (n < size) {
			if (n + 1 != size) {
				memmove(&nd->u.shop.shop_item[n],
						&nd->u.shop.shop_item[n + 1],
				...
				case UMOB_SLAVECPYMSTRMD:
					if (value > 0) {
						...
						if (!md->master_id || !(md2 = map_id2md(md->master_id))) {
							ShowWarning(
								"buildin_setunitdata: Trying to set UMOB_SLAVECPYMSTRMD on mob without master!\n");
							break;
						}
			...
			switch (type) {
				...
				case UPET_X:
					if (!unit_walktoxy(bl, (short)value, pd->bl.y, 2)) {
						unit_movepos(bl, (short)value, pd->bl.y, 0, 0);
					}
					break;
				case UPET_Y:
...
BUILDIN_FUNC(unitwalk) {
	...
	if (!strcmp(cmd, "unitwalk")) {
		...
		if (script_pushint(st, unit_can_reach_pos(bl, x, y, 0))) {
			...
			add_timer(gettick() + 50,
					  unit_delay_walktoxy_timer,
					  bl->id,
...
BUILDIN_FUNC(unitskilluseid) {
	...
	if (script_rid2bl(2, bl)) {
		...
		unit_skilluse_id2(bl,
						  target_id,
						  skill_id,
						  skill_lv,
						  (casttime * 1000) + skill_castfix(bl, skill_id, skill_lv),
						  cancel,
						  ignore_range);
	}
...
BUILDIN_FUNC(bg_monster_set_team) {
	...
	int32 id = script_getnum(st, 2), bg_id = script_getnum(st, 3);
	...
	switch (type) {
		...
		case BG_INFO_MAPS: {
			...
			for (i = 0; i < bg->maps.size(); i++) {
				setd_sub_str(
					st, nullptr, ".@bgmaps$", static_cast<int>(i), mapindex_id2name(bg->maps[i].mapindex), nullptr);
			}
...
BUILDIN_FUNC(party_destroy) {
	...
	if (i >= MAX_PARTY || !party->data[i].sd) { // leader not online
		...
		for (j = 0; j < MAX_PARTY; j++) {
			...
			if (sd) {
				party_member_withdraw(party->party.party_id,
									  sd->status.account_id,
									  sd->status.char_id,
									  sd->status.name,
				...
				intif_party_leave(party->party.party_id,
								  party->party.member[j].account_id,
								  party->party.member[j].char_id,
								  party->party.member[j].name,
								  PARTY_MEMBER_WITHDRAW_LEAVE);
			}
		}
...
BUILDIN_FUNC(channel_ban) {
	...
	if (!idb_exists(ch->banned, char_id)) {
		...
		if (tsd) {
			...
			safesnprintf(
				output, CHAT_SIZE_MAX, msg_txt(tsd, 769), ch->alias, tsd->status.name); // %s %s has been banned.
		}
		else {
...
BUILDIN_FUNC(channel_unban) {
	...
	if (idb_exists(ch->banned, char_id)) {
		...
		if (tsd) {
			safesnprintf(
				output, CHAT_SIZE_MAX, msg_txt(tsd, 770), ch->alias, tsd->status.name); // %s %s has been unbanned
		}
		else {
...
static inline bool mail_sub(struct script_state *st,
							struct script_data *data,
							map_session_data *sd,
							int32 i,
							const char **out_name,
...
BUILDIN_FUNC(getitempos) {
	...
	if (current_equip_item_index == -1) {
		ShowError(
			"buildin_getitempos: Invalid usage detected. This command should only be used inside item scripts.\n");
		script_pushint(st, 0);
		return SCRIPT_CMD_FAILURE;
	}
