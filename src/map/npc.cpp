...
static int npc_selllist_sub(map_session_data* sd, int list_length, PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list, struct npc_data* nd)
{
	...
	for( i = 0; i < list_length; i++ )
	{
		...
		if (sd->inventory.u.items_inventory[idx].card[0] == CARD0_CREATE) {
			auto creator = MakeDWord(sd->inventory.u.items_inventory[idx].card[2], sd->inventory.u.items_inventory[idx].card[3]);
			script_setarray_pc(sd, "@sold_creator", i, creator, &key_creator);
		}
	}
