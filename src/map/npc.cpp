...
static void npc_parsename(
	struct npc_data* nd, const char* name, const char* start, const char* buffer, const char* filepath);
	...
	if (this->nodeExists(node, "Items")) {
		for (const ryml::NodeRef& itemNode : node["Items"]) {
			...
			if (this->nodeExists(itemNode, "Zeny")) {
				...
				if (zeny > MAX_ZENY) {
					this->invalidWarning(itemNode["Zeny"],
				...
				for (const ryml::NodeRef& requiredItemNode : itemNode["RequiredItems"]) {
					...
					if (item->requirements.size() >= MAX_BARTER_REQUIREMENTS) {
						this->invalidWarning(requiredItemNode["Index"],
											 "barter_parseBodyNode: Failed at Index %hu. Too many requirements, "
											 "Barters support up to %d.\n",
											 requirement_index,
					...
					if (this->nodeExists(requiredItemNode, "Item")) {
						...
						if (data == nullptr) {
							this->invalidWarning(requiredItemNode["Item"],
					...
					if (this->nodeExists(requiredItemNode, "Amount")) {
						...
						if (amount > MAX_AMOUNT) {
							this->invalidWarning(requiredItemNode["Amount"],
					...
					if (this->nodeExists(requiredItemNode, "Refine")) {
						...
						if (data->flag.no_refine) {
							this->invalidWarning(requiredItemNode["Refine"],
...
bool npc_enable_target(npc_data& nd, uint32 char_id, e_npcv_status flag) {
	if (char_id > 0 && (flag & NPCVIEW_CLOAK)) {
		...
		map_foreachinmap(npc_cloaked_sub,
						 nd.bl.m,
						 BL_PC,
						 nd.bl.id); // Because npc option has been updated we remove the npc id from sd->cloaked_npc
	}
...
struct timer_event_data {
	...
	int32 time; // holds total time elapsed for the script from when timer was started to when last time the event
				// triggered.
};
	...
	if (nd && nd->bl.type == BL_NPC) {
		...
		if (ev && ev->nd != nd) {
			ShowWarning("npc_timerevent_quit: Unable to execute \"%s\", two NPCs have the same event name [%s]!\n",
						script_config.timer_quit_event_name,
						buf);
			ev = nullptr;
		}
		if (ev) {
...
int32 npc_event_sub(map_session_data* sd, struct event_data* ev, const char* eventname) {
	...
	safesnprintf(ontouch_event_name,
				 ARRAYLENGTH(ontouch_event_name),
				 "%s::%s",
				 ev->nd->exname,
				 script_config.ontouch_event_name);
	safesnprintf(ontouch2_event_name,
				 ARRAYLENGTH(ontouch2_event_name),
				 "%s::%s",
				 ev->nd->exname,
...
int32 npc_event(map_session_data* sd, const char* eventname, int32 ontouch) {
	...
	npc_event_sub(
		sd,
		ev,
		eventname); // Don't return this value so npc_enable_sub doesn't attempt to "click" the NPC if OnTouch fails.
	return 0;
}
...
int32 npc_touchnext_areanpc(map_session_data* sd, bool leavemap) {
	...
	sd->npc_ontouch_.erase(
		std::remove_if(
			sd->npc_ontouch_.begin(),
			sd->npc_ontouch_.end(),
			[&](const int32& current_npc_id) {
		...
		if (x >= mapdata->npc[i]->bl.x - xs && x <= mapdata->npc[i]->bl.x + xs && y >= mapdata->npc[i]->bl.y - ys &&
			...
			switch (mapdata->npc[i]->subtype) {
				case NPCTYPE_WARP: {
					...
					if (unit_warp(
							&md->bl, warp_m, mapdata->npc[i]->u.warp.x, mapdata->npc[i]->u.warp.y, CLR_OUTSIGHT) == 0) {
						return 1; // Warped
					}
				} break;
				case NPCTYPE_SCRIPT:
					...
					safesnprintf(eventname,
								 ARRAYLENGTH(eventname),
								 "%s::%s",
								 mapdata->npc[i]->exname,
 ...
 **/
static enum e_CASHSHOP_ACK npc_cashshop_process_payment(struct npc_data* nd,
														int32 price,
														int32 points,
														map_session_data* sd) {
	int32 cost[2] = {0, 0};
	...
	switch (nd->subtype) {
		...
		case NPCTYPE_POINTSHOP: {
			...
			if (!set_reg_num(nullptr,
							 sd,
							 add_str(nd->u.shop.pointshop_str),
							 nd->u.shop.pointshop_str,
							 cost[0] - (price - points),
							 nullptr)) {
				return ERROR_TYPE_PURCHASE_FAIL;
			}
			...
			sprintf(
				output, msg_txt(sd, 716), nd->u.shop.pointshop_str, cost[0] - (price - points)); // Your '%s' is now: %d
...
int32 npc_cashshop_buylist(map_session_data* sd, int32 points, std::vector<s_npc_buy_list>& item_list) {
	...
	if (!nd ||
		(nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP)) {
...
static int32 npc_selllist_sub(map_session_data* sd,
							  int32 list_length,
							  const PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list,
							  struct npc_data* nd) {
	char npc_ev[EVENT_NAME_LENGTH];
...
static void npc_parsename(
	...
	if ((dnd = npc_name2id(nd->exname)) != nullptr) { // duplicate unique name, generate new one
		...
		strcpy(other_mapname,
			   (dnd->bl.m == -1 ? "(not on a map)" : mapindex_id2name(map_getmapdata(dnd->bl.m)->index)));
 ...
 */
static const char* npc_parse_warp(
 ...
 * @return new index for next parsing
 */
static const char* npc_parse_shop(
	char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	char *p, point_str[32];
	...
	switch (type) {
		...
		case NPCTYPE_POINTSHOP: {
			...
			switch (point_str[0]) {
				...
				case '\'':
					ShowWarning(
						"npc_parse_shop: Invalid item cost variable type (must be permanent character or account "
						"based) in file '%s', line '%d'. Ignoring the rest of the line...\n * w1=%s\n * w2=%s\n * "
						"w3=%s\n * w4=%s\n",
						filepath,
						strline(buffer, start - buffer),
						w1,
			...
			if (point_str[strlen(point_str) - 1] == '$') {
				ShowWarning(
					"npc_parse_shop: Invalid item cost variable type (must be integer) in file '%s', line '%d'. "
...
#if PACKETVER < 20131223
			ShowError(
				"npc_parse_shop: (MARKETSHOP) Feature is disabled, need client 20131223 or newer. Ignoring file '%s', "
				"line '%d\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				filepath,
				strline(buffer, start - buffer),
				w1,
		...
		switch (type) {
			case NPCTYPE_MARKETSHOP:
				...
				if (sscanf(p, ",%u:%11d:%11d", &nameid2, &value, &qty) != 3) {
					ShowError(
						"npc_parse_shop: (MARKETSHOP) Invalid item definition in file '%s', line '%d'. Ignoring the "
						"rest of the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
						filepath,
						strline(buffer, start - buffer),
						w1,
 ...
 * @return new index for next parsing
 */
static const char* npc_parse_script(
	char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	int16 dir = 0;
...
const char* npc_parse_duplicate(char* w1,
								char* w2,
								char* w3,
								char* w4,
								const char* start,
...
void npc_market_delfromsql_(const char* exname, t_itemid nameid, bool clear) {
	...
	if (clear) {
		...
		if (SQL_ERROR ==
				stmt.Prepare(
					"DELETE FROM `%s` WHERE `name`='%s' AND `nameid`='%u' LIMIT 1", market_table, exname, nameid) ||
			SQL_ERROR == stmt.Execute()) {
			SqlStmt_ShowDebug(stmt);
		}
	}
...
static const char* npc_parse_mob(
	char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if (battle_config.force_random_spawn || (mob.x == 0 && mob.y == 0) ||
		(mob.xs == 1 && mob.ys == 1 &&
		 !map_getcell(mob.m, mob.x, mob.y, CELL_CHKREACH))) { // Force a random spawn anywhere on the map.
 ...
 *------------------------------------------*/
static const char* npc_parse_mapflag(
	char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	int16 m;
	char mapname[MAP_NAME_LENGTH_EXT];
	bool state = true;
	...
	switch (mapflag) {
		...
		case MF_NOSAVE: {
			...
			if (state && !strcmpi(w4, "SavePoint")) {
				...
				if (!args.nosave.map) {
					ShowWarning(
						"npc_parse_mapflag: Specified save point map '%s' for mapflag 'nosave' not found (file '%s', "
						"line '%d'), using 'SavePoint'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
						savemap,
						filepath,
						strline(buffer, start - buffer),
						w1,
		...
		case MF_PVP_NIGHTMAREDROP: {
			...
			if (sscanf(w4, "%15[^,],%15[^,],%11d", drop_arg1, drop_arg2, &args.nightmaredrop.drop_per) == 3) {
				if (!strcmpi(drop_arg1, "random")) {
					...
					ShowWarning(
						"npc_parse_mapflag: Invalid item ID '%d' supplied for mapflag 'pvp_nightmaredrop' (file '%s', "
						"line '%d'), removing.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
						args.nightmaredrop.drop_id,
						filepath,
						strline(buffer, start - buffer),
						w1,
						w2,
		...
		case MF_SKILL_DAMAGE: {
			...
			if (!state) {
				...
				if (sscanf(w4,
						   "%30[^,],%23[^,],%11d,%11d,%11d,%11d[^\n]",
						   skill_name,
						   caster_constant,
						   &args.skill_damage.rate[SKILLDMG_PC],
	...
	if (sd != nullptr) {
		...
		if (sd->npc_id == nd->bl.id || sd->npc_shopid == nd->bl.id) {
			...
			nd->dynamicnpc.removal_tid =
				add_timer(nd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout,
						  npc_dynamicnpc_removal_timer,
						  nd->bl.id,
						  (intptr_t) nullptr);
			return 0;
		}
		...
		if (DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction) < battle_config.feature_dynamicnpc_timeout) {
			nd->dynamicnpc.removal_tid =
				add_timer(nd->dynamicnpc.last_interaction + DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction),
						  npc_dynamicnpc_removal_timer,
						  nd->bl.id,
						  (intptr_t) nullptr);
			return 0;
...
struct npc_data* npc_duplicate_npc_for_player(struct npc_data& nd, map_session_data& sd) {
	...
	dnd->dynamicnpc.removal_tid = add_timer(dnd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout,
											npc_dynamicnpc_removal_timer,
											dnd->bl.id,
											(intptr_t) nullptr);
