...
HANDLER_FUNC(userconfig_save) {
	...
	SqlStmt stmt{*handle};
	if (SQL_SUCCESS != stmt.Prepare("SELECT `data` FROM `%s` WHERE (`account_id` = ? AND `world_name` = ?) LIMIT 1",
									user_configs_table) ||
	...
	if (SQL_SUCCESS != stmt.Prepare("REPLACE INTO `%s` (`account_id`, `world_name`, `data`) VALUES (?, ?, ?)",
									user_configs_table) ||
		SQL_SUCCESS != stmt.BindParam(0, SQLDT_INT, &account_id, sizeof(account_id)) ||
		SQL_SUCCESS != stmt.BindParam(1, SQLDT_STRING, (void *)world_name.c_str(), world_name.length()) ||
...
HANDLER_FUNC(userconfig_load) {
	...
	if (stmt.NumRows() <= 0) {
		...
		if (SQL_SUCCESS != stmt.Prepare("INSERT INTO `%s` (`account_id`, `world_name`, `data`) VALUES (?, ?, ?)",
										user_configs_table) ||
			SQL_SUCCESS != stmt.BindParam(0, SQLDT_INT, &account_id, sizeof(account_id)) ||
			SQL_SUCCESS != stmt.BindParam(1, SQLDT_STRING, (void *)world_name, strlen(world_name)) ||
