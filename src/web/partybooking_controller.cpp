...
bool party_booking_read(std::string& world_name,
						std::vector<s_party_booking_entry>& output,
	...
	std::string query =
		"SELECT `account_id`, `char_id`, `char_name`, `purpose`, `assist`, `damagedealer`, `healer`, `tanker`, "
		"`minimum_level`, `maximum_level`, `comment` FROM `" +
...
HANDLER_FUNC(partybooking_add) {
	...
	if (SQL_SUCCESS !=
			stmt.Prepare("SELECT 1 FROM `%s` WHERE `leader_id` = ? AND `leader_char` = ?", party_table, aid, cid) ||
		SQL_SUCCESS != stmt.BindParam(0, SQLDT_UINT32, &aid, sizeof(aid)) ||
		SQL_SUCCESS != stmt.BindParam(1, SQLDT_UINT32, &cid, sizeof(cid)) || SQL_SUCCESS != stmt.Execute()) {
		SqlStmt_ShowDebug(stmt);
		csl.unlock();
		res.status = HTTP_BAD_REQUEST;
		res.set_content("Error", "text/plain");
	...
	StringBuf_Printf(&buf,
					 ...
					 "`damagedealer`, `healer`, `tanker`, `minimum_level`, `maximum_level`, `comment` ) VALUES ( ",
					 partybookings_table);
...
HANDLER_FUNC(partybooking_delete) {
	...
	if (SQL_ERROR == Sql_Query(handle,
							   "DELETE FROM `%s` WHERE `world_name` = '%s' AND `account_id` = '%d'",
							   partybookings_table,
							   world_name.c_str(),
							   account_id)) {
