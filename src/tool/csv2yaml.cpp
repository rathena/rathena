...
static void skilltree_txt_data(const std::string &modePath, const std::string &fixedPath) {
	...
	if (fileExists(modePath + "/skill_tree.txt")) {
		sv_readdb(modePath.c_str(),
				  "skill_tree.txt",
				  ',',
				  3 + MAX_PC_SKILL_REQUIRE * 2,
...
bool Csv2YamlTool::initialize(int32 argc, char *argv[]) {
	...
	if (fileExists(mob_db.getDefaultLocation())) {
		...
		sv_readdb(path_db_mode.c_str(),
				  "mob_db.txt",
				  ',',
				  31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
				  31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
				  -1,
				  &parse_mob_constants_txt,
				  false);
		sv_readdb(path_db_import.c_str(),
				  "mob_db.txt",
				  ',',
				  31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
				  31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
...
#include <map/script_constants.hpp>
	...
	if (!process("GUILD_SKILL_TREE_DB",
				 1,
				 root_paths,
				 "guild_skill_tree",
				 [](const std::string &path, const std::string &name_ext) -> bool {
					 return sv_readdb(path.c_str(),
									  name_ext.c_str(),
									  ',',
									  2 + MAX_GUILD_SKILL_REQUIRE * 2,
									  2 + MAX_GUILD_SKILL_REQUIRE * 2,
									  -1,
	...
	if (!process("READING_SPELLBOOK_DB",
				 1,
				 root_paths,
				 "spellbook_db",
				 [](const std::string &path, const std::string &name_ext) -> bool {
					 return sv_readdb(
						 path.c_str(), name_ext.c_str(), ',', 3, 3, -1, &skill_parse_row_spellbookdb, false);
				 })) {
		return false;
	}

	if (!process("MOB_AVAIL_DB",
				 1,
				 root_paths,
				 "mob_avail",
				 [](const std::string &path, const std::string &name_ext) -> bool {
	...
	if (!process(
			"QUEST_DB", 3, root_paths, "quest_db", [](const std::string &path, const std::string &name_ext) -> bool {
				return sv_readdb(path.c_str(),
								 name_ext.c_str(),
								 ',',
								 3 + MAX_QUEST_OBJECTIVES * 2 + MAX_QUEST_DROPS * 3,
								 100,
								 -1,
								 &quest_read_db,
								 false);
	...
	if (!process(
			"ITEM_DB", 3, {path_db_mode}, "item_db", [](const std::string &path, const std::string &name_ext) -> bool {
	...
	if (!process("RANDOM_OPTION_GROUP",
				 1,
				 root_paths,
	...
	if (!process(
			"MOB_DB", 3, {path_db_mode}, "mob_db", [](const std::string &path, const std::string &name_ext) -> bool {
				return sv_readdb(path.c_str(),
								 name_ext.c_str(),
								 ',',
								 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
	...
	if (!process(
			"MOB_DB", 3, {path_db_import}, "mob_db", [](const std::string &path, const std::string &name_ext) -> bool {
				return sv_readdb(path.c_str(),
								 name_ext.c_str(),
								 ',',
								 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
	...
	if (!process(
			"CASTLE_DB", 1, root_paths, "castle_db", [](const std::string &path, const std::string &name_ext) -> bool {
				return sv_readdb(path.c_str(), name_ext.c_str(), ',', 4, 4, -1, &guild_read_castledb, false);
			})) {
		return false;
	}

	if (!process("GUILD_EXP_DB",
				 1,
				 {path_db_mode},
				 "exp_guild",
				 [](const std::string &path, const std::string &name_ext) -> bool {
	...
	if (!process(
			"CONSTANT_DB", 1, root_paths, "const", [](const std::string &path, const std::string &name_ext) -> bool {
...
static bool skill_parse_row_skilldb(char *split[], size_t columns, size_t current) {
	...
	if (it_unit != skill_unit.end()) {
		...
		if (it_unit->second.target_str.size() > 0) {
			if (it_unit->second.target_str.compare("noenemy") == 0 ||
				it_unit->second.target_str.compare("friend") == 0) {
...
static bool pc_readdb_job_exp_sub(char *fields[], size_t columns, size_t current) {
	int32 level = atoi(fields[0]), jobs[CLASS_COUNT], job_count = skill_split_atoi(fields[1], jobs, CLASS_COUNT),
		  type = atoi(fields[2]);
...
static bool pc_readdb_job_basehpsp(char *fields[], size_t columns, size_t current) {
	...
	for (int32 i = 0; i < job_count; i++) {
		...
		if (it_level != exp_base_level.end()) {
			...
			ShowError(
				"pc_readdb_job_basehpsp: The job_exp database needs to be imported into memory before converting the "
				"job_basehpsp_db database.\n");
			return false;
		}
